 ...
 */
export const getVersionsFromDoc = (
	frontMatter: FrontMatter[],
	docPath: string,
) => {
...
async function getAllFilesFrontMatter(): Promise<FrontMatter[]> {
	...
	for await (const platform of await opendir(platformsPath)) {
		...
		const commonFileNames: string[] = (
			await getAllFilesRecursively(commonPath)
					...
					async (f) => {
						...
						if (noFrontMatter) {
							...
							allFrontMatter.push({
								...
								sourcePath:
									"docs/" + f.commonFileName.slice(docsPath.length + 1),
							});
						}
					},
					{ concurrency: FILE_CONCURRENCY_LIMIT },
...
export async function getFileBySlug(slug: string): Promise<SlugFile> {
	...
	const configPath = path.join(
		root,
		slug.split(VERSION_INDICATOR)[0],
		"config.yml",
	);

	let configFrontmatter: PlatformConfig | undefined;
	try {
		configFrontmatter = yaml.load(
			await readFile(configPath, "utf8"),
		) as PlatformConfig;
	} catch (err) {
	...
	if (
		...
		(
			await Promise.allSettled(
				[mdxPath, mdxIndexPath, mdPath, mdIndexPath, versionedMdxIndexPath].map(
					(p) => access(p),
				),
			)
		).every((r) => r.status === "rejected")
		...
		if (
			...
			versionedMdxIndexPath = getVersionedIndexPath(
				root,
				commonFilePath,
				".mdx",
			);
	...
	if (isVersioned(slug)) {
		try {
			...
			mdxIndexPath = addVersionToFilePath(
				mdxIndexPath,
				slug.split(VERSION_INDICATOR)[1],
			);
		} catch (err) {
