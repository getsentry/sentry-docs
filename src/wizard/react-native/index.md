...
Complete stack traces will be shown for your React Native Javascript error by default using the [automatic source maps upload](/platforms/react-native/sourcemaps). For manual source maps upload follow [this guide](/platforms/react-native/sourcemaps).

You'll also need to [upload the debug information files](/platforms/react-native/upload-debug/) generated by the native iOS and Android tooling for native crashes.
...
Source Context will be shown for React Native Javascript error by default if source maps are uploaded. For source maps upload follow [this guide](/platforms/react-native/sourcemaps).

To enable source context for native errors, you'll need to upload native debug symbols to Sentry by following the [Uploading Source Code Context With Sentry Gradle Plugin](/platforms/react-native/upload-debug/#uploading-source-context-with-sentry-gradle-plugin) guide and [Uploading Source Context With Xcode](/platforms/react-native/upload-debug/#uploading-source-context-with-xcode).
