<Note>

Sentry's Django integration enables automatic reporting of errors and exceptions.

Sentry's Django integration reports all exceptions leading to an Internal Server Error and creates a separate scope for each request. The integration supports [Django Web Framework from Version 1.8 and above](https://www.djangoproject.com/). Django applications using Channels 2.0 will be correctly instrumented using Python 3.7. Older versions of Python require installation of `aiocontextvars`.

</Note>
