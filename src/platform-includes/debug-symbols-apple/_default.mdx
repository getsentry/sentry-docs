...
Sentry can display snippets of your code next to the event stacktraces. This feature is called <PlatformLink to="/data-management/debug-files/source-context/">source context</PlatformLink>.

To use this feature you have three options:

### 1.Manual use of sentry-cli

By adding `--include-sources` to the sentry-cli command, sentry-cli will scan your debug files to find references to the source code files, resolve them in the local file system, bundle them up, and upload them to Sentry.
...
You can upload your source to sentry after every build when using Xcode. To do this, add `--include-sources` to the sentry-cli command in the [Upload debug symbols](#xcode-build-phase) Xcode build phase script.
