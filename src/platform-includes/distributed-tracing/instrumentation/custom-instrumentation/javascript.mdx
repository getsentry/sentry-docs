...
In this example we create a new transaction, that is attached to the trace that was specified in the information in the `sentry-trace` and `baggage` HTML `meta` tags.

## Inject Tracing Information to Outgoing Requests

On outgoing XHR/fetch requests you have to add the two headers `sentry-trace` and `baggage` that you extracted and stored in the `pageLoadTransaction` earlier.
...
Now tracing information is propagated to the project running at `https://example.com`. If it has a Sentry SDK running, it will extract and save the tracing information for later use.
