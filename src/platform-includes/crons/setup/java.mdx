...
We offer a helper that takes care of wrapping your code and sending a check-in at the start as well as one at the end. It checks for exceptions and sets the status of the check-in accordingly.
...
CheckInUtils.withCheckIn("<monitor-slug>", MonitorConfig(MonitorSchedule.interval(3, MonitorScheduleUnit.MINUTE))) {
    // Execute your scheduled task here...
}
```
