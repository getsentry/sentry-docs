...
the Sentry server to look up sources in the source bundle with matching ID.
...
```
sentry-cli debug-files bundle-jvm --output path/to/store/bundle --debug-id A_VALID_UUID path/to/sources
```
