...
In practice, matching the paths for artifacts discovery is prone to mismatches that might be caused by a wrong configuration of your bundler, CI/CD pipeline...
...
| Pros | Cons |
|:--|:--|
| With Debug IDs, configuring your build pipeline to upload artifacts is straightforward. | Requires a version update to Javascript SDKs, bundler plugin ( and `sentry-cli`). |
