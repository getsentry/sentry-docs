...
If you're using one or more of these older versions of the tools, you can still upload source maps; however, the process will differ. Instead of using Debug IDs, you'll use releases to match an event's stack frame with its corresponding minified source and source map file (known as artifacts).
...
## Uploading Source Maps using `@sentry/webpack-plugin` on version `1.x`
...
## Using bundler plugins on version `2.x`
