...
Chances are, you set up source maps for Sentry a while ago. Since then we updated a lot of our tooling to make the process around source maps easier.
We recommend you upgrade your tooling to the newest versions, as that is usually easier than to troubleshoot the legacy method.

If you still want to find information on the legacy method of uploading source maps, please see the <PlatformLink to="/sourcemaps/troubleshooting_js/uploading-without-debug-ids">guide to upload source maps without debug IDs</PlatformLink>.
...
To use the automated verification process, [install and configure](/product/cli/) `sentry-cli`. Once completed, use the `sourcemaps explain` command, by calling it with the event ID that can be found in the top-left corner of the **Issue Details** page in [sentry.io](https://sentry.io). For example, "Event ID: c2ad049f".
...
Bundlers and tools (like `tsc`) that generate code, generally require you to manually set specific options in order for source maps to be generated.
...
Inside the JavaScript files you uploaded to Sentry, search for code that roughly looks like `e._sentryDebugIds=e._sentryDebugIds||{}` (note that this snipped might be looking different depending on how you process your code). Having this code inside a bundle means that it will be able to be matched to a source file. Every bundle you deploy in your app needs to have this snippet in order to be correctly source mapped.
