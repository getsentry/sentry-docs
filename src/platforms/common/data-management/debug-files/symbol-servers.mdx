...
## Notes about Portable PDBs

Portable PDBs are generally treated like native PDBs with, the following amendments:
- The `age` part of the id is masked with `FFFFFFFF`.
- Downloading a Portable PDB from Nuget requires the debug checksum to be supplied
  via the `SymbolChecksum` header.
