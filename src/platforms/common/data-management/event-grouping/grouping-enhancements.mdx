...
If you use stack traces for grouping, the stack trace rules influence the data that feed into that algorithm. These rules can be configured on a per-project basis under **Settings > Projects > [project] > Issue Grouping > Stack Trace Rules**.
