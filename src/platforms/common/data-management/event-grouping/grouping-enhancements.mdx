...
If you use stack traces for grouping, the enhancements rules influence the data that feed into that algorithm. These rules can be configured on a per-project basis under **Settings > [organization] > [project] > General: Grouping Settings**.
...
- **flag**: Flags identify the action to be taken if all matchers match. A flag needs to be prefixed with `+` to set it or `-` to unset it. If this expression is prefixed with a `^`, it applies to frames above the flag (toward the crash). If prefixed with `v` it applies to frames below the flag (away from the crash). For instance, `-group ^-group` removes the matching frame and all frames above it from the grouping.
