...
sidebar_order: 1
description: "Learn about server-side fingerprinting, matchers for fingerprinting, how to combine matchers, and using variables for fingerprinting."
---
...
These rules can be configured on a per-project basis under **Settings > Projects > [project] > General Settings > Grouping Settings**. Group Settings has input fields where you can write custom fingerprinting rules. The syntax follows the syntax from [Discover queries](/product/discover-queries/). If you want to negate the match, prefix the expression with an exclamation mark (`!`).
