...
This is a practical example which would group exceptions of a specific type
together:
...
Matchers generally use globbing syntax. The following matchers exist:
...
descriptions (values) associated with them. This mathchers lets you do a case
insensitive match on it.
...
This matcher matches on a log message. This will also automatically check for
...
Matches on the name of the logger. This is useful to combine all messages of a
logger together. The match is done case sensitively.
...
Matches on the log level. The match is done case insensitively.
...
This matcher matches case insensitively on the path of an event. It uses path
...
instead. The match is performed case sensitively and regular globbing rules
...
glob. The match is done case sensitively:
...
name then a match only exists if a frame exists that matches both the function
name and the module name. It's not enough for a frame to exist with the
...
This fills in the value of a tag into the fingerprint. This can for instance
...
error.type:DatabaseUnavailable -> system-down
error.type:RedisConnectionError -> system-down
...
error.type:TimeoutError -> {{ transaction }}

# group all JavaScript errors in a module by the topmost module and
# function name
stack.abs_path:**/some-module/** -> {{ stack.module }}, {{ stack.function }}

# force all memory allocation errors to be grouped together
family:native stack.function:malloc -> memory-allocation-error

# group all allocation errors by the package (dylib, executable)
# that caused them
stack.function:*alloc* -> {{ stack.package }}
