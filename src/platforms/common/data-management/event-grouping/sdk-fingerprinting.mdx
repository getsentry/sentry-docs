...
In supported SDKs, you can override the Sentry default grouping passing the `fingerprint` attribute an array of strings. The length of a fingerprints array is not restricted. This works similar to the [server side functionality](../server-side-fingerprinting/) which is always available and
