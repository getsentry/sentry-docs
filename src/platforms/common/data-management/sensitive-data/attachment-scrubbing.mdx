...
Events can have attachments which may also contain PII.
[Minidumps](/platforms/native/guides/minidumps) are a common attachment type for native crash reports which can contain PII in various sections of the file.
...
This is done by understading enough of the file format and using custom selectors which will try and only let you modify certain parts of the file, [see below](#source-selectors) for more information on the source selectors.

However when actually modifying some data in the file, we can not modify the length of the file, this has implications for how the [Methods](../advanced-datascrubbing/#methods) behave:

- Any modification which results in a replacement text which is **longer** than the original will be **truncated**.
- Any modification which results in a replacement text which is **shorter** than the original will be **padded** using `x` as padding character.
...
- *Hash*: Hasing happens as usual, however the **binary representation** of the match is hashed, regardless of the encoding the match was made in.  The resulting hash is padded or truncated as described above.
- *Replace*: This behaves like normal, with the replacement being padded or truncated as described above.

## Source selectors

All attachments can be select using the `$attachments` [Value Type](../advanced-datascrubbing/#value-types) selector at the root of the selector, this can be followed by the specific filename itself.
The filename part need to be quoted to write the actual filename but wildcards can be used as well.

For example if your filename is `minidump.dmp` you can select all fields which can be scrubbed using `$attachments.'minidump.dmp'.**`.
To write the same using a wildcard to select all attachments you would write `$attachmetns.*.**` or even simply `$attachments.**`.

### Minidump selectors

A minidump is always an attachment, but it also has its own [value type](../advanced-datascrubbing/#value-types) of `$minidump`.
This allows you to select all fields which can be scrubbed in a minidump using `$minidump.**`.
This really is a shorthand for selecting the fields of all attachments which are minidumps: `$attachments.$minidump.**`.

So far we have repeatedly referred to all fields in a minidump which can be scrubbed.
For the puprpose of PII scrubbing we currently represent the minidump as a flat structure with the following fields:
...
- The **stack memory**: These are memory regions in the minidump which are used as stack memory by a thread.
  ...
  Additionally the regular expression is applied to any valid UTF-16LE strings found withing the binary data.
...
- A minidump also contains a list of **code modules** which were loaded at the time the minidump was created.
  ...
  As these fields are pathnames they could contain a user's home directory, so it makes sense to apply the *Usernames in filepaths* [data type](../advanced-datascrubbing/#data-types) to these fields.  The fields can be selected individually path their respective field names:
  ...
  An example of how the UI would display a rule for these fields would be:
