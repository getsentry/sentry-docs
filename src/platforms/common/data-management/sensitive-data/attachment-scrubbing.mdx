...
- *Hash*: Hashing happens as usual, however the **binary representation** of the match is hashed, regardless of the encoding the match was made in.  The resulting hash is padded or truncated as described above.
- *Replace*: This behaves like normal, with the replacement being padded or truncated as described above.

## Source selectors

All attachments can be selected using the `$attachments` [Value Type](../advanced-datascrubbing/#value-types) selector at the root of the selector, this can be followed by the specific filename itself.
The filename part needs to be quoted to write the actual filename but wildcards can be used as well.
