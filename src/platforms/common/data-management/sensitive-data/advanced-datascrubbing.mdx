...
In addition to using [`beforeSend`](../#scrubbing-data) in your SDK or our [regular server-side data scrubbing features](../#server-side-controls) to redact sensitive data, Advanced Data Scrubbing is an alternative way to redact sensitive information just before it is saved in Sentry. It allows you to:
...
- `$stack_memory`: Matches all stack memory regions used in native crash errors, see [Attachment Scrubbing](../attachment-scrubbing) for details.
- `$heap_memory`: Matches all heap memory regions ussed in native crash errors, see [Attachment Scrubbing](../attachment-scrubbing) for details.
- `$binary`: Matches fields containing binary data and are not coverd by a more specific value types like `$stack_memory` and `$heap_memory`.
