...
If you want to filter out transactions before sending them to Sentry, ( to get rid of health checks, for example), you can use <PlatformLink to="/configuration/options/#before-send-transaction"><PlatformIdentifier name="before-send-transaction" /></PlatformLink> to drop them. <PlatformLink to="/configuration/filtering/#using-platformidentifier-namebefore-send-transaction-">Here's an example</PlatformLink>.
