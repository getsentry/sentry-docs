...
The User Feedback Widget offers a large amount of customization and if the available options are still insufficient, you're also able to [use your own UI](#). The following image shows what is available to be customized. The configuration keys on the left side of the image correspond to [text customization](#text-configuration) and the configuration keys on the right side of the image are for [theme customizations](#theme-customization).
...
Colors can be customized via the Feedback class constructor or by defining CSS variables on the injected button. If you use the default button, it will have an `id="sentry-feedback`, meaning you can use the `#sentry-feedback` selector to define CSS variables to override.
