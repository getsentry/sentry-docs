...
Sentry now allows you to collect user feedback from anywhere inside of your application, without requiring an error event to occur. This requires a minimum SDK version of [#TODO](#TODO). The previous [Crash-Report Modal](#crash-report-modal) still exists to handle user feedback that is associated with an error event.

## User Feedback Widget

Our embeddable JavaScript widget allows your users to submit feedback from anywhere inside of your application. This differs from the [Crash-Report Modal](#crash-report-modal) which is reactive feedback that is specifically tied to an error event.
