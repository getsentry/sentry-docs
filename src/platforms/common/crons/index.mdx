...
: Time (in minutes) of how long Sentry should wait for your check-in until it is considered missed ("grace period"). Optional.

<Note>

We recommend that your check-in margin to be less or equal to your interval.

</Note>

`max_runtime`:

: Time (in minutes) your job is allowed to run before it is considered failed. Optional.
