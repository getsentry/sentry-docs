...
<ConfigKey name="consider-frames-in-app-by-default" supported={["dart"]}>
...
- If `considerFramesInAppByDefault` is true you only need to maintain `inAppExcludes`.
- If `considerFramesInAppByDefault` is false you only need to maintain `inAppIncludes`.
