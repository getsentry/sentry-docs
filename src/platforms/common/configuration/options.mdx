...
<ConfigKey name="before-send" notSupported={["elixir"]}>
...
<ConfigKey name="before-send-event">

This function is called with a [`Sentry.Event`](https://hexdocs.pm/sentry/Sentry.Event.html), and can return a modified event struct, or `nil` to skip reporting the event. This can be used, for instance, for manual PII stripping before sending.
