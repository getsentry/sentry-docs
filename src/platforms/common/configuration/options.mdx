...
When enabled, a new monitor thread will be spawned that will perform health checks on the SDK. If the system is unhealthy, the SDK will keep halving the `traces_sample_rate` set by you in 10 second intervals till recovery. This downsampling will ensure that system stays stable and reduces SDK overhead under high load.
