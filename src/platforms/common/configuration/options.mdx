...
An optional property that configures which features are under experimental mode, this property is either an `Object Type` with properties or a key/value `TypedDict` depending the language. Experimental features are still in-progress and may have bugs. We recognize the irony.
