...
Set this boolean to `false` to disable the native nagger alert be shown.
...
Set this boolean to `true` to automatically attach all the threads to all logged events on Android.

</ConfigKey>

<ConfigKey name="enableAutoPerformanceTracking" supported={["react-native"]}>

Set this boolean to `false` to disable the auto [performance monitoring](product/performance/) tracking.
...
Set this callback that is called after the Sentry React Native SDK initialized its Native SDKs (Android and iOS).
