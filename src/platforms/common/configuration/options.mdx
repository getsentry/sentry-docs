...
Sentry differentiates stack frames that are directly related to your application ("in application") from stack frames that come from other packages such as the standard library, frameworks, or other dependencies. The application package is automatically marked as `inApp`. The difference is visible on [sentry.io](https://sentry.io), where only the "in application" frames are displayed by default.
...
<ConfigKey name="enableAutoPerformanceTracking" supported={["react-native", "flutter"]}>
