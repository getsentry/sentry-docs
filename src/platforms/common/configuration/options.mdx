...
<ConfigKey name="stack-trace-mode" notSupported={["dotnet"]}>
...
Specifies a local directory used for caching payloads. When this option is enabled (i.e. when the directory is set), Sentry SDK will persist envelopes locally before sending them over to Sentry. This is useful if you expect your application to run in environments where internet connectivity is limited.
...
When caching is enabled (i.e. <PlatformIdentifier name="cache-directory-path" /> is set), this option controls the timeout which limits how long the Sentry SDK will attempt to flush existing cache during initialization. Note that flushing cache involves sending the payload to Sentry in a blocking operation. Setting this option to zero will mean that Sentry will not attempt to flush cache during initialization, but will do so on the next queued up payload instead.
