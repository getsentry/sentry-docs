...
When set to `true` the SDK will send session events to Sentry. This is supported in all Browser SDKs. How it works is that one session per pageload will be emitted to Sentry.
