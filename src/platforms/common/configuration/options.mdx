...
<PlatformSection notSupported={["native", "javascript.electron", "unity", "dotnet", "python", "php"]}>
...
<ConfigKey name="attach-stacktrace" notSupported={["elixir"]}>
...
<ConfigKey name="server-name" supported={["python", "node", "ruby", "php", "java", "dart", "dotnet", "elixir"]} notSupported={["android"]}>
...
<ConfigKey name="before-breadcrumb" notSupported={["native", "unreal", "elixir"]}>
...
<PlatformSection supported={["elixir"]}>
...
<PlatformSection supported={["javascript", "java", "python", "node", "php", "dotnet", "dart", "elixir"]}>
...
<ConfigKey name="send-result" supported={["elixir"]}>
...
<ConfigKey name="send-max-attempts" supported={["elixir"]}>

The maximum number of attempts to send an event to Sentry. Defaults to `4`.

</ConfigKey>

<ConfigKey name="client" supported={["elixir"]}>
...
<ConfigKey name="hackney-pool-timeout" supported={["elixir"]}>
...
If <PlatformIdentifier name="traces-sample-rate" /> is 0 this means that no new traces will be created. But if you have another service (for example an JS frontend) that makes requests to your service that include trace information, that traces will be continued and thus transactions will be sent to Sentry.

If you want to disable tracing for good you need to set <PlatformIdentifier name="traces-sample-rate" />`=None` or set <PlatformIdentifier name="enable-tracing" />`=False`. In this case now new traces will be started and no incoming traces will be continued.
