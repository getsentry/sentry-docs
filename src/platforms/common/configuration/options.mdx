...
Client SDKs built on top of the Sentry SDK for .NET such Xamarin and Unity have this feature enabled by default. For example, in the case of Unity, the default value is `Application.persistentDataPath`. You can disable offline caching by setting it to `null`.
