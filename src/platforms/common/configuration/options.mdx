...
- If `considerFramesInAppByDefault` is set to `true`, you only need to maintain `inAppExcludes`.
- Conversely, if `considerFramesInAppByDefault` is set to `false`, you only need to maintain `inAppIncludes`.
