...
Specifies whether this SDK should send events to Sentry. Defaults to true. `enabled: false` doesn't prevent all overhead from Sentry instrumentation. To disable Sentry completely depending on environment, call `Sentry.init` conditionally.
