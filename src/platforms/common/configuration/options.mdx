...
<ConfigKey name="transaction_style" supported={["python.django"]}><markdown>

This can be used in our Django integration to set the transaction to `/myurl/{myid}` if you set `transaction_style="url"`, which matches the behavior of the deprecated Raven SDK. Alternately, use `my_function` if you set transaction_style="function_name"` as displayed in the example.
