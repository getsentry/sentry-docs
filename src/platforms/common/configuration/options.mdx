...
Sets the url that will be used to transport captured events instead of using the DSN. Can be used to work around ad-blockers issues or to have more granular
control over events sent to Sentry. This feature requires an implementation of custom server endpoint, see [Dealing with Ad-Blockers](platforms/javascript/troubleshooting/#dealing-with-ad-blockers) for more details and examples.
