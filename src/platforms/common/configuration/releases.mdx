...
The Crash Free Session Rate requires [native support](platforms/unity/configuration/native-support/). On platforms that don't yet have native support, the SDK reports sessions that did not end gracefully as _Abnormal_ which leads to 100% crash free sessions.
