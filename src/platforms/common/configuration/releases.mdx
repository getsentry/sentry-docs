...
A release is a version of your code that is deployed to an [environment](/platforms/python/configuration/environments/). When you give Sentry information about your releases, you can:
...
Include a release ID (often called a "version") when you initialise the SDK.
...
Setting the release name tags each event with that release name. We recommend that you tell Sentry about a new release before sending events with that release name, as this will unlock a few more features. See our documentation about [releases](/product/releases/).

If you do not tell Sentry about a new release, Sentry will automatically create a release entity in the system the first time it sees an event with that release ID.
...
A session represents the interaction between the user and the application. Sessions contain a timestamp, a duration, a status (if the session was OK or if it crashed) and others.

Sessions are always linked to a release and are used to monitor user adoption, usage of the application, percentage of [crashes](/product/releases/health/#crash), and [session data](/product/releases/health/#session) of that release.

This will give you insight into the impact of crashes and bugs as it relates to user experience, and reveal trends with each new issue through the release details, graphs, and filters.
