...
Your <PlatformIdentifier name="error-sampler" /> function must return a valid value. A valid value is a **floating-point number** between `0.0` and `1.0` (inclusive) indicating the probability an error gets sampled or a **boolean** indicating whether or not to sample the error.
