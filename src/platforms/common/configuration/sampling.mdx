...
<PlatformSection supported={["apple"]}>
In Sentry SDK for Apple, for convenience, the <PlatformIdentifier name="traces-sampler" /> function can return a SentrySampleDecision, so that a parent's decision can be returned directly if that's the desired behavior.
</PlatformSection>
