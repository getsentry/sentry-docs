...
View Hierarchy captured by the SDK represents the native user interface generated based on React Native components structure. Generally each React Element maps to a native component, but there are exception like [View Flattening](https://reactnative.dev/architecture/view-flattening) described in the React Native documentation.

Part of View Hierarchy are also native components wrapping the app created by React Native.

### iOS

The code snipped below show React Native Component and next to it native UIKit components as they will be included in the view hierarchy.
...
The code snipped below show React Native Component and next to it native Android components as they will be included in the view hierarchy.
