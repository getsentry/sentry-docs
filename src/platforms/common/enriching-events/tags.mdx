...
_Tag keys_ have a maximum length of 32 characters and can contain only letters (`a-zA-Z`), numbers (`0-9`), underscore (`_`), period (`.`), colon (`:`), and dash (`-`).

_Tag values_ have a maximum length of 200 characters and they cannot contain newline (`\n`) character.

<PlatformSection notSupported={["native.minidumps", "native.wasm", "php", "php.laravel", "php.symfony", "unreal"]}>
