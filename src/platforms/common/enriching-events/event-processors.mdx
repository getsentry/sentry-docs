...
- <PlatformIdentifier name="before-send" /> and <PlatformIdentifier name="before-send-transaction" /> are guaranteed to be run last, after all other event processors, which means they get the final version of the event right before it's sent (hence the name). Event processors added with either of the methods below run in an undetermined order, which means changes to the event may still be made after the event processor runs.
