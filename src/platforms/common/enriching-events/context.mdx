...
## Structured Context (It would be great to link depths here with structured context that are nested https://docs.sentry.io/platforms/javascript/configuration/options/#normalize-depth)
...
In addition to structured contexts, Sentry supports adding unstructured "Additional Data" via <PlatformIdentifier name="set-extra" />. **Additional Data is deprecated** in favor of structured contexts and should be avoided when possible. (Are there any cases when setExtra makes sense, can those situations be mentioned here?  Are both equally applicable to setting global metadata and event-level metadata?)
