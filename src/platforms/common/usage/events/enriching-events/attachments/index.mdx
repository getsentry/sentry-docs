...
<PlatformContent includePath="usage/events/enriching-events/attachment-init-with-path" />

Alternately, use `bytes` to initialize an attachment. When doing so, you also need to specify a filename.

<PlatformContent includePath="usage/events/enriching-events/attachment-init-with-bytes" />
...
Unity features a global <PlatformLink to="/usage/events/enriching-events/scopes/">scope</PlatformLink> to which you add an attachment that will be sent with every event.
...
<PlatformContent includePath="usage/events/enriching-events/attachment-max-size" />
