...
If a generic error, such as a database connection error, has many different stack traces and never groups together, you can completely overwrite Sentry's grouping by omitting `{{ default }}` from the array:
