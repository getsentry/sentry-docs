...
The [AssetBundle](https://api.flutter.dev/flutter/services/AssetBundle-class.html) Instrumentation provides insight into how long your App takes to load its assets, e.g. files.

The `AssetBundle` instrumentation can be enabled by wrapping up the `runApp` method with a `DefaultAssetBundle` and `SentryAssetBundle`.
...
The `SentryAssetBundle` instrumentation starts a span out of the active transaction bound to the scope for each `load`, `loadString` and `loadStructuredData` call, The SDK sets the span `operation` to `file.read`.
...
The `enableStructuredDataTracing` Feature is available in Beta are still in-progress and may have bugs. We recognize the irony.
