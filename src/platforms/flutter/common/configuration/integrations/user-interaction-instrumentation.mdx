...
The UI instrumentation tracing is disabled by default, but you can enable it by setting the `enableUserInteractionTracing` option to `true` and wrapping your app with the `SentryUserInteractionWidget` widget:
...
Breadcrumbs don't contain the `Widget`'s labels, texts, etc due to PII concerns, but in case you want to capture them, you can enable them by setting the `sendDefaultPii` option to `true`.
