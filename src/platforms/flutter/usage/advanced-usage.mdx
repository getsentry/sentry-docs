...
- Flutter `split-debug-info` and `obfuscate` flag isn't supported on iOS yet, but only on Android, if this feature is enabled, Dart stack traces are not useful
