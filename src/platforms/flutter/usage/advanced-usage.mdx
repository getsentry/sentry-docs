...
- Flutter `split-debug-info` and `obfuscate` flag aren't yet supported on iOS; they are supported only on Android. If this feature is enabled, Dart stack traces are not human readable.
