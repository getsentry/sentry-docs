...
The Sentry Flutter SDK includes Sentry Android SDK which bundles multiple native libraries, for multiple ABIs.
When building a Flutter app for a specific ABI using the `--target-platfort` argument, for example an ARM 32bit apk:

```sh
flutter build apk --target-platform=android-arm --split-per-abi
```

Make sure to include the `--split-per-abi` flag.
