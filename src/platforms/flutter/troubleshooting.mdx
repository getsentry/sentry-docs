...
- Issue's titles might be obfuscated as we rely on the `runtimeType`, but they may not be human-readable, See: [Obfuscate Caveat](https://flutter.dev/docs/deployment/obfuscate#caveat)
...
Flutter `split-debug-info` and `obfuscate` flags are supported on iOS/macOS; it requires compiling your app using the latest Flutter's `master` branch and the Sentry Flutter SDK `6.10.0` and above.
