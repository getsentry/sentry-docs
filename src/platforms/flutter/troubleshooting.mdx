...
For the symbolication of Native events (C/C++) on Android, you've to make a configuration which is described on the [Android Native Development Kit](/platforms/android/using-ndk/) page.

Specifically about `android:extractNativeLibs="true"` in the `AndroidManifest.xml` or `android.bundle.enableUncompressedNativeLibs=false` in the `gradle.properties`, otherwise you might not have a good symbolication experience.
