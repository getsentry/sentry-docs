...
For the symbolication of Native events (C/C++) on Android, follow the configuration guidelines in the [Android Native Development Kit](/platforms/android/using-ndk/) documentation.

It's important that you set `android:extractNativeLibs="true"` in the `AndroidManifest.xml` or `android.bundle.enableUncompressedNativeLibs=false` in the `gradle.properties`, or you might not have a good symbolication experience.
