...
There is a [Flutter issue](https://github.com/getsentry/sentry-dart/issues/1669) where apps can crash if taking screenshots in the background. 
...
For prior versions, you can configure the SDK to only take screenshots when the app is in the `resumed` state. Set `SentryFlutterOptions.attachScreenshotOnlyWhenResumed` to `true`.
