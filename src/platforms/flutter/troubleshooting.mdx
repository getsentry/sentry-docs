...
For prior version, you can configure the SDK to only take screenshots when the app is in the `resumed` state. Set `SentryFlutterOptions.attachScreenshotOnlyWhenResumed` to `true`.