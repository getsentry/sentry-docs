...
Tap on the button specified in step 2 of the configure section, wait for `idleTimeout` seconds (default is 3s) then check the performance page for a new transaction.
...
User Interactions tracing is enabled by default.
...
User Interction breadcrumbs are enabled by default.
...
Breadcrumbs by default exclude Widget labels and text to avoid capturing Personal Identifiable Information (PII).
...
You can also disable the idle timeout by setting it to `null`, but the transaction must be finished manually in this case.
