...
To _automatically_ capture transactions, you must first <PlatformLink to="/performance/">set up tracing in your app</PlatformLink> if you haven't already.
...
Currently, by default, the React Native SDK will only create child spans for fetch/XHR transactions out of the box. This means once you are done setting up your routing instrumentation, you will either see just a few fetch/XHR child spans or no children at all. To find out how to manually instrument your app, review our <PlatformLink to="/performance/manual/">Manual Instrumentation</PlatformLink>.
