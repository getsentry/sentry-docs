...
## Verify artifacts distribution match value configured in your SDK

Whenever you are using distribution identifier (`dist` configuration option in the SDK), it has to be matched during the source maps upload as well.
To do this, use `--dist` flag in the `sentry-cli` or `dist` option if you are using `@sentry/webpack-plugin`, and make sure that the same value is used in both places.

To verify that the distribution has been used and set correctly, open up the issue from the Sentry UI and check if the `dist` tag is present.
For the artifacts, go to the `Source Maps` page in the project settings, and verify that required files have this value set. If not, it will say `none` instead.
