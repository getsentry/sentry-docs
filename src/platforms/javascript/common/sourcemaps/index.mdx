...
Instead of using `SENTRY_RELEASE` directly, you can change it for an [appropriate env variable](https://github.com/getsentry/sentry-javascript/blob/master/packages/gatsby/gatsby-node.js) based on the environment you are going to deploy your application on.
...
Always make sure that the release value you are providing during Source Maps upload, matches the value used in the `Sentry.init` call.
