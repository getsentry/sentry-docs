...
In order to have working SourceMaps when using the Loader Script, you will have to [upload & host them publicly](/platforms/javascript/sourcemaps/uploading/hosting-publicly/).
...
If Performance Monitoring and/or Session Replay is enabled, we immediately need to fetch and initialize the bundle to make sure we are able to capture transactions/replays when the page loaded.
...
The loader script always includes a call to `Sentry.init` with some default configuration, including your DSN. If you want to [configure your SDK](../../configuration/options/) beyond that, you'll need a second script tag, in which you'll call `Sentry.onLoad`. This script must come _after_ the main loader script.
...
For Performance Monitoring, by default the SDK will be initialized with `tracesSampleRate: 1` and for Session Replay `replaysSessionSampleRate: 0.1` and `replaysOnErrorSampleRate: 1`. Put it words, we capture all pages with Performance Monitoring, Replays of 10% of all sessions as well as Replays for all sessions with an error.

## **Limitations for error-only capturing**
