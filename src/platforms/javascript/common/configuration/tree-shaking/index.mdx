...
: Replacing this flag with `true` will tree shake all code in the SDK that's related to the included compression web worker for Session Replay. This is only relevant when using [Session Replay](../../session-replay/). Enable this flag if you want to host a compression worker yourself - see XXXX for details. **We do not reccomend to enable this flag unless you are providing a custom worker URL.**
