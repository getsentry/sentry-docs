...
By default, the Sentry SDK sets up a list of <PlatformLink to="/integrations/">default integrations</PlatformLink> that extend your SDK functionality. You can also add <PlatformLink to="/integrations/custom/">custom integrations</PlatformLink> to your SDK configuration.

If you don't want to include default integrations in your config, you can <PlatformLink to="/configuration/options/#integration-configuration">disable</PlatformLink> them and add your custom array of integrations.

However, if you also want to tree shake the unused default integrations, you can do so by creating a `Client` yourself. By doing this, you essentially bypass `Sentry.init` which normally creates a `Client` for you.
