...
We recommend using [Sentry's Webpack plugin](https://github.com/getsentry/sentry-webpack-plugin) to configure source maps and uploadsthem automatically during the build:
...
Sentry needs access to your source maps in order to effectively fingerprint errors (via source code context expansion).
