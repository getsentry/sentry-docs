...
sidebar_order: 200
keywords: ["micro frontend", "multiplexed transport", "module federation", "module metadata"]
---
...
init({
  ...
  beforeSend: (event) => {
    if (event?.exception?.values?.[0].stacktrace.frames) {
      ...
      const routeTo = frames.findLast(frame => frame.module_metadata && frame.module_metadata.dsn) || [];
