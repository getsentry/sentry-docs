...
This integration captures all `Console API` calls and redirects them to Sentry using the `captureMessage` or `captureException` call depending on the level. It then retriggers to preserve default native behavior.
