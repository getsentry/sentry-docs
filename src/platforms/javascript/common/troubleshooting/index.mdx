...
Starting version `6.5.0` of the JavaScript SDK, it is possible to provide a `tunnel` option, which will tell the SDK to deliver the event to configured url, instead of using the DSN. This allows it to remove `sentry_key` from the query params, which is the main cause of being blocked by various ad-blockers, as well as not sending preflight requests, which was one of the requirements that forced us to send `sentry_key` this way.

To enable it, provide either a relative, or absolute url as `tunnel` option in your `Sentry.init` call:
...
Once configured, all events will be sent to `/tunnel` endpoint. This solution however, requires an additional configuration on the server, as the events now need to be parsed and redirected to Sentry. Here is an example code written in PHP:
...
With this setup, no events should ever get blocked by the ad-blocker, as they are never sent to 3rd party url.

### Using JavaScript Proxy API
