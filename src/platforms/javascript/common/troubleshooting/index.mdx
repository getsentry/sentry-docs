...
### Using a `tunnel` Option

Starting with version `6.5.0` of the JavaScript SDK, it is possible to provide a `tunnel` option, which tells the SDK to deliver the event to the configured URL, instead of using the DSN. This allows the SDK to remove `sentry_key` from the query params, which is the main reason events are blocked by ad-blockers. It also stops the SDK from sending preflight requests, which was one of the requirements that necessitated sending the `sentry_key` this way.

To enable it, provide either a relative, or absolute URL as a `tunnel` option in your `Sentry.init` call:
...
Once configured, all events will be sent to the `/tunnel` endpoint. This solution, however, requires an additional configuration on the server, as the events now need to be parsed and redirected to Sentry. Here is example code written in PHP:
...
With this setup, no events should ever be blocked by the ad-blocker, as they are never sent to a third party URL.
