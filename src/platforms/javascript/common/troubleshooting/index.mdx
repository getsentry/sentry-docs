...
If you are using the [Vite Bundler](https://vitejs.dev/) and a Sentry NPM package and you get an error like so:

```
Error: Could not resolve './{}.js' from node_modules/@sentry/utils/esm/index.js
```

This might be because [`define` option in your Vite config](https://vitejs.dev/config/shared-options.html#define) is string replacing some variable used by the Sentry SDK, like `global`, causing build errors. Vite recommends using `define` for CONSTANTS only, and not putting `process` or `global` into the options. To fix this build error, remove or update your `define` option functionality, like what is shown below:
