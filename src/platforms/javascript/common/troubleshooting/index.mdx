...
To enable the `tunnel` option, provide either a relative or an absolute URL in your `Sentry.init` call. A URL is relative to the current origin and this is the form that we recommend. Using relative url will not trigger a pre-flight CORS request and no events will be ever blocked as ad-blocker will not treat it as a 3rd party request.
