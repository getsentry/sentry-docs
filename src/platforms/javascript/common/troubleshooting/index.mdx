...
If your application started to misbehave because of performing additional OPTIONS requests, it is most likely the issue with unwanted `sentry-trace` request header.
This can happen when you are using too generic configuration for our Tracing Integration in the Browser SDK.
In order to fix this, change `tracingOrigins` option during SDK intialization.
For more details, see [Automatic Instrumentation](/platforms/javascript/performance/capturing/automatic/#tracingorigins) page of our Performance Monitoring docs.
