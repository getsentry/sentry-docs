...
Starting with version `6.5.0` of the JavaScript SDK, it is possible to use the `tunnel` option to tell the SDK to deliver events to the configured URL, instead of using the DSN. This allows the SDK to remove `sentry_key` from the query params, which is the main reason events are blocked by ad-blockers. It also stops the SDK from sending preflight requests, which was one of the requirements that necessitated sending the `sentry_key` this way.

To enable it, provide either a relative, or an absolute URL as the `tunnel` option in your `Sentry.init` call:
