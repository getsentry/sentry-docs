...
To capture additional information such as request and response headers or bodies, you'll need to opt-in via `networkDetailAllowUrls`. This gives you control to add only URLs that are safe to capture bodies, avoiding any endpoints that can contain personal identifiable information (i.e., PII).
...
If you want to capture additional headers, you'll have to configure them with the options `networkRequestHeaders` and `networkResponseHeaders`.

<Note>
  Captured bodies will be truncated to a maximum size of 150k characters. Content in
