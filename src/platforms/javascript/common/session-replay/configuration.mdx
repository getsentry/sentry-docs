...
To capture additional information such as headers or bodies, you'll need to opt-in via `networkDetailAllowUrls`. This gives you control to add only URLs that are safe to capture bodies, avoiding any endpoints that can contain personal identifiable information.
