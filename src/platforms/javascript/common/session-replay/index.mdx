...
Session Replay uses a WebWorker to perform work off the main UI thread so as not to degrade the performance of your application. Add the below entry to make sure that workers can be loaded from our CDN:


```
worker-src https://browser.sentry-cdn.com
```
