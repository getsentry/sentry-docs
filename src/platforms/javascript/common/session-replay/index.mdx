...
The session will capture any pageloads, refreshes or navigations as long as the SDK is re-initialized on the same domain, and in the same borwser tab, each time. Sessions continue capturing data until 5 minutes passes without any user interactions **or** until a maximum of 60 minutes have elapsed. Closing the browser tab will end the session immediately, according to the rules for [SessionStorage](https://developer.mozilla.org/en-US/docs/Web/API/Window/sessionStorage).
