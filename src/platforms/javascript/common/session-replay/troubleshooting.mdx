...
The Sentry SDK attempts to minimize potential [performance overhead](/product/session-replay/performance-overhead/#how-is-session-replay-optimized) in a few different ways. For example, by keeping track of how many DOM mutations are happening, then disabling recording when a large number of changes are detected. Many simultaneous mutations can slow down a web page whether Session Replay is installed or not, but when a large number of mutations happen, the Replay client can incur an additional slowdown because it records each change.
...
We've identified two major potential reasons for impacted performance and have added several performance degradation safeguards as a result. Please update the SDK to the latest version to benefit from all these improvements. The two culprits are websites that have a large numbers of mutations (Read the entry above to learn more.) and large console messages. 

We've added improvements, including [truncating the console](https://github.com/getsentry/sentry-javascript/pull/7917) and disabling Replay recordings on pages that have a large number of mutations. To see these updates in action, be sure you have the latest version of our SDK.
