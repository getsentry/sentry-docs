...
## Text in the iframe is not getting masked
  
If your iframe relies on `srcdoc`, the contents are taken as-is. As opposed to loading a resource via `src`. For that reason, our masking logic doesn't run through it.
<PlatformLink to="/session-replay/privacy/">In such case it's best to block the iframe</PlatformLink>.
