...
Another reason you might not see the body is if the format is not supported. We inspect the body and make an educated guess if it is JSON (starts and ends with `[`, `]` or `{`, `}`), otherwise we treat it as a string/array of bytes and take the first 150k bytes. XML is stringified, and will be captured (as well as other strings). `FormData` (and `URLSearchParams`) datatypes are also captured. Bodies for files or images are not captured.

More details about this feature in the <PlatformLink to="/session-replay/configuration/">configuration page</PlatformLink>.
