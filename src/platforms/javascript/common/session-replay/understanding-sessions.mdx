...
If it is sampled, it will start recording in `session` mode and begin sending Replay data immediately.

Else, if `replaysOnErrorSampleRate > 0`, it will start recording in `buffer` mode.
In this mode, when an error occurs, `replaysOnErrorSampleRate` is used to check if it should be sampled. If it is sampled, then the Replay will be uploaded to Sentry and continue recording normally.
...
In `buffer` mode, Replay will continuously record data, but will only keep up to the last 60 seconds in memory. When an error occurs, `replaysOnErrorSampleRate` will be checked and if it's sampled, the replay will be uploaded to Sentry and continue recording normally. After 15 minutes of user inactivity, or a maximum duration of 60 minutes, the session will end and the replay will stop.
