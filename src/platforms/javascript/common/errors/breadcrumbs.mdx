...
SDKs will automatically start recording breadcrumbs by enabling integrations, such as `Sentry.Integrations.Breadcrumbs`. Learn more in our documentation for [Default Integrations](../integrations/default/). In addition, the JavaScript SDK will automatically record all location changes and automatically records certain events, such as changes to the URL and XHR requests to add context to an error.
