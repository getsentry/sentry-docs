...
If you are using the CDN version or the Loader, we provide a standalone file for every integration, which you can use as follows:
...
<script
  ...
  integirty="sha384-{{ packages.checksum('sentry.javascript.browser', 'ember.min.js', 'sha384-base64') }}"
  crossorigin="anonymous"
></script>
