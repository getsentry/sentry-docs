...
The default behaviour is to only send sessions on error or when the main process ends. To send sessions when the main process is created, enable the `sendOnCreate` option.
