...
_(Available in version 4.13.0 and above)_
...
Sentry.enableANRDetection({ mainProcess: { captureStackTrace: true } }).then(
  () => {
...
// await Sentry.enableANRDetection({ mainProcess: { captureStackTrace: true } });
// runApp();
```
