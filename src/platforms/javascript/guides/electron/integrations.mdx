...
The Sentry Electron SDK uses `@sentry/node` in the main process and `@sentry/browser` in the renderers. A number of integrations are included by default that setup communication between Electrons multiple processes and include Electron specific context and breadcrumbs.
...
### `ElectronBreadcrumbs` (default)

Captures breadcrumbs for events for many of Electrons built-in modules. The breadcrumbs captured for each module can be configured `true` to capture all events, `false` to capture no events. Alternatively a method which is passed the event name and returns `true` or `false` depending on whether the event should be captured.
...
### `SentryMinidump` (default)

Captures minidumps and sends them with full context to the Sentry Envelope endpoint using a custom uploader.

### `ElectronMinidump`

Captures and sends minidumps via Electrons built in `crashReporter`. Renderer and GPU crashes will include minimal context.

If you add the `ElectronMinidump` integration, the `SentryMinidump` integration will be automatically removed.
