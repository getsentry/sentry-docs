...
If you change the `userData` directory used by your application, ensure this change is made before you configure the SDK as this path is used to cache scope and events between application restarts.
...
## Bunder Configuration

The SDK makes an effort to ensure JavaScript bundlers pick up the correct entry point for the Electron process type, but it's not foolproof. If the incorrect entry point is selected, your bundler may throw an error or failing that, an error will be thrown at runtime. If this occurs you can use relative imports to ensure that the correct code is bundled for the process.
...
To give detailed information for all events, the Electron SDK merges context, scope and breadcrumbs from all processes in the Electron `main` process.

By default, the SDK attempts to establish communication from `renderer` to `main` process via Electron's IPC APIs and if that fails, falls back to using a custom HTTP protocol. You can change this default via the `ipcMode` which can be one of `Classic`, `Protocol` or `Both`.
...
The SDK attempts to inject a preload script via the Electron [`session.setPreloads(preloads)` API](https://www.electronjs.org/docs/latest/api/session#sessetpreloadspreloads) and by default only does this for the `defaultSession`. If you are using other sessions, you can pass custom sessions via the `getSessions` option in the `main` process:
...
Sentry.init({
  dsn: "___PUBLIC_DSN___",
  getSessions: () => [session.fromPartition("persist:my-session")],
});
```

If you bundle the `main` process JavaScript, the SDK cannot automatically inject preload scripts because the file will be missing from the packaged application. In this case, the SDK will send events and scope updates via a custom HTTP protocol and `window.fetch`.

### Manual Preload

If you would like to manually bundle and configure the preload script, you should import the SDK preload into your own preload script:

```javascript
import "@sentry/electron/preload";
```

This script exposes IPC via Electrons `contextBridge` API.

See the [example apps](https://github.com/getsentry/sentry-electron/tree/master/examples) for how to do this.
