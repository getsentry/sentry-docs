...
Learn how to further configure and use the Sentry Error Boundary in <Link to="./components/errorboundary/">React Error Boundary</Link>
...
Learn more about how to manually capture errors or enable message capture with Sentry's React SDK in <Link to="./errors/capture">Capture Errors</Link>.
...
Learn more about the data sent with events in <Link to="./errors/">Enrich Event Data</Link>.
...
Learn more about what releases can do, including using a repository integration, creating the release and associated commits, and telling Sentry when you deploy a release in <Link to="/workflow/releases/">our documentation for Releases</Link>.
...
When you’re using the Sentry React SDK, the SDK automatically fetches the source code and source maps by scraping the URLs within the stack trace. If you need to disable the JavaScript source fetching, log in to Sentry and toggle off `Allow JavaScript source fetching` in your organization's settings for Security & Privacy. See our <Link to="/platforms/javascript/sourcemaps">source maps documentation</Link> to learn more.

Upload source maps using either our [Webpack plugin](/platforms/javascript/sourcemaps/#webpack) or the <Link to="cli/">Sentry CLI</Link>. Sourcemaps can be generated by [Uglify JS](/platforms/javascript/sourcemaps/#uglifyjs), [System JS](/platforms/javascript/sourcemaps/#systemjs), or [TypeScript](/platforms/javascript/sourcemaps/#typescript).
...
Learn more about using and customizing the React Profiler in <Link to="./components/profiler/">Profiler</Link>.

If you are using `react-router`, we provide `react-router` instrumentation you can use alongside Performance Monitoring with `@sentry/tracing`. Please see the <Link to="./integrations/react-router/">React Router Integration documentation</Link> for more details on setting it up.

Performance data is transmitted using a new event type called “transactions”, which you can learn about in <Link to="/performance-monitoring/distributed-tracing">Distributed Tracing</Link>. To capture transactions, you must install the performance package and configure your SDK to set the `tracesSampleRate` configuration to a nonzero value. The example configuration above will transmit 100% of captured transactions; lower this value in production to avoid consuming your quota too quickly.

Learn more about sampling in <Link to="./config/filter/">Filtering Events Reported to Sentry</Link>.
...
Learn more about the redux integration and it's options in <Link to="./integrations/redux/">Redux Integration</Link>

## Next Steps

- **<Link to="./components/">View Custom Components</Link>**
...
- **<Link to="configuration/">Manage Configuration Options</Link>**
...
- **<Link to="enriching-error-data/additional-data/">Enrich Event Data</Link>**
...
- **<Link to="./integrations/">Review and Manage Integrations</Link>**
