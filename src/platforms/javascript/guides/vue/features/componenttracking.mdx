...
description: "Learn how Sentry's Vue SDK allows you to track the rendering performance of your application and its components"
---
...
By default, the Vue SDK tracks the rendering performance of your app (i.e. its root component) on the initial page load. This operation is represented in the page load transaction by the **`ui.vue.render`** span.
...
To set it up, you onyl need to add a few options in your `Sentry.init` call:

### `trackComponents`

This is the main options that controls which child components should be tracked. Set it to `true` to track all or specify a list of individual components you want to track:
...
Control which lifecycle hooks should be tracked. This is helpful for example, if you want to know if some components are removed during the initial page load, in which case you can add a `destroy` hook to the default:
...
The default is `['activate', 'mount', 'update']`
...
Note that there is no change in behaviour as opposed to using the three top-level properties described above.

The defualt is value for `tracingOptions` is `undefined`.
