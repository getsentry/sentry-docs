...
To disable the automatic instrumentation of API route handlers and serverside data fetching functions, set the `autoInstrumentServerFunctions` to `false`.
...
If the automatic instrumentation doesn't work for your use case, you can turn it off globally and choose instead to only wrap specific API route handlers or data fetching functions.
...
Excluded routes can be specified either as strings (which must exactly match the full route you want to exclude, with a leading slash but no trailing one) or regexes.
