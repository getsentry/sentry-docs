...
To use Sentry with a NextJS application, you will need to use Sentry’s NextJS SDK: `@sentry/nextjs`.
...
Firstly, you need to extend NextJS’s usage of webpack by creating a custom `next.config.js` file (if there’s already one, we need to merge it manually):
...
Every error handled by the NextJS SDK has a `runtime` tag to indicate whether the error happened in the frontend (`browser`) or the backend (`node`).
