...
For any error, there’s a `runtime` tag to indicate whether the error happened in the frontend (`browser`) or the backend (`node`).
For example, since the error in the example above happens in the frontend, the issue contains the tag `runtime: browser`.
