...
redirect_from:
  ...
  - /sdks/nextjs/
  - /platforms/nextjs/
description: "Learn how to use Sentry’s Next.js SDK and how it automatically reports errors and exceptions in your application."
---

On this page, we provide concise information to get you up and running with Sentry’s Next.js SDK, automatically reporting errors and exceptions in your application.
...
To use Sentry with a Next.js application, you will need to use Sentry’s Next.js SDK: `@sentry/nextjs`.
...
- create `next.config.js` with the default configuration. If such file already exists, it will instead create `_next.config.js`, which will need to be merged manually.
- create `sentry.properties` with Sentry’s project-specific data. This file isn’t required if your environment variables are correctly set, see [Environment configuration](./configuration/environment-configuration) to learn more.
...
Firstly, you need to extend Next.js’s usage of webpack by creating a custom `next.config.js` file (if there’s already one, we need to merge it manually):
...
If you’re using TypeScript, make sure the compiler generates Source Maps:
...
To set Sentry’s project-specific data, you can create a `sentry.properties` file, or if you’re interested in using environment variables, see [Environment configuration](./configuration/environment-configuration) to avoid creating it:
...
You can trigger the first event from the development environment by raising an exception somewhere in the application. For example, throwing an error on a button click:
...
Every error handled by the Next.js SDK has a `runtime` tag to indicate whether the error happened in the frontend (`browser`) or the backend (`node`).
The error in the example above happens in the frontend, so the issue contains the tag `runtime: browser`.
