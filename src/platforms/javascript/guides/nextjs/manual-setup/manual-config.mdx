...
To initialize the SDK you must perform the initialization for both the client and the server in `sentry.client.config.js` and `sentry.server.config.js` respectively, in the root folder of your project.
Both files must initialize Sentry as in other SDKs:
