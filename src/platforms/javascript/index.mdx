...
If you include `defer` in your script tag, any errors that occur in scripts executed before the browser SDK script executes won’t be caught, since the SDK won’t yet be initialized. We strongly recommend that if you use `defer`, you both place the script tag for the browser SDK first and also mark it -- and all of your other scripts -- `defer` (but not `async`), guaranteeing that it’s executed before any of the others.

</markdown>
</Alert>

**Note:** If you have Content Security Policy (CSP) set up on your site, please see [our documentation about adding to the `script-src`](/platforms/javascript/common/troubleshooting/).

## Configure and Verify

`init` Sentry as soon as possible and include the verification of your setup with an intentionally sent error that breaks your application. For example, in JavaScript, calling an undefined function throws an exception::
