...
On this page, we get you up and running with Sentry's JavaScript SDK, automatically reporting errors and exceptions in your application.
...
If you are using CDN to install our SDK, please see our documentation for [installing with CDN](/platforms/javascript/configuration/install-cdn/).

If you have Content Security Policy (CSP) set up on your site, please see [our documentation about adding to the `script-src`](/platforms/javascript/troubleshooting/).
...
The snippet below includes an intentional error, so you can test that everything is working as soon as you set it up. To see the recorded error, log into sentry.io and open your project. Clicking on the error's title will open a page where you can see detailed information and mark it as resolved.
...
**Important:** Note your DSN. The _DSN_ (Data Source Name) tells the SDK where to send events. If you forget it, go to _Settings -> Projects -> Your Project -> Client Keys (DSN)_ in the Sentry web UI.

Resolve the created error by logging in to [sentry.io](https://sentry.io), then opening your project. On the **Issue Details** page, scroll down to the Exception stack trace and view the error.
