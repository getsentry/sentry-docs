...
Install performance monitoring using Sentry’s JavaScript SDK using both the `@sentry/browser` and `@sentry/tracing` packages, or link directly to our combined minified bundle via CDN:
...
If you are using CDN, please see our documentation for [installing with CDN](/platforms/javascript/configuration/install-cdn).
