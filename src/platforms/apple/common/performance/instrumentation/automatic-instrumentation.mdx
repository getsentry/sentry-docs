...
To enable this feature enable `AutoUIPerformanceTracking` and set a <PlatformLink to="/configuration/sampling/#setting-a-uniform-sample-rate">`tracesSampleRate`</PlatformLink> or a <PlatformLink to="/configuration/sampling/#setting-a-sampling-function">`tracesSampler`</PlatformLink>. The SDK differentiates between a cold and a warm start, and doesn't track hot starts/resumes.
