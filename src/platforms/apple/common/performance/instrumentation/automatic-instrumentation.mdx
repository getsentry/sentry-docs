...
The `UIViewController` Instrumentation captures transactions when your app loads an in-app [`UIViewController`][UIViewController]. However, the SDK doesn't capture transactions for the `UIViewController` of a third-party library or SwiftUI.
