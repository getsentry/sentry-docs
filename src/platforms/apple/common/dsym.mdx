...
At the start of Early Access the App Store Connect integration is limited to a single source per project.
Later on free tiers will remain limited to one App Store Connect source per project, Business plans will gain the ability to have multiple App Store Connect sources per project.
Take a look at our [plans](https://sentry.io/pricing/) or contact [sales@sentry.io](mailto:sales@sentry.io) if more than one App Store Connect source is required.
