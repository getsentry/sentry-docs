...
Note that the Cocoa SDK has no control over the stacktraces provided by MetricKit. Some stacktraces from MetricKit only offer a couple of frames. If these are not useful to you, you can drop such events in <PlatformLink to="/configuration/filtering/#filtering-error-events/">`beforeSend`</PlatformLink>. You can identify MetricKit events by looking at the `mechanism.type` of the `exception`, which are `mx_hang_diagnostic`, `mx_cpu_exception`, and `mx_disk_write_exception`.

To use this feature, enable it by setting the `enableMetricKit` option to `true`:
