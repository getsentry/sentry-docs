...
This feature is opt-in and can be enabled by setting the `enableCaptureFailedRequests` option to `true`:
...
This feature requires the setting `enableSwizzling` to be enabled as well.
...
Error events may contain PII data, such as `Headers` and `Cookies`, Sentry already does data scrubbing by default, but you can scrub any data before it is sent, learn more about it in the [Scrubbing Sensitive Data docs](/platforms/apple/guides/ios/data-management/sensitive-data/).
