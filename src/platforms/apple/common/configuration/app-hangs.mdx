...
The app hang detection integration has a default timeout of two (2) seconds, which means if the app becomes unresponsive for at least two seconds, the SDK creates an error event. We improved the algorithm to detect app hangs sooner with 7.24.0:
- Before 7.24.0 the app hang can last up to `appHangTimeoutInterval * 2` until the SDK reports an app hang.
- Since 7.24.0 the app hang can last up to `appHangTimeoutInterval * 1.2` until the SDK reports an app hang.
