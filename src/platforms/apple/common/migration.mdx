...
To upgrade from the 4.x version of the SDK to the 8.x version of the SDK, you must first migrate from 4.x to 5.x, then 5.x to 6.x, then 6.x to 7.x, and finally from 7.x to 8.x.
...
We removed the version of `SentrySDK.startWithOptions` which took a dictionary, and renamed `SentrySDK.startWithOptionsObject` to `SentrySDK.startWithOptions`. The recommended way to initialize Sentry has not changed:
...
The minimum Xcode version has been bumbed to 13.
