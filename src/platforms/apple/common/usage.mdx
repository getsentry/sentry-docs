...
Before version 6.3.0, the SDK marked all frames inside the application bundle as `inApp`. This change doesn't impact applications using static libraries, as when
using static libraries, all of them end up in the main executable. For dynamic libraries, this means that the SDK marked libraries inside the application bundle
as `inApp` before version 6.3.0.
