...
To highlight essential frames of your stacktrace, the Sentry UI only displays frames related to your application by default. Sentry marks such frames as inApp.
To achieve this, the SDK only sets the inApp flag for frames originating from the main executable by using the [CFBundleExecutable].
Public frameworks such as UIKitCore, CoreFoundation, GraphicsServices, etc., are always marked as not inApp.
For private frameworks, such as Sentry, there is a difference between dynamic or static frameworks. If you are not familiar with these terms, we recommend
...
you can use inAppIncludes or inAppExcludes of the SentryOptions.
...
originates from your application or a private framework and marks all of them as inApp. To improve your experience, use <PlatformLink to="/data-management/event-grouping/stack-trace-rules/#mark-in-app-frames">stack trace rules</PlatformLink> to tell Sentry which it should mark not as inApp.
