...
Sentry needs to write files in a specific path in the app cache directory. This path is created during the SDK initialization.
If you see a log that looks like this: `[SentryFileManager:439] Failed to write data to path` it means that the cache path don`t exist anymore.
Check your app for any procedure that cleans the cache of your app during runtime, and avoid deleting Sentry directory.
