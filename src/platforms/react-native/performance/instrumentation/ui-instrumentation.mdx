...
<Note>
Capturing UI transactions requires that you first set up routing instrumentation if you haven't already.
</Note>
...
Transactions are always bound to the Scope automatically if there's none set. Because of that, you can create spans using manual instrumentation (/platforms/react-native/performance/instrumentation/custom-instrumentation/#retrieve-a-transaction), and those spans will be automatically associated with the running UI transaction.
