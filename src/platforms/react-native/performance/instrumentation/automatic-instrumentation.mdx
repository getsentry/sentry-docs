...
When no routing instrumentation is used, a transaction for `App Start` is automatically captured. That stops being sent when one of the routing integrations below is added. That information lives as a Span in a transaction captured by the routing instrumentation instead.
