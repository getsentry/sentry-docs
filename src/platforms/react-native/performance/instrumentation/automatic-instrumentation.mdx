...
The UI instrumentation captures transactions and adds breadcrumbs for touch interactions. Gesture support using React Native Gesture Handler is also available with the `sentryTraceGesture` wrapper. Learn more about the [User Interaction Instrumentation](/platforms/react-native/performance/instrumentation/user-interaction-instrumentation).
