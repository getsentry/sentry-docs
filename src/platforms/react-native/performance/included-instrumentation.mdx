...
sidebar_order: 10
<<<<<<< HEAD:src/platforms/common/performance/capturing/automatic.mdx
supported:
  - javascript
  - react-native
  - dotnet
...
<PlatformSection supported={["javascript", "dotnet"]} notSupported={["react-native"]}>
...
Automatic transaction instrumentation is currently only supported in `Sentry.AspNetCore` package, starting with version 3.0.0. We will continue adding supprot for other platforms in the future. If you are not using ASP.NET Core, you can still benefit from Sentry's Performance Monitoring using <PlatformLink to="/performance/capturing/manual/">Manual Instrumentation</PlatformLink> instead.
</PlatformSection>
...
<PlatformSection supported={["dotnet"]}>
The ASP.NET Core integration creates a new transtion for each incoming HTTP request to your web application. Each transaction is identified by the route that handled the request. The integration currently does not automatically create spans for outgoing HTTP requests.

Learn more about [traces, transactions, and spans](/product/performance/distributed-tracing/).
</PlatformSection>
...
<PlatformSection supported={["dotnet"]}>
To enable automatic tracing for ASP.NET Core, install the `Sentry.AspNetCore` NuGet package and include Sentry's tracing middleware in your request pipeline:
...
public class Startup
{
    ...
    {
        // If you use the static files middleware, place it before
        // the tracing middleware to avoid capturing requests to static
        // files.
        app.UseStaticFiles();

        // Routing middleware needs to be inserted before the tracing middleware
        app.UseRouting();

        // Enable the tracing integration
        app.UseSentryTracing();

        // Endpoints middleware needs to be inserted after the tracing middleware
...
Automatic instrumentation is currently in beta, available on the Sentry React Native SDK version ≥ 2.2.0-beta.0. As a result, the API may change without warning.
>>>>>>> upstream/master:src/platforms/react-native/performance/included-instrumentation.mdx
</Alert>
