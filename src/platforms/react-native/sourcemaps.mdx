...
Before going ahead and disabling the automatic source maps upload, you may set the `release` and `dist` as environment variables, `SENTRY_RELEASE` and `SENTRY_DIST` respectively. The script that does the automatic source maps upload will use those values instead the default ones and they will match the given `release` and `dist`.
...
In case you've exported the environment variables and they match the given `release` and `dist` during the `Sentry.init` call, you can skip the next steps.
