...
If you're using [expo-dev-client](https://docs.expo.dev/development/introduction/#what-is-an-expo-dev-client), you might notice that transactions never finish in your dev builds. This is due to logs that the dev client is sending to the development server. To fix this behavior in your dev builds, you can stop creating spans for the HTTP requests to the log endpoint. To do this, add the following to your `Sentry.init()`:
