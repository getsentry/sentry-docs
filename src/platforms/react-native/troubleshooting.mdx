...
You may need to perform some extra steps when using the source maps with the Hermes engine. [You can follow the guide here.](/platforms/react-native/manual-setup/hermes/#custom-source-maps) Although the build script should already handle this for you most of the time.

### Different Bundles for Different Platforms

Your app could have different bundles for each platform that your app supports such as iOS and Android. To associate the correct bundle and sourcemap to each platform you will need a unique release/dist combination for each platform, an easy way to do this is to include the platform name in the `dist` value.
