...
In cases, you are building your app without access to `sentry.io` or some other unexpected error occurs. You can allow failure of the `sentry-cli` step by adding an environmental variable `SENTRY_ALLOW_FAILURE` equal to `true`. The CLI will still run, but will not fail the build.
