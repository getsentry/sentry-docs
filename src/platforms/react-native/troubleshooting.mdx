...
If you are using nvm or notion, Xcode has trouble locating the default node binary, you can add `node` binary path to the _Bundle React Native code and images_ build phase like this:
...
## Setup SKD v4 with React Native 0.69 and Higher

To install an older version of the `@sentry/react-native` you need to specify it in the install command. Similarly, you will need to use compatible `@sentry/wizard` as shown in the script below.
...
If your build is failing due to outdate fields in `react-native.config.js`. Update the config file, located in `node_modules/@sentry/react-native/react-native.config.js`. This is required on React Native 0.69. The newer versions only show warning, but that can change.
