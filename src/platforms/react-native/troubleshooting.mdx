...
This should work on Android, however, on iOS, it'd still fail in the `Bundle React Native code and images` build step phase and for that, you've to revert the patched `shellScript` in the `ios/$projectName.xcodeproj/project.pbxproj` file where `$projectName` is likely your project's name.
...
Because of that, the auto-uploading of sourcemaps won't work on iOS and you've to do it manually, you'll find more info on how to do that manually in the sourcemaps [docs](/platforms/react-native/sourcemaps/).
