...
## Transaction Data Contains Sensitve Information

The SDK might send sensitive data such as username and password for auto-generated transactions before 5.15.2 from navigation routing parameters. You can fix this issue by either updating to [5.15.2](https://github.com/getsentry/sentry-react-native/releases/tag/5.15.2) or higher or dropping the data manually with `beforeSendTransaction`:
...
You can also use [Advanced Data Scrubbing](/product/data-management-settings/scrubbing/advanced-datascrubbing/) to filter out sensitive data before it is stored on the server side:
