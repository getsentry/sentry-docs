...
New feature that enables sending runtime JS dependencies in Events for React Native apps was added in `4.9.0` release. There are no manual steps required to enable this on Android builds. For iOS, you need to patch the `shellScript` in the `ios/$projectName.xcodeproj/project.pbxproj` file, where `$projectName` is likely your project's name. Search for the `Bundle React Native code and images` phase and patch the `shellScript`.
