...
The following metadata are sync from `sentry-cocoa` to `react-native`:
- `tags`
- `extra`
- `fingerprint`
- `level`
...
The synchronized `fingerprint` might affect grouping of issues in Sentry.
...
`sentry-wizard` doesn't patch iOS project for React Native 0.68 to 0.65. These have to be patched [manually](/platforms/react-native/manual-setup/manual-setup/#bundle-react-native-code-and-images).
