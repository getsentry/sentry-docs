...
1. Bundle/minify with `metro` (`react-native`) to get the bundle (`.bundle` or `.jsbundle`) and packager source map (`.packager.map`):
...
mv main.jsbundle.packager.map main.jsbundle.map
```
