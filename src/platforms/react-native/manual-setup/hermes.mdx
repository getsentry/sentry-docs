...
If you are using Hermes with **React Native 0.68 or below** use:
...
Since **React Native 0.69** `hermes` has been [shipped with React Native](https://reactnative.dev/blog/2022/06/21/version-069#bundled-hermes) so we will need to use `hermesc` from `react-native`:
```bash {tabTitle:Android}
