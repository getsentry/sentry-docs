...
`--version-name` comes from appcenter and has the `{}@{}+codepush:{}` pattern (see [here](https://github.com/getsentry/sentry-cli/blob/e08e7c31d93a29797edf7d1e5ec42ee0453ecc49/src/utils/appcenter.rs#L191-L194)), it should be the value within the 2nd `{}`.
...
If you don't want to use the automatically computed release format, instead of passing `--bundle-id` and `--version-name` you will need to pass the `--release-name` parameter.
