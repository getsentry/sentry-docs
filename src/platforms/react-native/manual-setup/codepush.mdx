...
When you set a custom `release` and `dist` or manually upload source maps, you will need to disable the automatic source map upload script, check out the [Disable the automatic source maps upload script](/platforms/react-native/sourcemaps/#2-disable-the-automatic-source-maps-upload-script) docs.
