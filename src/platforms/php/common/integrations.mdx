...
Also, `E_ALL` defaults to the current PHP reporting level. You can explicitly set the option by setting `error_types` to a different constant in `\Sentry\init()`.
