...
Also, `E_ALL` defaults to the value returned by the `error_reporting()` function, which can change at runtime. Alternately, you can set the option to a fixed value by setting `error_types` to a different constant in `\Sentry\init()`. In this case, Sentry will log errors of that specific type regardless of the current reporting level.
...
You can customize the list of integration without disabling the default integration using the `integrations` option. You can pass a callable in advanced needs, though a fixed list of integrations will work in use cases.
