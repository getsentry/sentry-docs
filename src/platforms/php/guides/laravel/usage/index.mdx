...
Separately to capturing you can also record the breadcrumbs that lead up to an event. Breadcrumbs are different from events: they will not create an event in Sentry, but will be buffered until the next event is sent. Learn more about breadcrumbs in our <PlatformLink to="/usage/events/enriching-events/breadcrumbs/">Breadcrumbs documentation</PlatformLink>.
