...
Starting with Laravel 5.3 we can automatically add the authenticated user id to the scope if [`send_default_pii`](configuration/filtering/#send-default-pii) option is set to `true` in your `config/sentry.php`.
