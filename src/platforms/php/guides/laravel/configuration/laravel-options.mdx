...
Sometimes the SDK requires a closure as an option value. The causes problems when using `php artisan config:cache` resulting in the `Your configuration files are not serializable` error.closure
