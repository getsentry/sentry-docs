...
The Excimer PHP extension supports PHP 7.2 and up. Use Excimer version 1.1.0 for PHP 8.2. 
...
## Installation
Here's how to install the Linux package manager, PECL, or build from a source.
...
Sentry's performance monitoring product has to be enabled in order for Profiling to work. Check out the <PlatformLink to="/performance/">performance setup</PlatformLink> documentation for more detailed information.
...
Using the profiling capabilities in your Laravel application may has some impact on your overall response time (depending on the `traces_sample_rate` you configured).
