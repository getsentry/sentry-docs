...
### Sentry Reporting Using Exception Handler
...
### Alternative: Log Channel

Add Sentry as a log channel in `config/logging.php`. Then configure your app to both log to a log file and to Sentry by modifying the log stack:
...
You can further [configure the log channel](usage/#log-channels) to set the logging level, so you can log `info` and `debug` as well.

<Alert level="info" title="Note">

If you're using log channels to log your exceptions and are also logging exceptions to Sentry in your [exception handler](#sentry-reporting-via-exception-handler), exceptions might show up in Sentry twice.
