...
### Sentry reporting via Exception Handler
...
Add Sentry as a log channel in `config/logging.php`, and configure your app to both log to a log file and to Sentry by modifying the log stack:
...
If you're using log channels to log your exceptions and are also logging exceptions to Sentry in your [exception handler](#sentry-reporting-via-exception-handler), exceptions might end up twice in Sentry.
