...
If you use Monolog, you can use it to report events instead of the normal error listener approach. To do so, you must turn `register_error_listener` to `false` and configure our `Handler` as a service, like this:

```yaml {filename:config/packages/sentry.yaml}
sentry:
  dsn: "___PUBLIC_DSN___"
  register_error_listener: false  # avoids double reporting
  # ...
