...
The plugin will automatically generate appropriate ProGuard mapping files and upload them when you run `gradle assemble{BuildVariant}` or `gradle assembleRelease`. Release is the default, but the plugin works for other variants if you have enabled ProGuard/R8.
The credentials for the upload step are loaded through environment variables or from a `sentry.properties` file in your project root.
