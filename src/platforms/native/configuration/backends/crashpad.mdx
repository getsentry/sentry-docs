...
Multi-process use cases are also supported. Initialize Sentry individually
in each process which spawns its own Crashpad handler either in a separate
database path, or using a shared database path. Each run uses its individual
run directory, and there is filesystem-level locking in place.
