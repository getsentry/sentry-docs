...
- [`crashpad`](crashpad/): This uses the out-of-process crashpad handler. It is currently
...
`Breakpad` and `inproc` being "in-process" means that they run in the same process
as the application whose crashes they are meant to capture. Consequently they cannot
send a crash to Sentry immediately after it happens. Instead, the crash report is
written to disk and sent the next time the application is run. `Crashpad`, running
in a separate process, does not suffer from this limitation.
