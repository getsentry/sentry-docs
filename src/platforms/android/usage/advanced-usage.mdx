...
To prevent this, disable the [`allowNativeHeapPointerTagging`](https://developer.android.com/guide/topics/manifest/application-element#allowNativeHeapPointerTagging) configuration option. You only need to do that if you are using `sentry-android` < 3.1, as this is fixed in `sentry-android` >= 3.1.
