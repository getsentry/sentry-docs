...
Often users want to set context data in the Java layer, and get that when a native crash happen. For that, we need to synchronize the Java `Scope` with the 
