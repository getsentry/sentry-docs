...
* `sentry-android-okhttp`, `sentry-android-fragment` and `sentry-android-timber` packages now declare their upstream dependencies as `compileOnly`. Please make sure to have `okhttp`, `fragment` and `timber` dependencies on the classpath if you are using those integrations, otherwise, the application will crash with a `NoClassDefFoundError` exception.
