...
- Minimal supported API level is updated to 19 (`minSdk`)
- `sentry-android-okhttp` classes were deprecated in favour of `sentry-okhttp`, which is a pure Java module and can be used in non-Android projects
  - `SentryOkHttpUtils` class was removed from public API. If you were using it, consider filing a [feature request](https://github.com/getsentry/sentry-java/issues)
- If you're using `sentry-kotlin-extensions`, it requires `kotlinx-coroutines-core` version `1.6.1` or higher now
- Move `enableNdk` and `enableScopeSync` from `SentryOptions` to `SentryAndroidOptions`
- `SentryApolloInterceptor.BeforeSpanCallback` changed return type from `ISpan` to `ISpan?`

### Behavioural Changes

- `Sentry.getSpan()` returns the root span/transaction instead of the latest span
  - This will make the span hierarchy leaner and better readable
- Capture [failed HTTP requests](/platforms/android/integrations/okhttp/#http-client-errors) by default
- Add deadline timeout for automatic transactions
  - This affects all automatically generated transactions on Android (UI, clicks), the default timeout is 30s, meaning the automatic transaction will be force-finished with status `deadline_exceeded` when reaching the deadline
- Set ip_address to {{auto}} by default, even if sendDefaultPII is disabled
  - Instead use the "Prevent Storing of IP Addresses" option in the "Security & Privacy" project settings on sentry.io
- The `maxSpans` setting (defaults to 1000) is enforced for nested child spans which means a single transaction can have `maxSpans` number of children (nested or not) at most
