...
The package `io.sentry.core` has been renamed to `io.sentry`, do a String replace on all the usages from `io.sentry.core.` to `io.sentry.` that it should compile normally.

Other than that, the APIs didn't change. Release heath is enabled by default though, if you prefer not to track the health of your releases, disable it with:
