...
## Sentry Android Gradle Plugin fails; how do I debug it?

You can set the debug log flag as environment variable, which is picked up by [Sentry CLI](/product/cli/).
And in addition, set the ``-stacktrace` flag:

```
export SENTRY_LOG_LEVEL=debug && ./gradlew build --stacktrace
```

With that information it might be more clear what's going on. If not, please provide this information when opening an [issue on GitHub]([url](https://github.com/getsentry/sentry-android-gradle-plugin)). 
