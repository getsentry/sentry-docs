...
If you are facing a `StackOverflowError` when trying to trigger a Gradle Sync in Android Studio or during the Gradle build, most likely this is [the issue](https://issuetracker.google.com/issues/232075280) on the Android Gradle Plugin's (AGP) site. The issue is only present in version `7.2.0` of AGP and can be fixed as follows:
...
If the problem persists, most likely this is caused by some other Gradle plugin that is processing `.pom` files. In this case, the problem can be worked around by upgrading the `sentry-android` or `sentry-android-core` dependency to version `6.3.0` or above:
...
If you're declaring `sentry-android-core:6.x` dependency **not in** your main `app` module, but in one of the submodules and are using the Sentry Android Gradle Plugin version `3.1.0`, the application will crash at runtime with a `NoSuchMethodError`, because the plugin will try to install the `sentry-android-ndk` with an incompatible version `5.7.0`. To fix this problem you can update the Sentry Android Gradle Plugin to version `3.1.1` or above:
