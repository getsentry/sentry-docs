...
The SDK uses the `SentryOkHttpInterceptor` to support [Distributed Tracing](https://docs.sentry.io/product/sentry-basics/tracing/distributed-tracing/) and creates a span for any http call.
Through the `SentryOkHttpEventListener`, the SDK can give more in depth information, creating a span for each operation reported by OkHttp: dns setup, proxy selection, http connect, ssl setup, send request headers, send request body, receive response headers and receive response body.
...
The event listener can propagate calls to another `EventListener` or `EventListener.Factory`, set on the `SentryOkHttpEventListener` once you create your `OkHttpClient` instance.
