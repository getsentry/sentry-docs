...
implementation 'io.sentry:sentry-apollo3:{{ packages.version('sentry.java.apollo3', '6.1.4') }}'
```

For other dependency managers, see the [central Maven repository](https://search.maven.org/artifact/io.sentry/sentry-apollo3).
...
import io.sentry.apollo3.ApolloBuilderExtensionsKt;
...
When using a custom `NetworkTransport`, the `SentryInterceptors` need to be added manually, because then `HttpInterceptors` need to be added to the `NetworkTransport`.
