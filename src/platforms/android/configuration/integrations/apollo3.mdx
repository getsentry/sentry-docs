...
To customize or drop the error event, you'll need to do a [manual initialization](/platforms/android/configuration/manual-config/#initialize) of the Sentry Android SDK. The captured error event can then be customized or dropped with a `BeforeSendCallback`:
