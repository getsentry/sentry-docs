...
In general, no further configuration required as the auto-instrumentation is enabled by default. If you would like to disable the file I/O instrumentation feature, we expose a configuration option for that:
...
To view the recorded transaction, log into [sentry.io](https://sentry.io) and open your project. Clicking on the Performance tab will open a page with transactions, where you can select just recorded transaction with the name `Track Interaction`. The event will look similar to this:
...
Alternatively, if you are not using our Gradle Plugin, the Android SDK provides a capability to manually instrument File I/O operations through the custom Sentry-specific implementations.
