...
The `sentry-android-navigation` library provides [AndroidX Navigation](https://developer.android.com/jetpack/androidx/releases/navigation) support for Sentry using the [SentryNavigationListener](https://github.com/getsentry/sentry-java/blob/a5f30b43b1dad2634ce020809f3b52e0d564a22a/sentry-android-navigation/src/main/java/io/sentry/android/navigation/SentryNavigationListener.kt). The source can be found [on GitHub](https://github.com/getsentry/sentry-java/tree/main/sentry-android-navigation/src/main/java/io/sentry/android/navigation).

On this page, we get you up and running with Sentry's Navigation Integration, so that it will automatically add a breadcrumb and start an idle transaction for each navigation event.

## Install

Sentry captures data by adding a `NavController.OnDestinationChangedListener`. To add the Navigation integration, initialize the [Android SDK](/platforms/android/), then add the `sentry-android-navigation` dependency. Using Gradle:
...
Configuration should happen in the respective lifecyce callbacks, once you retrieve your `NavController` instance.
...
By default, the navigation transaction finishes automatically after it reaches the specified [idleTimeout](/platforms/android/configuration/options/#idle-timeout) and all of its child spans are finished. The `idleTimeoout` defaults to `3000` milliseconds (three seconds), but you can customize it to your needs.
...
## Customize the Recorded Breadcrumb/Transaciton

By default, the Navigation integration captures route arguments as additional data on breadcrumbs and transactions. In case the arguments contain any PII data, you can strip it out via `BeforeBreadcrumbCallback` and `EventProcessor` respectively:
