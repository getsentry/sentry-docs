...
This feature relies on Automatic Activity Instrumentation, check the <PlatformLink to="/performance/instrumentation/automatic-instrumentation/">performance documentation</PlatformLink> on how to get started first.

</Note>

Once enabled you'll see two spans inside your transactions:
* `ui.compose.composition`, describing the time it took to execute your `@Composable`
* `ui.compose.rendering`, describing the time it took to render your `@Composable` onto the canvas

![Jetpack Compose Performance Metrics](jetpack-compose-performance-metrics.png)

In order to use it, you need to decorate your `@Composable` body with the `SentryTraced` function.
```kotlin
...
The `SentryTraced` function also provides a `enableUserInteractionTracing` argument, which automatically applies a `Modifier.testTag()`, thus automatically enabling the _User Interactions_ feature mentioned below as well.
...
The Sentry User Interactions feature for Jetpack Compose can detect click, scroll and swipe gestures. For every gesture the SDK can automatically collects breadcrumbs and launch transactions.
