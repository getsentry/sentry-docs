...
The `sentry-compose` library provides [Jetpack Compose](https://developer.android.com/jetpack/androidx/releases/compose) support for the following interactions:
* User interactions like clicks, swipe and scroll gestures
...
The Sentry SDK utilizes the built-in `Modifier.testTag("<tag>")` of a Composable to determine it's identifier. Here's an example:
...
If you want to customize the recorded breadcrumbs / transactions you can skip [to section below](#customize-the-recorded-breadcrumbtransaction).

</Note>

## Jetpack Compose Navigation

On this section, we get you up and running with Sentry's Compose Navigation Integration, so that it will automatically add a breadcrumb and start a transaction for each navigation event.
