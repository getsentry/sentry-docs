...
With Jetpack Compose performance metrics the Sentry Android SDK can automatically measure the initial `composition` and `rendering` time of your `@Composable` UI elements.
...
The `SentryTraced` function also provides a `enableUserInteractionTracing` argument, which automatically applies a `Modifier.testTag()`, thus automatically enabling the User Interaction feature mentioned below as well.
