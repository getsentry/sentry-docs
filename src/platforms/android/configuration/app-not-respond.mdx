...
In case [ApplicationExitInfo#getTraceInputStream](https://developer.android.com/reference/android/app/ApplicationExitInfo#getTraceInputStream()) returns `null`, the SDK no longer reports an ANR event as these events are not actionable without it.
...
By default, the SDK only reports and enriches the latest ANR and only this one counts towards ANR rate. However, there's
an `setReportHistoricalAnrs` option available in `SentryOptions`, which enables the SDK to report all of the ANRs from the [getHistoricalExitReasons](https://developer.android.com/reference/android/app/ActivityManager?hl=en#getHistoricalProcessExitReasons(java.lang.String,%20int,%20int)) list:
...
This option is useful when updating the SDK to the version where the new ANR implementation has been introduced, to report 
all ANRs happened prior to the SDK update. After that, the SDK will always pick up the latest ANR from the historical exit 
reasons list on next app restart, so there will be no historical ANRs to report.

The integration reports ANR events with `mechanism:HistoricalAppExitInfo`.

### Attaching Thread Dump

The SDK provides a capability to send ANR thread dump from [ApplicationExitInfo#getTraceInputStream](https://developer.android.com/reference/android/app/ApplicationExitInfo#getTraceInputStream()) as attachment. This might be useful to perform 
deeper investigations based on all available information from the OS (in addition to the SDK parsing the thread dump into 
threads with stacktraces):
