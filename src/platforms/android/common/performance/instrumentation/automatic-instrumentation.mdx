...
_(New in version 6.10.0)_
 ...
 The SDK composes the transaction name out of the host `Activity` and the `tag` set via `Modifier.testTag("<tag>")` of the Composable. For example, `LoginActivity.login_button`. The transaction operation is set to `ui.action` plus the interaction type (one of `click`, `scroll` or `swipe`).
