...
This is a preview API, available in `5.1.0` and above. Features available in a preview API are still in progress and may have bugs. We recognize the irony. The API may be renamed, changed, or removed in a future version.
...
Sentry uses the AndroidX.Core library for detecting Slow and Frozen Frames. This is necessary for having an accurate result across all the Android OS versions.

We check at runtime for availability, so if you're not using AndroidX.Core, you can remove it from Sentry's transitive dependencies.
...
Be aware that by removing this transitive dependency, the Slow and Frozen Frames won't be reported.
