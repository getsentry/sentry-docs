...
At the moment, only a standard `androidx.room` usage is supported, i.e. SQL queries for any other `SupportSQLiteOpenHelper.Factory` than [androidx.sqlite](https://github.com/androidx/androidx/tree/androidx-main/sqlite) will not be reported. It's worth reporting an issue though, so we can add a new integration, check [this issue](https://github.com/getsentry/sentry-android-gradle-plugin/issues/234) for more details.
