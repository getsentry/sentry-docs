...
- **Slow frames** are captured by the SDK when the app needs more than 16 ms for a frame. Typically, a phone or tablet renders with 60 frames per second (fps). With 60 fps, every frame has 16 ms to render; slower than that, and the app has slow frames.
