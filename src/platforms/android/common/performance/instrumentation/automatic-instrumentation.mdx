...
The [Sentry Android Gradle Plugin](/platforms/android/gradle/) does the tracing auto instrumentation using bytecode manipulation for `java.io.FileInputStream`, `java.io.FileOutputStream`, `java.io.FileReader` and `java.io.FileWriter`.

The plugin replaces the aforementioned classes with custom Sentry-specific implementations.

The Sentry-specific File I/O implementation starts a span out of the active span bound to the scope for each File I/O operation. The SDK sets the span `operation` to `file.write`/`file.read` and `description` to the SQL Query if available.
