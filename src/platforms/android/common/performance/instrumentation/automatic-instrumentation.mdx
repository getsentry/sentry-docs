...
### Android's Fragment's Instrumentation

The Fragment's instrumentation, once enabled, starts a span for each launch of a Fragment. The SDK sets the Span operation to `ui.fragment.load` and the Span description to the Fragment's name, for example, `LoginFragment`.

The span starts after each Fragment's `onCreate` and before `onCreateView` method is called.

The span finishes after each Fragment's `onResume` method is executed.

The Android's Fragment's Instrumentation depends on having an active transaction bound to the scope, ideally it'd be used along with the [Android's Activity Instrumentation](/platforms/android/performance/instrumentation/automatic-instrumentation/#androids-activity-instrumentation) which starts a transaction and bind it to the scope automatically.

For more information see our [Fragment](/platforms/android/configuration/integrations/fragment/).
