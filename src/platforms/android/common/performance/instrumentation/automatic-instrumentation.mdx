...
If the `@Composable` doesn't have a `testTag` modifier applied, the transaction won't be captured because it can't be uniquely identified. Thus either a `testTag` modifier needs to be applied or the automatic `@Composable` tagging needs to be enabled.
