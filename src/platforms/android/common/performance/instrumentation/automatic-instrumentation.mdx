...
If the view does not have the `id` assigned, the transaction will not be captured, because it cannot be uniquely identified otherwise.

</Note>

_The UI instrumentation is disabled by default, but you may enable it by setting:_
...
The transaction finishes automatically after the specified [idleTimeout](/platforms/android/configuration/options/#idle-timeout) and all of its child spans are finished. The `idleTimeoout` defaults to 3 seconds; you can also disable the idle timeout by setting it to `null` - the transaction must be finished manually in this case.

<Note>

If the UI transaction has idled, but didn't have any child spans added, it will be dropped, because the transaction alone does not bring any value.

</Note>

To finish the transaction manually, you can do:
...
In case the host Activity transitions into a non-interactive state (e.g. `onPause`), the UI transaction will be scheduled to finish automatically (as soon as all of its child spans are finished).

Transactions are always bound to the Scope automatically if there's none set. Because of that, you may create spans using manual instrumentation, and those spans will be automatically associated with the running UI transaction.
...
When the UI transaction is not finished yet, but the user makes a new interaction, the SDK automatically finishes the previous UI transaction. This is due to the fact that only one transaction at a time can be bound to the Scope. Note, however, that if the same view has been interacted with (e.g. a `RecyclerView` was scrolled again within the 3 seconds window), the transaction time will be extended with the `idleTimeout` value.
