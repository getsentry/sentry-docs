...
The scope inside of the `withScope()` callback is only valid inside of the callback - once the callback ended, the scope will be removed and no longer applied. The callback scope is only applied to events that are captured inside of the callback. `withScope()` will clone (or fork) the current scope, so that the current scope is not modified. This allows you to
