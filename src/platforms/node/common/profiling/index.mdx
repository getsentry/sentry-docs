...
The Profiling package enables you to run the profiler in either [eager or lazy mode](https://v8docs.nodesource.com/node-18.2/d2/dc3/namespacev8.html#a874b4921ddee43bef58d8538e3149374). During initial tests, when we started a profile while no other profiles were running, we saw that the default profiling mode (lazy) sometimes caused startProfiling calls to take a long time, (sometimes in the range of a couple hundred milliseconds). The benefit of lazy logging is that it doesn't add any overhead when profiles aren't running.
