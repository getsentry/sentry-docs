...
- Allows you to <PlatformLink to="/configuration/filtering/#filtering-transaction-events">modify the transaction sample rate</PlatformLink> using `tracesSampleRate`.
...
- Request data is attached to all events: **HTTP method, URL, headers, form data, JSON payloads**. Sentry excludes raw bodies and multipart file uploads. Sentry also excludes personally identifiable information (such as user ids, usernames, cookies, authorization headers, IP addresses) unless you set `sendDefaultPii` to `True`.
