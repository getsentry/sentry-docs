...
Currenty, it is not possible for the SDK to distinguish between the user calling `Debug.LogException` and the SDK capturing an unhandled excpetion. To capture an exception and mark it as handled you can call `SentrySDK.CaptureException` instead.
