...
### Unable to add package Sentry Unity Package from Git
...
```
Check if the URL that you tried to add contains no additional spaces or lines, Unity will fail to find the git repository if the URL contains extra lines or spaces. 

![Url link with an additional line](unity-git-extra-line.jpg)
...
In order to send events to Sentry, you will need to activate the InternetClient Capability on your Player Settings.
