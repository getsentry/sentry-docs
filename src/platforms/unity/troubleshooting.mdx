...
This is a [known limitation](https://github.com/getsentry/sentry-unity/issues/884#issuecomment-1233035296) of the Mono JIT Scripting Backend due to native code not being able to see the actual generated code at the time of the crash. There are no debug files we can use to provide more information to stack traces since all code is generated when running the managed code. Using IL2CPP scripting backend may provide more information, if that's an option for you.
