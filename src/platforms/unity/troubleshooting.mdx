...
You can resolve this issue by creating a clean build or chosing **Replace** when prompted. Instead of relying on Unity to copy the `SentryNativeBridge.m` over to the generated Xcode project, starting 0.12.0, the SDK copies it manually to `**/Libraries/io.sentry.unity/SentryNativeBridge.m`.
