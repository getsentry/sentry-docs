...
Sentry requires [debug information files](/platforms/android/data-management/debug-files/) to symbolicate your crash logs. The Unity SDK provides an automated upload functionality for those symbol files that relies on the [sentry-cli](/product/cli/). We included the executables for Windows, macOS, and Linux inside the Unity SDK package.
...
2. `BCSymbolMap` files that get created during the archiving process
