<Note>
This is preview version. Features available in a preview version are still in-progress and may have bugs. We recognize the irony, and we'd appreciate your feedback.

If you want to use a stable version, please see our [Unity Lite content](/platforms/unity/unity-lite/).
...
Install it from [`Unity UPM`](https://docs.unity3d.com/Manual/upm-ui.html) via _git url_:
...
Create `TestMonoBehaviour.cs` which contains an intentional error (in this case `Debug.LogError`, but can be any exception like the C# expression `throw null`):
