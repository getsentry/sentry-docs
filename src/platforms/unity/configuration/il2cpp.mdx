...
Currently, line number support is limited when programmatically subscribing to button click events via `Button.onClick.AddListener`. This can be worked around by setting up callbacks through the editor.
