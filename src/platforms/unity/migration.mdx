...
You no longer need to to call `SentryUnity.Init` by yourself. Instead you can let the SDK self-initialize and modify the options taken from the Sentry Unity editor configuration window by providing a ScriptableOptionsConfiguration object.
...
- Click `Create options configuration`.
  ...
  - It will also create an instance of the ScriptableOptionsConfiguration and set in in the configuration window.
- Add your code to newly created script.

Browse the documentation to learn more about [options](/platforms/unity/configuration/options/).
