...
Currently, the Unity SDK offers native support for `Android`, `iOS`, and `Windows`. The support is `enabled` by default and you can opt out for each platform individually in the configuration window.

## Windows

The Unity SDK initialized the Native SDK through the C# layer, and this means that we currently do not capture errors of the Unity engine itself.
