...
title:  Editor Issues
sidebar_order: 100
description: "Issues relating to the Unity Editor"
---
...
The SDK provides you hooks in the editor window to add to scriptable objects to allow for [programmatic configuration](/configuration/options/#programmatic-configuration). This allows for not only disabling the SDK from initializing during runtime but also gives control over the configuration of native SDKs that get set up during buildtime.

### Build input file cannot be found: '**/Libraries/io.sentry.unity/Plugins/iOS/SentryNativeBridge.m'

This issue can come up when upgrading from version 0.12.0. You can resolve this by creating a clean build or chosing "Replace" when prompted. Alternatively, you can remove the unused `SentryNativeBridge.m`from the **UnityFramework Target > Compile Sources** in the Xcode project settings. Instead of relying on Unity to copy the `SentryNativeBridge.m` over to the generated Xcode project, the SDK copies it manually to `**/Libraries/io.sentry.unity/SentryNativeBridge.m`.
If you're still encountering this issue on a version later than 0.12.0, please let us know [with an issue report](https://github.com/getsentry/sentry-unity/issues/new?assignees=&labels=&template=BUG_REPORT.md).

### Failed to locate the Sentry package

This happens if you've copied some of the SDK files directly to the Assets folder. You can resolve this issue by either installing the SDK via UPM or by moving the Sentry SDK into your project's package directory.
...
You can resolve this issue by either installing the SDK via UPM or by moving the Sentry SDK into your project's package directory.
