...
Two separate settings define the minimum log level to keep the log entry as a `Breadcrumb` and to send an `Event` to Sentry. The events include any stored breadcrumb on that [scope](usage/events/enriching-events/scopes/).
...
You can provide a `user` entry matching the following example user. See [Identify Users](usage/events/enriching-events/user/) for more information.
