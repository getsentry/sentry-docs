...
The simplest way to initialise Sentry is ASP.NET Core is by using the `UseSentry` extension method from the `Sentry.AspNetCore` package in conjunction with the [framework configuration system](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/configuration/?view=aspnetcore-7.0).
...
app.UseSentryTracing(); // <- To enable automatic tracing
```
...
public partial class Startup
{
    ...
    {
        app.UseRouting();
        // Put this immediately after `UseRouting()` on .NET 5 or below
        app.UseSentryTracing() // <- To enable automatic tracing
    }
}
```
...
Note `TracesSampleRate` in this example. For automatic tracing to work, this must be configured in addition to the calling `UseSentryTracing()`. See <PlatformLink to="/performance/instrumentation/automatic-instrumentation">Automatic Instrumentation</PlatformLink> for further information.
