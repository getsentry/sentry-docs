...
using the assembly method `FunctionsStartup` to include the type `SentryStartup`, for example:
...
To load the configurations, you will need to use the assembly `[assembly: FunctionsStartup(typeof(SentryStartup))]`. After initialized, the framework automatically loads `appsettings.json` and environment variables, binding to `SentryAspNetCoreOptions`.
