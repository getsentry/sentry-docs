...
> Looking for `EntityFramework Core`? That integration is achieved through the <Link to="/platforms/dotnet/microsoft-extensions-logging/">Sentry.Extensions.Logging</Link> package.
...
All queries executed are added as breadcrumbs and are sent with any event which happens on the same <Link to="enriching-error-data/scopes/">scope</Link>. Besides that, validation errors are also included as `Extra`.
...
!<Link to="dotnet-entityframework.png">Sample breadcrumbs in Sentry</Link>
