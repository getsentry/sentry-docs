...
All queries executed are added as breadcrumbs and are sent with any event which happens on the same [scope](usage/events/enriching-events/scopes/). Besides that, validation errors are also included as `Extra`.
