...
One breaking change that affects objects set as [Context](../usage/events/enriching-events/context/) and `Extra` is that `Newtonsoft.Json` was previously able to serialize some types, which is not possible with `System.Text.Json`.
...
You can add attachments to events. Please refer to our [Attachments documentation](../usage/events/enriching-events/attachments/).

#### User Feedback

You can add user feedback to events via a .NET API. Please refer to the [User Feedback documentation](../usage/events/enriching-events/user-feedback/).
