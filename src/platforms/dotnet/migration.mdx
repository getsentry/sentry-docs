...
On version 3.0.0 the Sentry SDK for .NET changed from `Newtonsoft.Json` to `System.Text.Json`.
This introduces some advantages, such as no dependency is needed (when targeting .NET Core 3.0, .NET 5 and later version) and much faster performance.

One breaking change in behavior that came up was that `Newtonsoft.Json` was able to serialize some types that are not possible with `System.Text.Json`.
This affects objects set as [Context](../enriching-events/context/) and `Extra`.

Setting an instance of `Exception` that was caugh in a `catch` block could fail to serialze, and the event would be dropped by Sentry (this wouldn't break your app, but the error wouldn't make into Sentry):
...
To avoid such error, make sure to pass a serializable object into Sentry. For example, map the instance of `Exception` to an anonymous object:
