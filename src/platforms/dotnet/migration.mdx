...
Upgrading from Sentry SDK v2.x to Sentry SDK v3.x includes both breaking changes and new features.
...
ASP.NET (**not Core**) users need to install an additional packag, `Sentry.AspNet`. Without this package, HTTP related information will not be added to events.
...
### New Method for `IHub` and `ISentryClient`
