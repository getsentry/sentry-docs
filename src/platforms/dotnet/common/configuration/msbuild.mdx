...
<PropertyGroup>
  ...
  <!--
    Optionally, you can upload sources to Sentry - enabling Source Context.
    Note, symbols are sent by default for Release configuration, but sending sources is always an opt-in feature.
  -->
  <SentryUploadSources Condition="'$(Configuration)' == 'Release'">true</SentryUploadSources>

  <!--
    If you *always* want to send symbols and sources to Sentry, you can uncoment the lines below instead.
    (But note that this will slow down your local dev builds.)
  -->
  <!--
...
Controls whether source code files are uploaded to Sentry during the build.  Defaults to `false`, regardless of build configuration.
