...
- `IHub`: holds a client and the current [scope](../usage/events/enriching-events/scopes/). In fact, it extends `ISentryClient` and is able to dispatch calls to the right client depending on the current scope.
