...
The `IHub` on the other hand, holds a client and the current <Link to="../enriching-error-data/scopes/">scope</Link>. In fact, it extends `ISentryClient` and is able to dispatch calls to the right client depending on the current scope.
