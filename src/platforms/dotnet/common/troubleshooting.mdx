...
Starting with C# 7.1, a program's [`Main` method](https://docs.microsoft.com/dotnet/csharp/fundamentals/program-structure/main-command-line) can be declared either synchronously or asynchronously.  Unfortunately, the asynchronous approach is not fully compatible with the Sentry SDK.  If your application uses an async `Main` method, the Sentry SDK will be disposed before an unhandled exception could possibly be captured.
