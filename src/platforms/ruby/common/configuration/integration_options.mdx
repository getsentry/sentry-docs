...
: Tracing subscribers are responsible for capturing tracing spans from Rails instrumentation.


The default subscribers are:
...
: `sentry-delayed_job` captures exceptions whenever they occur in a job execution. You can make it wait utill the retry is exhausted. Default is `false`.
