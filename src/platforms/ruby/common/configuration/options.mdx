...
: Provide a lambda or proc. This will be `called` before sending a transaction event to Sentry. Receives an `event` and `hint` as parameter. `hint` is currently empty but might be augmented in the future. It is possible to mutate the event, also if this function returns `nil` the event will be dropped and not sent.
