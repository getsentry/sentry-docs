...
class OrdersController < ApplicationController
  def create
    order = Order.new
    # get the scope's current span (may not be the root transaction)
    span = Sentry.get_current_scope.get_span
    # start a child span
    child_span = span.start_child(op: :process_items, description: "process order's items")
