...
Senty's Ruby SDK uses a unified structure, which introduces two new components: `Hub` and `Scope` ([which are both documented here](/platforms/ruby/usage/events/enriching-events/scopes/)). Most users won't interact with `Hub` directly, but will need `Scope` to configure contextual data.
