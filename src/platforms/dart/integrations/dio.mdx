...
The Dio instrumentation, once enabled, adds spans to the current active transaction on the scope for each request.
...
It is required to have an active transaction running as the span is attached to it.
...
After executing `makeWebRequestWithDio()` check Sentry's performance page to see the transaction.
