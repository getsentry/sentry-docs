...
To get started automatically sending traces you can set the `traces_sample_rate` in your SDK config. This should be a number between `0` and `1`. (For example, to send 20% of transactions, set traces_sample_rate to `0.2`.)
...
Without sampling, our automatic instrumenation will send a transaction any time any user loads any page or navigates anywhere in your app. Sampling allows you to send representative data without overwhelming either your system or your Sentry transaction quota.
