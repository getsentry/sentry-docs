...
[Ariadne GraphQL library](https://ariadnegraphql.org/). These can then be viewed
...
the GraphiQL graphical user interface.

Enter `{ hello }` into the query field
and press the Run button. Your web app will be queried and it will encounter the
`ZeroDivisionError` error we've snuck into the `resolve_hello` resolver function.
...
The Ariadne integration can capture request and response payload
for each GraphQL error that happens. Since these may contain sensitive data, the
SDK needs to be initialized with the
[send_default_pii](/platforms/python/configuration/options/#send-default-pii) option set to `True`. By default, no request and response
...
Since `send_default_pii` is a global SDK option, setting it to `True` affects all integrations, not just Ariadne. Please make sure to take care of
