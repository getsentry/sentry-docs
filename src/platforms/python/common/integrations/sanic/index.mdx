...
If you are using performance monitoring, you can configure the Sanic integration to not send transactions for requests which have resulted in certain HTTP statuses using the `unsampled_statuses` option.

By default, the Sanic integration generates transactions for all requests, except requests which result in a 404 HTTP status. If you wish to generate transactions for all HTTP statuses, including 404, or change the HTTP statuses which have no transactions generated, you can manually instantiate the Sanic integration when initializing the Sentry SDK, like so:
...
If you wish to specify a different set of HTTP statuses for which no transactions should be generated, set `unsampled_statuses` to a set containing the HTTP statuses which should have no transactions generated. For example, if you wish to have no transactions for requests resulting in a 200 or 404 HTTP status, use `SanicIntegration(unsampled_statuses={200, 400})`.
