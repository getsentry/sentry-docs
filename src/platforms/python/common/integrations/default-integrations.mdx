...
sidebar_order: 1
redirects_from:
  - /platforms/python/default-integrations/
---
...
- Any outgoing HTTP request done with `httplib` will result in a <Link to="../../enriching-error-data/breadcrumbs/">breadcrumb</Link> being logged. `urllib3` and `requests` use `httplib` under the hood, so HTTP requests from those packages should be covered as well.

- _(New in version 0.10.0)_ Subprocesses spawned with the `subprocess` module will result in a <Link to="../../enriching-error-data/breadcrumbs/">breadcrumb</Link> being logged.
...
See <Link to="/platforms/python/logging/">_Logging_</Link>
