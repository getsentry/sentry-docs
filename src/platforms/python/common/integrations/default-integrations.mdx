...
- Any outgoing HTTP request done with `httplib` will result in a <PlatformLink to="/usage/events/enriching-events/breadcrumbs/">breadcrumb</PlatformLink> being logged. `urllib3` and `requests` use `httplib` under the hood, so HTTP requests from those packages should be covered as well.

- Subprocesses spawned with the `subprocess` module will result in a <PlatformLink to="/usage/events/enriching-events/breadcrumbs/">breadcrumb</PlatformLink> being logged.
