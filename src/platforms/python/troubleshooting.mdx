...
## Using the Legacy Raven SDK

If you are still using our legacy Raven SDK, you can access the <Link rel={`nofollow`} to={`/platforms/python/legacy-sdk/`}>legacy SDK documentation</Link>, until further notice. Migrating from older versions is documented <PlatformLink to="/migration/">here</PlatformLink>.
