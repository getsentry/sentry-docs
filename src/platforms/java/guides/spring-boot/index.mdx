...
To use Sentry Logback integration in Spring application you must opt-out from `application.properties` based Spring Boot logging configuration and configure `SentryAppender` in `logback-spring.xml` file:
...
To use Sentry Log4j2 integration in Spring application configure `SentryAppender` in `log4j2.xml` file:
