...
If used decides to opt-out from `application.properties` based Spring Boot logging configuration and configures logging in `logback-spring.xml` file, `SentryAppender` can be configured in the following way:
