...
To use Sentry's Log4j2 integration in Spring Boot application, you must include a dependency to the `sentry-log4j` module. Then [follow the guide on configuring Log4j2 with Spring Boot](https://docs.spring.io/spring-boot/docs/current/reference/html/howto.html#howto-configure-log4j-for-logging) and configure `SentryAppender` in the `log4j2.xml` file:
...
```sbt {tabTitle:SBT}
libraryDependencies += "io.sentry" % "sentry-log4j2" % "5.0.1" }}'
```
