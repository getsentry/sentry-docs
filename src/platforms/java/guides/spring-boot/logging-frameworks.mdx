...
<dependency>
    ...
    <version>{{ packages.version('sentry.java.log4j2', '5.1.1') }}</version>
</dependency>
```

```groovy {tabTitle:Gradle}
implementation 'io.sentry:sentry-log4j2:{{ packages.version('sentry.java.log4j2', '5.1.1') }}'
```

```scala {tabTitle: SBT}
libraryDependencies += "io.sentry" % "sentry-log4j2" % "{{ packages.version('sentry.java.log4j2', '5.1.1') }}"
```
