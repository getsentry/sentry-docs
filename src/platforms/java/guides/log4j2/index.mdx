...
If the DSN is configured using system property or the environment variable, it is possible to use [Log4j2 capabilities](https://logging.apache.org/log4j/2.x/manual/configuration.html) to set it on `SentryAppender`.

Following example shows how to configure DSN from `SENTRY_DSN` the system property.
...
Following example shows how to configure DSN from `SENTRY_DSN` the environment variable.
