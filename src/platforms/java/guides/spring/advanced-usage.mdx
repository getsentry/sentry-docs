...
To record the user's IP address and `Principal#name` as the username:
...
2. Register servlet filter bean `SentryUserFilter`:
...
By default, the username is retrieved from `HttpServletRequest#userPrincipal`. To retrieve username from Spring Security context, register `SpringSecuritySentryUserProvider` bean:
