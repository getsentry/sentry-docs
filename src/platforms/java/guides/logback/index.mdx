...
If the DSN is configured using system property or the environment variable, it is possible to use [Logback capabilities](http://logback.qos.ch/manual/configuration.html#variableSubstitution) to set it on `SentryAppender`.

Following example shows how to configure DSN from `SENTRY_DSN` system property or environment variable.
