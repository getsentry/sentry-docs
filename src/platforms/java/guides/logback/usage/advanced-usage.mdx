...
Starting with Sentry version 6.0.0 and up, you can define a list of `MDC tags` that will be viewable as `Tags` in the Sentry UI. `MDC Tags` not in this list will be viewable as `Context Data`. To define a tag, add a `<context-tag>` entry inside the `<options>` of the Sentry appender configuration in `logback.xml`
