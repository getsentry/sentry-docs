...
`InvalidDsnException` has been removed in favor of `IllegalArgumentException`.

`EventProcessor` interface has a new `default` method and this could break the instantiation when using trailing lambdas.
