...
## Migrating from `io.sentry:sentry` `4.3.0` to `io.sentry:sentry` `4.4.0`

`Sentry#startTransaction` by default binds created transaction to the scope. Bound transaction can be retrieved with `Sentry#getSpan`.
To start transaction without binding to the scope, use one of the new overloaded `startTransaction` methods taking `bindToScope` parameter and set it to `false`.
