...
The plugin automatically adds available Sentry integrations as dependencies if it detects a library dependency we support. So if your project has a dependency on `graphql-java` the plugin will automatically add `sentry-graphql` as an additional dependency.
