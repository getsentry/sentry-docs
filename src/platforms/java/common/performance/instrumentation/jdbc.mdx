...
By default, in addition to spans, P6Spy logs JDBC statements to a log file, which can grow rapidly. We recommend disabling creating a log file by setting a System Property `p6spy.config.modulelist` to `com.p6spy.engine.spy.P6SpyFactory` or creating a `spy.properties` file in `src/main/resources` with a content:
