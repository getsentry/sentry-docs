...
In cases where the decision if thrown exception should be reported to Sentry happens later in the future after span is already finished, to connect error with a span you must:

1. Link exception with a Span by calling `setThrowable` method:
...
2. Retrieve span context associated with exception when reporting an error with `IHub#getSpan`:

```java
SentryEvent event = new SentryEvent(throwable);
SpanContext spanContext = HubAdapter.getInstance().getSpan(ex);
if (spanContext != null) {
