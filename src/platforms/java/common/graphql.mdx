...
sidebar_order: 7
description: "Learn how to capture exceptions and the performance of queries executed with GraphQL Java ."
notSupported:
  - java.logback
  - java.log4j2
  - java.jul
...
<dependency>
    ...
    <version>{{ packages.version('sentry.java.openfeign', '5.4.0') }}</version>
</dependency>
```

```groovy {tabTitle:Gradle}
implementation 'io.sentry:sentry-graphql:{{ packages.version('sentry.java.jdbc', '5.4.0') }}'
```

```scala {tabTitle: SBT}
libraryDependencies += "io.sentry" % "sentry-graphql" % "{{ packages.version('sentry.java.jdbc', '5.4.0') }}"
```

For other dependency managers, check out the [central Maven repository](https://search.maven.org/artifact/io.sentry/sentry-grapql).

## Capture Exceptions

`SentryDataFetcherExceptionHandler` captures exception, sends it to Sentry and calls the delegate responsible for handling the exception.
