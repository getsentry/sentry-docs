...
```scala {tabTitle:SBT}
libraryDependencies += "io.sentry" % "sentry" % "3.0.0"
```
...
Starting from the version 3.0 there is no built in support for reading configuration options from the environment variables, system properties or `sentry.properties` file. When Sentry is used in manual way, configuration options must be passed to a static `Sentry#init` method.
