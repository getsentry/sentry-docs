---
title: Apollo Android Integration
sidebar_order: 20
description: "Learn how to capture the performance of Apollo Android GraphQL client."
---
...
Sentry Apollo integration provides the `SentryApolloInterceptor`, which creates a span for each outgoing HTTP request executed with a [Apollo Android](https://github.com/apollographql/apollo-android) GraphQL client.
...
<dependency>
    ...
    <version>{{ packages.version('sentry.java.apollo', '5.1.0') }}</version>
</dependency>
```

```groovy {tabTitle:Gradle}
implementation 'io.sentry:sentry-apollo:{{ packages.version('sentry.java.apollo', '5.1.0') }}'
```

```scala {tabTitle: SBT}
libraryDependencies += "io.sentry" % "sentry-apollo" % "{{ packages.version('sentry.java.apollo', '5.1.0') }}"
```
...
<Note>
In a Global Hub Mode, all threads use the same Hub.
</Note>
