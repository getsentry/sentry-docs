...
Navigate to the editor's menu **Project Settings > Plugins > Sentry**, set the Sentry _Unreal Engine Endpoint_ in the corresponding input field and click the **Update global settings** button to apply changes. Once _Crash Reporter_ is not needed anymore default endpoint value can be restored with the **Reset** button.
