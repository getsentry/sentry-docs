...
Navigate to the editor's menu **Project Settings > Plugins > Sentry**, set the Sentry _Unreal Engine Endpoint_ in the corresponding input field and click the **Update global settings** button to apply changes. Once the _Crash Reporter_ is not needed anymore the default endpoint value can be restored through the **Reset** button.
