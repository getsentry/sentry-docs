...
Unreal Engine application or game since Sentry supports the _UE Crash Reporter_.

To integrate your UE game or application with Sentry, the following steps are required:

1. Include the _UE Crash Reporter_ to your game or application.
...
<Alert level="info" title="Using UE Crash Reporter on Windows">

UE Crash Reporter still works with Sentry SDK installed along. In this case Sentry SDK can help with configuring UE Crash Reporter, uploading debug symbols and enriching crash reports with custom data. However it's important to keep in mind that starting from UE 5.2 user has to choose between the two available crash capturing mechanisms - UE Crash Reporter or `sentry-native` library that's integrated into the Sentry plugin. By default, second option is enabled (see `Enable automatic crash capturing (Windows, UE 5.2+)` in plugin settings) and thus UE Crash Reporter is suppressed. Both solutions can't be used simultaneously since they are mutually exclusive.
...
To get the most out of captured crash reports [debug symbols need to be uploaded to Sentry](#upload-debug-symbols).
...
Configuring Crash Reporter attributes should be completed before initializing Sentry SDK if one is enabled. Otherwise, some information could be overwritten and lost.

</Note>

<Note>

Sentry SDK provides API allowing to set Crash Reporter attributes as well.
...
Sentry SDK can hadle [debug symbols uploading](/platforms/unreal/configuration/debug-symbols/) automatically at build time.
...
Event ingestion imposes limits on the size of UE crash reports. These limits
...
For tips on troubleshooting the UE Crash Reporter, [check out this thread in Sentry's forum](https://forum.sentry.io/t/unreal-engine-crash-reporter-cant-get-it-to-work/7643) where community members have delved into potential solutions.
