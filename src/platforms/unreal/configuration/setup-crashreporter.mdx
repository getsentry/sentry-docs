...
The UE Crash Reporter works if the Sentry SDK is installed. It can help with configuring the UE Crash Reporter, uploading debug symbols, and enriching crash reports with custom data. However, it's important to keep in mind that **starting from UE 5.2** users have to choose between the two available crash capturing mechanisms - the UE Crash Reporter or the `sentry-native` library that's integrated into the Sentry plugin. By default, the second option is enabled and the UE Crash Reporter is suppressed. (See "Enable automatic crash capturing (Windows, UE 5.2+)" in the plugin settings.) The two solutions are mutually exclusive and can't be used simultaneously.
