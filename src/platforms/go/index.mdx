...
More on this in <Link to="/platforms/go/config/">Configuration</Link> section.
...
The other two being `CaptureMessage` and `Recover` which you can read more about in [Error Reporting](/error-reporting/quickstart/?platform=) and <Link to="/platforms/go/panics/">Capturing Panics</Link>.
