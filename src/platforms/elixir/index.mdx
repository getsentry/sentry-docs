...
Sentry has multiple options for including contextual information. They are organized into "Tags", "User", and "Extra", and Sentry's documentation on them is <Link to="enriching-error-data/additional-data/">here</Link>. Breadcrumbs are a similar concept and Sentry's documentation covers them <Link to="enriching-error-data/breadcrumbs/">here</Link>.
...
By default, Sentry aggregates reported events according to the attributes of the event, but users may need to override this functionality via <Link to="/data-management/event-grouping/">fingerprinting</Link>.
...
- <Link to="/clients/elixir/usage/">Usage</Link>
- <Link to="/clients/elixir/config/">Configuration</Link>
