...
For example, if you are using Capacitor with Ionic-Angular, upload your `www` folder on **every build**. The values for `<release_name>` and `<dist>` must match the values passed into `Sentry.init` for events to be deminified correctly.
