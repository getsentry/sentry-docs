...
`SentryAppender` does not support Log4j2's [async mode](https://logging.apache.org/log4j/2.x/manual/async.html). Sentry Java SDK itself is already asynchronous and does not perform any blocking operation on the caller thread.
