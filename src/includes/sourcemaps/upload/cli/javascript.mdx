...
You need to set up your build system to both create a release and attach the various source files. For Sentry to de-minify your stack traces, provide both:
- the minified files (for example, `app.min.js`)
- the corresponding source maps

If the source map files do not contain your original source code (`sourcesContent`), you must also provide the original source files. Alternatively, Sentry CLI will automatically embed the sources (if missing) into your source maps.
...
You don't _have_ to upload the source files (referenced by source maps), but without them, the grouping algorithm will not be as strong, and [sentry.io](https://sentry.io) will not show any contextual source.

For more information, see our [Releases API documentation](/api/releases/).

<PlatformSection notSupported={["node"]} >

It’s not uncommon for a web application to be accessible at multiple origins. See our documentation on [multiple origins](../../uploading/multiple-origins/) on how to handle this.
