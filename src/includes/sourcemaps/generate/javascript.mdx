...
We recommend using our Webpack plugin to integrate your source maps with Sentry.
...
Sentry provides a convenient [Webpack plugin](https://github.com/getsentry/sentry-webpack-plugin) that configures source maps and uploads them to Sentry during the build.

This process is an example of how to generate and upload your sources:
...
You don't need to install the Webpack plugin if you are using Sentry's Next.JS SDK because the SDK automatically handles and uploads your source maps without requiring any configuration. 
...
Set up the Webpack plugin as the last running plugin; otherwise, the resulting source maps the plugin receives may not be the final one.
...
You can configure Rollup to generate source maps:
