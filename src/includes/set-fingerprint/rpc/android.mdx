...
SentryAndroid.init(this, options -> {
  options.setBeforeSend((event, hint) -> {
    if (event.getOriginThrowable() instanceof MyRpcException) {
      MyRpcException exception = (MyRpcException) event.getOriginThrowable();
...
SentryAndroid.init(this) { options ->
  options.beforeSend = BeforeSendCallback { event, hint ->
    if (event.originThrowable is MyRpcException) {
      val exception = event.originThrowable as MyRpcException
