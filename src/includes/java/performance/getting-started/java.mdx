...
In future, if a thrown exception should be reported to Sentry after the span has already finished, you must link the exception with the span by calling `setThrowable` method to connect the error:
