...
For HTTP communications, Sentry's SDK providers `SentryHttpMessageHandler`, which can be used to [automatically attach trace headers to outgoing HTTP requests](/platforms/dotnet/performance/included-instrumentation/).

</Note>

The code snippet below demonstrates propagating the trace programatically. Even though `SentryHttpMessageHandler` does that automatically for your, this illustrates the mechanism so it can be replicated in other protocols and technologies such a messaging systems.
