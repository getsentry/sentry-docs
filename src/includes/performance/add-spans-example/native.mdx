...
void perform_checkout() {
    ...
    sentry_transaction_t *tx = sentry_start_transaction(tx_ctx);

    // Validate the cart
    sentry_span_t *validation_span = sentry_start_transaction_child(
        tx,
        "validation",
    ...
    // Process the order
    sentry_span_t *process_span = sentry_start_transaction_child(
        tx,
        "process",
