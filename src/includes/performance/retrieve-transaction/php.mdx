...
In cases where you want to attach spans to an already ongoing transaction you can use `getTransaction()` on the current hub. This property will return a `\Sentry\Tracing\Transaction` in case there is a transaction set otherwise it returns `null`.
...
In cases where you want to attach spans to an already ongoing span you can use `getSpan()` on the current hub. This property will return a `\Sentry\Tracing\Span` in case there is a span set otherwise it returns `null`. You can also set the current span after you started one and restore it back to the parent after your span ended.
