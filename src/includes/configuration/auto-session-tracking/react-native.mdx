...
We mark the session as an error if the SDK captures an event that contains an exception (this includes manually captured errors) for _unhandled error_ or _unhandled promise rejection_, but for native crashes (Android and iOS), they will be marked as crashed.
