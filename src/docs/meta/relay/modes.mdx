...
In Sentry, event processing is configured according to both project and organization settings. Some settings, such as privacy controls, are set at the organization level, then inherited by all projects in that organization; other settings are specified per project. For Relay, events are processed according to the inherited project settings to which the event belongs.

Configuration for Relay is refreshed in regular intervals. See [Configuration Options] regarding configuration of intervals, timeouts, and retries.
...
the settings for a particular project, all messages for that project will
be discarded.
