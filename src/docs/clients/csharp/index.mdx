...
Sentry supports a concept called [Breadcrumbs](/platforms/dotnet/enriching-error-data/breadcrumbs/), which is a trail of events which happened prior to an issue. Often times these events are very similar to traditional logs, but also have the ability to record more rich structured data.
