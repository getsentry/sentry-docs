...
To integrate Sentry into your Xcode project using SPM, open your App in Xcode and open File -> Add Packages. Then add sentry-cocoa by entering the git repo url `https://github.com/getsentry/sentry-cocoa.git` on the top-right search field. Define you dependecy rule by selecting the SDK version (or branch), then press "Add Package" button.
