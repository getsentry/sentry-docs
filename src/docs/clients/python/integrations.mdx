...
[Django](http://djangoproject.com/) versions 1.4 to 2.0 are supported. Since this SDK is being phased out, newer versions of Django are only supported by <Link to="/platforms/python/">the new Python SDK</Link>.
...
See the <Link to="/clients/javascript/">_Raven.js documentation_</Link> for more information.
...
For more details on this feature, see the <Link to="/platforms/python/enriching-error-data/user-feedback/">_User Feedback guide_</Link>.
