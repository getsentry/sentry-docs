...
- If you can’t install a repository integration, send commit metadata using the [create release endpoint](/product/releases/adding-commits/#using-the-api) or omit the `set-commits` line (`set-commits` is required for suspect commits).
