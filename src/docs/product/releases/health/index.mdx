...
Users that started the application at least once in the last 24 hours.

### Crash

The app had an explicit unhandled error or hard crash. You'll typically be able to view the corresponding issue in [sentry.io](https://sentry.io) that captures this event, and errors that did not cause the end of the application should not be included. To search for them in **Discover** or on the **Issues** page, filter by `error.unhandled:true `. The number of unhandled events is not expected to match the number of crashed sessions because sessions are not subject to [Inbound Filters](/product/data-management-settings/filtering/) and [Sampling](/platform-redirect/?next=/configuration/sampling/).
...
Percentage of sessions in the specified time range that did not end by a crash of the application.
...
Depending on selected Display option, we show either session or user based adoption.

User Adoption is the number of users that started at least once during the last 24 hours of a specific release.
Session Adoption is the number of sessions in the last 24 hours of a specific release.
