...
When a project is selected from the project selecter on the top left of the page, if it has release health set up and if it has releases and user/session data in the selected time period, there will be a chart with the adoption percentages for each release for each data point.

Adoption data is stored in buckets of 10m, 1h and 1d, the last point on the graph will show the data so far for the last bucket.

Clicking on a line in the graph you can navigate to that release's [Release Details](/product/releases/health/release-details/) page.
