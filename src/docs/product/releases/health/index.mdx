...
You can monitor the health of releases by observing user adoption, usage of the application, percentage of [crashes](#crash), and [session data](#session). Release health data provides insight into the impact of crashes and bugs related to user experience and reveals trends with each new issue through the release details, graphs, and filters on the. This data is primarily displayed on the [Releases](/product/releases/) and [Release Details](/product/releases/release-details) pages, but is also reflected in other parts of [sentry.io](https://sentry.io).
...
## Release Adoption Data {#release-adoption}
...
The adoption progress bar is calculated as a percentage of sessions/users in this release vs all releases in the project, in the last 24 hours:
...
Adoption stage labels are only displayed for mobile projects when you filter down to a single environment.
...
Your release can have one of the following statuses:
