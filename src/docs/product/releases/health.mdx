...
: The app had an explicit unhandled error or hard crash. You'll typically be able to see the corresponding issue in Sentry that captures this event. To search for them in _Discover_ or _Issue Stream_, filter by `handled:no`.
...
: The app shut down normally, but there were handled errors in the session. Most likely, there are issues/errors in Sentry.
