...
Monitor the health of releases by observing user adoption, usage of the application, percentage of [crashes](#crash), and [session data](#session). Release health provides insight into the impact of crashes and bugs as it relates to user experience, and reveal trends with each new issue through the release details, graphs, and filters.

Sentry's [iOS](/platforms/apple/guides/ios/), [Android](/platforms/android/), [React Native](/platforms/react-native/), [Rust](/platforms/rust/), C and C++ SDKs are automatically configured to track release health. You can manually configure Python and all of its frameworks to track release health (see the full list [here](/platforms/python/)). We have limited support for release health in [Electron](/platforms/javascript/guides/electron/).

<Note>

Release health is in Beta for JavaScript

</Note>

## How Release Health Works

Once configured, Sentry's SDKs will send health data about the usage of your application.

The primary component that Sentry uses to monitor health is a _session_. A session represents the interaction of the user with the application and starts when a user opens the application. When a user closes the application or has a prolonged period of inactivity, the session ends.
...
In sentry.io, release health uses the following terms --  Crashes, Healthy, Abnormal, Errored, Crashed, Crash Free Users, and Crash Free Sessions -- the definition of which naturally varies to be compatible with the SDK. We describe each term fully in our documentation regarding [Release Health Terms](/product/releases/sdk-terms/)
