...
If you're using one of our [release automation](/product/releases/setup/release-automation/), suspect commits should already be enabled and this step is automated for you.

However, if you don't have a repository integration installed, you can tell Sentry about your raw commit metadata using the [Sentry CLI](#using-the-cli) or [our API](#using-the-api).
...
If you're using one of our [release automation](/product/releases/setup/release-automation/) options, suspect commits should already be enabled. However, if you don’t want to install a repository integration so Sentry can connect to your repository, or you’re using an unsupported repository provider or VCS (for example, Perforce), you can tell Sentry about your raw commit metadata using the [Sentry CLI](#using-the-cli) or [our API](#using-the-api).
...
To tell Sentry about your commit metadata using the API use the [create release endpoint](/api/releases/create-a-new-release-for-an-organization/).
