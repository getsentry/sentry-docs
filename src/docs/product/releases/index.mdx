...
Marking an issue as [resolved](/product/issues/states-triage/#resolve) in a release or in the next release allows you to mark an issue as resolved until the issue is seen again in a future release.

If you'd like the issue to be marked as unresolved the next time an event is seen in any release, use the main "Resolve" button.
If you'd like to ignore events until an event is seen in a future release, you can mark it resolved in the current release or the next release.

When using "Resolved in the current release", Sentry records the current release, displayed in the UI, at the time of resolution. New events for this issue are compared with the recorded release version. If the release version is using semver, the issue will be marked as unresolved if the release is from a semver version that is greater than the resolved version. If the release is not using semver, the release creation dates are compared.

"Resolved in the next release" works similar to resolved in current release except that we can't record the resolution version since it hasn't been created yet. Instead, we record the current release version and update it when a newer release is created.

"Resolved in commit" waits for a release with the specified commit to be created. After the release is created, new events are checked against the release version or release date.

If the new event is from a newer release, the issue is marked as a regression and will appear in your [For Review tab](/product/issues/states-triage/) for triage.
