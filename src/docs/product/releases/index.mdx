...
**Resolved in the current release** records the current release, displayed in the UI and new events for this issue are compared with the recorded release version to determine if the issue should unresolve. If the release version is using semver, the issue will be marked as unresolved if the release is from a semver version that is greater than the resolved version. If the release is not using semver, the release creation dates are compared.
