...
As a result, you will see information about the release, such as new issues and regressions introduced in the release.
...
To notify Sentry  of a release:

1. [Associate commits with a release](#associate-commits-with-a-release)
2. [Finalize the release](#finalize-the-release)
3. [Tell Sentry when you deploy](#tell-sentry-when-you-deploy-a-release)

#### Associate Commits with a Release

In your release process, add a step to create a release object in Sentry and associate it with commits from your linked repository. We recommend using Sentry’s Command Line Interface as illustrated in the following example or [our API](product/releases/manual-setup/use-the-api) to add this step in your release process, especially if you're using [Release Automation](/product/releases/release-automation/).
...
Tell Sentry when you deploy a release to a new environment, so we can automatically send an email to Sentry users who have committed to the release that is being deployed.

![release ui](release-ui.png)

To let Sentry know you’ve deployed, just send an additional request after creating a release:
