...
Since the received event from the deleted release would automatically recreate it, Sentry has an option to hide releases permanently from the UI. Archiving releases can be useful when you're no longer interested in seeing them or you've accidentally created a release you no longer want.
