...
Marking an issue as [resolved](/product/issues/states-triage/#resolve) in a release or in the next release allows you to mark an issue as resolved until the issue is seen again in a future release. Marking an issue as resolved in a release is different from just marking it resolved as resolved will unresolve on the next event, whereas resolved in release will only unresolve when the event is seen in a future release.
