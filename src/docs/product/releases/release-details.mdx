...
Click on any option in the graph legend to hide it. For example, in a "Session Count" graph, you might hide "Healthy" sessions to see "Crashed" ones in better scale. In the image below, the healthy sessions are displayed, but there are so many of them compared to the crashed sessions that the crashed ones become invisible in the graph:

![Session count graph with healthy sessions displayed](healthy_sessions_displayed.png)

By hiding the healthy sessions, you can actually see the very small number of crashed sessions:

![Session count graph with healthy sessions hidden](healthy_sessions_hidden.png)
