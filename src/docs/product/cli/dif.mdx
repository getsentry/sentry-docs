...
`sentry-cli` requires an Auth Token to be authenticated with a set of [Permissions & Scopes](/api/permissions/) to be able to upload Debug Information Files and for that you must have either a `project:releases` or `project:write` Scope.
