...
It can happen whenever you modify the commit in question, for example by ammending it, rebasing or squashing multiple commits together.
In this case, Sentry CLI will be unable to find it, and will throw an error explaining what happened.

Whenever this happens, you can pass an additional `--ignore-missing` flag, which will allow the command to fall back to the default behavior,
