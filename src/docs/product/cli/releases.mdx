...
: Sets the distribution identifier for uploaded files. This identifier is the used to make a distinction between multiple files of the same name within a single release. See [Troubleshooting Source Maps](/platforms/javascript/guides/cordova/sourcemaps/troubleshooting_js/#verify-artifact-distribution-value-matches-value-configured-in-your-sdk) page for more details.
