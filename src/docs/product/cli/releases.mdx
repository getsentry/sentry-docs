...
The most common use case is to upload files. For the generic upload the `sentry-cli files upload -r RELEASE` command can be used. However since most release artifacts are JavaScript source map related we have a [Upload Source Maps](#sentry-cli-sourcemaps) convenience method for that.
...
```bash
sentry-cli files upload -r "$RELEASE" /path/to/file '~/file.js'
```
...
```bash
sentry-cli sourcemaps upload -r "$RELEASE" /path/to/sourcemaps
```
...
```bash
sentry-cli files list -r "$RELEASE"
```
...
```bash
sentry-cli files delete -r "$RELEASE" NAME_OF_FILE
sentry-cli files delete -r "$RELEASE" --all
```
...
```bash
sentry-cli deploys new -r "$RELEASE" -e ENVIRONMENT
```
...
...
now=$(date +%s)
sentry-cli deploys new -r "$RELEASE" -e ENVIRONMENT -t $((now-start))
```

Deploys can be listed too (however they cannot be deleted):

```bash
sentry-cli deploys list -r "$RELEASE"
```
