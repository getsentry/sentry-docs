...
While we default the baseline calculation to use P50 you can change it via the dropdown, other options include P75, P95, P99 and average. The baseline is used to determine the percentage change but is also used for the comparison view found in the dropdown for each transaction. This comparison view finds the transaction in each period with the duration closest to the baseline and compares their spans.
