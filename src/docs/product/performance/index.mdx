...
The `Confidence` option allows you to filter trends based on High or Low statistical significance. By default, only high confidence, statistically significant results are shown. Sentry uses a [t-test](https://en.wikipedia.org/wiki/Welch%27s_t-test) to compare the before and after periods of the transaction and assign a confidence score. 
...
While we default the baseline calculation to use P50, you can change the baseline calculation via the dropdown. Other options include P75, P95, P99, and average. The baseline determines the percentage change, but you can also use it for the comparison view found in each transaction's dropdown. The comparison view finds the transaction in each period with the duration closest to the baseline and compares their spans.
