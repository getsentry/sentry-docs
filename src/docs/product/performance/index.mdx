...
For organizations participating in the limited release of [metrics](https://docs.sentry.io/product/sentry-basics/metrics/), the transaction filter on the Performance page has been simplified. The search bar can only be used to filter by transaction name, rather than to search for transactions matching the query. See [transaction search](/product/performance/filters-display/#transaction-search).
