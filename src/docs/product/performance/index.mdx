...
For certain users, the transaction filter on the performance page has been simplified to support metric enhanced performance. The search bar can only be used to filter by transaction name. See [performance filters](/product/performance/filters-display/#search-condition-filters).
