...
The "Display" dropdown automatically selects a set of [metrics](metrics/) and table columns based on the platform of the selected project. The "Frontend (Pageload)" and "Frontend (Other)" options additionally filter by pageload and non-pageload transactions.
...
In Display: Frontend (Other) mode, duration information is shown. Additionally, transactions are automatically filtered by `transaction.op:!pageload`. This generally corresponds to page transitions handled internally in SPA frameworks such as React or Vue. Currently, this will also display backend transactions if a backend project is selected.

### Display: Backend

In Display: Backend mode, duration, throughput, failure rate, and apdex information is shown. The Apdex threshold can be configured in the project settings. Currently, this will also display frontend transactions if a frontend project is selected.
