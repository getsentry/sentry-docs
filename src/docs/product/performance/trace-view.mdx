...
As well broken subtraces can occur when sentry received an error for a trace, but the corresponding transaction does not exist. This means that the error will not appear on the trace view, and the quick trace will be empty.
...
- [Quota Limits](/product/accounts/quotas/) differ between transactions and errors which means that an error was received, but not the corresponding transaction.
