...
Within a project, some transaction types occur more often than others, this is a similar problem to the one discussed above, Low Volume Projects, but it applies to various transaction types within a project. To avoid high volume transaction types drowning out low volume transaction types, we prioritise the retention of transaction samples from low volume transaction types, in order to ensure that you have enough samples from all transaction types.

<Note>
When we talk about transaction types we refer to the transaction name. For example, a typical scenario for web service is to set the transaction name to the URL of the particular request.
In an online shop application, calls to browsing various items may be orders of magnitude higher that calls to the final payment page, Low Volume Transaction adjustment tries to ensure that you will have
enough samples from transactions from the payment page in addition to samples from the browsing page.
</Note>
