...
The operations breakdown corresponds to the waterfall span view as a legend. The operation type, duration, and percentage of the total duration are shown. Spans can overlap but the calculation is based on operations that do not overlap and contribute to the overall duration. See the http example below.
