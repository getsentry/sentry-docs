...
Entry points to an event detail are either through a [Transaction Summary](/product/performance/transaction-summary/) or [Query Builder](/product/discover-queries/query-builder/). In [Discover](/product/discover-queries/), performing a search with the `event.type:transaction` or going into a pre-built query such as "Transactions by Volume" will get you a list of event IDs. Event IDs will be linked to open the corresponding Event Detail.
...
Information about the specific event is located at the top, such as the Event ID, project, duration, time of occurrence, and status. More can be found under Tag Details.
...
![Quick trace showing related events](./quick-trace-open.png)

If a node is associated with errors, it is displayed in red and has a flame icon next to it. Click "Trace ID" to open the [Trace View](/product/performance/trace-view).
...
The operations breakdown corresponds to the waterfall span view as a legend. The operation type, duration, and percentage of the total duration are shown.
