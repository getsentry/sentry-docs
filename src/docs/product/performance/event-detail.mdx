...
From <Link to="/product/performance/">Performance</Link> and <Link to="/product/discover-queries/">Discover</Link>, you can drill all the way down into a span for a single transaction and traverse multiple directions. This will accelerate your ability to debug slow HTTP requests, database queries, identify associated errors, and root out other bottlenecks.

!<Link to="./perf-event-detail.png">Discover span showing the map of the transactions (aka minimap) and the black dashed handlebars for adjusting the window selection.</Link>
...
!<Link to="./span-details.png">Span detail view shows the span id, trace id, parent span id, and other data such as tags.</Link>

To find these views, you can either go through the <Link to="/product/performance/transaction-summary">Transaction Summary</Link> or <Link to="/product/discover-queries/query-builder/">Query Builder</Link>. Event IDs will be linked to open the corresponding Event Detail.

**With Transaction Summary**

Select the <Link to="/product/performance/">Performance Homepage</Link>, then click the affected transaction to display the trace data.
...
_Note_: Users on the Team or Business plans can also view a list of transaction events by clicking on the "Transactions" pre-built query in <Link to="/product/discover-queries/">Discover</Link> or by performing a search with the `event.type:transaction` condition the Discover Query Builder view.
...
Gaps between spans may be marked as "Missing Instrumentation." This means a duration in the transaction that isn't accounted for by any of the transaction's spans. It likely means that you need to manually instrument that part of your process. Go back to <Link to="/product/performance/getting-started">Getting Started</Link> for details.
