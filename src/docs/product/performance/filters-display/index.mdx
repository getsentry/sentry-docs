...
The transaction filter on the performance page has been simplified to support metric enhanced performance. The search bar can only be used to filter by transaction name, rather than to search for transactions matching the query
