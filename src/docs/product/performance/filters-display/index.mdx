...
For certain users, the transaction filter on the performance page has been simplified to support metric enchanced performance. The search bar can only be used to filter by transaction name.
...
1. Clicking on a transaction name will apply a filter on the performance page to only display transaction data that matches the name.
2. Clicking the upwards arrow on a search result will take you to that transactions summary page.
