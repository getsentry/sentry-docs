...
If your organization is in the Early Adopter program, you will see tabs representing the options in the "Display" filter, rather than the dropdown list. 
...
The widgets offer visualizations that can be changed to best match your workflow. The first row offers timeseries and distributions of common metrics for the performance type selected. You can compare these side by side to find any correlations. The second row offers actionable widgets that can show you areas of concern in your application. These are coupled with direct links into other parts of the product which you can follow to see additional context. Each widget has a menu in the top right that can change the widget shown. These changes will be persisted locally either for the session or until application storage is cleared.
