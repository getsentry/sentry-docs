...
A query's source is the location in your application's code that triggered the query. Sentry automatically determines the query source by examining the stack traceh and attaches the information to the query data. In order to reduce the SDK's performance impact on your application, Sentry only attaches query sources to queries that are slower than a <PlatformLink to="/configuration/options">configurable threshold</PlatformLink>. If query sources are often missing, you can reduce this threshold.
