...
The Sentry JavaScript SDK collects Web Vitals information from the browser (where supported) and adds that information to frontend transactions. These Web Vitals are then summarized in several graphs for a quick overview of how each frontend transaction is performing for your users.
