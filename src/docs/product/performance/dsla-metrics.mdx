...
Although you can set custom metrics in the SDK, the ability to see the data generated by these metrics is in beta and is available only if your organization is participating in its limited release and you're on a plan that includes Performance Monitoring. Features in beta are still in-progress and may have bugs. We recognize the irony.
...
- Indexed events - A sub-set of your processed transaction events, based on your settings in [Dynamic Sampling](/product/data-management-settings/dynamic-sampling/dsla/). Underlies events-based data.

We capture metrics from all the transaction events that you send to Sentry, processed and indexed. However, if you want to improve the accuracy of your metrics, you only need to increase the number of processed events you send to Sentry. Learn more about how we sample transaction events in our [full Sampling documentation](/product/sentry-basics/sampling/dsla/).

Note that if you don't set a server-side sampling rate in **Dynamic Sampling**, or if your server-side sampling rate is the same as your client-side rate, then all the transaction events you send to Sentry will be processed _and_ indexed.
