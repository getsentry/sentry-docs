...
Retention priorities are Sentry's way of determining the types of data that are most important to keep so you can get a complete picture of your application's performance. They are enabled by default, but can be updated on a per-project basis to better fit your organization's needs.
...
Below is a list of the transactions we prioritize and deprioritize by default. Most can be customized on a per-project basis.
...
<div style="position: relative; padding-bottom: calc(52.60977630488816% + 41px); height: 0; width: 100%"><iframe src="https://demo.arcade.software/ETgxCQqyWN5uuCityFxT?embed" frameborder="0" loading="lazy" webkitallowfullscreen mozallowfullscreen allowfullscreen style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;color-scheme: light;" title="General Settings — totes-ma-gotes — Sentry"></iframe></div>
## Deciding on Your SDK Sample Rate

To give Sentry the fullest, most accurate picture of your application's health, we recommend that you send us 100% of your transactions. But if sending all your transactions doesn't fit your needs, we offer two SDK sampling configuration options:

The `tracesSampleRate` option enables you to control the percentage of transactions you send to Sentry.

The `TracesSampler` function (where available), allows you to manually target the specific transactions you send to Sentry and controls the sample rate based on the transaction itself and the context in which it's captured.

The two options are mutually exclusive. If you set up both, `tracesSampler` will take precedence.
