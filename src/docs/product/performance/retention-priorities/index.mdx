...
- [Admin-level permissions](/product/accounts/membership/).
...
Our automated retention priorities work well generically in order to collect a baseline of samples, because they prioritize retaining data that is valuable for any customer at any given time. But there are certain scenarios, such as investigating a particular issue, when very specific data becomes more important temporarily. In such cases, sometimes the automated sampling priorities do not ensure sufficient or even any samples to use during this investigation.

Investigation rules are sampling rules that can be created by users through the Sentry UI, to temporarily boost the amount of transactions collected for a particular scenario (for example transactions with specific tag values, etc.), when there are no or too few samples already available.

#### Creating a new Investigation Rule

When you perform a query on transactions either in [Discover](/product/discover-queries/) or in [Transaction Summary](/product/performance/transaction-summary/) and receive fewer than 5 samples, you are given the possibility to request retaining samples of transactions for the respective query. You can do so by clicking on the button labeled Get Samples which appears in the control panel.

![Investigation Mode in Discover](/product/performance/retention-priorities/investigation-bias-discover.png)

<Note>

Please note that because this feature only applies to transactions, in Discover it’s necessary to specify `event.type:transaction` in the query in order to access the feature, whereas in Performance the event type is implicit.
...
![Rule successfully created](/product/performance/retention-priorities/investigation-bias-rule-created.png)
...
![Investigation Mode in Transaction Summary](/product/performance/retention-priorities/investigation-bias-tx-summary.png)

#### Notes and current limitations

Once created, a rule remains active for up to 48h or until 100 samples are collected (this may change in the future). You will be notified via email once the collection is completed.
...
<div style="position: relative; padding-bottom: calc(62.74089935760171% + 41px); height: 0; width: 100%">
  <iframe
