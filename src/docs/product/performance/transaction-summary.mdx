...
While you can use the search field to narrow down your event search, you can also filter by http, db, browser and resource by toggling between operations in the Filter. This will be reflected in the rest of the page to enable targeted drilldowns.
...
On initial load, the table displays slow occurrences of the transaction along with the Event ID, User, [Operations Breakdown](/product/performance/event-detail/#operations-breakdown), Total Transaction Duration, and Timestamp of the event. Click on the Event ID to open the [event detail](/product/performance/event-detail/) or the Trace ID to view the [full trace](/product/performance/trace-view). Other sets of transactions you can browse through are below.
