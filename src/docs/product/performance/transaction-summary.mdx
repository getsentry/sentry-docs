...
The transaction summary includes a list of suspect tags that corresponds to where most of the time is spent. By default, we sort spans by the total exclusive time. An example event is provided and will take you directly to the span in question.
...
The **Spans** tab displays a list of suspect spans that corresponds to where most of the time is being spent. Choose from one of the metrics to identify different types of problems and filter down to the span operation you are interested in to find spans where performance problems lie. Example events are available and will take you directly to the span in question.

Suspect spans are determined using a span's exclusive time rather than its duration. The exclusive time of a span is the time spent in a span excluding the time spend in any child spans.

### Metrics

- **Cumulative Time**: The sum of the exclusive time for a span. This metric is useful to identify the span where the majority of the time is being spent.
- **Avg Occurrences**: The average number of times a span appears in a transaction. This metric is useful to identify redundant work being done within a transaction. It is often indicative of a n + 1 problem.
- **Occurrences**: The total number of times a span appears across all transactions. This metric is useful to identify spans that appear most frequently.
- **p50 Duration**: The 50th percentile of the exclusive time for a span. This metric is useful to identify spans that take a long time individually.
- **p75 Duration**: The 75th percentile of the exclusive time for a span. This metric is useful to identify spans that take a long time individually.
- **p95 Duration**: The 95th percentile of the exclusive time for a span. This metric is useful to identify spans that take a long time individually.
- **p99 Duration**: The 99th percentile of the exclusive time for a span. This metric is useful to identify spans that take a long time individually.
