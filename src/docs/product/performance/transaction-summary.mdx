...
The time series graph in the **Anomalies** tab shows your event throughput over time. Our anomaly detection highlights areas that have gone above or below the expected throughput for that time of day, week, or other period, based on your historical data. The different colors for anomalies represent the level of confidence we have that that area is anomalous.
