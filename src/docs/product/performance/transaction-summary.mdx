...
On this tab, suspect tag keys that often correspond to slower transactions are listed. Toggle through them to have corresponding tag values reflected in the heat map. Similar to a histogram, events are distributed by duration. Click on any purple box to view a list of events or continue to filter down the events by targeting values in the table below the heat map.
...
On this tab, the table offers a full list of transactions broken down similar to the abbreviated table in the overview. Filters are carried across from other views. You can also narrow the events down by percentile.

## Additional Actions

### Starring Transactions
