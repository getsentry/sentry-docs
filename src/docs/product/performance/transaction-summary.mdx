...
For this transaction, we'll list out suspect tags that often correspond to slower transactions. By default, we sort tags by the total time lost. Additional information includes:

- **Tag Key** is the tag category (ex. device, geo, etc.)
...
- **Comparison to Avg** notes how much slower or faster this tag is than the average duration of this transaction
- **Total Time Lost** calculates how much total time you've lost in the selected time range when comparing the average tag duration v. the average transaction duration
