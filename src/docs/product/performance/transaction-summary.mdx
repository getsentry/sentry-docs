...
The timeseries graph in **Anomalies** tab shows your event throughput over time. Our anomaly detection will highlight areas that have gone above or below the expected throughput for that time of day, week etc. based on your historical data. The different colors for anomalies represent the confidence we have that that area is anomalous.
