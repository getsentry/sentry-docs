...
![Example of Transaction Summary](./perf-summary-overview.png)
...
For this transaction, we'll list out suspect tags that we believe are causing some of your worst slowdowns. By default, we sort tags by the total time lost. Additional information includes:

- **Tag Key** is the tag category (ex. device, country, etc.)
- **Tag Value** is the specific tag value (ex. android, south america, etc.)
- **Avg (Tag) Duration** provides the average duration for a transaction with this tag
- **Comparison to Avg** notes how much slower or faster this tag is than the average duration of this transaction with other tags
- **Total Time Lost** calculates how much total time you've lost in the selected time range when coming the average tag duration v. the average transaction duration with other tags

To investigate why these tags are causing you problems, click on the tag value to filter this transaction summary further.
