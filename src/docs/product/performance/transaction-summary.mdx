...
On initial load, the table displays slow occurrences of the transaction along with the Event ID, User, Operation Breakdown, Total Transaction Duration, and Timestamp of the event. Click on the Event ID to open the [event detail](/product/performance/event-detail/) or the Trace ID to view the [full trace](/product/performance/trace-view). Other sets of transactions you can browse through are below.
