...
The time series graph in the **Anomalies** tab shows your transaction throughput over time. Our anomaly detection highlights areas that have gone above or below the expected throughput for that time of day, week, or other period, based on your historical data. The shading indicates our confidence that a particular time interval is anomalous.
