...
- **User Misery:** This graph displays the [User Misery](/performance/metrics/#user-misery) score for the transaction. User Misery is a score that represents the number of unique users who have experienced load times four times the project's configured threshold. 
