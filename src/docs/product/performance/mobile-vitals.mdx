...
Mobile Vitals are a set of metrics to measure how fast your app starts, the duration of HTTP requests, the number of slow and frozen frames, and more. Each metric provides insights into the overall performance of your mobile application. These metrics give you the details you need to not only prioritize critical performance issues, but to trace issues down to the root cause to solve them faster.
...
The frame rate can also be higher, especially as 120 fps displays are becoming more popular, which will change the associated slow frame measurement. This is reflected in the graphs on the **Performance** page when the "Display" selection is "Mobile".
