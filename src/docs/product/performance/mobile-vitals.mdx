...
- **Time to initial display**: tracks how long your mobile application takes to produce its first frame, including app start time on the first screen loaded. It doesn't include any content loaded lazily after the first frame is drawn.
- **Time to full display**: tracks how long your mobile application takes to produce its first frame with full content, including content loaded asynchronously after the first frame. For example after loading some content from the network.
Time to initial display is automatic and enabled by default.
Time to full display is opt-in and requires you to manually call the API to report the screen has loaded all of its content and is fully displayed.
