...
The Python SDK starting with version 1.30.0 manages backpressure internally by reducing sampling dynamically when the system is found to be in an unhealthy state. This ensures better system stability and lower SDK overhead under heavy load. We will gradually roll this out to other backend languages in the future.
