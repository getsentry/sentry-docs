...
First Input Delay measures the response time when the user tries to interact with the viewport. Actions may include clicking a button, scrolling or other custom Javascript controller. It is key in helping the user determine if a page is usable or not.

## Cumulative Layout Shift (CLS)

INSERT CLS IMAGE HERE

Cumulative Layout Shift is the sum of individual layout shift scores for every unexpected element shift that occurs during the rendering process. Imagine navigating to an article and trying to click a link before the page finishes loading. Before your cursor even gets there, the link may have shifted down due to an mage render. Rather than using duration for this web vital, the score represents how disruptive and visually unstable the shifts are.
