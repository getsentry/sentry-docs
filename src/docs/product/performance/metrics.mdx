...
For organizations in the Early Adopter program, you can improve the accuracy of your metrics by increasing the number of processed transaction events that you send to Sentry. Processed transaction events are events for which we capture metrics and event counts, but not event details. Learn more in our [full Sampling documentation](/product/sentry-basics/sampling/).
...
Custom performance metrics on transactions allow you to define metrics (beyond the ones mentioned above) that are important to your application and send them to Sentry. Currently, you can set these in the [JavaScript (version `7.0.0` or higher)](/platforms/javascript/performance/instrumentation/performance-metrics/) and [Python (version `1.5.12` or higher)](/platforms/python/performance/instrumentation/performance-metrics/) SDKs.

## Metrics and Sampling

<Include name="early-adopter-note.mdx" />

Performance metrics are generated based on the transactions that you send to Sentry and these transactions fall into one of two types of events:

- Processed events - All the transaction events that you send to Sentry, based on your client-side sample rate in your SDK.
- Indexed events - A sub-set of your processed transaction events, based on your settings in [Dynamic Sampling](/product/data-management-settings/dynamic-sampling/). Underlies events-based data.

We capture metrics from all the transaction events that you send to Sentry, processed and indexed. However, if you want to improve the accuracy of your metrics, you only need to increase the number of processed events you send to Sentry. Learn more about how we sample transaction events in our [full Sampling documentation](/product/sentry-basics/sampling/).
