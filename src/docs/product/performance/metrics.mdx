...
The p50 threshold is the value at which 50% of transaction durations are greater than the threshold, when all the durations in a data set are compared to each other. This is also the median. For example, if the p50 threshold is 10 milliseconds, then 50% of transactions exceeded that threshold, taking longer than 10 milliseconds.

### P75 Threshold

The p75 threshold indicates the value at which 25% of transaction durations are greater than the threshold, when all the durations in a data set are compared to each other. For example, if the p75 threshold is 10 milliseconds, then 25% of transactions exceeded that threshold, taking longer than 10 milliseconds.

### P95 Threshold

The p95 threshold is the value at which 5% of transaction durations are greater than the threshold, when all the durations in a data set are compared to each other. For example, if the p95 threshold is 50 milliseconds, then 5% of transactions exceeded that threshold, taking longer than 50 milliseconds.

### P99 Threshold

The p99 threshold is the value at which 1% of transaction durations are greater than the threshold, when all the durations in a data set are compared to each other. For example, if the p99 threshold is five seconds, then 1% of transactions exceeded that threshold, taking longer than five seconds.
...
This feature is only available to organization on our latest plans which include [Dynamic Sampling](/product/data-management-settings/dynamic-sampling/). Customers on legacy plans must move to one of these plans in order to access custom metrics. For general questions about how you can move to plan that includes Dynamic Sampling and custom metrics, please contact [support@sentry.io](mailto:support@sentry.io). For information on volume pricing, please contact [sales@sentry.io](mailto:sales@sentry.io).
