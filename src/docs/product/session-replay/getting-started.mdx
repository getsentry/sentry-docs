...
Backend error linking works by integrating replays with Sentry's tracing data model. When you have Sentry set up for both your frontend and backend, there's end-to-end data connectivity on your APIs which can link replays to backend endpoints.

If backend error linking is enabled, you'll be able to view backend errors in the Timeline, Breadcrumbs, Errors tab, and other places when you go to the **Replay Details** view. You'll also see a Replays tab and replay previews in the **Issue Details** page for any backend error that has a linked replay.
...
Currently, backend error linking is supported for the following SDK versions or newer:
