...
While the performance overhead of our client SDK will be imperceptible to end-users most of the time, it can vary based on the complexity of your application. If an application has a large DOM and numerous DOM node mutations, it will have a higher overhead than simpler, mostly static sites.
...
1. Pageload
   ...
   - The size of your initial HTML page affects how long the SDK takes to complete the initial snapshot and send it over the wire. Using your browser devtools, filter the Network panel to show "type=document" and look at the size to get a sense of the impact on your site. For example, if you show a loading screen on a single-page-app then the overhead is negligible.
2. User scrolling & consumption
   - Mouse-move & hover events are incredibly small; overhead is negligible.
