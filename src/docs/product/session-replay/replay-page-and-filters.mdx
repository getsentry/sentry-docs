...
- **Dead Clicks**: A user click that does not result in any page activity after 7 seconds (i.e. no HTML was added, removed, or updated; no visual changes were observed in the page.) They only affect `a` and `button` tags.
...
The minimum SDK version required to see rage and dead click data on the Replay index page is 7.60.1.
