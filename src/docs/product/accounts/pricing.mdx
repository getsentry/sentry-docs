...
When you buy reserved volume (or quota), you prepay for data at significant discounts. Your prepaid, reserved volume is always applied before on-demand volume kicks in. Unused reserved volume expires at the end of each billing month.

Each Sentry plan includes different amounts of reserved volume. For example, our Team plan includes a minimum of 50k errors, 100k transactions, 500 replays, and 1GB of attachments per month. To see prepaid pricing for each plan, check out our [per-data-type-pricing](#per-data-type-pricing).
