...
This page gives you an in-depth understanding of out pricing and billing structure, including:

- [How our pricing works](#pricing-how-it-works), including [per-data-type-pricing](#per-data-type-pricing)
...
We bill based on the amount of data you want us to process. Our Developer, Team, and Business plans each come with a pre-set number of events (such as errors and transactions) that are included in the price. 
...
- **[On-demand volume:]**(#on-demand-capacity) On-demand volume: An overage budget that kicks in only if your prepaid volume has been used up. Having this makes it possible to control the maximum amount you will spend for overages each month. 
- 
...
| >10M-15M | $0.000039 | $0.000051 | $0.000074 | $0.000096 |
...
| >25k-100k | 25k | $0.00 | $0.003705 | 
| >100k-1M | $0.002565 | $0.000195 | $0.003335 | 
...
| Errors Volume&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; | Purchase Increment | Transaction Volume | Purchase Increment | Attachments Volume | Purchase Increment |Replay Volume | Purchase Increment |
|---|---|---|---|---|---|---|---|
| >50k-100k | 50k | >100k-250k | 150k | >25 - 100 (GB) | 25 (GB) | >10k - 25k | 15k |
| >100k-500k | 100k | >250k-500k | 250k | >100 - 10k (GB) | 100 (GB) |>25k - 100k | 25k |
| >500k-2M | 500k | >500k-10M | 500k |  |  |>100k - 1M | 100k |
| >2M-20M | 1M | >10M-100M | 5M |  |  |  |  |
| >20M-50M | 2M |  |  |  |  |  |  |

<!-- prettier-ignore-end -->

If the amount of prepaid data you want to buy falls in-between increments, you may want to consider setting an on-demand budget, (which is based on a dollar amount rather than data volume), instead. 

Use the slider on the [pricing](https://sentry.io/pricing/) page to determine the amount of prepaid data you need, based on the event types you send. 
...
Your _volume_ is the total number of [events](/product/sentry-basics/key-terms/) and attachments — or the total amount of [data](/product/sentry-basics/key-terms/) — sent by and billed to your organization. You can purchase volume in two ways: [prepaid (or reserved)](#prepaid-reserved-capacity) or [on-demand](#on-demand-capacity).

### Prepaid (Reserved) Volume {#prepaid-reserved-capacity}

When you buy _reserved_ volume (or quota), you prepay for data at significant discounts. Your prepaid, reserved volume is always applied before on-demand volume kicks in. Unused reserved volume expires at the end of each billing month.

Each Sentry plan type includes different amounts of reserved volume. For example, our Team plan includes a minimum of 50k errors, 100k transactions, 500 replays, and 1GB of attachments per month:

![Reserved Volume settings in Subscriptions](reserved-volume.png)

To see prepaid pricing for each plan type, check out our [per-data-type-pricing](#per-data-type-pricing).

### On-Demand Volume {#on-demand-capacity}

Buying _on-demand volume_, lets you control the maximum amount you’ll pay for overage each month after your reserved volume has run out. If you go over your on-demand volume,  you’ll still have access to previous event data, but any new data you send will be rejected and you won’t be charged for it.

With on-demand volume, you pay for any event or attachment processed, (rounded up to the nearest cent), after your reserved volume has run out. On-demand volume is billed at the end of each billing month. To see on-demand pricing per plan type, check out our [per-data-type-pricing](#per-data-type-pricing).
...
- **Shared:** Your on-demand budget is shared between errors, transactions, replays and attachments.

- **Per-category:** Separate on-demand budgets for errors, transactions, replays and attachments. Any overages in one category will not consume the budget of another category.
