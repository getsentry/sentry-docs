...
Sentry has moved to unit-based Performance pricing where customers are charged based on a volume of total units for two different Performance event types:

- **Transactions** - Consumes 1 Performance Unit and is sent by projects with only [Tracing](/product/sentry-basics/tracing/) enabled.
- **Transactions with Profiling** - Consumes 1.3 Performance Units and is sent by projects that have both [Tracing](/product/sentry-basics/tracing/) and [Profiling](/product/profiling/) enabled.

_For example, if you send 1M Transactions and 1M Transactions with Profiling, you will be consuming 2.3M units of your total quota [(1M at 1 unit) + (1M at 1.3 units)]._
...
Performance Units can be used towards multiple Performance event types, depending on what you have enabled:

| Event Type | Unit Value | |
|---|---|---|
| Transactions | 1 Unit | Sent by SDKs with only [Tracing](/product/sentry-basics/tracing/) enabled |
| Transactions with Profiling | 1.3 Units | Sent by SDKs with both [Tracing](/product/sentry-basics/tracing/) and [Profiling](/product/profiling/) enabled |
