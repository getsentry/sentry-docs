...
On-demand volume is billed at the end of each billing month. You are charged per processed event, rounded up to the nearest cent. You have the ability to set on-demand budgets (or quotas) so you can stay on budget. Your on-demand volume is determined by your on-demand budget, which can be configured on either a shared or per-category basis. Learn more in [On-Demand Budgets](#on-demand-cap).
