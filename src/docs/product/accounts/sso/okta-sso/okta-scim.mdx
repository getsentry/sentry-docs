...
Once these changes have been made, newly assigned users will be sent an invitation email. If a user gets un-assigned, their membership status in Sentry will be deleted.

You can use "Push Groups" to sync and assign groups in Okta; they'll be reflected in Sentry.
...
- The Import Users feature is not currently supported because Sentry's SCIM API doesn't support the User `name` attribute fields: `firstName` and `lastName`at this time. For compatibility purposes, they're returned with `N/A` values. 
