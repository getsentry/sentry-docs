...
Creating users through Okta will lock the created user’s membership within Sentry.

</Alert>

![Okta Provisioned User Unable to Leave](./okta-provisioned-unable-to-leave.png)

![Okta Unable to Remove Provisioned User](./okta-provisioned-unable-to-remove.png)
...
Setting the organization-level role for a user through Okta will lock that user's role within Sentry.

</Alert>

Users who have had their role assigned via Okta will be unable to change their role from within the Sentry application.
...
#### I'm unable to remove a member from my organization from within Sentry because their organization membership is managed by an identity provider

An organization member whose membership is managed by an identity provider (e.g. Okta) can only be removed from the organization in Sentry by removing them via the associated identity provider.

#### I'm unable to add or remove a member from my team in Sentry

Groups in Okta that have been pushed to Sentry will create teams that have their team membership locked. Adding or removing members from these teams must be done through Okta.

#### I removed a user from the Sentry app in Okta, but the user still exists in my Sentry organization

Make sure that `Deactivate` User is enabled in the "Provisioning" tab of your Sentry application in Okta. Re-provision the user to your Sentry organization via Okta, then remove them again.

#### How do I unset the organization-level role for a user?

If the user is re-provisioned with a blank (default) role, then the user's role in Sentry won't be restricted via Okta.
