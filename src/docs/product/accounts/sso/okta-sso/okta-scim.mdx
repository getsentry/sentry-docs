...
### Requirements
- You must be on a Sentry Business Plan or above.
- You must have configured SAML SSO for okta. See [documentation here](/product/accounts/sso/okta-sso/), or the Okta sign-on tab in your Sentry Okta application for configuring SAML.
- In your Sign On configuration, **You must select Email for the Application username format** on the Sign On application tab in Okta.
...
- The User `name` attribute will contain fields firstName and lastName with the values of `N/A`. Sentry's SCIM API does not currently support these fields but returns them for compatibility purposes.
...
- When provisioning a new team, Sentry will normalize and convert the team `displayName` uppercase to lowercase, and spaces to dashes.
- The GET /Groups endpoint will return a maximum of 10000 members in a group.
- See [SCIM API documentation](/api/scim/) for more information.
