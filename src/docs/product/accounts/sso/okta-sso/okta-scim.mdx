...
Sentry currently only supports updating user attributes for organization-level roles. Once you have added the `sentryOrgRole` attribute to your Okta application profile, you can edit the attribute for users assigned to your application.

![Okta Edit Attribute](okta-edit-sentryOrgRole-attribute.png)

Editing a user's role attribute to blank will remove the organization-level role in Sentry. The user's role in Sentry will default to the organization role selected in your Auth settings.
...
Edit the user's role attribute to blank.
