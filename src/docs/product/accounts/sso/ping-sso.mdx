...
1. Go to "Adminstrators" environment for your organization.

1. If you don't require a permission group for Sentry, skip to Step 5.

1. In the sidebar, click on "Identities" then "Groups", and navigate to it. From there, create a new group, add yourself and other members to it. For more details about group creation, see the [Ping docs](https://docs.pingidentity.com/bundle/pingone/page/vdz1610472224361.html).
...
1. For Section (3), labeled "Attribute Mappings". For "Application Attribute" ← "Outgoing Value", use:
   ...
   - `last_name` ← `Family Name`
     ...
     You can use custom key values, but you will need to amend the steps below. If you would like to use custom attribute mapping, see

1. Once the application is created, click on the Sentry application and navigate to "Configuration" and copy "IDP Metadata URL"

   ![Ping configuration for Sentry](ping-configure-sentry.png)

1) Go back to Sentry. Navigate to your **Org Settings > Auth** (or go to `https://sentry.io/settings/YOUR_ORG_SLUG/auth/`) and click on "Configure" for SAML2.

1) Paste the IDP Metadata URL from above and click "Get Metadata".

1) In the next page, enter the following keys in their respective fields to map the Ping ID attributes to Sentry, and then save them.
   ...
   - Last Name: `last_name`
     ...
     For more details about mappings for custom configs, see [this guide on Ping](https://support.pingidentity.com/s/article/PingOne-Using-Advanced-Attribute-Mapping).

1) Sentry will attempt to authenticate and link your account with Ping. After successful authentication, you'll be redirected to Sentry's SSO configuration page, where you can take the following actions:
