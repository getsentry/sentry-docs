...
1. Provide the settings. Note that for "1. General Settings", you will not need to provide any special settings. For "2. Sign On Options", provide the following:
   ...
   - Base URL: `https://sentry.io` <Alert level="warning">Ensure that the URL doesn't have a trailing slash or whitespace at the end as it will break the redirect from Okta to Sentry.</Alert>
