...
<Alert level="warning" title="No Trailing Spaces">

Confirm the URL doesn't have a trailing whitespace at the end as it will break the redirect from Okta to Sentry.
...
## SCIM Integration
<Alert level="warning" title="SCIM is not available for general use yet.">

SCIM functionality is currently in early access and is not available to all organizations. Please reach out to support@sentry.io if you'd like to participate in early access.
</Alert>
