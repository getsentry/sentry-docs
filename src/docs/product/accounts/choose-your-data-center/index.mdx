...
<Note>
  Once selected, your data storage location can’t be changed. The only way to
  switch regions is by creating a new account.
</Note>
...
Here’s a list of the types of data that will be stored in whichever local region (US or EU) you select.
...
At this time, your data storage location can’t be changed if you have an existing SaaS organization. The only way to switch regions is by creating a new organization.

### Self-hosted

If you have a self-hosted Sentry account, you can [follow these instructions](https://docs.sentry.io/product/accounts/migration/) to migrate your data.
...
If you have multiple orgs, they will be treated like separate accounts regardless of where data is stored. This means that billing, quota, user management, and so on, should all be managed separately for each org.
