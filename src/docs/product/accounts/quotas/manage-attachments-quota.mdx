...
Once applied, you can track the filtered attachments (numbers and cause) using the graph provided at the top of the "Inbound Filters" page.

![Built-in Inbound Filters](manage-event-stream-03.png)

<Note>

The **Inbound Data Filters** page in [sentry.io](https://sentry.io) only refers to errors, but it applies to attachments as well.
...
Sentry supports filtering out a specific or certain kind of attachments as well. Navigate to **[Project] > Settings > Inbound Filters**, then add the attachment name to the "Error Message" field.

<!--how do we do this exactly for attachments? or do we? -->
...
To avoid using up your attachments quota — whch is based on size and not number of instaces of data sent — or having the event dropped entirely, consider limiting the size of values passed into Sentry's APIs. For example, if your application attaches application state or request bodies to Sentry events, truncate them first.
