...
Events and quotas are interconnected in Sentry. At the most basic level, when you [subscribe to Sentry](https://sentry.io/pricing/), you pay for the number of events — errors and transactions — to be tracked. Each of these has a quota, and when Sentry accepts an event, it counts toward your quota for that type of event. We also track and provide a quota for for attachments.
