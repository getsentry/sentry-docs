...
- **Event** - An event is one instance of you sending Sentry data. Generally, this data is an error or a transaction.
- **Issue** - An issue is a grouping of similar error events, which all share the same fingerprint. Every error event has a set of characteristics, called its fingerprint. For example, Sentry groups events together when they are triggered by the same part of your code. Learn more in our full [Issue Grouping documentation](/product/data-management-settings/event-grouping/).
...
3. **Quota availability**
   ...
   Events that exceed your quota are not accepted. Once your event volume is approaching or has exceeded the quota, teammates with the "owner" organization permission level will receive [notification](/product/alerts/notifications/#quota-notifications) emails. To add to your quota or review what happens when you exceed it, see [Increasing Quotas](#increasing-quotas).

4. **Event repetition**

    Event repeition only applies to error events, not transactions or attachments. The following rules apply for event repeition and your quota:

    - If you have intervened to "Delete & Discard" an issue, then _future_ error events with the same fingerprint do not count toward your quota.
    - If the issue was previously was resolved, this error event counts toward your quota because it may represent a regression in your code.
    - If you have intervened to ignore alerts about error events with the same fingerprint, this event counts toward your quota because the event is still occurring. For more information, see [Inbound Filters](#inbound-data-filters).

5. **Spike protection**

   Sentry’s spike protection prevents huge overages in error events from consuming your event capacity. Spike protection is not currently available for transactions. For more information, see [Spike Protection](#spike-protection).
...
6. **Rate limit for the project**
   ...
   If the error event rate limit for the project has been exceeded, and your subscription allows, the event will not be counted. For more information, see [Rate Limiting in our guide to Manage Your Event Stream](/product/accounts/quotas/manage-event-stream-guide/#6-rate-limiting) or [Error Event Limits](/product/accounts/quotas/#error-event-limits). This does not apply to transactions or attachments.

7. **Inbound filters**

   If any inbound filter is set for this type of error event, and your subscription allows, the event will not be counted. For more information, see [Inbound Filters in our guide to Manage Your Event Stream](/product/accounts/quotas/manage-event-stream-guide/#3-inbound-data-filters) or [Inbound Data Filter](#inbound-data-filters). This does not apply to transactions or attachments.
