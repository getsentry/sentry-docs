...
This page is accessible to all members of your organization, so a Billing or Owner member in your Sentry org can share this page with the developers directly responsible for a given project. Also, you can come back to this page to check if the changes you've made are having the desired effect.
...
**We also include a one-time grace period for all paid accounts.** If an account depletes its event volume and has never previously triggered a grace period, we continue to accept events for the next three days at a maximum rate of 10,000 events per hour. If you choose not to increase your volume, Sentry will stop accepting events for the remainder of the current billing month. However, if you increase your volume, events accepted during the grace period will be counted as consumed volume.

<!-- does this belong with spike protection or is this an entirely separate thing? does spike protection have to be enabled for the grace period to kick in?? -->

## How Does Spike Protection Work?

Because Sentry bills based on monthly event volume, spikes can easily consume your volume for the rest of the month. Sentry's spike protection prevents these types of overages from consuming your error event volume by dropping error events during the spike.
...
To manage or avoid future spikes, we recommend taking the following steps:
...
<!-- this last sentence sounds like a penalty of some sort; is it? or is just a fact? -->
...
Once your event volume is approaching or has exceeded the quota, teammates with the "Owner" organization permission level will receive [notification](/product/alerts/notifications/#quota-notifications) emails.
...
Events dropped because of rate limiting generate a 429 error code.
...
1. Based on that, choose a **daily** maximum value or ceiling. In this example, we set a daily maximum of approximantely 35K, which is around 1458 events an **hour**, or about 24 events a **minute**.
...
After these checks are processed, events and attachments that aren't dropped based on these filters toward your quota. They're accepted into Sentry, where they're persisted and stored.
...
If a sample rate is defined for the SDK, the SDK evaluates whether this event should be sent as a representative fraction of events. Setting a sample rate is \* <PlatformLink to="/configuration/sampling/">documented for each SDK</PlatformLink>.
...
The Sentry SDKs have several configuration options that can be used to filter unwanted errors from leaving your application's runtime. A lot of these options are platform-specific, so make sure you look for yours in our docs under [Platforms](/platforms/) and [Configuration](/platform-redirect/?next=/configuration/). Following are some configuration examples for [JavaScript](#javascript) and [other SDKs](#other-sdks).
