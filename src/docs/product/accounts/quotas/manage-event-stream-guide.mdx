...
Limited availability spike protection is a project-level tool that helps prevent quota overconsupmption. It's enabled for every project by default, and when it's enabled, Sentry continually monitors for spikes. You can confirm that it's enabled in **[Project] > Settings > General Settings**.
...
- Compares this final number against a minimum number of events to trigger a spike that is determined using your quota
...
In the graph below, we can see a zoomed in perspective of the 12-hour period of the spike, along with a line indicating the spike limit as it’s being recalculated over the course of the spike:
