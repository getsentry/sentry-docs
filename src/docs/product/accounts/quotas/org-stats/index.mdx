...
The cards on the page provide high-level usage stats about events, including which ones were *Accepted*, *Filtered, or Dropped*. Filtered events are events that were blocked due to your inbound data filter rules. Dropped events are ones that the system discarded due to rate limits, quotas, spike protection, or because of invalid data.

## Summary Chart

The chart on this page shows a breakdown of events over the selected time range. Depending on the time range that you set the chart to display, each bar in the chart represents a different amount of time. For example, if you set the chart to display 7 days, each bar in the chart represents one hour, but if you set it to display 90 days, each bar represents one day. You can also use the “Type” dropdown to set whether the chart shows a cumulative count of events or a daily (or hourly) count.

## Project Usage Stats Table

The “Project Usage Stats” table provides insight into per-project usage of your errors, transactions, and attachments over time. The table shows you the total errors, transactions, or attachments and then those events are further broken down into the Accepted, Filtered, and Dropped categories.
