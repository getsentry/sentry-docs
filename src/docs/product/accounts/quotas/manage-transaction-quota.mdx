...
In most cases, sending all [transaction](/product/performance/transaction-summary/#what-is-a-transaction) events to Sentry could generate more performance data than you'd find useful and might use up your transactions quota too quickly. Sentry provides tools to control the _type_ and _amount_ of transactions that are monitored. This allows you to have transaction data that's actionable and meaningful, and to manage your quota.

Applying the proper SDK configuration is an iterative and on-going process, but these tips will show you how to best use Sentry's tools to get the most out of your transaction events. These tips are ordered from easiest or least time-consuming to most challenging or potentially time-consuming. The first two options are things you can do in [sentry.io](https://sentry.io), whereas the remaining ones are things you'll do in the SDK.
...
<!-- Screenshot showing transactions? -->

### What are my busiest projects?

The "Project" table in the "Usage Stats" tab of **Stats** breaks down your events by project so you can see which ones are consuming your quota. Clicking on the settings icon next to a project name in the table will open the project's settings, where you can manage its [inbound filters](#3-inbound-data-filters).
...
The **Inbound Data Filters** page in [sentry.io](https://sentry.io) only refers to errors, but it is applicable for transactions as well.
...
After these checks are processed, events that aren't dropped based on these filters toward your quota. They're accepted into Sentry, where they're persisted and stored.
...
If you discover a problematic release causing excessive noise, Sentry supports ignoring all events from that release. Navigate to **[Project] > Settings > Inbound Filters**, then add the release to the "Releases" field.
...
Sentry supports filtering out a specific or certain kind of transaction as well. Navigate to **[Project] > Settings > Inbound Filters**, then add the transaction name to the "Error Message" field.
