...
When a spike is detected, team members with Billing or Owner-level permissions in Sentry will be notified via email:

**We recommend taking the following steps for managing your spikes:**

- Check to see which issues are [consuming your quota](https://sentry.sentry.io/stats/).
- Set [rate limits](https://docs.sentry.io/product/accounts/quotas/manage-event-stream-guide/#6-rate-limiting) on the DSN keys for the projects related to the spike.
- Set up [metric alerts](https://docs.sentry.io/product/alerts/alert-types/#metric-alerts) for the number of errors in a project.
- If a specific release version has caused the spike, add the version identifier to the project's [inbound filters](https://docs.sentry.io/product/accounts/quotas/manage-event-stream-guide/#3-inbound-data-filters) to avoid accepting events from that release.
- Set up an [on-demand budget](https://docs.sentry.io/product/accounts/quotas/manage-event-stream-guide/#on-demand-budget) to make sure you have time to adjust your volume in the event of a future spike.
...
By default, all team members with Billing or Owner-level permissions will receive email notifications for projects that have Spike Protection turned on. To disable this, go to **Organization Settings > Spike protection > Delete**.

![Delete a spike protection notification](spike-protection-delete-notification.png)

You can also add an action to send spike protection notifications for a project to Slack or PagerDuty from this page.

![Send spike protection notifications to Slack or PagerDuty](spike-protection-slack-pagerduty.png)

To turn off notifications only for yourself, go to **User Settings > Notifications > Spike Protection**. You can choose to turn off notifications for all projects, or customize notifications on a per-project basis.
