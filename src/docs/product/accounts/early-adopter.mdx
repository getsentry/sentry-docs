...
- Organization Domains - Instead of being a path segment, organization slugs are used as a subdomain for [sentry.io](https://sentry.io). For example, `https://acme.sentry.io/issues/` will replace `https://sentry.io/organizations/acme/issues/`. API URLs are unchanged. 
