...
Alerts when the number of crashed sessions exceeds the threshold you set.

A session begins when a user starts the application and ends when it’s closed or sent to the background. A crash is when a session ends due to an error.

#### Crash Free User Rate

Alerts when the overall user experience dips below the threshold you set.

Crash Free Users represents the percentage of individual users who haven’t experienced a crash.

### Performance

Application performance alerts can help you pinpoint and identify specific problems that could be causing a suboptimal experience for your users.
...
This alert can be set to monitor how long it takes for transactions to complete. It can be customized with flexible aggregates like percentiles, averages, and min/max.

#### Apdex

Alerts on the Apdex score (the ratio of satisfactory, tolerable, and frustrated requests in a specific transaction or endpoint).
...
Custom alerts that can be set up to be triggered by metrics which are not listed above, such as first paint (FP), first contentful paint (FCP) and time to first byte (TTFB).
