...
The **Alert Details** page shows you the number of times an issue alert rule was triggered over time, grouped in one hour time buckets. Clicking on the Alert Rule name on the "Alert Rules" tab, or the notification you receive will take you to this page. The page also includes details such as the alert rule conditions, the current status of the alert (Warning, Critical, or Resolved), and its details such as when it was created, last modified, and the Team that owns the alert.

The **Alert Details** page also includes a list of issues that triggered the alert. You can see what issues caused the alert, and then open that issue's page to find more information.
