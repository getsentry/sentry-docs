...
Filtering metric alerts by additional tags or properties is possible, however we will only start collecting these metrics once the alert is saved. If such a field is used, we will provide an estimation for the previous period during alerts creation.
