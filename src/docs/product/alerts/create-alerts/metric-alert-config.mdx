...
For "Crash Free Session Rate" and "Crash Free User Rate" alerts, these thresholds are percentages rather than numerical values. Use these to tell you when the crash free percentage for either [sessions or users](/product/releases/health/#active-sessionsusers) falls below a specific threshold, which could happen because of a spike in the number of session or user [crashes](/product/releases/health/#crash).
