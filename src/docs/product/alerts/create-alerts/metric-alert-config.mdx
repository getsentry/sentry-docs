...
![Configuring a metric alert.](../metric-alert-wizard.png)
...
There are two threshold types: fixed (such as when there’s 100 errors in a period of time) and percent change (when there’s 10% more errors in this time period compared to a previous period). By default the fixed threshold is set.

<Note>
This feature is available only if your organization is on either a Business or Trial plan.
</Note>

Percent change is useful when you want to know when the metric is significantly different from what’s normal. Once selected, you’ll need to set the time period to compare to. One example could be comparing the number of errors in the last hour to the same time period 1 week ago. If errors jump by 10% in the last hour then an alert will trigger.

## Set threshold to trigger alert

You can set the status of an alert rule when a thresholds is met, such as:
