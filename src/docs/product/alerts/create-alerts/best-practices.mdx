...
- **Percent of sessions affected by an issue**: Error counts and users affected a) can be hard to reason about, b) require constant manual adjustments as your traffic patterns change, and c) are not well suited to deal with seasonality (for example, fewer errors on the weekend). In such cases, if you've configured your project to capture session data, you can opt for alerting when an issue affects a certain percentage of user sessions.
