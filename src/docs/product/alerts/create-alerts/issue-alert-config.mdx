...
The preview displays a list of issues that would have triggered the alert you've configured and when they would have last triggered it.

Rules with any of following are not previewable:
- The issue is assigned to {no one/a team/a member}.
- The event is from the latest release.
- An issue affects more than [{X} percent of sessions in {time}](#percent-based-alerts)
- No ["When" conditions](#when-conditions-triggers)
- [Issue frequency](#change-alerts) conditions and [event filters](#if-conditions-filters) (filters that start with "The event")
