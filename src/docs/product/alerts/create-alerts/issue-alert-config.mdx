...
![Configuring an issue alert.](../issue-alert-wizard.png)
...
## Set Conditions to Trigger the Alert

To trigger an alert on issues you can set the "When", "If", and "Then" conditions.

![The conditions that can be set to alert on an issue.](../issue-alert-conditions.png)

### When

"When" conditions specify what type of activity you’d like monitored for the issue. These triggers are optional and if you don’t select a "When" condition then all events that Sentry detects will meet this condition.

You can set an alert to trigger when [an issue’s state changes](/product/issues/states-triage/), such as:

- When an issue is first seen by Sentry
- When an issues changes state from `resolved` or `ignored` to `unresolved`

You can also trigger an alert when an issue’s events and users increases, for example:

- When an issue has a high number of events
- When an issue affects a lot of unique users
- When an issue affects more than a percentage of [sessions](/product/releases/health/#session)
