...
- The number of events in an issue is more than a certain number or [percent change](#change-alerts) in an interval
- The number of unique users is more than a certain number or [or percent change](#change-alerts) in an interval
...
When selecting the number of events or users affected by an issue, you’ll have the option to set a threshold based on percent change. Percent change thresholds are useful for alerting you when the number of errors in an issue or the number of unique users affected is significantly different from normal.

For example, an alert can be triggered when an issue is affecting 10% more unique users in an hour compared to a week ago. Or when the number of events in an issue is 40% higher this week compared to 30 days ago.
