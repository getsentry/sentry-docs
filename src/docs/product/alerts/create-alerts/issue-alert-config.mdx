...
The preview displays a list of issues that would have triggered the alert you've configured and when they would have last triggered it. Previews can't be generated for certain alert rules and previews containing issue frequency conditions are approximated. Support for certain low priority conditions have not yet been added, as well as rules that use both frequency conditions and event filters.
