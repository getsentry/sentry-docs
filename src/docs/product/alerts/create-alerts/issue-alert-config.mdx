...
Suggested Assignees are a combination of [Ownership Rules](/product/issues/ownership-rules/) and [Suspect Committers](/product/issues/suspect-commits/).

Suggested Assignees can receive notifications by email or Slack depending on the [notification settings](/product/alerts/notifications/notification-settings/) of the assignee when an alert is triggered.

If a suggested assignee is not configured or not found, the notification will be sent to the teams or members specified in the fallback notification setting:
