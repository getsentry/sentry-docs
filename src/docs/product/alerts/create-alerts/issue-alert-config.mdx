...
- *Beta* An issue affects more than {X} percent of [sessions](/product/releases/health/#session) in {time}. This is an approximation and will trigger when the ratio of the issue’s frequency to the number of sessions exceeds the threshold.
