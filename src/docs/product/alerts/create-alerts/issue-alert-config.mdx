...
If an issue owner is not configured or not found, the notification will be sent to the teams or members specified in the fallback setting:

The following members will be notified based on the option selected and their notification settings:
- All Project Members: All members that are on a team that has access to the project will be notified. This has the potential to notify a large number of people. If two teams have access to the project, all members of both teams will be notified.
- Team Admins + Recently active members: Team admins of teams that have access to the project along with 10 of the most recently logged in members.
- Nobody: If the issue has no issue owner, no one will be notified.

![Issue owners with notify all project members selected.](issue_owners_all_members.png)
