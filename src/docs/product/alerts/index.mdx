...
[Issue alerts](/product/alerts/alert-types/#issue-alerts) trigger whenever any issue in a project matches specified criteria, such as:

- When a new issue is created by Sentry
- When the number of errors or users affected by an issue increases
- When resolved and ignored issues become unresolved
...
- When the total errors in your project increases
- When [Latency](/product/performance/metrics/#latency) changes: min, max, average, percentile
- When the [Failure rate](/product/performance/metrics/#failure-rate) increases
- When a custom metric changes
