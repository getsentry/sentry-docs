...
- **Issue owners**: Use [ownership rules and code owners](/product/issues/issue-owners/) to let Sentry automatically send alerts to the right people, as well as to ease configuration burden. You can configure ownership rules in **[Project] > Settings > Issue Owners**. When there are no matching owners, the alert goes to all project members by default. If this is too broad, and you'd like a specific owner to be the fallback, end your ownership rules with a rule like `*:<owner>`.
