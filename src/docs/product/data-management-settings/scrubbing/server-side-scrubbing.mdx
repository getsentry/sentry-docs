...
Using the default options would cause the _object_ `credentials` to be set to `null`, and the object itself will not be entered. To prevent this from happening the keyname can be added to Safe Fields, which will leave the `credentials` object intact with all its contents.
