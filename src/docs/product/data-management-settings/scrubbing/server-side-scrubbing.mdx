...
Sentry will redact arrays and literal values, but also the entire objects. For example:
...
Using the default options would cause the _object_ `credentials` to be set to `null`, and the object itself will not be entered. To prevent this from happening the keyname can be added to Safe Fields (`credentials` in this case), which will leave the `credentials` object intact, but all of its entries would be subject to scrubbing.
So `password` would be redacted by default, and adding `cats`, `username`, and/or `lastLogin` to the list of additional fields would cause those values to be redacted as well.
