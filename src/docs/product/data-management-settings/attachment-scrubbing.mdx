...
When scrubbing attachments, and especially minidump, it is often important to do so without destroying the original file format.
We do this by providing some built-in knowledge of some file formats and provide you with custom [source-selectors](#attachment-source-selectors) which help you write scrubbing rules without damage to the file if used carefully.
...
- The **stack memory**: These are memory regions in the minidump that are used as stack memory by a thread.
  ...
  Because of this there is no way to select the stack memory by a more generic selector like a [value type](../advanced-datascrubbing/#value-types), you must select it explicitly e.g. using `stack_memory` or `$minidump.stack_memory` for it to be scrubbed.
...
- A minidump also contains a list of **code modules**,  which are loaded at the time the minidump is created.
  ...
  * `debug_file`
  ...
  This example is how the UI displays a rule to scrub these fields:
...
A process usually contains a struct with it's environment variables early in the stack memory, in C semantics this can often this can be consumed by the process using the `char *envp[]` parameter ot `main()`.
...
- Early on the stack, as passed vian `**envp`.
