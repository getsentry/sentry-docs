...
In addition to <PlatformLink to="/data-management/sensitive-data/">using hooks in your SDK</PlatformLink> or our [server-side data scrubbing features](/product/server-side-scrubbing/) to redact sensitive data, Advanced Data Scrubbing is an alternative way to redact sensitive information just before it is saved in Sentry. It allows you to:
