...
The following examples illustrates when different sampling methods might be useful. In the examples, the system is composed of:
...
After the deployment of new mobile application UI 1.2 and the updated backend API 2.0 the system occasionally generates errors.
...
For targeting specific versions of a software in services that do not initiate a trace, as is the case in our example for the API
and S1 layers we need to write events.
...
Looking at errors and transactions in isolation does reveal any pattern.
To understand what is going on we need to enable traces to try to understand exactly in what circumstances the error
occur.
...
In addition to the system and type of message we show information from the trace context (Tr...) and information
from the (Ev...).
We have shown a representative trace for all four possible paths a call can go through (T1...T4)
...
|6 | S1 | T | T2 | UI 1.1 | p1 | E6 | S1-1.4 | p3 |
|7 | UI | T| T3 | UI 1.2 |  p1 | E1 | 1.1 | p1 |
|8 | API | T | T3 | UI 1.2 | p1 | E2  | API-1.0  | p2  |
|9 | S1 | T | T3 | UI 1.2 | p1 | E3 | S1-1.4 | p3 |
|10 | S1 | E | T3 | UI 1.2 | p1 | E3 | S1-1.4 | p3 |
|11 | UI | T| T4 | UI 1.2 |  p1 | E4 | 1.1 | p1 |
|12 | API | T | T4 | UI 1.2 | p1 | E5  | API-2.0  | p2  |
|13 | S1 | T | T4 | UI 1.2 | p1 | E6 | S1-1.4 | p3 |

Each trace has 3 transactions, one for each system it passes through.

Looking through a few traces we notice an interesting pattern: only traces that go through UI 1.2, API 1.0 and S1 1.4
generate errors.
...
It could be argued that this is really a bug
...
discarded together, so you can understand the path of the request through the system and clearly identify
