...
You can use Dynamic Sampling rules in any project and with any SDK for transaction rules. To sample distributed traces, however, you need to use one of the supported SDKs:
...
When using transaction rules setting the release, etc. will have the expected effect on sampling regardless of whether the event is the first or not in the trace.
...
Although transactions have a lot of attributes only a limited number of them are
available in the rules. We are working to expand the availability of attributes for the rules in the Dynamic Sampling.
...
For example if we want to sample at 50% transactions originating from the environment "production" `and` having release "1.2"
we can create a rule with:
...
If we wanted to sample at 50% transactions originating either from the environment "production" `or` having release "1.2" we
could still do it by writing two rules and having them one after the other
