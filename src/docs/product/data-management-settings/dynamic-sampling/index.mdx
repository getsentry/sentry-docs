...
Sentry SDKs are able to sample events by setting `traces_sample_rate` or `traces_sampler` during SDK initialisation
(TODO add link to .../PerformanceMonitoring/SamplingTransactions).
While this mechanism is very simple and works well in certain scenarios it has the downside of being difficult to
change for scenarios where the application is distributed and redeployment is either not an option or very
difficult (e.g. mobile applications).
