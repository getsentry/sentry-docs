...
Every time you create a new release, there is a good chance you will want to get more visibility during the early adoption phase. As a result, to have the ability to catch new issues as soon as your release is being adopted, we prioritize data from the latest release.

_While prioritizing data from the latest release, we will also take into consideration your environment because sometimes you might keep the same release and only change the environment. In that case, we also assume you would expect to get more visibility on those transactions._
