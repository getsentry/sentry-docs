...
Sentry stores transactions from all projects at an equal rate, based on the fidelity target
(ratio of stored vs. total) of a customer package. But some projects are very high volume compared to others
and this means some low-volume projects might be "drowned out", especially at lower fidelity rates.
We launched new rebalance algorithms which take the edge off of very larger projects and "redistribute" it to smaller ones.
