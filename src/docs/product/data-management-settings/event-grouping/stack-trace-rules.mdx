...
If you use stack traces for grouping, the stack trace rules (previously known as grouping enhancements) influence the data that's fed into the grouping algorithm. These rules can be configured on a per-project basis under **[Project] > Settings > Issue Grouping > Stack Trace Rules**.
