...
The syntax follows the syntax from [Discover queries](/product/discover-queries/). If you want to negate the match, you can prefix the expression with an exclamation mark (`!`). If a line is prefixed with a hash (`#`), it's a comment and ignored.
...
Frame categorization is heavily used by our newest grouping algorithm, so [if you have it enabled](TODO),
there is a variety of categories which you can match on, including:

* `system` - detected system libraries
* `std` - detected standard libraries
* `ui` - UI frameworks
* `driver` - graphics drivers, etc.
* `telemetry` - crash reporting & analytics frameworks
...
Sometimes, information about the surrounding frames is necessary to apply a rule to a frame.
For this purpose, use caller / callee matching syntax, e.g.
...
A **flag** identifies the action to be taken if all matchers match and uses these prefixes:
...
- `category`: Set a custom category for the frame.
...
Sentinel & Prefix Frames are available only if you're in the Early Adopter program. Features available to Early Adopters are still in-progress and may have bugs. We recognize the irony.
...
In [the Grouping Breakdown tab](src/docs/product/sentry-basics/guides/grouping-and-fingerprints/grouping-breakdown), you can adjust the level of detail by which
an issue is grouped. To enable this, Sentry looks for interesting parts of the stack trace,
adding a new level for every interesting group of frames it can find.

To mark frames as interesting, use the `+sentinel` and `+prefix` actions. For the first level, our algorithm looks for the first frame `X` which is marked as sentinel. The fingerprint of this frame is added to the level. If `X` is also a prefix frame, the next frame `Y` is also added. If `Y` is again a prefix frame, the next frame `Z` is added, etc.

If no sentinel frame is found, every frame which contributes to grouping according to the usual
stack trace rules (e.g. `+group`) constitutes its own level of detail.
...
You can see that the first occurence of a sentinel frame (`b`) is used for grouping.
Because `c` is a prefix frame, `d` is also used:

![Example stack trace with Sentinel & Prefix Frames](screenshots/stacktrace_sentinels.png)

This becomes visible in the [Breakdown tab](/product/sentry-basics/guides/grouping-and-fingerprints/grouping-breakdown):

![Example Breakdown with Sentinel & Prefix Frames (level 1)](screenshots/breakdown_sentinels_level_01.png)

Increasing the level adds the second sentinel:

![Example Breakdown with Sentinel & Prefix Frames (level 2)](screenshots/breakdown_sentinels_level_02.png)

Finally, the deepest level contains the entire stack trace, ignoring Sentinel & Prefix Frames:

![Example Breakdown with Sentinel & Prefix Frames (level 3)](screenshots/breakdown_sentinels_level_03.png)
