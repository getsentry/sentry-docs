...
The syntax follows the syntax from [Discover queries](/product/discover-queries/). If you want to negate the match, you can prefix the expression with an exclamation mark (`!`). If a line is prefixed with a hash (`#`), it's treated as a comment and ignored.
...
there are a variety of categories on which you can match, including:
...
For this purpose, use caller/callee matching syntax. For example:
