...
Sentry attempts to match against all values that are configured in the fingerprint, only applying the first matching rule (top-bottom). In the case of stack traces, all frames are considered. If the event data matches all the values in a line for a matcher and expression, then the fingerprint is applied:
