...
These rules can be configured on a per-project basis under **[Project] > Settings > Issue Grouping > Fingerprint Rules**. This setting has input fields where you can write custom fingerprinting rules. To update a rule:

1. Identify the match logic for grouping issues together.
1. Set the match logic and its fingerprint for it, in your project's settings.
...
```
Now, all the events with the error type `DatabaseUnavailable` or `ConnectionError` will be grouped into an issue with the type `system-down`. All events with the error value `connection error` will be grouped into an issue with a type of the same name and the name of the transaction appended.
<!-- is what i'm saying about transaction correct here? not sure -->
...
<!--
**TO DO - delete all this and any unused screenshots after draft PR is active**
