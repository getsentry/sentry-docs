...
A fingerprint is a way to uniquely identify an event, and all events have one. Events with the same fingerprint are grouped together into an issue. So if two events have the same stack trace, then they will group into one issue.

By default, Sentry will run one of our built-in grouping algorithms to generate an event fingerprint based on information available within the event, such as `stacktrace`, `exception`, and `message`. If the way that Sentry is grouping issues for your organization is already ideal, you don't need to make any changes. However, you can extend the default grouping behavior or change it completely, using a combination of the following options:
...
4. In your project, using [Grouping Breakdown](./grouping-breakdown). (Early Adopters only)
...
You can see a fingerprint by opening an issue, clicking the JSON link, and finding the _fingerprint_ property. If the default grouping was used, you'll see default written there. If a different grouping was used, you'll see the actual fingerprint value itself.
...
If a set of issues looks similar but they aren't being automatically grouped, there is something that still differs: the stack trace, and therefore the fingerprint. Let's compare two similar looking issues side-by-side:
...
Sometimes two stack traces are the same function execution path but differ by one frame. This can be due to several things like middleware you've configured, node_modules, the framework itself, or library imports. To have greater control over which stack frames are included or excluded, you can update your grouping algorithm.

There are four different approaches for updating how events group into issues in [sentry.io](https://sentry.io):

- [Fingerprint Rules](/product/data-management-settings/event-grouping/fingerprint-rules/): Sets a new fingerprint for incoming events based on matchers. This doesn't affect already existing issues.
- [Stack Trace Rules](/product/data-management-settings/event-grouping/stack-trace-rules/): Sets rules for how incoming events should be treated based on matchers. This doesn't affect already existing issues.
- [Merging Issues](/product/data-management-settings/event-grouping/merging-issues/): Merge together similar issues that have already been created. No settings or configuration changes are required for this.
- [Grouping Breakdown](/product/data-management-settings/event-grouping/grouping-breakdown/): Creates coarser issue groups by considering the most relevant part of the stack trace.
