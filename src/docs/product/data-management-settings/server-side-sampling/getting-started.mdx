...
Your client-side sample rate determines how many events are processed, or ingested, while the server-side sample rate determines how many events are then indexed, or retained. We use all your processed events to derive [performance metrics](/product/sentry-basics/metrics/), which makes us better able to provide you with statistically significant insights into the performance of your application. 
