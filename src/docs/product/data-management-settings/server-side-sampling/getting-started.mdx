...
**Dynamic Sampling** is currently only available for the following SDKs:

<Include name="sampling-supported-sdks.mdx" />

If your application relies on any Sentry SDK that isn't specified above, then you won’t be able to use dynamic sampling.
...
Next, go to **[Project] > Settings > Dynamic Sampling** in [sentry.io](https://sentry.io) and set a sampling rate for all transactions: this is the number of transactions that Sentry should accept from your application:
