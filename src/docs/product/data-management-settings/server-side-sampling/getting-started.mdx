...
- [Trace rules can only select based on the attributes of the initial transaction](/product/data-management-settings/server-side-sampling/current-limitations/#trace-rules-can-only-select-based-on-the-attrictures-of-the-initial-transaction)
...
- [Limited number of rule conditions](/product/data-management-settings/server-side-sampling/current-limitations/#limited-number-of-selection-rule-conditions)
...
Based on your quota and usage, in this step, we’ll suggest sampling rates for both Sentry servers and your Sentry SDK. However, you can enter a custom sampling rate by overriding our recommended values. For example, you can enter your current sampling rates to understand how our proposed sampling configuration would impact your quota within the scope of this project. Learn more about [How Sentry Samples](/product/data-management-settings/server-side-sampling/#how-sentry-samples).

<!-- fix paragraph above to reflect UI behaviour: I'm nit-picking here, but the user doesn't need to enter, rather they can simply select the "Current" option. -- Ah okay, I forgot about the radio buttons. So just so I'm understanding, the graph reflects whichever row of values you've selected AND if you've selected the Recommended row, you can then adjust the numbers? -- You got it! -->
