...
While server-side sampling allows you to focus in on smaller number of indexed (or retained) events that you can explore more deeply, it also allows you to send a larger number of processed (or ingested) events to Sentry from which performance metrics are derived. With this larger set of processed events, we're better able to provide you with statistically significant insights into the performance of your application. Learn more in our [full Metrics documentation](/product/sentry-basics/metrics/).
