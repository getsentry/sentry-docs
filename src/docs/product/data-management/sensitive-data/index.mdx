...
Our newer SDKs do not purposefully send PII to stay on the safe side. This behavior is controlled by an option called [`send-default-pii`](/platform-redirect/?next=/configuration/options/#send-default-pii).
...
If you _do not_ wish to use the default PII behavior, you can also choose to identify users in a more controlled manner, using our [user identity context](/platform-redirect/?next=/enriching-events/identify-user/).
...
For more details, see [_Filtering Events_](/platform-redirect/?next=/configuration/filtering/).
