...
You can import and incrementally add the mappings between your source control teams/users to Sentry Teams/Users using [external team/user mappings](/product/issues/ownership-rules/#external-teamuser-mappings). Sentry uses the external team/user mappings to convert the owners in your CODEOWNERS file into their equivalents in Sentry. Sentry will automatically ignore rules that are missing team/user mappings.
...
Once ownership rules are configured, it's applied in the following ways.
...
You can automatically assign issues to their owners by enabling the following setting in **[Project] > Settings > Ownership Rules**.
