...
This process assumes that source maps — or your platform-specific files for mapping transformed source code to the original source — have been uploaded (check out the docs for your [specific platform](/platforms/)). Note that Sentry may not display suspect commits that were committed prior to when the integration was set up.
...
We support a [manual process](/product/releases/associate-commits/) for sending us your commit data if
- you don’t want Sentry to connect to your repository
- your organization does not have a GitHub or Gitlab integration
- your integration has become disconnected

Sentry will attempt to find suspect commits via your Github or Gitlab integration. If this fails and you've setup the manual process, Sentry will fall back to using the release commit data to find suspect commits. 
