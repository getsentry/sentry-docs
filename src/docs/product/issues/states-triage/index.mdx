...
When any of the following conditions occur for an issue, that issue is displayed in the For-Review tab, or Review List:
...
You can mark issues as reviewed, and [sentry.io](https://sentry.io) removes the reason label (the issue stays in the Unresolved state). Unlike ignoring an issue, this is a way to acknowledge an issue that you intend to fix at some point. This is only available on issues flagged for review.
