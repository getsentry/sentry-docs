...
The N+1 API Calls detector looks for groups of simultaneous network calls to the same URL. It uses has the following criteria for detection:

- applies to `GET` requests only
- a minimum of 10 network calls need to happen at the same time
- the network calls must be a minimum of 50s in duration, and within 5ms of each other
- the calls must not be to a GraphQL endpoint, or to `_next/data`
- the calls must not be fetching a static resource (e.g., `.png` files)
...
![N+1 API Calls span evidence](n-plus-one-api-calls-span-evidence.png)
...
![N+1 API Calls in an example application](n-plus-one-queries-in-book-application.png)
