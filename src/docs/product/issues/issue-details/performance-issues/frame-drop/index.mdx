...
There is no predefined way to fix performance of custom algorithms, due to their nature.

In our example above, we were analyzing each number, then performed a search to find the right index to put the number, and finally inserting the item in the list, for every item on the list.

By using two separated lists for even and odd numbers, we can completely avoid the search, as the right index will always be the end of the list.
...
With the considerations above, the same function can be made faster:
...
There are several considerations to keep in mind to improve performance of an algorithm:

- You may be able to parallelize some work with the use of Threads (or Kotlin coroutines).
- Explore and find the best data structures you can for your specific scope.
- Some structures could be cached and reused throughout your function, reducing memory usage.
