...
In our example above, for every item on the list we analyze each number, perform a search to find the right index to put the number, and finally insert the item in the list.
