...
[Profiling](/product/profiling) must be enabled for Sentry to detect Function Regression issues. Once set up, Sentry will periodically check the 95th percentile function duration of the most common application functions in your project. When a significant change in the p95 value for function duration is detected and has been sustained for some time, a regression issue is reported.
...
A common cause of regressions is an increase in the throughput of a function. For example, when the throughput increases in a code path that contains a mutex, this can cause lock contention.
...
- A sparkline showing the p95 function duration of the regressed function within the transaction.
- The p95 function duration of the regressed function before and after the regression.
