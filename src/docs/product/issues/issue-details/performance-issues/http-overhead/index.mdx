...
The detector for this performance issue looks for a set of overlapping HTTP/1.1 spans that are getting slower because the request itself being delayed, which is a problem for HTTP/1.1 because of browser connection limits.
...
- All spans must share a common host (either `/` if the HTTP connection is the same origin, or the domain name otherwise).

Since all HTTP spans have some amount of wait time, while they will still be included in the detection to ensure the connection limit is hit, only HTTP spans over `100ms` will be marked as 'offending' and show up in the span evidence.
...
## Fixing HTTP Overhead

The causes of HTTP Overhead issues can be solved primarily by upgrading to allow HTTP/2 or higher connections to your backend. This would involve your load balancers if you have them in front of your servers, or your server if accessed directly.
Since this may not be possible in all cases, other solutions are to perform domain sharding, or reduce the number of simultaneous requests to your backend, but these solutions require more manual intervention and maintenance over the long term.
