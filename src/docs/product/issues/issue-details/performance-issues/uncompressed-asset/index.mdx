...
Uncompressed asset issues are when one of the files your users are downloading to load your browser page was not compressed when transferred. This likely indicates a misconfiguration of the server or CDN (content delivery network) serving the asset file.

## Detection Criteria

The detector for this performance issue looks for specific asset (resource) spans with the following criteria:

- The span has to be one of either `resource.link` or `resource.script` spans, which in practice means it is a `.css` or `.js` file.
- It has to be over `512kb` in size, to rule out smaller files less likely to benefit from being compressed.
- The span duration has to be over `200ms`, to rule out faster transfers or assets served in internal networks.
