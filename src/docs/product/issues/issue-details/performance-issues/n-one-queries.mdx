...
You can get information about your N+1 queries problem by looking at three main aspects in the "Span Evidence" section:
...
The detector for performance issues looks for a set of sequential spans, including at least one database span, and follows a recurring pattern. It also looks for the following criteria:

- Total duration of involved spans must exceed a threshold (usually 100 ms)
- Total count of involved spans must exceed a threshold (usually ten spans)
- At least one involved span must be a database span with a full query as its description. (Some SDKs truncate queries and add an ellipsis at the end.)

If Sentry isn't detecting an MN+1 issue where you expected one, it's probably because the transaction didn't meet one of the above criteria.
