...
We can detect some specific causes for a main thread stall, like decoding [images](LINK) or [JSON](LINK), or [searching strings with regexes](LINK), but there are many other things that could cause a stall. If a main thread stall causes your app to drop UI frames, but doesn't match one of our specific detectors, it will get reported under the generic Long Task Causing Frame Drops issue type.
