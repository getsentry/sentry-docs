...
Performance could be improved by moving the long computation off of the main thread by dispatching it to a lower Quality of Service (QoS) queue (or `NSOperationQueue`):
...
Specifically for loops, another avenue to consider is to parallelize iterations. There are several options for doing this:
