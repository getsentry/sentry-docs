...
The quick trace (below the date) shows an abbreviated view of the related [trace](/product/performance/distributed-tracing) for the current transaction.
The quick trace will show up to 6 nodes representing the different groups of the event's trace.
- **Root**: The transaction that started the trace
- **Ancestor**: The events that are descendants of the root, and are directly related to the current event
- **Parent**: The event that directly started the current event
- **This Event**: A node representing the current event that you are viewing
- **Children**: Events that the current event directly started
- **Descendants**: Any future generations of events that children of this event create

A node will turn red if there are any errors associated with events in that group.
Clicking View Full Trace will bring you to the [Trace View](/product/performance/trace-view)
