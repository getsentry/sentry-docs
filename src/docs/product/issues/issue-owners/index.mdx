...
Sentry offers multiple ways to define the "ownership" of an issue. With ownership defined, we can automatically assign issues and send alerts to the owner. Sentry defines ownership with Code Owners and Ownership Rules. Code Owners lets you import your CODEOWNERS file and we will assign according to those filepaths. Ownership Rules allows you to override the assignments from Code Owners and provide advanced matcher types (ex: urls, tags) and can match on file paths of files in the stack trace, URL of the request, or event tags.
...
Import your CODEOWNERS file and use it alongside your Ownership Rules to assign Sentry issues. Prerequiste for this feature is the **Code Mappings** from [Stack Trace Linking](/product/integrations/source-code-mgmt/github/#stack-trace-linking).

To quickly setup a **Code Mapping** for a project:
 1. Go to an issue in your project. You should see the following in the stack trace.
...
Now that we have Code Mappings, we are able to transform the paths in your CODEOWNERS file to your expected build output. For example, you may build your program in a `dist/` folder, but that will not be in your CODEOWNERS. However the `dist/` prefix is captured in the stacktrace's file paths. Code Mappings allows Sentry to figure out how to normalize the file paths between your build output and the CODEOWNERS paths.
...
And the last rule matching returns the assignment.
...
1. Evaluate against the Code Owners top-to-bottom, we get match.
2. Evaluate against the Ownership Rules top-to-bottom, we get 2 matches
The matches are in order
```
[
  "codeowners:*.js                    #ecosystem",
  "path:*.js                          #ecosystem",
