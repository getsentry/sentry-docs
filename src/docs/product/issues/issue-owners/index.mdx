...
Import your CODEOWNERS file and use them alongside your Ownership Rules to assign Sentry Issues. You can import and incrementally add the mappings between your source control teams/users to Sentry Teams/Users using [External Team/User Mappings](/product/issues/issue-owners/#external-teamuser-mappings). We will automatically ignore rules that are missing team/user mappings.

This feature supports GitHub and GitLab CODEOWNERS file syntax with the following exceptions:
 - Escaping a pattern starting with # using \ so it is treated as a pattern and not a comment
 - Using ! to negate a pattern
 - Using [ ] to define a character range
 - GitLab Premium syntax
...
Create external team/user mappings for your GitHub/GitLab teams and users.
