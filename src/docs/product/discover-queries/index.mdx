...
The <SandboxLink scenario="discover" projectSlug="react">Discover Homepage</SandboxLink> offers views of all your saved and pre-built queries so you can quickly dive into areas that need immediate attention. For example, if a spike occurred in the past 24 hours for a query on errors, a user may investigate that first. Each query is saved as a card that displays a summarized view of the data.
...
From the **Discover** Homepage, you can build a <SandboxLink scenario="oneDiscoverQuery" projectSlug="react">query</SandboxLink> in three ways.
