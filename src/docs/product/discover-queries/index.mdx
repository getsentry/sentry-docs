...
- Errors by URL: Users can see the pages that generated the most errors and different types of issues. As a reminder, issues are a group of fingerprinted Events. For more details, see the [full documentation on fingerprinting](/platform-redirect/?next=/data-management/event-grouping/).
...
Share your queries as often as you want. You can share URLs with other users who also have access to the same organization. As each part of the query is built, the results update and the URL is updated so that in-progress searches can be shared in an email, chat, etc.
