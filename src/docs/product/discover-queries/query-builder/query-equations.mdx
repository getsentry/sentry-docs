...
In **Discover**, you can create equations based on the columns of your query. Each column acts like a variable since it represents the result of the query, which is a number. This number is used in the equation.
...
The equation will take the result of each of those query columns and use those numbers in the calculation. In this case, the results of those query columns are:
...
- [The percentage of transactions that completed within a threshold](#calculate-a-status-percentage)
- [A custom Apdex score](/#calculate-a-custom-apdex)
- [The average non-database times](#find-largest-average-non-database-times)
...
- Contain a mix of fields or functions
...
Equations can only operate on columns and functions that are numbers, which includes:
...
Apdex is defined as (satisfied + tolerable*0.5 + frustrated*0)/total. If you wanted to change the the weight of tolerable to 0.2, you would create the following query and equation.
