...
The Interval selector will allow you to change the date groupings in the graph. The interval selector has bounds that are based on the currently selected time range, this means for example if you're doing a query over 90 days the largest interval you can select is 45 days and the smallest you can pick is 1 hour.
