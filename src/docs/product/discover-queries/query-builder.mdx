...
Discover queries use sampled transactions, or events-based data. You can compare your query results against the data based on total transactions by enabling the 'Total Transactions' toggle on the chart footer. For example, if you were investigating performance issues on your latest release, you can display `p95(transaction.duration)` for your latest release and the total transactions baseline to visualize how much worse it is than the baseline.

There are a few reasons why the 'Total Transactions' toggle might be disabled:
- Selected display is not supported, only available on Total Period display.
- Selected y-axis is not supported, not available for equations, `apdex()`, `user_misery()` and aggregations performed on fields and tags (for example `count_unique(device.os)`)
- You are querying both error and transaction data (which is default Discover behaviour), ensure you are querying only transactions data by adding `event.type:transaction` to the query search bar
- Your project has an outdated SDK which is incompatible with [Dynamic Sampling](/product/data-management-settings/dynamic-sampling/), confirm that your application relies on [supported SDKs](/product/data-management-settings/dynamic-sampling/getting-started/#current-limitations)
- You have some unparameterized transactions which are incompatible with [Dynamic Sampling](/product/data-management-settings/dynamic-sampling/)
