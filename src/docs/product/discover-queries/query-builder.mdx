...
You can also search multiple values on the same key without an operator, and the search terms automatically use an `OR` operator. For example, "x:value1 x:value2" is the same as "x:value1 `OR` x:value2". For more details, go to [Multiple Values on the Same Key](product/sentry-basics/search/#multiple-values-on-the-same-key).
