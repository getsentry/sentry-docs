...
You can obtain the key by going into the _Sentry > Project Settings > Client Keys (DSN)_ . The public key can be extracted from the DSN. In this DSN, for example, `___DSN___`, the key is `___PUBLIC_KEY___`.
