...
|----------|------------------------------------------------------------------------------------------|
| CPU      | - **Required**: x86-64 (amd64) CPU architecture<br/> - Multi-core CPU [1](#multi-core-cpu) |
...
| Storage  | - By default Relay does not require the disk storage.<br/> - The storage is required only if [Spooling](/product/relay/options/#spooling) is configured, see `spool.envelopes.max_memory_size` configuration option. |
