...
In Docker, the configuration directory is provided using the standard
...
Refer to the [Logging](/product/relay/logging/) page for more information on how to configure Relay's logging.
...
Get the DSN of your project by navigating to your project settings ( _Settings > Projects > {YOUR_PROJECT_NAME} > Client Keys (DSN)_ ), and selecting one of the existing DSN value, which looks similar to:
...
You have a functioning Relay instance, great! Make sure to check [Operating Guidelines](/product/relay/best-practices/) to learn how to monitor and scale your Relay setup.
