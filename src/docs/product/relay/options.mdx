...
Configuration for Relay is recorded either the file `.relay/config.yml`, or when noted, also through an environment variable. To change the config location, pass the `--config` option to any Relay command:
