...
Configuration for Relay is recorded either in the file `.relay/config.yml`, or when noted, will be overridden through an environment variable. To change the config location, pass the `--config` option to any Relay command:
...
Credentials can be initiated as described in the [getting started page](https://sentry-docs-git-relayconfig.sentry.dev/product/relay/getting-started/?original_referrer=https%3A%2F%2Fgithub.com%2Fgetsentry%2Fsentry-docs%2Fpull%2F7062#creating-credentials), but can be overriden by setting some environment variables as described here:
...
: _String, default: `managed`, possible values: `managed`, `static`, `proxy`
and `capture`, environment variable: `RELAY_MODE`_
...
: _String, default: `https://sentry.io`, environment variable: `RELAY_UPSTREAM_URL`_,

Fully qualified URL of the upstream Relay or Sentry instance.

> enviroment variable: "RELAY_UPSTREAM_URL".
...
> enviroment variable: "RELAY_HOST".
...
> enviroment variable: "RELAY_PORT".
...
Relay reuses connections when the upstream
