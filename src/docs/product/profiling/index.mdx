...
There's an aggregate flamegraph as well as a digest of the slowest and most regressed functions on the left side of the page. As a rule of thumb, a good first thing to look for is wide frames that have no children. This will allow you to identify frames that are often occupying the stack and may be good candidates for optimization.

We recommend reading the [Flame Graphs and Aggregated Flame Graphs](/product/profiling/flame-charts-graphs) docs to get a better understand of this data.

An alternative way to visualize the flamegraph is by clicking on the "Call Tree" button. This will render the flamegraph as an inverted call tree with its leftmost nodes first, which will then expand to its parent nodes.

The sample column in the call tree represents the frequency with which profiling collected stack samples where a function was on a stack. Tree expansion can be done via click interaction on the caret before the function name (holding ⌘ while doing so will expand that entire branch all the way to the root.)
