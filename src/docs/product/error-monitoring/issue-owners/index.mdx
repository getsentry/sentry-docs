...
1. Module: matches against the module in the event's stack trace
...
: Can be either `path`, which matches on paths, `module`, which matches on module, `url`, which matches the specified URL, or `tags.TAG_NAME` which matches the specified tag.
