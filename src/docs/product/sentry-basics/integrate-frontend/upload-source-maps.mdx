...
3. To find the `SENTRY_ORG` and `SENTRY_PROJECT` values:
   ...
   - Your organization ID, or the `SENTRY_ORG` value, is part of the browser URL (for example, https://sentry.io/settings/**SENTRY_ORG**/projects/). In the sample screenshot below, the SENTRY_ORG value would be `testorg-az`.
   - The `SENTRY_PROJECT` value is the name that appears in the project tile. In the sample screenshot below, the SENTRY_PROJECT value would be `store-frontend`.
...
Now that we have the details of the error, along with a human-readable stack trace, the next step is to assign the right developer to handle the Issue.
