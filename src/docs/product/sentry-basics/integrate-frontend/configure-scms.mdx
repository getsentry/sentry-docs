...
Sentry uses commit metadata from your source code repositories to pinpoint the source of errors and help you resolve issues faster. Sentry embeds _suspect commits_ – commits likely to have introduced an error – directly into your **Issue Details** page. We also recommend the authors of these commits as _suggested assignees_ who can resolve the issue.
...
4. Add a code mapping between the `frontend-monitoring` repository and your Sentry project. Select the `main` or `master` branch, depending on your repository's default branch name, and tell Sentry that your components live in the `src/` directory:
...
Congratulations! Let’s recap what you’ve just done:

- Set up Sentry in a frontend project
- Configured conditions for an Alert that will notify you of new errors via email
- Triggered an error
- Uploaded source maps to deobfuscate minified stack traces
- Enabled suspect commits via Sentry’s Github integration, allowing you to see likely commits responsible for causing errors
- Set up stack trace linking, letting you jump directly from the stack trace embedded into a Sentry issue to its corresponding location in your Github source code.

## What’s Next?
- Fine-tune your alerts with [Alerts Best Practices](https://docs.sentry.io/product/alerts/best-practices/)
- [Integrate Sentry into a backend application](https://docs.sentry.io/product/sentry-basics/integrate-backend/)
- Define more [issue ownership rules](https://docs.sentry.io/product/issues/issue-owners/) to suggest assignees for an issue”
