...
In some cases, a span may be the parent of one or more transactions. Spans that have children are identified by the expand icon, and clicking this icon will fetch and display the first child transaction. When that transaction is displayed, the collapse icon will appear so that you can hide the transaction again.
You can also click any part of the row itself to investigate the span details further.
