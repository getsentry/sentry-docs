...
The operations breakdown shows you how much time was spent in each operation for the duration of a transaction. The operation type, duration, and percentage of the total duration are shown.

![Operations breakdown in the event detail.](operations-breakdown.png)

Spans can overlap, but the total duration is based on operations that do not overlap and contribute to the overall duration. See the http example below.

![Operations calculation method.](./operations-calc.png)

This visualization can help you find transactions with unusual distributions of operations, which might point to an underlying problem.
...
In some cases, spans may be the parent of one or more transactions, and these spans are identified by the expand icon. Clicking this icon will pull in the first child transaction and display its span tree nested beneath the parent span. When that transaction is displayed, the collapse icon will appear so that you can hide the transaction again.

![Embedded child transaction.](./embedded-transaction.png)
