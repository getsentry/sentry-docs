...
The operations breakdown corresponds to the waterfall span view as a legend. The operation type, duration, and percentage of the total duration are shown. Spans can overlap, but the total duration is based on operations that do not overlap and contribute to the overall duration. See the http example below.
...
#### Embedding and Traversing Transactions

In some cases, a span may be the parent of one or more transactions. Spans that have children are identified by the expand icon, and clicking this icon will display the first embedded child transaction. When that transaction is displayed, the collapse icon will appear so that you can hide the transaction again.

You can also click any part of the row itself to investigate the span details more where some Span IDs will have a "View Child" or "View Children" button. These will potentially lead to another transaction or a list of transactions.
