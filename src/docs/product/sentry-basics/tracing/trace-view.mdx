...
- [Server-side sampling](/product/data-management-settings/server-side-sampling/)  isn't enabled. Client-side sampling may prevent the SDK from sending a transaction. Turn on server-side sampling to give you greater control over which transactions are processed and indexed.
