...
- [Dynamic Sampling](/product/data-management-settings/dynamic-sampling/) isn't enabled. Client-side sampling may prevent the SDK from sending a transaction. Turn on the **Dynamic Sampling** feature to give you greater control over which transactions are processed and indexed.
