...
- [Turn on Server-side Sampling](/product/data-management-settings/server-side-sampling/). Client-side sampling may prevent the SDK from sending a transaction, server-side sampling gives you greater control over which Transactions are stored and processed.
