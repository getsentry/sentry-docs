...
Sentry automatically creates environments when it receives an event with the environment tag. Environments are case sensitive. You can also create an environment when you first `init` your SDK, [as documented for each SDK](/platforms/javascript/configuration/environments).
