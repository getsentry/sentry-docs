...
If you'd like a specific owner to be the fallback, end your ownership rules with a rule like `*:<owner>`
...
- If you're repeatedly seeing alerts you've seen before, try filtering your alerts to issues created in the last few days, or [ignoring](https://docs.sentry.io/product/alerts-notifications/alerts/#ignore) the issue.
- Ignored issues don't alert you when ignore conditions are met, they only become unresolved. So if you're worried about ignored issues disappearing, create an alert to notify you when issues change state from ignored to unresolved.
- Often there are transient issues that happen only a few times in quick succession and never again. Use the "issue has happened at least {X} times" filter.

### Release-based Alerting  [Still undecided about this one...]

- Use the release = latest filter to have your alert apply only to the latest release.

### Tagging

- Use [tags](https://docs.sentry.io/platforms/javascript/enriching-error-data/additional-data/adding-tags/) to categorize errors. For example you can filter on the auto-captured `url` tag to identify business-critical pages, or filter on a custom tag like `VIP customer` to treat those alerts with more importance.

### Webhooks

- Need to send alerts to webhooks? [Create a webhook destination](https://docs.sentry.io/workflow/integrations/integration-platform/webhooks/?platform=browser) and it will be available in the alert actions menu. Example use cases:
    - Sending alerts to integrations not supported natively
    - Aggregating alerts from your different monitoring systems
    - Writing custom rules on your end to route alerts more intelligently
