...
If you want to know about a sustained (not transient) change in a metric, set a threshold for the “Resolved Status” when you create your alert. When the metric falls below this threshold, the alert will automatically resolve. For example, if there's an occasional latency spike in an endpoint caused by a single user, that's just noise. You can set that alert to auto-resolve.

On the other hand, if you get an alert for a spike in errors, that could indicate a real incident even if it subsides. You probably want the alert to remain active and show up in your default view. Therefore, you should not set a “Resolved Status” threshold for that alert.
