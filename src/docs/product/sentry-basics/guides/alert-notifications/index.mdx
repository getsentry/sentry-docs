...
When you create a new project in [sentry.io](https://sentry.io), you can choose from a few default alerts, or choose to create alerts at a later time. Choosing a default alert, such as getting notified only once when an error is first seen, is generally useful for catching new errors in real time, but means Sentry will not alert you about subsequent instances of the same error. With only the default alert rule in place, you might be losing visibility into critical issues building up in your code. Instead, we recommend you create alerts that provide you with the kind of information your team needs.

To create an alert, navigate to **Alerts** and click "Create New Alert". Select your project, and then select whether to create an alert about errors or performance. Learn more about creating [metric alerts](/product/sentry-basics/guides/alert-notifications/metric-alerts/) or [issue alerts](/product/sentry-basics/guides/alert-notifications/issue-alerts/) in the full documentation.
