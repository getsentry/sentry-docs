...
!<Link to="alerts_notifications_01.png">Default Alert</Link>
...
- **Regressions** – A regression happens when the state of an issue changes from `resolved` back to `unresolved`. An email is sent to all project team members.
  ...
  !<Link to="alerts_notifications_05.png">Workflow Issue Regression Alert</Link>
...
- **Assignment** - when a team member is assigned to an issue, they are notified via email.
  ...
  !<Link to="alerts_notifications_03.png">Workflow Assignment Alert</Link>

For more information, take a look at the <Link to="/workflow/notifications/">Notifications</Link> documentation.

## Next

<Link to="/guides/alert-notifications/creating-custom-rules/">Creating Custom Alert Notifications</Link>
