...
The following steps demonstrate a potential use-case that involves an example web application with a JavaScript front-end and a Node.js server. The examples assume that both services are already configured with Sentry's <Link to="/platforms/javascript/">JavaScript SDK</Link>.
...
!<Link to="tracing_tags.png">Image of the request_id grouped with other tags.</Link>

### Searching Request IDs in Sentry

If you also need several example cases to supplement a unique request ID, you can use Sentry's <Link to="/product/discover-queries/">Discover</Link> functionality to search for all request ids you've seen on a specific URL.

!<Link to="tracing_with_discover.png">Image of the request_id grouped with other tags.</Link>
