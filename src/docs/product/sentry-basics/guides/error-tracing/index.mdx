...
sidebar_order: 1
redirect_from:
  - /enriching-error-data/error-tracing/
  - /enriching-error-data/tracing/
description: "Learn more about correlating errors from multiple services to uncover a significant story surrounding a break."
---
...
NGINX has the [request ID variable](https://nginx.org/en/docs/http/ngx_http_core_module.html#var_request_id) already built-in. After accessing the variable, you can configure your NGINX server to include `$request_id` in your access logs and pass this value downstream to your application servers. First, declare a new log format, because the default log format doesn't include `$request_id`.
...
![Image of the request_id grouped with other tags.](tracing_tags.png)

### Searching Request IDs in Sentry

If you also need several example cases to supplement a unique request ID, you can use Sentry's [Discover](/product/discover-queries/) functionality to search for all request ids you've seen on a specific URL.

![Image of the request_id grouped with other tags.](tracing_with_discover.png)
