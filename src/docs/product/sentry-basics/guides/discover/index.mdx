...
- Table columns: `release` (field), `count()`, `count_unique(issue)`

![Issues per release](012.png)

From here you can add a release as a filter and change the columns to `count()` and `title` to see similar errors, and then click the Open Group icon to drill down to individual events. You can also open the release in **Releases** from the "Results" table:
