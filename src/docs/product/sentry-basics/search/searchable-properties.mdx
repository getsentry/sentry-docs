...
Sentry's search provides you with reserved keywords, like `is`, `user`, `server`, and `browser`, that you can use to search on the properties of issues and events (as well as the special case of releases). You can also create [custom tags](#custom-tags) on which to search. This page includes [a canonical list of all available issue and event search terms](#properties-table) as well as some guidance for how Sentry search works.

## Issue Properties

[Issues](/product/issues/) are an aggregate of one or more events. Searchable properties include status, assignment, aggregate counts, and age. You can search by issue properties in the **Issues** page and in **Dashboards** in the widget builder, depending on your dataset selection.

### Event Properties

Events are the underlying event data captured using Sentry SDKs (read: errors and transactions).

When searching on event properties within the **Issues** page, the search will return any issue that has _one or more events_ matching the supplied event filters. You can search by event properties in the following [sentry.io](https://sentry.io) pages:

- Discover
- Dashboards - within the widget builder, depending on dataset selection
- Performance
- Issues - where indicated
- Alerts - when creating a metric alert

### Release Properties

You can only search by release properties in **Releases**, but these properties are also searchable in the other pages of [sentry.io](https://sentry.io) listed above (under Event Properties).

## Searchable Properties Tips

Search terms should auto-complete, and when they don't that means you have an invalid data set. If you enter an invalid search term and tap Return or Enter, an error message is displayed. For example, in **Discover**, you cannot search using a grouping function (properties with a parentheses in them, for example `epm()`) if you don't already have a grouping function in your "Results" table columns.

When searching using a property that's a function (properties with a parentheses in them, for example `epm()`), if the function takes a parameter, that's part of the key/token and you still need to add a value for the search to work. For example, the function count_unique(field) takes whichever field you choose as the parameter and then you add a number as your value. So a typical search might look like: `count_unique(add a field)=>20`.

Other things to note:

- Search properties that are duration or number types are typically used with a [comparison operator](/product/sentry-basics/search/#comparison-operators) rather than an exact match, as an exact match would be unlikely to exist.
- Properties with the notation "Doesn't take a parameter" still take a value, but don't take a paramter inside the parentheses. For example, `epm():>12`.
- Default parameters in search functions can be updated. For example, the default parameters of column, operator, and value can be updated for count_if(column,operator,value).

## Properties Table

Below is a list of issue-level and event-level keys and tokens reserved and known to Sentry.
...
| error.handled | Indicates whether the user has handled the exception — for example, using try...catch. Values are `1/0` or `true/false` | x | x | boolean |
...
| stack.abs_path | The absolute path to the source file. In **Discover**, this is an array; in **Issues**, this is a single value. | x | x | array, single value |
| stack.colno | Column number of the call, starting at 1. | x |  | array |
| stack.filename | The path to the source file relative to the project root directory. In **Discover**, this is an array. In **Issues**, this is a single value. | x | x | array, single value |
| stack.function | Name of the function being called. In **Discover**, this is an array. In **Issues**, this is a single value. | x | x | array, single value |
...
| stack.module | Platform-specific module path. For example, `sentry.interfaces.Stacktrace`. In **Discover**, this is an array. In **Issues**, this is a single value. | x | x | array, single value |
| stack.package | The "package" the frame was contained in. Depending on the platform, this can be different things. For C#, it can be the name of the assembly. For native code, it can be the path of the dynamic library or something else. In **Discover**, this is an array. In **Issues**, this is a single value. | x | x | array, single value |
