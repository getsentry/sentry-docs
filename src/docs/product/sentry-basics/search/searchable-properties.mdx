...
[Session Replay](/product/session-replay/) objects are not represented within the existing event data tables inside Sentry. As such, they have share some common field names, but also have unique fields as well.

You can search by session replay properties on the **Replays** page.
...
The Xs in these columns of the table indicate whether the property is an event property, an issue property, or a combination of these.
...
|---|---|---|---|---|---|
| activity | Replay activity is calculated based on the number of errors, number of ui events, and factors in total duration. It's represented as a number from `1` to `10`. |  |  | x | number |
...
| countErrors | The number of errors within a replay. |  |  | x | number |
| countSegments | The number of segments within a replay. More segments represent more activity over time. |  |  | x | number |
| countUrls | The number of URLs that the user visited during a replay recording. |  |  | x | number |
...
| device.model | Internal hardware revision to identify the device exactly. |  | | x | n/a |
...
| releases | A release is a version of your code deployed to an environment. You can create a token with an exact match on the version of a release, or `release:latest` to pick the most recent release. |  |  | x | string |
...
| replayType | The reason a replay was triggered. For example, `session` when `replaysSessionSampleRate` takes effect, or `error` when the session is not sampled, but `replaysOnErrorSampleRate` is instead. |  |  | x | string |
...
| urls | A specific URL that the user visited during the replay. |  |  | x | string |
...
| user.ipAddress | User's IP address. If the user is unauthenticated, Sentry uses the IP address as a unique identifier for the user. |  |  | x | string |
| user.name | Username, which is typically a better label than the `user.id`. |  |  | x | string |
