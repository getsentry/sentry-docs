...
| apdex() | Returns transactions with the [Apdex score](/product/performance/metrics/#apdex) that was passed in (the word Apdex would be linked) | Discover | number |
| avg() | Average the numeric/duration values for a column | Discover | duration | <!-- not search -->
| count() | Returns table rows that match the result of the count value and operator. | Discover | number | <!-- does this make sense? -->
| count_if(column,operator,value) | The count of events that satisfy the condition passed to the parameters | Discover | number | <!-- Updated this one -->
| count_miserable() | Count of unique users that fall above the miserable threshold | Discover | number |
| count_unique() | Count of the unique number of occurances of the field passed in | Discover | number | <!-- can this be value instead of field-->
...
| device.brand | Brand of the device | Discover, Issues |  |
|  |  |  |  |
...
_ Discover, Performance = events
_ Issues = issues
_ Releases = it’s own beast
_ Dashboards within the widget builder = issues & events & releases
