...
|---|---|---|---|---|---|
| activity | Replay activity is calculated based on the number of errors & ui events factoring in duration. It's represented as a number from `1` to `10`. |  |  | x | number |
...
| browser.name | Name of the users webbrowser. For example, `Chrome`, `Firefox` or `Safari` |  |  | x | string |
