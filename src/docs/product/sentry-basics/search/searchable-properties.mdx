...
| replay.click.class | The classname of an element that was clicked. No leading `.` is nessessary. For example `btn-primary` would match the element `<a class="btn btn-primary">Save</a>` | | | x | string |
...
| replay.click.tag | The tagName of an element that was clicked. For example `input` would match `<input name="username" />` | | | x | string |
