...
| replay.click.alt | The `alt` of an element that was clicked. For example `"a good dog"` would match the element `<img src="/lassie.jpeg" alt="a good dog" />` | | | x | string |
| replay.click.class | The `class` of an element that was clicked. No leading `.` is nessessary. For example `btn-primary` would match the element `<a class="btn btn-primary">Save</a>` | | | x | string |
| replay.click.id | The `id` of an element that was clicked. No leading `#` is nessessary. For example `reset-password` would match the element `<a id="reset-password">Reset</a>` | | | x | string |
| replay.click.label | The `aria-label` of an element that was clicked. For example `Expand` would match the element `<button aria-label="Expand"><img src="/icons/expand.png"/></button>` | | | x | string |
| replay.click.role | The `role` of an element that was clicked. For example `button` would match both `<button>Save</button>` and `<a role="button">Submit</a>` | | | x | string |
| replay.click.tag | The tag name of an element that was clicked. For example `input` would match `<input name="username" />` | | | x | string |
| replay.click.testid | The `data-test-id` or `data-testid` of an element that was clicked. For example `save-button` would match the element `<button data-test-id="save-button">Save</button>` | | | x | string |
| replay.click.textContent | The immediate `textContent` of an element that was clicked. For example `Save` would match `<button>Save</button>` but would not match `<button><h1>Save</h1></button>` | | | x | string |
| replay.click.title | The `title` of an element that was clicked. For example `Save this comment` would match the element `<a title="Save this comment" class="btn btn-primary">Save</a>` | | | x | string |
