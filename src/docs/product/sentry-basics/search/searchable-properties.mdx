...
| message | Restrict results to events with a matching message. This filter will also match on any message containing the supplied value: `match:undefined` will match an event with a message of `undefined is not an object`.<br /><br />  Raw token searches are searched against `message`. |
