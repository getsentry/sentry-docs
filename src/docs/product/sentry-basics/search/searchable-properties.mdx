...
| assigned | Returns issues assigned to the defined user(s) or team(s). Values can be a user ID (your email address), `me` for yourself, `me_or_none` for yourself or no assignee, `my_teams` or `#team-name` for teams you belong to. |  | x |  | team or org user |
