...
Metric properties underly [processed events](/product/sentry-basics/metrics/). In **Dashboards**, for the ["Errors & Transactions"](/product/dashboards/widget-builder/#errors--transactions) dataset, we default to metric-backed data if all search conditions and/or columns are metric properties.
