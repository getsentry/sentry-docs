...
| assigned | Returns issues assigned to the defined user. Values can be a user ID (your email address), `me` for yourself, `me_or_none` for yourself or no assignee, or # followed by the team name, for example `#team-name`. |  | x | team or org user |
...
| count() | Returns results with a matching count. (Same as `timesSeen` in issue search) No parameter. | x |  | number |
...
| failure_rate() | Returns results with a matching rate of failing transactions. That is, `failure_count()` divided by the `count()` (total count). | x |  | number |
...
| last_seen() | Datetime when the event was last seen. Equivalent to `max(timestamp)`. | x |  | datetime |
| lastSeen | Datetime when the event was last seen. |  | x | datetime |
...
| measurements.fcp | [First Contentful Paint (FCP)](/product/performance/web-vitals/#first-contentful-paint-fcps) measures the time for the first content to render in the viewport. | x |  | duration |
