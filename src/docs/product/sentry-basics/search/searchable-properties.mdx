...
In the case of transaction events, we also refer to these types of properties as _indexed event properties_ because they are transaction events for which we store all of the data. This isn't the case for [processed event properties](#processed-event-properties). **For error events, we currently store all data for all events sent to Sentry, and the concept of processed events doesn't apply.** Learn more about indexed and processed transaction events in our full [Sampling documentation](/product/sentry-basics/sampling/index/).
...
Metric properties are in beta and are only available if your organization is participating in its limited release. Features in beta are still in-progress and may have bugs. We recognize the irony. If you’re interested in participating, [join the waitlist](https://sentry.io/for/performance/#updates-signup).

</Note>

The Xs in these columns of the table indicate whether the property is an event property, an issue property, a metric property, or a combination of these.
