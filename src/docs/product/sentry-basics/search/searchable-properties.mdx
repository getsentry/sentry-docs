...
| release | A release is a version of your code deployed to an environment. You can create a token with an exact match of the version of a release, or `release:latest` to pick the most recent release. | x | x |  | string |
| releases | A release is a version of your code deployed to an environment. You can create a token with an exact match of the version of a release, or `release:latest` to pick the most recent release. |  |  | x | string |
...
| user.ip | User's IP address. Sentry uses the IP address as a unique identifier for unauthenticated users. | x | x |  | string |
| user.ipAddress | User's IP address. Sentry uses the IP address as a unique identifier for unauthenticated users. |  |  | x | string |
