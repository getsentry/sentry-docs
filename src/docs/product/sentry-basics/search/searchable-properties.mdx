...
| click.class | The `class` of an element that was clicked. No leading `.` is nessessary. For example `btn-primary` would match the element `<a class="btn btn-primary">Save</a>` | | | x | string |
| click.id | The `id` of an element that was clicked. No leading `#` is nessessary. For example `reset-password` would match the element `<a id="reset-password">Reset</a>` | | | x | string |
...
| click.selector | An element identified using a subset of CSS selector syntax. For example `span#section-1` or `span.active` or `span[role=button]` or `span#section-1.active[role=button]` would all match the element `<span id="section-1" class="active" role="button"/>`. CSS combinators, pseudo selectors, and attr selectors other than `=` are not supported. | | | x | string |
| click.tag | The tag name of an element that was clicked. For example `input` would match `<input name="username" />` | | | x | string |
| click.textContent | The immediate `textContent` of an element that was clicked. For example `Save` would match `<button>Save</button>` but would not match `<button><h1>Save</h1></button>` | | | x | string |
