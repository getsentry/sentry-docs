...
In **Releases**, you can only search by by properties that include `release` in the key, but these properties are also searchable in the other pages of [sentry.io](https://sentry.io) listed above (under [Event Properties](#event-properties)).

## Search Tips

Search terms should auto-complete, and when they don't that means you have an invalid data set. If you enter an invalid search term and tap Return or Enter, an error message is displayed. For example, in **Discover**, you cannot search using a grouping function (properties with a parentheses in the key, for example `epm()`) if you don't already have a grouping function included in the "Results" table columns.

When you search using a property that's a function, if the key includes a parameter, you still need to add a filter or value to have a complete search token. For example, the function `count_unique(user)` takes whichever field you choose as the parameter and then you add a number as your value to filter on. So a typical search might look like: `count_unique(field)>=20`.
...
Below is a list of issue-level and event-level keys and tokens reserved and known to Sentry. The Xs in the "E" and "I" columns of the table indicate whether the property is an event property, an issue property, or both.
