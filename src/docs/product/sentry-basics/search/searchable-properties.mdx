...
| replay_click.testid | The `data-test-id` or `data-testid` of an element that was clicked. For example `save-button` would match the element `<button data-test-id="save-button">Save</button>` | | | x | string |
