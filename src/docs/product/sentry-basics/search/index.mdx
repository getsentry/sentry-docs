...
When you search multiple values on the same key without an operator, the search terms automatically use an `OR` operator. For example, "x:value1 x:value2" is the same as "x:value1 `OR` x:value2". When you do this, the search returns issues/events that match any search term.

An example of searching on the same key without an operator:

```
release:12.0 release:13.0
```
...
By default, search terms with different keys use the `AND` operator; that is, they return the intersection of issues/events that match all search terms.
