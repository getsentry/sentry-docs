...
For organizations participating in the limited release, the transaction filter on the performance page has been simplified to support [metrics](https://docs.sentry.io/product/sentry-basics/metrics/). The search bar can only be used to filter by transaction name. See [transaction search](/product/performance/filters-display/#transaction-search).
