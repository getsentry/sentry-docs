...
Discover is Sentry's powerful query builder for aggregating raw event data and has its own unique syntax not covered here. For more information, see <Link to="/product/discover-queries/">full Discover documentation</Link>.
...
NOTE: `OR` and `AND` search conditions are only available for <Link to="/product/discover-queries/">Discover</Link>, <Link to="/product/performance/">Performance</Link>, and [Metric Alerts](/product/alerts-notifications/alerts/#metric-alerts).
