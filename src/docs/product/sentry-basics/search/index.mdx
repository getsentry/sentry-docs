...
Page filters enable you to narrow down the results shown on a page by restricting data to a certain project, environment, or date range.

  ![Page Filters](search-page-filters.png)

These filters are normally only persistent within the current page, but you can choose to "lock" the current filter selection such that any changes made to the "locked" filter will be persisted throughout the application and even remembered between sessions.
