...
**Sentry SDK** - Sentry's software development kit. SDKs are programming language/framework-specific. When you instrument your application with one of our SDKs, your application sends event data to Sentry so we can provide you with information about how your software is performing.
...
**attachments** - Attachments are files uploaded in the same request, such as log files. Unless the option to store crash reports is enabled, Sentry will use these files only to create an event, and then drop them. <!-- same request as what-->
