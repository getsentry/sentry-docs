...
- **event** - An error or a transaction.
  ...
  <!-- we need to figure out a new way of defining an event since the old definition now really describes processed events -->
...
- **processed events** - A processed event is one instance of you sending data to Sentry. This can be all the events that occur in your application or a sub-set. The number you send is based on the sample rate that you set in the SDK. For events that are processed but not sampled, Sentry doesn't store the full transaction data, only aggregate data. Learn more in [Metrics](/product/sentry-basics/events-metrics/).

- **sampled events** - Events that Sentry stores for you based on your [dynamic sampling]() settings. In the case of transactions, we store the complete transaction data. Learn more in [Metrics](/product/sentry-basics/events-metrics/).

- **metrics** - Metrics are generated from processed, but not sampled events that you send to Sentry. Learn more in [Metrics](/product/sentry-basics/events-metrics/).
