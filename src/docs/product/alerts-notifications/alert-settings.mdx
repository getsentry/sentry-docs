...
The “Alert Rules” tab displays your alert rules, along with their current status, project, team, and creation date. The list is filtered so that only alerts associated with the teams you're a member of are displayed. You can change this using the "Filter" button. The name of the button changes to reflect the number of filters applied, so when you first navigate to the page, it's labeled "2 Active Filters".

If you have [Performance Monitoring](/product/performance/) configured for your organization, the **Alerts** page also displays a “History” tab where you can find a list of triggered alerts, along with visual graphs, associated projects, and the last time the alert was triggered. In the “History” tab, the default view displays unresolved alerts.

![Alert rules are divided by projects, their conditions/triggers, and action(s).](alert-list.png)

<Note>

Currently, the "History" tab doesn't display issue alerts.
...
The digests feature works only for **issue alert emails** and limits alerts across issues within a project. This project-level setting allows you to batch issue alerts to limit the total number of emails people in your organization receive for that project. Use the sliders to control the frequency.
