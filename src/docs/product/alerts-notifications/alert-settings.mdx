...
The “Alert Rules” tab displays your alert rules, along with their current status, project, team, and creation date. By default, the list is filtered so that only alerts associated with the teams you're a member of, as well as ones that aren't associated with any team, are displayed. You can change this using the "Filter" button. The name of the button changes to reflect the number of filters applied, so when you first navigate to the page, it's labeled "2 Active Filters".
