...
Sentry users with owner permissions and above can create notifications, change default notification settings, and configure notifications according to a project's needs. Notifications are categorized as [Workflow](#workflow), [Deploy](#deploy), and [Quota](#quota).
...
**Resolved**: Resolve an issue to indicate that no more events are expected. Beware of [regressions](/product/alerts-notifications/troubleshooting/#notifications).

Ways to resolve an issue: 1) Manually changing the state on the issue page UI, 2) Submitting a fix with the associated issue ID, 3) When the project’s auto-resolve feature is configured. Auto-resolve is located in **Project Settings > General Settings**. For more details about submitting a fix, see documentation on [resolving issues](/product/releases/#after-associating-commits).
...
You can configure your personal [workflow](#workflow-notification) and [deploy](#deploy-notification) notifications in your [account settings](https://sentry.io/settings/account/notifications/). You **cannot configure** quota notifications.
...
Configure your deploy notifications broadly for all notifications and/or by organization by navigating to **Setting > Account > Notifications**.
