...
The rate limit controls how often the alert rule can be triggered for a particular issue. If alert conditions match an issue, actions are only executed if no other actions have been executed within the rate limit period. The limit is set to perform the action according to one of these intervals:
