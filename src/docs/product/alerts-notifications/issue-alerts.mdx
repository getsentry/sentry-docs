...
Conditions are evaluated for an issue alert **each time** the issue receives a new event, subject to [rate limits](/product/alerts-notifications/alerts/#rate-limit). Alert conditions have three parts:
...
Note: Check for conflicting conditions. In the example below, the alert will not fire because an event **cannot satisfy both** of these conditions simultaneously: Sentry detects/creates a new issue, _and_ the issue has happened ten times.

![The alert condition in this example shows the conditions are conflicting with each other.](alert-condition-conflict.png)
