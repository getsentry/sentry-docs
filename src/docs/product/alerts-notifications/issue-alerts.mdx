...
Issue alerts apply only to event errors, and trigger whenever any issue in the project matches the specified criteria, such as a resolved issue re-appearing or an issue affecting many users. To change the frequency of your alerts, configure your [Alert Subscription](/product/alerts-notifications/alert-subscription/).
