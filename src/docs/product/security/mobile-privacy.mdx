...
Sentry SDKs are not officially supported and recommended for use in third-party SDKs and libraries due to technical limitations.
...
- [Signal handlers](https://en.wikipedia.org/wiki/C_signal_handling) may overwrite each other leading to one of the Sentry SDKs not operating normally if multiple Sentry SDKs are installed in the same application.
