...
Sentry's dashboard and API are served from a single IP address for all web traffic. The IP address is:
...
Sentry's Event Ingestion respects two domains within a Data Source Name (DSN): [Organization Subdomains](https://forum.sentry.io/t/organization-subdomains-in-dsns/9360) and Sentry's apex domain.

Sentry's primary domain accepts events from the same IP address as the Dashboard and API:

```python
35.188.42.15/32
```

Starting $DATE, Sentry's Organization Subdomains will migrate from the prior IP address to the following single address:
