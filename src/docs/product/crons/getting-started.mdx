...
Sentry Crons can help you better understand your job check-ins by storing a file, such as a log output. You'll be able to use this to debug check-in failures or track job statuses. To set up your health check, execute an initial HTTP POST check-in request to the following endpoint:
