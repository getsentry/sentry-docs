...
Sentry Cron Monitoring allows you to monitor any job running on a scheduled interval. To get started, first create a new cron monitoring service for one of your projects:

1. First, access the Cron Monitor product area by clicking **“Crons”** in the sidebar.
2. From here, click the **"Set Up Cron Monitor"** button.
    ![Set Up Cron Monitor](cron-setup.png)
3. In the details section:
    - **Project:** Select a project to which you'd like your Cron Monitor to be attached. The project selected is where any issues in your job execution will be created.
...
Creating these health checks can be accomplished automatically using the Sentry's CLI or manually via HTTP. To get started, [install](/product/cli/installation) and [configure](/product/cli/configuration) your CLI, or simply use the example below to construct an HTTP request in your favorite language:
...
: Required. The status of your job execution, if it was completed successfully or unsuccessfully. Values accepted are `in_progress`, `ok` or `error`.
