...
Click "Add Alert Rule" in the integration configuration page or go to **Alerts > Create Alert** to set up a new Alert Rule for the PagerDuty Integration.

Select PagerDuty as an [issue alert](/product/alerts/alert-types/#issue-alerts) action:

![](pd_alert_rule.png)

Select PagerDuty as a [metric alert](/product/alerts/alert-types/#metric-alerts) action:

![](pd_metric_alert_rule.png)

If a PagerDuty incident is created from a metric alert critical trigger, it will resolve itself when the Sentry metric alert is resolved.
