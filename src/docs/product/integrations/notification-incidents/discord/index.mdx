...
Triage, resolve, and archive Sentry issues directly from Discord.
...
2. Click "Add Installation". A pop up window should open.

   ![Install Discord integration](discord-integration-details.png)

3. Log into your Discord account. (If you're already logged in, this step won't be displayed.)
...
5. Finally, once the server is selected, click "Authorize". The integration will be created and the Sentry Discord bot will join the selected server.
...
Currently our Discord integration supports issue and metric alert rules, with personal notifications planned for future updates to the integration.
...
3. Configure your issue alert conditions as desired.

4. To set up Discord notifications, select "Send a Discord notification" in the "THEN" actions dropdown.

5. Select the Discord server and channel you'd like to send the alert to. You can optionally specify tags here to include in the notification.

   The channel field must be filled in with a Discord channel ID, _not_ a channel name. To get this channel ID, right click your channel, click "Copy Link", then select the string of numbers at the end. For example, if the URL is `https://discord.com/channels/server-id/channel-id`, the ID is `channel-id`.
...
2. Select "Number of Errors" or "Users Experiencing Errors" and then click "Set Conditions".

3. Configure your metric alert conditions as desired.

4. To set up Discord notifications, click "Add Action" then select your Discord server from the second dropdown.

5. Add the channel ID you'd like to send the alert to.

   The channel field must be filled in with a Discord channel ID, _not_ a channel name. To get this channel ID, copy the link to your channel and select the string of numbers at the end. For example, if the URL is `https://discord.com/channels/server-id/channel-id`, the ID is `channel-id`.
