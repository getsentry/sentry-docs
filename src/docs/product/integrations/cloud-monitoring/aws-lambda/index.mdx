...
Looking for instructions to set up the serverless SDK manually? Check out these docs for [Node](/platforms/node/guides/aws-lambda/) or [Python](/platforms/python/integrations/aws-lambda/) instead.
...
1. Wait for the stack to be created, which is confirmed by seeing the status change to **Create Complete**, then click **I've created the stack** in the Sentry popup.
...
Run your AWS Lambda function. Performance data will be sent to Sentry and will show up in the "Performance" section of Sentry. If your function raises an error it will show up in the "Issues" section of Sentry.
...
If you are using [Serverless Framekwork](https://www.serverless.com/) note that this integration will configure your existing lambda functions correctly to use the SDK layer and set the appropriate environment variables as per [how this integration works](/product/integrations/cloud-monitoring/aws-lambda/how-it-works/). You can check the output `aws lambda get-function-configuration --function-name <yourFunctionName>` and this works.
...
See also [How this integration works](/product/integrations/cloud-monitoring/aws-lambda/how-it-works/) for more information.
