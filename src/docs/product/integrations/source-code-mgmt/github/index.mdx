...
<Note>
Sentry will automatically try to set this up for your organization for Python and JavaScript projects for organizations with the Github integration installed. You are still welcome to add code mappings youself, however, we will still try to do so. Support for other languages and other source code integrations will follow.
</Note>
...
5. Fill out the form, then click **Save Changes**. Each form field is described below:
   ...
   - **stack trace root** and **source code root** (optional):
     ...
     2. If the filename in your Sentry stack trace frame doesn't match the path to your source code, you will need to replace the **stack_root** part of the filename with your **source_root** to make the filename match the source code path.
...
Sometimes when you visit an issue details page, you will see a message asking you tell us where your source code is. This process will also set up the code mapping for your project.

1.  Visit an issue in your project. If it has a stack trace, you should see the message "Tell us where your code is" in the expanded frame.
2.  When you click the link, a modal will open. Enter the source code URL from GitHub corresponding to the stack trace filename and we'll automatically create the code mapping and set up stack trace linking for the project.
