...
![Suspect commits](azure-global-suspect-commit.png)
...
When Sentry sees this, we’ll automatically annotate the matching issue with reference to the commit, and later, when that commit is part of a release, we’ll mark the issue as resolved. The commit must be associated with a release or Sentry won't know when the commit has been released and if the commit is squashed, the issue may never be marked as a regression.
