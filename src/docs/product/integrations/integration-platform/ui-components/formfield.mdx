...
- `label` - (Required) Label text to be rendered for the form field
- `name` - (Required) Value to use in `name` attribute of the field
- `uri` - (Required if developer doesn't provide `options`) URI to retrieve values from. See [URI Guidelines](/product/integrations/integration-platform/ui-components/#uri-guidelines) for formatting help.
