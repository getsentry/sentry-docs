...
The issue linking and alert rule action UI components can utilize `FormField` components to display attributes in their respective modals. Some UI Components (e.g. [Issue Link](/product/integrations/integration-platform/ui-components/issue-link), [Alert Rule Actions](/product/integrations/integration-platform/ui-components/alert-rule-action)) can render custom forms with inputs specified according to the schemes below. They behave similarly to their HTML equivalents.
