...
All public integrations can be installed on a fixed external url that you can expose to your users. That url is: `https://sentry.io/sentry-apps/<your-integration-slug>/external-install/`. For example, here is the one for `Linear`: https://sentry.io/sentry-apps/linear/external-install/. If a user has multiple organization they will be asked to pick an organization for the installation in a dropdown.
