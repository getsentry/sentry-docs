...
Published integrations are required to have a logo image uploaded. The requirements for those are fairly rigid:
* Logo size must be between 256 &times; 256px and 1024 &times; 1024px
* The file format must be a PNG
* The background must be transparent (unless the logo takes up the entire space)

If the integration has a [UI component](#ui-components), it must also upload an icon that follow these requirements:
* Icon size must be between 256x256 px and 1024x1024px
* The file format must be a PNG in RGBA format
* All pixels must be black or transparent

Internal integrations can have uploaded logos and icons, though they are not required.
