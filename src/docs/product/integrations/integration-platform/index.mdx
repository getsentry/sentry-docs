...
Users will have the option to install your integrations on the "Integrations" page in [sentry.io](https://sentry.io). If your integration is still in an unpublished state, the only Sentry organization that will be able to install it will be the organization that created the integration. Clicking the name of the integration will allow users to see a description of your integration and the permissions that will be granted if the user chooses to install.

![A list of integrations that a user can install.](integration-directory.png)

### OAuth Process

After installation, if your user has approved of all permissions, Sentry will generate a grant code and an installation ID. This information is sent by way of the `installation.created` webhook to the webhook URL specified in your configuration.
...
When you create an internal integration, a token is automatically generated. Should you need multiple tokens, or you need to get a new one, you can go into your **Settings > Developer Settings > [Your Internal Integration]** and do so.
...
Personal auth tokens are tokens a user can use to invoke APIs directly and have access to all the resources tied to that user. A Sentry integration doesn't represent a single user, but rather makes requests as "itself".
