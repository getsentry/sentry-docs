...
For more information, check out the [full documentation on UI Components](/product/integrations/integration-platform/ui-components/).
...
Permissions specify what level of access your service requires of Sentry resources. For public integrations, Sentry will prompt users to approve these permissions upon installation. For more information, check out the [full documentation on Permissions](/api/permissions/).
...
Instead of keeping track of times and passively refreshing when a token expires, one painless way you can handle refreshing tokens is to actively capture exceptions raised by requests that receive a 401 unauthorized response from Sentry, refresh the token, and remake the request.
...
If you're looking for any of the new features below, we recommend you switch to using Sentry integrations and build an [internal integration](#internal-integrations).
