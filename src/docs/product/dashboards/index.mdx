...
Dashboards are made up of one or more widgets, and each widget visualizes of one or more [Discover queries](/product/discover-queries/).
...
a timeseries. Each chart type allows you to solve different problems:
...
on a world map. An example use would be 'in which countries are users experiencing the most errors'.
...
suited for high level aggregates. An example use would be 'p95 of all
transactions'.

## Filter by Global Selection Header

Once you've created a dashboards, you can specify which projects, environments,
...
Timeseries visualizations have an interactive graph that allows you to zoom in.
