...
Add another query for the tolerable condition by clicking the "Add Query" button. In this example, we're using the Apdex definition where tolerable response times are between the satisfactory threshold and four times the satisfactory threshold.
...
Monitoring the cardinality of issues alongside the total number of errors in your projects can give you a sense of the impact of issues in your project. For example, low cardinality in issues and a high number of total errors indicates you have a few very high impact issues. Modify the "Latest Unresolved Issues" widget by selecting the "All Events" data set. Update the search condition to filter to error events.
