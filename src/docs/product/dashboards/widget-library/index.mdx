...
The widget library contains a collection of prebuilt widgets you can add to your [custom dashboards](/product/dashboards/customize-dashboards/). To access it, click on "Add Widget" and switch to the "Widget Library" tab in the modal that appears.
...
Update columns to add `project` so you can see the associated project with each issue, and modify the “sort by” to `Events` so the widget displays issues with the most error events in descending order:
...
The issues data set is unique in that you can add a free form [token](https://docs.sentry.io/product/sentry-basics/search/#syntax) to your search condition that will filter by issue titles containing the token. For example, you can search `integrity` to filter by all issue titles containing integrity. Update the search condition with a token of your choice:

- Search condition: `is:unresolved is:for_review token`

Update the "sort by" to Priority so the widget displays issues with that have been trending upward recently:
