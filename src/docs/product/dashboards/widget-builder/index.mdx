...
The option for setting thresholds is only available for [Big Number visualizations](/product/dashboards/widget-builder/#big-number). Thresholds help identify problematic widgets, using coloured indicators. Hovering over the coloured indicators at the dashboard level displays the thresholds associated with a widget. Units can be attached to thresholds if the involved metric is of type duration or rate. An example metric would be “p95 of all transactions”.
