...
For performance widgets, data based on [processed transaction events](/product/sentry-basics/search/searchable-properties/#processed-event-properties) is the default. If you make a selection in configuring your widget that's not compatible with processed event data, then your widget will use indexed transaction events, or events-based data, and a message saying this will be displayed.

<Note>

Processed transactions data is in beta and is only available if your organization is participating in its limited release. Features in beta are still in-progress and may have bugs. We recognize the irony.
