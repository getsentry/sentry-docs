...
When the "Show Templates" toggle is enabled, there are several dashboard templates that come by default. Each template can be added directly to the list of dashboards by clicking "Add Dashboard" or can be previewed first by clicking "Preview". Templates must be added first before the resulting dashboard can be edited.
