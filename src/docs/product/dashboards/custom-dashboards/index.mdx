...
Choosing "All Events" allows you to query and aggregate individual errors and transactions in the same way you would for a [Discover query](/product/discover-queries/). This is data set fits most common use cases. Some widget examples include:
- Tracking performance on an endpoint.
- Amount of activity by country.
- Users most affected by errors.

Choosing "Issues" allows you to query using issues filters, such as `is:unresolved` for unresolved issues, and to display and sort by issues fields. Query filters are only applied to the issue group itself and not on the individual error events that make up an issue. Choose this data set if you want to customize a filtered list of issues on your dashboard. Some widget examples include:
- Issues assigned to your team.
- Most frequently occuring issues.

The "Issues" data set is only available [table](#table-results) visualization widgets.
