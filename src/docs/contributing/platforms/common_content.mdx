...
Common content lives in the `/platforms/common/` directory for all platforms, or the `<platform_name>/common/` for content specific to a platform and its frameworks (which we call guides or children).
...
To ensure that common content can scale as we suppport more platforms and frameworks, we use `include` files. These files can range from, most typically, a code sample specific to a platform or framework to information that augments the core content for a specific platform or framework.

### Simple Example

Let's look at the `/platforms/common/configuration/environments.mdx` file as a simple example of a code sample tuning the common content:
...
The common Getting Started content examplifies further how we avoid duplication in our content with extensive tuning. This file is `/platforms/common/index.md`:
