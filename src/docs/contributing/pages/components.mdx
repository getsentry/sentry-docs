...
Render terms in the correct case, for each platform, within the body text of a page (not in code samples):

```markdown {tabTitle:Example}
<PlatformIdentifier name="before-send" />
```

When you use <PlatformIdentifier name="before-send" />, it will be converted to:

- `beforeSend` if case_style=camelCase
- `before_send` if case_style=snake_case
- `BeforeSend` if case_style=PascalCase

This component enforces the case_style setting of the platform and only works properly in platform pages.
