...
- For SDKs that support Release Health, we control the display of content with a `PlatformSection supported` statement. For these SDKs (so far, only the Apple family), provide the following code samples:
