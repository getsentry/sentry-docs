...
The Configuration section of the content covers, unsurprisingly, how customers can configure an SDK. In most cases, the SDK will adopt the common content, which is stored in `/docs/platforms/<SDK>/configuration/` or `/docs/platforms/<SDK>/common/configuration/` - feel free to peruse the files in that directory to help answer any questions.
