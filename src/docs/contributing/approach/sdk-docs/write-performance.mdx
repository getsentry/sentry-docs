...
1. Determine if the paragraph regarding pageload and navigation applies to the SDK. If so, add the SDK to the `Supported` list in the `<PlatformSection>` surrounding **Connecting Services**. If not, add the SDK to the `notSupported` list, which will prevent the content from displaying for this SDK.
