...
If you're building on top of Sentry's API (i.e using <Link to="/api/auth/">Auth Tokens</Link>), you'll need certain scopes to access
different API endpoints.

If you're looking for information on membership roles please visit the
<Link to="/accounts/membership/">membership</Link> documentation.
...
<Link to="/api/releases/">Releases</Link> section of the API Documentation.
...
Be aware that if you're using `sentry-cli` to <Link to="/cli/releases/">manage your releases</Link>, you'll need a token which also has `org:read` scope.
