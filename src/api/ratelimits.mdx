...
Sentry rate limits every API request made to prevent abuse and resource overuse. The limit is applied to each caller per endpoint.
...
Every API request response will include the following headers:
...
- `X-Sentry-Rate-Limit-Reset`
  - The time when the next rate limit window begins and the count resets
  - Measured in UTC seconds from epoch
...
Rate limits for internal integrations may differ from other rate limits, and tend to be higher. We reccomend using internal integration bearer tokens if you anticipate needing to make a large volume of calls.
