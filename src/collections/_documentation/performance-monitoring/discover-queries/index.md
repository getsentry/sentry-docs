...
The Event Detail view varies depending on event type (i.e. error, transaction, csp). [Transaction event details](/performance-monitoring/performance/event-detail/) may have an entirely different view from error event details (as shown above). For example, errors may have a related issue shown in the sidebar. This gives you a quick glance at the event volume for that issue and lets you quickly navigate to the related issue. You may also find stack traces, breadcrumbs and more in these views. 