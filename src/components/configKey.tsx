...
export function ConfigKey({
  ...
  let configKeyClassName = 'config-key';

  if (deprecated === true) {
    configKeyClassName += ' config-key-deprecated ';
  }
  ...
  return (
    <PlatformSection
      ...
      platform={platform}
    >
      <div className={configKeyClassName}>
        <h4 id={name}>
          <a
            href={`#${name}`}
        ...
        <div className="config-description">{children}</div>
      </div>
    </PlatformSection>
  );
}
