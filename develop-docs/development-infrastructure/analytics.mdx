...
Run the tests that touch the endpoint to ensure everything works.
...
All you'll actually need is to import analytics from utils and call it wherever you need. Keep in mind the effect of React lifecycles on your data. In this case, we only want to send one event when the component mounts, so we call it in `useEffect` with an empty dependency array.
...
Track aggregate stats with [Metrics](/backend/application-domains/metrics/). For example, this can be used to track aggregate response codes for an endpoint.
