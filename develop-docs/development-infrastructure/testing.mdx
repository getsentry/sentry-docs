...
The test suite is aware of [silo modes](/application-architecture/#silo-modes) and by default tests will run in `REGION` mode. To have tests run in `CONTROL` mode, you can use `control_silo_test` annotation. To have tests run in all silo modes you can use `all_silo_test`:
