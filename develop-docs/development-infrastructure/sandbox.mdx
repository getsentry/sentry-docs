...
1. Forward service ports from the sandbox:

```shell
devservices sandbox port-forward my-sandbox --ports services
```

2. Stop local devservices if running (to free up ports):

```shell
devservices down
```

3. Stop the remote devserver on the sandbox:

```shell
devservices sandbox exec "sudo systemctl stop sandbox-devserver"
```

<Alert title="Note">
  The sandbox devserver should be stopped in hybrid mode since you're running the devserver locally. This frees CPU/RAM on the VM for the Docker services.
</Alert>

4. Start the local devserver — it connects to the forwarded ports:
...
<Alert title="Note">
  This requires a fully set up local environment (`devenv sync`). Every database query and Redis call goes over the SSH tunnel, which adds latency.
</Alert>
