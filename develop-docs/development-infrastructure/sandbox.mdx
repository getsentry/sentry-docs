...
5. Open `/opt/sentry` or `/opt/getsentry`
6. Set the Python interpreter to `/opt/sentry/.venv/bin/python`
...
2. Run SSH config setup (above)
3. Open Gateway, go to **SSH** tab, and select `sandbox-my-sandbox`
4. Choose your IDE (PyCharm, IntelliJ, WebStorm)
5. Set the project directory to `/opt/sentry` or `/opt/getsentry`
...
| `sandbox logs <name> [service]` | View devserver or service logs |
