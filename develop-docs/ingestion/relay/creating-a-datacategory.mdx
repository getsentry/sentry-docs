...
3. Update `librelay` by running `make header` in the `relay-cabi/` directory.
4. Add your data category to the Relay `CHANGELOG.md`.
5. Add your data category to the Python librelay `py/CHANGELOG.md`.

You can also use this [Example PR](https://github.com/getsentry/relay?tab=readme-ov-file#development) as a reference.


## Creating a librelay Release


To release a new version of `librelay` you have to trigger the [Release Library](https://github.com/getsentry/relay/actions/workflows/release_library.yml),
