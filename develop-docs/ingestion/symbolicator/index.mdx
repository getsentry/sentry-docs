...
A project can be assigned to the *low priority queue* for a platform if it uses an unusual amount of symbolication time. We have Symbolicator pods set aside for projects on the LPQ so that such projects don’t block symbolication for other projects.
