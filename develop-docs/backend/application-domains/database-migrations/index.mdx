...
There is a [useful script](https://github.com/getsentry/sentry/blob/master/bin/update-migration) for automatically moving your migration to the top of the migration history. To use the script, first make sure your local master branch is up to date. then, on your migration branch, run `git rebase master`. You will have conflicts, after which you can run:
