...
This document contains a set of design principles and requirements whish should be applied to all Sentry APIs. These are _requirements_ when designing public APIs, but internal APIs should attempt to adhere to these principles as well.

The most important thing you should know up front:

**Every endpoint in Sentry's API is public.**

You should design it as such. You should **not** expose internal designs, including parameters which control behaviors of the underlying system.
...
- **Do** use `snake_case` for route parameters. e.g. `organizations/\{organization}/projects/`.
...
- **Do** prefix resource collections with `organizations/\{organization}`.
- **Do** prefix resource endpoints with `projects/\{organization}/\{project}`.
- **Do not** expose endpoints which require the `organization` as a query parameter.
...
- **PUT** - Update a resource - must support partial updates
- **DELETE** - Delete a resource

Do not use **PATCH** as it is redundant. Use **PUT** instead. If you have an operation which is intended to _replace_ a resource, use `POST /resources/{id}`.

### Batch Operations

Resources can get complicated when you need to expose batch operations vs single resource operations.
