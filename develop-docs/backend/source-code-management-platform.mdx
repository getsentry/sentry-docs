...
scm = SourceCodeManager.make_from_repository_id(organization_id=1, repository_id=2, fetch_repository=..., fetch_provider=...)
```

This will initialize a new SCM class with its capabilities scoped to what the repository's service-provider can offer.

If you're in the `getsentry/sentry` repository a helper function can orchestrate the scm instance for you.

```
from sentry.scm import new

scm = new(organization_id, repository_id)
```
...
This pattern is discouraged. It is preferred that you interact with service-providers on a capability basis so that your feature is automatically enabled for new service-providers but we understand that this is not always desired.
...
The Source Code Management Platform exposes an RPC server and may be accessed with the `scm` library in your service.

```python
from scm.rpc.client import SourceCodeManager

scm = SourceCodeManager.make_from_repository_id(organization_id=1, repository_id=1)
