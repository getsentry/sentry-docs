...
If the bumped conventions need to be deployed urgently, reach out in `#discuss-ingest` to coordinate a deployment. Otherwise, no further action is required.
...
#### When does an attribute need Sentry code adjustments?

[Step 3](#sentry-backend) is required when the attribute needs special adjustments in the Sentry UI.
