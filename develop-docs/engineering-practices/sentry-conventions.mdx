...
- Set `pii` to `"maybe"` for string attributes unless scrubbing would break product features.
...
1. Update `pyproject.toml` to the new version pin (e.g. `sentry-conventions>=0.13.0`).
2. Run `uv lock` to update the lockfile.
3. Open + merge PR at [`getsentry/snuba`](https://github.com/getsentry/snuba).
...
5. Open + merge PR at [`getsentry/sentry`](https://github.com/getsentry/sentry).
