...
Django Rest Framework's serializers are used to handle input validation and transformation.

### Example

The version of [Django Rest Framework](http://www.tomchristie.com/rest-framework-2-docs/) we currently use is **2.5**. We use Django Rest Framework serializers for data coming into Sentry. In the typical serializer, the fields are specified so that they validate the type and format of the data to your specifications. Django Rest Framework serializers can also save the information into the database if written to fit to the model.
