...
- span data must follow the [Span Data Conventions](/sdk/telemetry/transactions/#span-data-conventions)
...
-  While the keys of sensitive HTTP headers (e.g. `Authorization` and `Cookie`) are included, their values must be replaced with `"[Filtered]"` (also see <Link to="/sdk/foundations/data-scrubbing/#sensitive-data">Data Handling: Sensitive Data</Link>).
...
The Exception Interface, see the <Link to="/sdk/telemetry/errors/">Errors spec</Link> for details.
...
- Set the <Link to="/sdk/telemetry/errors/#exception-mechanism">Exception Mechanism</Link> with a proper `type` such as `SentryOkHttpInterceptor`.
