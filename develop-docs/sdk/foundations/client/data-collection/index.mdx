...
This tier is **not** gated by the `dataCollection` configuration. SDKs **SHOULD** collect it by default.
...
This tier **MUST** be disabled by default unless the user opts in via `includeUserInfo` or explicit `collect` allowlists. See [`includeUserInfo`](#include-user-info-behavior), [`collect` options](#collect-option-behavior), and [Default Denylist](#default-denylist).
