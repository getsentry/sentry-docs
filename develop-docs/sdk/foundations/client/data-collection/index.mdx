...
<SpecSection id="data-sensitivity-levels" status="draft" since="1.0.0">
...
Personally Identifiable Information (PII) or user-linked data. This includes:
...
This sensitivity level **MUST** be disabled by default unless the user opts in via `includeUserInfo` or explicit `collect` allowlists. See [`includeUserInfo`](#include-user-info-behavior), [`collect` options](#collect-option-behavior), and [Default Denylist](#default-denylist).

#### 3. Sensitive Data

Credentials and secrets that **MUST NOT** be sent by default. This includes:
...
All data-collection options reside under a single top-level key: `dataCollection`. SDKs **MUST** support at least `includeUserInfo` and the `collect` object. SDKs **MAY** omit options that do not apply to their platform (e.g., `outgoingResponseBody` on a client-only SDK).
...
The `includeUserInfo` option controls whether the SDK automatically attaches user identity fields to events (e.g., `user.id`, `user.email`, `user.username`, `user.ip_address`). This serves as the primary PII gate; its value also sets the effective default for PII-heavy `collect` options.
...
Each key under `collect` maps to a category of automatically collected data. The configuration format depends on whether the data is structured as key-value pairs.
...
| `false` | Do not collect this category. |
| `{ deny: string[] }` | Collect this category. Apply the default denylist **plus** these additional key names. |
| `{ allow: string[] }` | Collect **only** keys in this list. The default denylist is bypassed, but sensitive values **MUST** still be scrubbed. |

> **Note:** Sensitive key **values** are always scrubbed (replaced with `"[Filtered]"`) regardless of configuration. The allow/deny lists control which keys are included, not whether scrubbing applies.
...
| PII Boolean (e.g. `incomingRequestBody`, `incomingResponseBody`) | `true` — attach | `false` — do not attach |
...
For key-value data (HTTP headers, cookies, URL query params), SDKs **MUST** apply a **default denylist** based on key name. Values corresponding to known-sensitive keys are replaced with `"[Filtered]"`. **Keys are never scrubbed** by the SDK.
...
When `includeUserInfo` is `false`, SDKs **MUST** apply the base denylist **and** treat the following as sensitive:

- Any data containing email, user ID, IP address, username, or machine name (if applicable).
...
Effective denylist when PII is disabled: Base list + `["forwarded", "-ip", "remote-", "via", "-user"]` (partial match, case-insensitive).
...
- **When individual cookie key-value pairs cannot be extracted** (e.g., malformed or opaque cookie string), the entire `Cookie` or `Set-Cookie` header value **MUST** be replaced with `"[Filtered]"`. Unfiltered raw cookie header values **MUST NOT** be sent. When in doubt, treat the entire cookie header as sensitive.
...
| `incomingResponseBody` | Boolean | TBD | 1.0.0 | Include full body of the incoming HTTP response. |
| `incomingRequestBody` | Boolean? | TBD | 1.0.0 | Include full body of the incoming HTTP request (applicable in server-environments). |
| `outgoingResponseBody` | Boolean? | TBD | 1.0.0 | Include full body of outgoing HTTP responses (applicable in server-environments). |
...
The most minimal configuration is just:
...
This behaves the same as explicitly setting `dataCollection.includeUserInfo: false` (the default) and keeping the default `collect` options.

If you want automatic user identity fields (legacy `sendDefaultPii: true` behavior), enable `includeUserInfo`:
...
For reference, this is an explicit representation of all defaults (with `includeUserInfo: false`):
