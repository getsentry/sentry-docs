...
When a user-provided callback fails due to a recoverable error, SDKs:
  ...
  - **MUST NOT** re-throw or capture the error as an event, unless strict mode is enabled.
...
SDKs **MAY** offer an opt-in strict mode, disabled by default. In strict mode, a callback failure **MUST** propagate as an unhandled error of the host application instead of being isolated. SDKs **MUST NOT** pass the event reporting that error through the callback that failed.
