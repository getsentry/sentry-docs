...
The telemetry buffer on the backend must follow the common [telemetry buffer requirements](/sdk/foundations/processor/#telemetry-buffer). Here are the additional requirements for the backend-specific implementation:
...
On the backend, use the same size limits as the [common requirements](/sdk/foundations/processor/#telemetry-buffer), except for spans, where we recommend **1000** because span volume is higher.

##### Span Buffer

The span buffer must follow the common [telemetry span buffer requirements](/sdk/foundations/processor/#span-buffer). Further requirements for the bucketed-by-trace buffer are:
