...
- In events reported from a Python/ASP.NET/Rails web backend, the OS context
  ...
  from the event's [Request Interface](/sdk/foundations/transport/event-payloads/request/) and create a _secondary_ OS
...
The data in the context abides to the [Cloud](https://opentelemetry.io/docs/specs/semconv/resource/cloud/) and [Host](https://opentelemetry.io/docs/specs/semconv/resource/host/) semantic conventions of OpenTelemenetry.
...
The following example illustrates the contexts part of the <Link to="/sdk/foundations/envelopes/event-payloads/">event payload</Link> and omits other attributes for simplicity.
...
: _Optional_. Short code identifying the type of operation the span is measuring. For more details, see [Sentry's conventions around span operations](/sdk/telemetry/transactions/#span-operations).
...
: _Optional_. The origin of the trace indicates what created the trace. For more details, see [trace origin](/sdk/telemetry/transactions/#trace-origin).
