...
Transactions must be wrapped in an [Envelope](/sdk/foundations/envelopes/) and therefore also be sent to the Envelope endpoint.

<Alert title="Note on backwards compatibility">
  Envelopes are a new format and therefore only work on Sentry >= 10. More
  information can be found on the <Link to="/sdk/foundations/envelopes/">Envelope</Link> page.
</Alert>

## Anatomy

A Transaction is basically a [Span](/sdk/foundations/transport/event-payloads/span/) combined with an [Event](/sdk/foundations/envelopes/event-payloads/). When using
...
Transactions are [Events](/sdk/foundations/envelopes/event-payloads/) enriched with [Span](/sdk/foundations/transport/event-payloads/span/) data.
