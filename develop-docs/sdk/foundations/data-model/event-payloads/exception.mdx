...
An <Link to="/sdk/foundations/data-model/event-payloads/">event</Link> may contain one or more exceptions in an attribute named `exception`.
...
: The type of exception (a string), e.g. `ValueError`. At least one of `type` or `value` is required, otherwise the exception is discarded.

`value`

: The value of the exception (a string). At least one of `type` or `value` is required, otherwise the exception is discarded.
...
: An optional value which refers to a thread in the [Thread Interface](/sdk/foundations/data-model/event-payloads/threads/).
...
: An optional stack trace object corresponding to the [Stack Trace Interface](/sdk/foundations/data-model/event-payloads/stacktrace/).
## Exception Mechanism
...
contains the exception part of the <Link to="/sdk/foundations/data-model/event-payloads/">event payload</Link> and omits other
