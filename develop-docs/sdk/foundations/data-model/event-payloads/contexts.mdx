...
- In events reported from a Python/ASP.NET/Rails web backend, the OS context
  ...
  from the event's [Request Interface](/sdk/foundations/data-model/event-payloads/request/) and create a _secondary_ OS
...
The following example illustrates the contexts part of the <Link to="/sdk/foundations/data-model/event-payloads/">event payload</Link> and omits other attributes for simplicity.
