...
Transactions must be wrapped in an [Envelope](/sdk/foundations/transport/envelopes/) and therefore also be sent to the Envelope endpoint.

<Alert title="Note on backwards compatibility">
  Envelopes are a new format and therefore only work on Sentry >= 10. More
  information can be found on the <Link to="/sdk/foundations/transport/envelopes/">Envelope</Link> page.
</Alert>
