...
Item type `"feedback"` contains an event with a feedback context in the payload encoded in JSON. See the <Link to="/sdk/telemetry/feedbacks/">Feedback spec</Link> for the full protocol, payload attributes, and examples.
