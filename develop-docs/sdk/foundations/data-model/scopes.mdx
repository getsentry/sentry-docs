...
Users MUST be able to attach attributes to any scope using a dedicated method (e.g., `scope.setAttributes()` or `scope.setAttribute()`). These attributes follow the structure defined in the [Attributes](/sdk/foundations/data-model/attributes) documentation.
...
- `unit` (optional): The unit of measurement. See [Units](/sdk/foundations/data-model/attributes#units) for the full list of supported units.
...
- `type` (optional): The type of the attribute. See [Attributes](/sdk/foundations/data-model/attributes) for the full list. SDKs SHOULD NOT expose this to users if they can reliably infer the type from the value. If not, SDKs MAY allow or require users to set the `type` explicitly.
...
See [Attributes](/sdk/foundations/data-model/attributes) for detailed information about attribute structure, supported types, units, and common attribute keys. Also see [Sentry Conventions](https://github.com/getsentry/sentry-conventions/) for the complete attribute registry.
