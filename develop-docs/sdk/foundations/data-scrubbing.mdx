...
- SDKs collect rich debugging context — including user identity (`user.*`) and HTTP bodies — by default, while never sending credentials or secrets through automatic instrumentation. The legacy [_send-default-pii_](https://docs.sentry.io/platforms/python/configuration/options/#send-default-pii) toggle is replaced by the structured `dataCollection` configuration; user identity collection is controlled by `dataCollection.userInfo` (default `true`) and can be disabled to opt out (see [Data Collection](/sdk/foundations/client/data-collection/)).
...
- For how `urlQueryParams` filtering applies to URL attributes, see [Data Collection — URLs](/sdk/foundations/client/data-collection/#urls).
