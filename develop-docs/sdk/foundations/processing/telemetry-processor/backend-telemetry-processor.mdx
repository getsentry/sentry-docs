...
Unlike the [push-based approach](/sdk/foundations/processing/telemetry-processor/#push-based-approach), the [telemetry scheduler](#telemetryscheduler) pulls data from the the telemetry buffer, by iterating through all buffers using weighted round-robin scheduling and flushing them, when it determines a buffer is ready. Still, the telemetry buffer **MUST** follow the common [telemetry buffer requirements](/sdk/foundations/processing/telemetry-processor/#telemetry-buffer). Here are the additional backend-specific requirements:
