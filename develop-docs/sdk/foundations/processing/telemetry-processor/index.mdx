...
The telemetry buffer batches high-volume telemetry items and forwards them to the telemetry scheduler.


## Common Requirements

This section covers the common requirements for all platforms:

1. The telemetry buffer **SHOULD** use separate buffers per telemetry item type (e.g., one for spans, one for logs, one for metrics).
...
4. For low-volume data, such as events, session replays or user feedback, the telemetry buffer **SHOULD** directly forward these items to the telemetry scheduler.
...
6. When the batch size is reached, the telemetry buffer **MUST** forward the batch to the telemetry scheduler.
