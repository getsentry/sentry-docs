...
For rules about which items can be combined in a single envelope, see [Envelope Item Constraints](/sdk/foundations/envelopes/envelope-items/#envelope-item-constraints). These constraints impact the design — for example, the [span buffer](#span-buffer) organizes spans per traceID because each envelope can only contain spans from a single trace, while logs and metrics can be batched across traces.
