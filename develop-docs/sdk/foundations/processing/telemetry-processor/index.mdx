...
The push-based approach is the default for most SDKs. The telemetry buffers push data to the telemetry scheduler when full or on timeout. For a pull-based alternative, where the telemetry scheduler polls data from the buffers, see the [Backend Telemetry Processor](./backend-telemetry-processor/).
