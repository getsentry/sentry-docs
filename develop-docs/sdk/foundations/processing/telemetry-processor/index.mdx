...
The telemetry scheduler **SHOULD** add [client reports](/sdk/telemetry/client-reports/) to outgoing envelopes or periodically send standalone client report envelopes to the transport, following the [SDK recommendations](/sdk/telemetry/client-reports/#sdk-recommendations).

Client report aggregation **SHOULD NOT** be implemented in the telemetry scheduler. Instead, the SDK **SHOULD** have a centralized `ClientReports` component per client, accessible throughout the SDK, since data can be dropped in multiple places, as shown in the diagram below. `ClientReports` handles aggregation internally. The telemetry scheduler **SHOULD** call an extraction method on `ClientReports` to atomically retrieve and reset the current aggregated report.
...
**Recording:** Multiple components call `Record` on `ClientReports` when they drop telemetry.
...
**Sending:** The component on the sending path extracts the aggregated report and attaches it to outgoing envelopes, or periodically sends it as a standalone envelope.

```mermaid
flowchart LR
  ClientReports["ClientReports"]
  TelemetryQueue["Telemetry Queue"]
  Sender["Telemetry Scheduler / Transport"]

  TelemetryQueue -- "envelope" --> Sender
  ClientReports -- "TakeReport()" --> Sender
  Sender -- "envelope + client report attached" --> Sentry
  Sender -- "standalone client report envelope\n(periodic, to avoid starvation)" --> Sentry
```
