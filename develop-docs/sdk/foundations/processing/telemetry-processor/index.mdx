...
For rules about which items can be combined in a single envelope, see [Envelope Item Constraints](/sdk/foundations/transport/envelope-items/#envelope-item-constraints). These constraints impact the design — for example, the [span buffer](#span-buffer) organizes spans per traceID because each envelope can only contain spans from a single trace, while logs and metrics can be batched across traces.
...
## Push-Based or Pull-Based Approach

SDKs **SHOULD** use the push-based approach, where the telemetry buffers push data to the telemetry scheduler when full or on timeout. For backend SDKs with high-throughput requirements, a [pull-based approach](./backend-telemetry-processor/) **MAY** be used. This section describes the push-based approach.
...
Client reports aggregation **SHOULD NOT** be implemented in the telemetry scheduler. Instead, the SDK **SHOULD** have a centralized `ClientReports` component per client, accessible throughout the SDK, since data can be dropped in multiple places, as shown in the diagram below. `ClientReports` handles aggregation internally. The telemetry scheduler **SHOULD** call an extraction method on `ClientReports` to atomically retrieve and reset the current aggregated client report.

See the [client reports spec](/sdk/telemetry/client-reports/#telemetry-processor-integration) for the full architecture details, per-client scoping requirements, and recording points.

### Recording

Multiple components call `Record` on `ClientReports` when they drop telemetry items.
...
The telemetry scheduler extracts the aggregated report and attaches it to outgoing envelopes, or periodically sends it as a standalone envelope following the [SDK recommendations](/sdk/telemetry/client-reports/#sdk-recommendations).
...
flowchart LR
  ...
  subgraph TelemetrySchedulerSubGraph["TelemetryScheduler"]
    ...
    TelemetryQueue -- "envelope" --> TelemetryScheduler
    TelemetryScheduler -- "add or remove envelope" --> TelemetryQueue
  end

  Transport["Transport"]

  ClientReports -- "TakeReport()" --> TelemetryScheduler
  TelemetryScheduler -- "sendEnvelope" --> Transport
```
...
It's worth noting that [rate limits](/sdk/foundations/transport/rate-limiting/) **MUST** still be tracked per each DSN. Whatever bookkeeping you do to track rate limits, it **MUST** be done per each DSN.
