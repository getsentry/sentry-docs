...
- `infer_ip`: Controls the behaviour of IP address inference based on request information. Following values are allowed:
  - `auto`: infer the IP address based on available request information. This is equal to [setting the IP address to `{{auto}}`](/sdk/foundations/transport/event-payloads/user/#automatic-ip-addresses).
