...
Several envelope item types ([spans](/sdk/telemetry/spans/span-protocol/#span-envelope-item-ingest-settings), [logs](/sdk/telemetry/logs/#log-envelope-item-ingest-settings), [metrics](/sdk/telemetry/metrics/#trace-metric-envelope-item-ingest-settings)) support an optional `ingest_settings` object on their envelope item payload. This object instructs Relay to infer certain data from the incoming HTTP request and attach it as attributes to the telemetry item.
