...
In addition to [attributes](/sdk/foundations/state-management/scopes/attributes/) and [breadcrumbs](/sdk/foundations/state-management/scopes/breadcrumbs/), SDKs **MUST** allow users to set user context, string tags, structured contexts, severity level, grouping fingerprints, and event processor callbacks on any scope. SDKs **MUST** also provide a method to reset a scope to its defaults while keeping registered event processors.

These methods have the same semantics as in the [Hub & Scope](/sdk/foundations/state-management/hub-and-scope/) spec. The scope type determines the propagation lifetime of the data.
