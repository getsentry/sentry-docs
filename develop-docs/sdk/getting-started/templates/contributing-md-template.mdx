...
- **Code review** — LOGAF scale (l/m/h), expected SLAs (1 BD internal / 2 BD external), ready-for-merge label. Link to the [reviewing a PR playbook](https://develop.sentry.dev/sdk/getting-started/playbooks/development/reviewing-a-pr/).
...
- [`getsentry/sentry-cocoa`](https://github.com/getsentry/sentry-cocoa/blob/master/CONTRIBUTING.md) — LOGAF examples, ready-for-merge label documented, copyright header rule
- [`getsentry/sentry-java`](https://github.com/getsentry/sentry-java/blob/master/CONTRIBUTING.md) — API compatibility validation workflow, clear step-by-step setup

## Template

Use the [`sentry-sdk-skills:contributing-md`](https://github.com/getsentry/skills/blob/main/plugins/sentry-sdk-skills/skills/contributing-md/SKILL.md) skill to generate initial content from your codebase — it detects commands, CI config, and ecosystem from the repo and fills in the placeholders below.
...
so we can align on scope and approach. See the
[handling external contributor PRs](https://develop.sentry.dev/sdk/getting-started/playbooks/development/handling-external-contributor-pr/)
playbook for what to expect.
...
4. Verify setup: `[smoke test command]`

For a general project overview see the [README](./README.md).

## Development Workflow

- **Commits:** follow `type(scope): description` format (e.g. `fix(transport): handle connection timeout`). See the [code submission standard](https://develop.sentry.dev/sdk/getting-started/standards/code-submission/).
- **Branches:** `{username}/{issue-number}-short-description`
...
1. Open the PR as a **draft** until it is ready for review.
2. Fill in the PR description template.
3. Ensure CI passes and the changelog is updated.

See the [opening a PR playbook](https://develop.sentry.dev/sdk/getting-started/playbooks/development/opening-a-pr/) for more detail.

## Code Review

We use the **LOGAF scale** to signal comment weight:

- `l` — Low: purely cosmetic, feel free to ignore
- `m` — Medium: worth fixing before merge
- `h` — High: must fix before merge

Expected SLAs: **1 business day** for Sentry engineers, **2 business days** for external contributors.

When all blocking comments are resolved, a maintainer will add the **ready-for-merge** label and merge.

See the [reviewing a PR playbook](https://develop.sentry.dev/sdk/getting-started/playbooks/development/reviewing-a-pr/).

## AI-Assisted Contributions

AI-generated code is welcome. Follow the [AI attribution standard](https://develop.sentry.dev/sdk/getting-started/standards/code-submission/#ai-attribution): include a `Co-Authored-By` trailer in the commit message for any AI tool that materially contributed to the change.

Maintainers reviewing AI-generated PRs follow the [reviewing AI-generated code playbook](https://develop.sentry.dev/sdk/getting-started/playbooks/development/reviewing-ai-generated-code/).

## Getting Help

- Join the [Sentry Discord](https://discord.gg/sentry) and find us in **#sdk-\[language\]**.
- For usage questions (not bugs), use [GitHub Discussions](../../discussions) or Discord.
