...
<Alert level="info">
The promotion criteria below were ratified for the spec framework introduced in [PR #16461](https://github.com/getsentry/sentry-docs/pull/16461). Stephanie Anderson owns the framework — check with her before changing them.
</Alert>
...
- `proposal` and `draft` specs **MUST** be `0.x`. The `0.` prefix signals "not yet stable — anything **MAY** change."
- Promoting to `candidate` **MUST** bump the version to `1.0.0`. A `1.0.0`-or-higher version means the spec has been deemed implementable.
...
<Alert level="warning">
Some existing `draft` specs are versioned at `1.0.0`, predating this rule. They **SHOULD** be renumbered to `0.x` when next edited.
</Alert>
