...
Any change to public API — adding, modifying, deprecating, or removing — **SHOULD** be approved by an [SDK Senior Engineer](https://github.com/orgs/getsentry/teams/sdk-seniors) or Engineering Manager.

What counts as "public" varies by language. As a general rule, public API includes anything a user can call, import, configure, subclass, or otherwise reference in documented or supported ways. Some SDKs have clear boundaries, others don't. If you're unsure whether something is public, discuss with the team rather than assuming either way — over-classifying blocks minor releases, under-classifying risks breaking users.
...
1. **Announce** in a minor release: add a compile-time deprecation warning (where possible), update the changelog and docs, and publish a migration guide. Any emitted warnings **MUST** link to the migration guide.
