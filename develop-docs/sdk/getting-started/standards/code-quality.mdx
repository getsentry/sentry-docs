...
<StatusBadge type="yes" /> Linter choice, rules, strictness. The standard is that a linter exists and is enforced.
...
<StatusBadge type="yes" /> Configuration and strictness. Gradual typing allowed with a tracked plan.
...
<StatusBadge type="no" /> Principle is universal. Language-specific anti-patterns documented per-SDK in AGENTS.md.
...
<StatusBadge type="yes" /> Budgets and measurement are SDK-specific. The standard is that budgets exist and are tracked.
