...
- [Documenting decisions](/sdk/getting-started/standards/coordination-maintenance#decision-documentation) — traceability requirement for decisions
...
Work with the engineering manager responsible for the SDK to get approval. The decision **MUST** be documented in Linear or GitHub per the [Documenting decisions](/sdk/getting-started/standards/coordination-maintenance#decision-documentation) standard.
...
You **MUST** mark the package as deprecated on its registry so users see the notice at install time:

- **npm**: `npm deprecate <package> "message"`
- **PyPI**: update classifiers to `Development Status :: 7 - Inactive`
- **NuGet**: mark as deprecated in the NuGet Gallery
- **Maven**: update the POM description and README
- **CocoaPods / Swift Package Manager**: update the podspec or `Package.swift` description

The deprecation message **MUST** name the successor SDKs.

#### 11. Archive the GitHub repo

Archive the repository to make it read-only. Update the repo description to reflect the deprecation. Add a topic tag (e.g., `tag-archived`) for discoverability.
...
- [Documenting decisions](/sdk/getting-started/standards/coordination-maintenance#decision-documentation) — traceability requirement
- [Cutting a Release](/sdk/getting-started/playbooks/sdk-lifecycle/cutting-a-release) — release process for the final release
