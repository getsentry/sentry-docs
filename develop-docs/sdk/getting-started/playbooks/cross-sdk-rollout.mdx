...
Related resources:
- [Cross-SDK Coordination Protocol](/sdk/getting-started/standards/coordination-maintenance#cross-sdk-coordination-protocol) — coordination requirements and process
...
- [Deprecating an API](/sdk/getting-started/playbooks/deprecating-an-api) — related playbook for API changes
...
You **MUST** create an RFC or issue describing:
...
You **SHOULD** use the cross-SDK proposal template if your organization has one.

#### 2. Cross-SDK review period

You **MUST** allow a minimum of 1 week for all affected SDK team leads to review and comment on the proposal. This ensures all teams have time to:
...
You **MUST** create a sub-initiative under the [Great Data Capture - SDKs](https://linear.app/getsentry/initiative/great-data-capture-sdks-650e07004f79/overview) initiative in Linear.

For each SDK team involved, you **MUST** create a separate **project** under this sub-initiative. Using a single project with multiple issues **MUST NOT** be done, as the project is only completed once **all** SDK teams finish their work.

Each project **MUST** contain at least one issue with the **same title and description** as the project. Additional issues **MAY** be added as needed but **SHOULD** remain within the scope of the alignment effort.

This approach ensures accurate per-team progress tracking while maintaining visibility of the overall alignment effort.

#### 6. Optionally create a public issue
...
#### 7. Each SDK team implements independently

Each team **MUST** follow their own PR process ([Opening a PR](/sdk/getting-started/playbooks/opening-a-pr), [Reviewing a PR](/sdk/getting-started/playbooks/reviewing-a-pr)). The Linear projects provide visibility, but each SDK maintains its own standards and timeline.
...
#### 9. Async retro after full rollout

After all SDKs have shipped, conduct an async retrospective documenting:
- What worked well
...
- [Cross-SDK Coordination Protocol](/sdk/getting-started/standards/coordination-maintenance#cross-sdk-coordination-protocol) — coordination process and requirements
