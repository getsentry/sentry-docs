...
SDKs must never capture Sentry events for exceptions happening within the SDK itself, including within user-defined [callbacks and hooks](/sdk/foundations/client/hooks/) such as `before_send` or `traces_sampler`. We are already in an event capturing flow where the [scope](/sdk/foundations/state-management/scopes/) has been applied - capturing another event at that point would lead to undefined behavior. In the worst case, it creates a busy loop of creating and sending events repeatedly until the system crashes. If the SDK throws, swallow gracefully and emit an error-level log. In mobile SDKs, unhandled crashes will still make it to Sentry via crash handlers. See [SDK Crash Detection](https://github.com/getsentry/sentry/tree/master/src/sentry/utils/sdk_crashes#sdk-crash-detection) for more details.
