...
Browser SDKs automatically create spans for the initial pageload as well as for subsequent soft navigations.

These spans however don't behave like conventional spans. We call them **idle spans** because they are started at some point (more on that later) but not explicitly ended.
...
It's important to understand this behavior to interpret the duration of the root span in a sensible way:

While at the end of the idle span, we shorten its duration to the longest child span, the duration does not necessarily represent the actual time a pageload or navigation took.
Due to the debounce mechanism, child spans that you'd intuitively not associate to the initial pageload might be created close enough to the ideal end of the pageload span to prolong it.
...
The pageload span is **earliest** ended, when the browser emits when `'interactive'` or `'complete'` in `WINDOW.document.readyState`. It might be very well prolonged by our instrumentation adding additional child spans to the pageload root span.
...
Framework-specific SDKs and router instrumentations might use other mechanisms to detect them.
...
Some frameworks like React, Angular or Vue have their own in-browser routing solutions (e.g. React Router).
...
2. Extract parameterized route names, meaning, we can show and group spans by the actual route (e.g. `/users/[id]` instead of `/users/12345`) instead of individual raw urls.
...
<Note>
  At the time of writing this page, we're transitioning to a more sophisticated
  LCP and CLS capturing technique that should improve the accuracy of these
  metrics. It will essentially work like INP collection works today.
</Note>

### Interaction To Next paint (INP)

The Interaction To Next Paint (INP) web vital treated a bit differently.
...
This is a know issue and we're working on an update to the trace lifetime model to address these issues.
...
The browser SDKs support tracing without performance (TwP). This means that the SDK attaches tracing headers to outgoing requests but does not collect any spans.
While in other SDKs this is the behaviour when neither `tracesSampleRate` nor `tracesSampler` are set, in browser, users still have to add `browserTracingIntegration`.
