...
- Rate limits **MUST** be tracked per DSN.
- SDKs **MAY** store rate limits in the transport or client. Once SDKs implement the [telemetry processor](/sdk/telemetry/telemetry-processor/), they **SHOULD** implement rate limit tracking outside the transport, since the telemetry processor **SHOULD** drop rate-limited data early to avoid overhead.
