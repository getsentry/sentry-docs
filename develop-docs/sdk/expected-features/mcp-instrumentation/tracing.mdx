...
- All [Session Data Attributes](#session-data-attributes) are automatically included when the transport has a session ID.
...
- All [Common Span Attributes](#common-span-attributes) SHOULD be set (all `required` common attributes MUST be set).
- [Session Data Attributes](#session-data-attributes) are extracted from this span's request and response, then attached to all subsequent spans for the session.
