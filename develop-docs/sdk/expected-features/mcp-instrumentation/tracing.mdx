...
MCP servers can be either **stateless** or **stateful**:

- **Stateless servers**: Each request is independent with no shared state between requests
- **Stateful servers**: Maintain session state across multiple requests, identified by a `mcp.session.id`

For stateful servers, Sentry enriches all spans within a session with additional metadata collected during the session lifecycle, particularly from the initialization phase. This session data provides valuable context about the client and server involved in the communication.

#### Session Data Attributes

When a session is established (typically during the `initialize` request), the following attributes are collected and automatically attached to all subsequent spans within that session:
...
This session enrichment ensures that every span within a stateful session contains complete context about the communication participants, making it easier to debug issues and understand the flow of requests. As the MCP specification evolves, additional session metadata may be captured and attached to spans.
