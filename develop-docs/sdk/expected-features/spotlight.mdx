...
SDKs MUST support the `SENTRY_SPOTLIGHT` environment variable. The value MUST be parsed according to the following rules:
...
- If the Spotlight server is unreachable, SDKs:
  - SHOULD log an error message (ideally once, not per-envelope)
