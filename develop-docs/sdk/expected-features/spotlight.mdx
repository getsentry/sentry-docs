...
URL of the Spotlight server. It also does some accomodations based on the target application, such as altering the host value for Docker Compose
while keeping the front-end ones the same. It will also support emulator-based development enviroments and will set the host value accordingly in
the near future.
