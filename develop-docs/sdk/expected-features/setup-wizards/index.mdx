...
For a step‑by‑step guide on **local development and debugging of `sentry-wizard` (including usage of `--url` with a local Sentry instance and the wizard UI)**,
see the <Link to="/frontend/debugging-setup-wizard/">Debugging the Setup Wizard and Wizard UI</Link> doc.
