...
When feedback is captured using the manual API (e.g., `captureFeedback()` method), the SDK must sample and flush the replay when the feedback capture method is called. Since there is no widget opening event to trigger replay capture, the SDK should call `captureReplay()` during the execution of the feedback capture API to ensure session context is preserved.
