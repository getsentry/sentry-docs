...
- User-specific information (e.g. the current user ID according to the used web-framework) is not collected and therefore not sent at all.
  - _Note_ that if a user manually sets a User object on the scope, nothing is stripped from that object. The above rule only applies to integrations that come with the SDK.
