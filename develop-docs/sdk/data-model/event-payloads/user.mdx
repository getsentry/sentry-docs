...
<Alert title="Note">
Titles of these well-known attributes are prettified in the UI. For example, `ip_address` is shown as `IP Address`. Other keys are shown as they were sent, with the prefix `data.`.
</Alert>
