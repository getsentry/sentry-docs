...
These keys are case-sensitive, so mistyping will lead to them being processed like custom attributes.
</Alert>
...
<Alert title="Note">
The listed attribute titles are prettified in the UI. For example, `ip_address` is shown as `IP Address`. Other keys are shown as they were sent, with the prefix `data.`. This also holds for mistyped keys, for example sending `Username` instead of `username` will show up as `data.Username`.
</Alert>
