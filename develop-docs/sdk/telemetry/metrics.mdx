...
- [Envelope Items](/sdk/foundations/data-model/envelope-items/) — `trace_metric` item type constraints
- [Attributes](/sdk/foundations/data-model/attributes/) — attribute types and structure
...
Metrics carry typed key-value pairs called **attributes** for dimensional filtering and grouping. Each attribute declares its type (`string`, `boolean`, `integer`, `double`). See [Attributes](/sdk/foundations/data-model/attributes/) for the full type specification.
...
1. `browser.name` — display name of the browser application. Maps to `name` in the [Browser Context](/sdk/foundations/data-model/event-payloads/contexts/#browser-context).
2. `browser.version` — version string of the browser. Maps to `version` in the [Browser Context](/sdk/foundations/data-model/event-payloads/contexts/#browser-context).
...
1. `server.address` — the address of the server that sent the metric. Equivalent to [`server_name`](/sdk/foundations/data-model/event-payloads/#optional-attributes) attached to errors and transactions.
...
| `item_count` | Number | **REQUIRED** | Number of metric items in the `items` array. |
...
| `attributes` | Object | **OPTIONAL** | 2.0.0 | Typed key-value pairs. See [Attributes](/sdk/foundations/data-model/attributes/) for supported types and structure. |
...
- `options` **Object, optional** — containing:
  - `unit` **String, optional** — the unit of measurement (only used for `distribution` and `gauge`). SDKs **MUST NOT** restrict what can be sent in (e.g. by using an enum) but **SHOULD** offer constants or similar that help customers send in <Link to="/sdk/foundations/data-model/attributes/#units">units we support</Link> (since 2.4.0).
