...
The backend synthesizes a virtual timestamp from the metric's integer millisecond and the sequence number. This limits the number of metrics that share the same integer millisecond timestamp to **1,000** per SDK instance. SDKs do not need to enforce this limit. It is a backend constraint that users should be aware of.
