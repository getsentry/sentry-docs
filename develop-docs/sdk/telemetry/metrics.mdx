...
<Alert title="Known Limitation" level="info">

The backend combines the metric timestamp in milliseconds with the sequence number as nanoseconds to synthesize a virtual timestamp. This limits the number of metrics that share the same integer millisecond timestamp to **1,000** per SDK instance. SDKs do not need to enforce this limit. It is a backend constraint that users should be aware of.
