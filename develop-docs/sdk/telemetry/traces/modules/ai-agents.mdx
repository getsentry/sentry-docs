...
The input list should include the most recent messages up to and including the most recent previous model response. The previous model response is idenfied with an "assistant" or "model" role in commmon frameworks. If there is no previous model response in the input array, then all input items which are not system instructions should be included. System instructions must be added in `gen_ai.system_instructions`, and are not included in the `gen_ai.input.messages` list.
