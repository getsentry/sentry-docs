...
The AI Agents module is library-agnostic. SDKs can instrument supported AI frameworks or libraries automatically (currently `openai-agents` in Python and `Vercel AI` in JavaScript). For other libraries, you may need to annotate spans manually.

## Span Conventions

For AI agent data to show up in the Sentry [Agents Dashboard](https://sentry.io/orgredirect/organizations/:orgslug/dashboards/), at least one AI span needs well-defined names and data attributes. If the required data (marked with **MUST** or required) is missing, the data will not show up in the Agents dashboard.

Sentry follows [v1.36.0](https://github.com/open-telemetry/semantic-conventions/blob/v1.36.0/docs/gen-ai/README.md) of the [OpenTelemetry Semantic Conventions](https://opentelemetry.io/docs/specs/semconv/gen-ai/) for Generative AI as closely as possible. Full compatibility is not yet possible because OpenTelemetry stores model input and output in span events, which Sentry does not support. Sentry stores that data as JSON-stringified span attributes instead.
...
| `gen_ai.pipeline.name`       | string | optional          | Name of the AI workflow, pipeline, or chain the tool belongs to.                                                | `"weather-pipeline"`                       |
