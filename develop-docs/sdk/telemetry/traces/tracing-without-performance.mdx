...
Any event created by an SDK in TwP mode must include the [`trace` context](/sdk/foundations/transport/event-payloads/contexts/#trace-context).
...
This usually corresponds with the lifetime of an [isolation scope](/sdk/foundations/state-management/scopes/#isolation-scope) (or current scope created within the isolation scope).
