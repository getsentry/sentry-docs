...
spec_id: sdk/telemetry/traces/tracing-without-performance
spec_version: 1.0.0
spec_status: stable
spec_depends_on:
...
spec_changelog:
  - version: 1.0.0
    date: 2024-11-06
    summary: Initial spec — TwP mode for trace propagation without span collection
---
...
Related specs:
- [Traces](/sdk/telemetry/traces/) — trace-level concepts and sampling
- [Dynamic Sampling Context](/sdk/telemetry/traces/dynamic-sampling-context/) — DSC propagation

---

## Behavior

<SpecSection id="twp-behavior" status="stable" since="1.0.0">
...
<SpecSection id="twp-implementation" status="stable" since="1.0.0">
...
- the SDK **MUST NOT** include a sampled flag in the [`sentry-trace` header](/sdk/telemetry/traces/#header-sentry-trace), meaning the header has the format `<traceId>-<spanId>`.
  More details on the [sampled flag](/sdk/telemetry/traces/#the-sampled-value).
- the [dynamic sampling context](/sdk/telemetry/traces/dynamic-sampling-context/), propagated via `baggage` **MUST NOT** contain the `sentry-sampled` key.

### Attaching Trace Data to Events and Envelopes

Any event created by an SDK in TwP mode **MUST** include the [`trace` context](/sdk/foundations/data-model/event-payloads/contexts/#trace-context).
...
This usually corresponds with the lifetime of an [isolation scope](/sdk/foundations/data-model/scopes/#isolation-scope) (or current scope created within the isolation scope).
...
The exact storage mechanism is up to the SDK implementation, but we recommend using the same mechanism as for regular traces.
Usually this means storing the data on the scope in a field called `propagationContext`.
...
In SDKs adapting OpenTelemetry's tracing capabilities ([POTel](/sdk/miscellaneous/hub_and_scope_refactoring/#f-use-otel-for-performance-instrumentation-potel)), the TwP trace data could also be stored in a non-recording span.
...
<SpecSection id="twp-configuration" status="stable" since="1.0.0">
