...
spec_id: sdk/telemetry/traces/dynamic-sampling-context
spec_version: 1.17.0
spec_status: stable
spec_changelog:
  - version: 1.17.0
    date: 2025-06-04
    summary: Updated DSC specification and propagation
  - version: 1.16.0
    date: 2025-05-20
    summary: Renamed org to org_id
  - version: 1.15.0
    date: 2025-04-28
    summary: Removed old DSC content, simplified propagation
  - version: 1.14.0
    date: 2025-04-08
    summary: Propagated sampling rates (sample_rand)
  - version: 1.0.0
    date: 2022-02-02
    summary: Initial spec — Dynamic Sampling Context
---
...
Related specs:
- [Dynamic Sampling](/dynamic-sampling/) — product overview
- [Performance / Tracing](/sdk/performance/) — `tracesSampleRate` and `tracesSampler` options
...
<SpecSection id="freezing-dsc" status="stable" since="1.0.0">
...
<SpecSection id="unified-propagation" status="stable" since="1.0.0">
...
SDKs can leverage <Link to="/sdk/foundations/data-model/event-payloads/transaction/#transaction-annotations">transaction annotations</Link> (in particular the `source` of the transaction name) to determine which transaction names have a good quality.
