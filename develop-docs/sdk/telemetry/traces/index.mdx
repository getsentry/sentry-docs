...
This must be a list/array of integers, denoting HTTP status codes.
...
The SDK should log an info message (when debug logging is enabled with `debug` == `true` in the client options) denoting why the transaction was dropped.
