...
If the value of this attribute matches one of the status codes in `traceIgnoreStatusCodes`, the SDK MUST set the transaction's [sampling decision](/sdk/telemetry/traces/#sampling) to `not sampled`.
