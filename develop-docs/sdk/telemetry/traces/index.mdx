...
Values for some of the attributes below may be unobtainable in specific situations for the SDK, and in these cases a subset MAY be provided.

The attributes are described in <Link to="/sdk/performance/span-data-conventions/">Sentry's Span Convention Documentation</Link>.
...
A threshold duration, which SHOULD be a floating point or integer value.
