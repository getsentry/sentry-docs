...
If suitable for the platform, the option MAY also admit pairs of integers (representable by an `Interval`/`Range` type if such type exists as a built-in), denoting HTTP status code ranges.
