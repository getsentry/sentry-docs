...
- [Envelope Items](/sdk/foundations/envelopes/envelope-items/) — item type reference
...
1. If an attachment is larger than ~100 MiB, upload it to the [TUS](https://tus.io/protocols/resumable-upload#creation-with-upload)-conformant upload endpoint:
   ...
   1. Upload the file:
      ...
      curl -X PATCH https://o0.ingest.sentry.io/api/0/upload/24e533e02ec3bc40c387f1a0e460e216/ \
        ...
        --data @myfile.log
      ```
1. Upon successful upload, attach a placeholder item to the event envelope, using the `Location` header from the upload response.
