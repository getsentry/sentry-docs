...
The `parentSpan` parameter has three distinct states: `undefined`, `null` and a span instance.
...
- If `parentSpan` is explicitly set to an ended span, the SDK SHOULD still create and send the new span as a child of that parent span.
