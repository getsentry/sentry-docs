...
To associate an attachment with a span, submit a [trace attachment](/sdk/data-model/envelope-items.mdx#trace-attachment) item with an additional `span_id` item header. The trace attachment _should_ be submitted in the same envelope as the span itself.
