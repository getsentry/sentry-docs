...
`startSpan` **MUST** always return a valid `Span` instance. When a span should not be recorded (e.g., due to [sampling](../sampling/) or [filtering](../filtering/) decisions), the SDK **MUST** return a Noop Span. This allows callers to use the span without null checks.
