...
SDKs **MAY** keep track of discarded events in the transport. Once SDKs implement the [telemetry processor](/sdk/foundations/processing/telemetry-processor/), they **SHOULD** track client reports outside the transport, since data can be dropped at multiple points throughout the SDK. Multiple components record into `ClientReports` and the [telemetry processor](/sdk/foundations/processing/telemetry-processor/) is responsible for extracting and sending the aggregated report.
