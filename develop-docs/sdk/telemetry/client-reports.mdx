...
spec_changelog:
  - version: 1.20.0
    date: 2026-03-04
    summary: Added architecture guidance
  - version: 1.19.0
    date: 2026-02-06
...
- [Rate Limiting](/sdk/foundations/transport/rate-limiting/) — data category definitions
...
A **discard reason** is a string that explains why a telemetry item was dropped. Each discarded item is categorized by reason and [data category](/sdk/foundations/transport/rate-limiting/#definitions).

### Data Categories

Data categories (e.g., `error`, `transaction`, `span`) identify the type of telemetry item that was discarded. These are the same categories used for [rate limits](/sdk/foundations/transport/rate-limiting/#definitions).
...
- **MUST** be safe for concurrent access, since multiple SDK components record into it simultaneously.
...
Early SDK implementations tracked client reports inside the transport, but as the number of telemetry data types has grown and data can be dropped at multiple points throughout the SDK, this approach no longer scales well. The transport only sees items at send time and has no visibility into earlier drops.

SDKs **SHOULD** own the `ClientReports` component at the client level, regardless of whether they implement the full [telemetry processor](/sdk/foundations/processing/telemetry-processor/) architecture. This keeps the transport focused on delivery.

Once SDKs implement the [telemetry processor](/sdk/foundations/processing/telemetry-processor/), the client **SHOULD NOT** forward reports to the transport directly. Instead, the telemetry processor **SHOULD** be responsible for extracting the aggregated report and attaching it to outgoing envelopes or sending it as a standalone envelope.

Client reports are separate from the normal telemetry data flow — they do not go through buffers or queues. See the [telemetry processor client reports section](/sdk/foundations/processing/telemetry-processor/#client-reports) for the full architecture diagram and recording points.
...
| `category` | String | **REQUIRED** | The [data category](/sdk/foundations/transport/rate-limiting/#definitions) for which the discard reason applies (since 1.2.0). |
