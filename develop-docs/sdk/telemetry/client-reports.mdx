...
SDKs **SHOULD** minimize unnecessary communication and **MUST NOT** send an envelope for each discarded event. Adjust these recommendations as needed:

1. For apps with low envelope frequency (e.g., mobile or web), SDKs **MUST** attach discarded events to scheduled envelopes. Fewer client reports are acceptable in such cases.
2. For backends or apps with continuous envelopes, SDKs **MUST** periodically flush or attach discarded events to scheduled envelopes.

The SDKs **MAY** keep track of the discard events in the transport. Once SDKs implement the [telemetry processor](/sdk/telemetry/telemetry-processor/), they **SHOULD** add an extra component, called client reports, to keep track of discarded events. The [telemetry scheduler](/sdk/telemetry/telemetry-processor/#telemetry-scheduler) is then responsible for passing client reports to the transport. It still **MUST** adhere to the requirements above.
