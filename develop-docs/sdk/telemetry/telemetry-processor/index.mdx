...
5. The telemetry buffer **MUST** define a size limit of x items. The size limit **MUST** respect the [envelope max sizes](/sdk/data-model/envelopes/#size-limits). When the size limit is reached, the telemetry buffer **MUST** forward all items to the telemetry scheduler.
...
For the telemetry span buffer, we recommend to use a map of [traceID to span buckets per trace](#buckets-per-traceid). SDKs may use alternatives, such as a fixed-ring buffer with tombstones. Regardless of the data structure, your SDK **MUST** meet these requirements:
