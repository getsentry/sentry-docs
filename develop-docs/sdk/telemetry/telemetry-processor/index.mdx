...
<Alert level="warning">
  ...
  If you have feedback or suggestions, please open an issue or PR and tag @philipphofmann, @isaacs, @giortzisg for review. Feedback and improvements are welcome while we confirm the approach makes sense.
</Alert>
...
Once an SDK adds the telemetry processor, its client **SHOULD** only forward data to the telemetry processor, and not to the transport. The telemetry processor consists of two major components:
...
Here's a simplified example of a log travelling through the telemetry processor:
...
3. The telemetry buffer SHOULD forward low-volume data, such as normal events, session replays, or user feedback, directly to the telemetry scheduler.
...
3. When the application shuts down gracefully, the telemetry buffer **SHOULD** forward all data in memory to the telemetry scheduler.
