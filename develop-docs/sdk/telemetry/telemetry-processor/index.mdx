...
Add the telemetry processor for high-volume data (spans, logs, metrics). SDKs without these features **MAY** omit it. Once added, clients **SHOULD** forward all data to the processor, not the transport. During migration, SDKs **MAY** temporarily send only some telemetry data through the processor.
...
<Alert>
Previously, the [unified API](/sdk/telemetry/unified-api/) managed rate limits and offline caching in the transport, forcing multi-platform SDKs (like Java) to maintain separate transports. To simplify, transports should now only send envelopes. The telemetry scheduler should handle rate limiting, offline caching, and prioritization, ensuring consistent logic across platforms. This keeps custom transports simple—SDKs need only adjust the scheduler for platform-specific needs. We recommend, but do not require, SDKs to adopt this architecture.
</Alert>
