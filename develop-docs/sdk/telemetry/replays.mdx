...
- [Envelope Items](/sdk/foundations/envelopes/envelope-items/) — `replay_event` and `replay_recording` item type constraints
...
| `replay_id` | String | **REQUIRED** | 1.0.0 | A unique ID for the replay. Follows the [same requirements](/sdk/foundations/envelopes/event-payloads/#required-attributes) as `event_id`: hexadecimal UUID v4, exactly 32 characters, lowercase, no dashes. |
...
The following attributes are a subset of the [optional attributes](/sdk/foundations/envelopes/event-payloads/#optional-attributes) of an Event.
