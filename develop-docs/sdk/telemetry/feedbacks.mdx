...
spec_changelog:
  - version: 1.5.0
    date: 2026-09-15
    summary: Added draft client-side input validation requirements, clarified Unicode message length and server limits
  - version: 1.4.0
    date: 2026-09-08
...
- [Event Payloads](/sdk/foundations/envelopes/event-payloads/) — base event attributes
...
A **feedback event** is an [event payload](/sdk/foundations/envelopes/event-payloads/) with a `feedback` context object. The feedback context carries user-provided data (message, contact info, URLs), while the event payload carries standard attributes (timestamp, event_id, platform, user, tags, etc.).
...
SDKs **MUST** apply [rate limiting](/sdk/foundations/transport/rate-limiting/) to feedbacks in the same way as it is applied to events.
...
<SpecSection id="input-validation" status="draft" since="1.5.0">

### Client-Side Input Validation

Feedback can pass transport checks but still be discarded during ingestion. SDKs **MUST** validate user input locally so users can correct their feedback before submitting it.

#### Payload Validation

These requirements apply to SDK-provided feedback forms and the manual capture API:
...
- `message` **MUST NOT** exceed **4,096 Unicode code points**. SDKs **MUST** count the value that will be sent, including any whitespace it retains. This matches Python's `len(message)` in Sentry. For well-formed Unicode, code points are equivalent to Unicode scalars, not UTF-8 bytes, UTF-16 code units, or user-perceived characters (grapheme clusters).
...
For example, precomposed `\u00E9` counts as one code point, while decomposed `e\u0301` counts as two. A character outside the Basic Multilingual Plane counts as one code point, even though UTF-16 represents it with two code units. Swift implementations can use `unicodeScalars.count`, Java can use `codePointCount`, and JavaScript can iterate over the string. Native string-length APIs and HTML `maxlength` alone do not consistently match this limit.

#### Feedback Forms

Forms **MUST** check the message before calling the capture API. When validation fails, the form **MUST** prevent submission, preserve the user's input, and display an accessible error explaining how to correct it. Forms **MUST NOT** silently truncate an overlong message and submit it as if the user had entered the shortened version.

Forms **MUST** enforce configured required name and email fields. Otherwise, name and email remain optional, and anonymous feedback **MUST** be accepted.

Forms **SHOULD** update validation as the user edits the input. A live character counter is **OPTIONAL**. If provided, it **MUST** use the same code-point count as validation and be accessible to assistive technology. This specification does not require a particular counter layout or visual design.

#### Server Limits

These limits distinguish the modern `feedback` item from the deprecated `user_report` item:

- **Sentry:** [Feedback ingestion](https://github.com/getsentry/sentry/blob/102b2390de99b983d6847bf8bc1864fee45559e3/src/sentry/feedback/usecases/ingest/create_feedback.py#L180-L214) rejects missing or whitespace-only messages, messages above the configured limit, and malformed associated event IDs. The [`feedback.message.max-size` option](https://github.com/getsentry/sentry/blob/102b2390de99b983d6847bf8bc1864fee45559e3/src/sentry/options/defaults.py#L593-L598) defaults to 4,096 code points. This is a rejection limit, not automatic truncation.
- **Relay:** The [feedback context](https://github.com/getsentry/relay/blob/800a2424667f6d4caa385e15d77e628570dcac66/relay-event-schema/src/protocol/contexts/mod.rs#L88-L90) has a shared `max_bytes = 8192` normalization budget. Its [schema](https://github.com/getsentry/relay/blob/800a2424667f6d4caa385e15d77e628570dcac66/relay-event-schema/src/protocol/contexts/user_report_v2.rs#L11-L20) does not declare separate message, name, email, or URL length limits. Relay's [trimming processor](https://github.com/getsentry/relay/blob/800a2424667f6d4caa385e15d77e628570dcac66/relay-event-normalization/src/trimming.rs) can trim or remove context data. This shared normalization budget is not an 8,192-character allowance for each field and does not replace Sentry's message validation. Feedback also remains subject to [envelope size limits](/sdk/foundations/envelopes/#size-limits).
- **Legacy user reports:** The [128-character name and 75-character email limits](https://github.com/getsentry/sentry/blob/102b2390de99b983d6847bf8bc1864fee45559e3/src/sentry/models/userreport.py#L16-L20) belong to `user_report`. They are not per-field limits for `contexts.feedback` and **MUST NOT** be imposed on modern feedback solely to match the legacy endpoint.

#### Implementation Coverage

This section is a draft because existing implementations do not yet meet all of these requirements:

- [JavaScript forms](https://github.com/getsentry/sentry-javascript/blob/352266199adb82af024b489483d37e94a2a17e81/packages/feedback/src/modal/components/Form.tsx) trim input, check required fields, and use the browser's email input validation, but do not enforce message length. [`sendFeedback`](https://github.com/getsentry/sentry-javascript/blob/352266199adb82af024b489483d37e94a2a17e81/packages/feedback/src/core/sendFeedback.ts) rejects empty messages, while the core [`captureFeedback`](https://github.com/getsentry/sentry-javascript/blob/352266199adb82af024b489483d37e94a2a17e81/packages/core/src/feedback.ts) API does not validate input.
- [Java's feedback model](https://github.com/getsentry/sentry-java/blob/cdb89fd96cbd13d6b53f9722c1096d7b015f54b8/sentry/src/main/java/io/sentry/protocol/Feedback.java#L91-L98) silently truncates messages to 4,096 UTF-16 code units. This can split a surrogate pair and is not equivalent to Sentry's code-point limit. The [Android form](https://github.com/getsentry/sentry-java/blob/cdb89fd96cbd13d6b53f9722c1096d7b015f54b8/sentry-android-core/src/main/java/io/sentry/android/core/SentryUserFeedbackForm.java#L301-L329) trims input and checks required fields, but does not show a length validation error before constructing that model.
- [Cocoa PR #8973](https://github.com/getsentry/sentry-cocoa/pull/8973) proposes managed-form validation using Unicode scalars and a live counter. It does not establish validation for every capture path or make the counter mandatory for other SDKs.

SDK validation tests **SHOULD** cover missing, empty, and whitespace-only messages, exactly 4,096 and 4,097 code points, decomposed characters, supplementary-plane characters, pasted or prefilled input, and messages modified by event processors or `beforeSendFeedback`.
...
A feedback is sent as a `feedback` envelope item containing a JSON object. The object is an [event payload](/sdk/foundations/envelopes/event-payloads/) with an additional `feedback` context object.
...
Below is a recap of the [required attributes](/sdk/foundations/envelopes/event-payloads/#required-attributes) for the event payload. For the full list of attributes, see [Event Payloads](/sdk/foundations/envelopes/event-payloads/).
...
| Field                 | Type   | Required     | Since | Description                                                                                                                                                                           |
| --------------------- | ------ | ------------ | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `message`             | String | **REQUIRED** | 1.0.0 | Comments of the user, describing what happened and/or sharing feedback. Max length: **4,096 Unicode code points**. See [Client-Side Input Validation](#client-side-input-validation). |
