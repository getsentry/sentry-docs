---
title: Feedback
description: Collecting qualitative user input with optional attachments, replay links, and error associations.
spec_id: sdk/telemetry/feedbacks
spec_version: 1.5.0
spec_status: stable
spec_depends_on:
  - id: sdk/foundations/transport/envelopes
    version: ">=1.0.0"
spec_changelog:
  - version: 1.5.0
    date: 2026-09-15
    summary: Added draft client-side input validation requirements, clarified Unicode message length and server limits
  - version: 1.4.0
    date: 2026-09-08
    summary: Disallowed sending attachments in separate envelopes
  - version: 1.3.0
    date: 2026-02-04
    summary: Broadened attachments from screenshots-only to any file type, added attachment examples to envelope
  - version: 1.2.0
    date: 2025-06-30
    summary: Added manual API replay sampling timing requirement
  - version: 1.1.0
    date: 2025-06-19
    summary: Added replay sampling timing requirement for feedback widgets
  - version: 1.0.0
    date: 2025-04-15
    summary: Initial spec — feedback protocol, envelope item constraints, context attributes, SDK pipeline, session replay integration, platform-specific guidance (onCrashedLastRun, user-facing/backend platforms)
---

<SpecRfcAlert />

<SpecMeta />

## Overview

User feedback allows Sentry to collect qualitative input directly from users — free-form comments describing what happened, optionally accompanied by contact information, linked errors, replay recordings, and file attachments. An SDK sends feedback as a `feedback` envelope item containing an event payload with a `feedback` context object.

Related specs:

- [Envelopes](/sdk/foundations/envelopes/) — transport format
- [Event Payloads](/sdk/foundations/envelopes/event-payloads/) — base event attributes
- [User Report (Deprecated)](/sdk/telemetry/user-reports/) — deprecated predecessor

---

## Concepts

### Feedback Events

A **feedback event** is an [event payload](/sdk/foundations/envelopes/event-payloads/) with a `feedback` context object. The feedback context carries user-provided data (message, contact info, URLs), while the event payload carries standard attributes (timestamp, event_id, platform, user, tags, etc.).

### Attachments

Files of any type (screenshots, logs, documents) can accompany a feedback event by sending them as [attachment items](/sdk/telemetry/attachments/) in the same envelope. Sentry uses the shared `event_id` to associate attachments with the feedback.

---

## Behavior

<SpecSection id="feedback-pipeline" status="stable" since="1.0.0">

### SDK Pipeline

A feedback captured by `capture_feedback` is processed through the SDK pipeline. The feedback event can be discarded at any stage, at which point no further processing happens.

SDKs **SHOULD** implement a `beforeSendFeedback` callback to allow users to modify or discard the feedback before it is sent. This callback **SHOULD** be similar to the existing [`beforeSend`](/sdk/foundations/client/hooks/#before-send-hook) callback used for error events. Feedback events **MUST NOT** go through the `beforeSend` hook.

SDKs **MUST** apply scope data to the feedback event, including tags, user, trace, and contexts. The scope's event processors **MUST** also be invoked.

SDKs **MUST** apply [rate limiting](/sdk/foundations/transport/rate-limiting/) to feedbacks in the same way as it is applied to events.

There is no sample rate for feedbacks — they are always sent. However, a feedback can be discarded at any pipeline stage (rate limiting, invalid message, etc.). SDKs **SHOULD** report dropped feedbacks through [client reports](/sdk/telemetry/client-reports/).

</SpecSection>

<SpecSection id="input-validation" status="draft" since="1.5.0">

### Client-Side Input Validation

Feedback can pass transport checks but still be discarded during ingestion. SDKs **MAY** validate user input locally so users can correct their feedback before submitting it.

#### Payload Validation

For SDKs that implement local payload validation, the following requirements apply to SDK-provided feedback forms and the manual capture API:

- `message` **MUST** be a string containing at least one non-whitespace character. Missing, empty, and whitespace-only messages **MUST NOT** be sent.
- `message` **MUST NOT** exceed **4,096 Unicode code points**. Sentry validates Python string length with `len(message)`, which counts Unicode code points. SDKs **MUST** match this count for the value that will be sent, including any whitespace it retains. For well-formed Unicode, counting Unicode scalars produces the same result. Counting UTF-8 bytes, UTF-16 code units, or user-perceived characters (grapheme clusters) does not.
- If an `associated_event_id` is supplied, the SDK **MUST** validate that it is a UUID before sending it, unless the SDK's identifier type already guarantees this.
- SDKs **MUST** validate the final feedback payload after event processors and `beforeSendFeedback` have run, before enqueueing it for transport. Feedback that fails validation **MUST** be discarded, using the SDK's existing failure reporting conventions and the [client report guidance](#sdk-pipeline).

For example, precomposed `é` (`\u00E9`) and decomposed `é` (`e\u0301`) both represent the visible character **é**. The precomposed form counts as one code point, while the decomposed form counts as two: `e` followed by a combining acute accent. A character outside the Basic Multilingual Plane counts as one code point, even though UTF-16 represents it with two code units. Swift implementations can use `unicodeScalars.count`, Java can use `codePointCount`, and JavaScript can iterate over the string. Native string-length APIs and HTML `maxlength` alone do not consistently match this limit.

#### Feedback Forms

Forms **MAY** check the message before calling the capture API. The following requirements apply to forms that implement this validation.

When validation fails, the form **MUST** prevent submission, preserve the user's input, and display an accessible error explaining how to correct it.

Forms **MUST** allow users to type or paste beyond the message limit rather than block further input or truncate the text. This preserves the full message so users can choose what to shorten.

Forms **MUST** enforce configured required name and email fields. Otherwise, name and email remain optional, and anonymous feedback **MUST** be accepted.

Forms **SHOULD** update validation as the user edits the input. To avoid visual clutter before the message approaches the limit, forms **MUST** hide the length counter below 90% of the maximum length. At **3,687 code points** (`ceil(0.9 × 4096)`) and above, forms **MUST** display a live label in the format `X / 4096`, where `X` is the current code-point count used for validation.

The label **MUST** be accessible to assistive technology, remain visible when the message exceeds the limit, and hide again if the count drops below 3,687. The same visibility rules **MUST** apply to typed, pasted, and prefilled input.

Forms **MUST** handle the message limit as follows:

- **At or below 4,096 code points:** Use neutral styling for the visible counter. Exactly 4,096 code points is valid, and submission **MUST** be allowed if all other fields are valid.
- **Above 4,096 code points:** Preserve all input, use error styling for the counter, and show an accessible error such as "Shorten your message to 4096 characters or fewer." Submission **MUST** remain blocked. If the user attempts to submit, the form **SHOULD** focus the message field.
- **After correction:** Clear the length error, restore the counter's neutral styling or hide it according to the 90% threshold, and allow submission if all other fields are valid.

Forms **SHOULD** defer length-error updates until an active input-method composition finishes, without interrupting typing. Assistive technology **SHOULD** announce changes in the validation state rather than every counter update.

#### Server Limits

These limits distinguish the modern `feedback` item from the deprecated `user_report` item:

- **Sentry:** [Feedback ingestion](https://github.com/getsentry/sentry/blob/102b2390de99b983d6847bf8bc1864fee45559e3/src/sentry/feedback/usecases/ingest/create_feedback.py#L180-L214) rejects missing or whitespace-only messages, messages above the configured limit, and malformed associated event IDs. The [`feedback.message.max-size` option](https://github.com/getsentry/sentry/blob/102b2390de99b983d6847bf8bc1864fee45559e3/src/sentry/options/defaults.py#L593-L598) defaults to 4,096 code points. This is a rejection limit, not automatic truncation.
- **Relay:** The [feedback context](https://github.com/getsentry/relay/blob/800a2424667f6d4caa385e15d77e628570dcac66/relay-event-schema/src/protocol/contexts/mod.rs#L88-L90) has a shared `max_bytes = 8192` normalization budget. Its [schema](https://github.com/getsentry/relay/blob/800a2424667f6d4caa385e15d77e628570dcac66/relay-event-schema/src/protocol/contexts/user_report_v2.rs#L11-L20) does not declare separate message, name, email, or URL length limits. Relay's [trimming processor](https://github.com/getsentry/relay/blob/800a2424667f6d4caa385e15d77e628570dcac66/relay-event-normalization/src/trimming.rs) can trim or remove context data. This shared normalization budget is not an 8,192-character allowance for each field and does not replace Sentry's message validation. Feedback also remains subject to [envelope size limits](/sdk/foundations/envelopes/#size-limits).
- **Legacy user reports:** The [128-character name and 75-character email limits](https://github.com/getsentry/sentry/blob/102b2390de99b983d6847bf8bc1864fee45559e3/src/sentry/models/userreport.py#L16-L20) belong to `user_report`. They are not per-field limits for `contexts.feedback` and **MUST NOT** be imposed on modern feedback solely to match the legacy endpoint.

</SpecSection>

<SpecSection id="session-replay-flush" status="stable" since="1.0.0">

### Session Replay Integration

When sending feedback, SDKs **SHOULD** flush the current Session Replay, if running. This ensures a meaningful replay recording exists and can be included in the `replay_id` attribute of the feedback context.

</SpecSection>

<SpecSection id="replay-widget-sampling-timing" status="stable" since="1.1.0">

### Replay Sampling Timing for Feedback Widgets

For feedback widgets specifically, when Session Replay's `onErrorSampleRate` is greater than 0, SDKs **MUST** sample and flush the replay when the widget **opens**, not when the user submits the feedback.

This timing is critical because:

- The replay buffer captures the user's session leading up to opening the feedback widget, providing context about what the user experienced before deciding to provide feedback.
- If sampling only occurs on submission, the buffer would primarily contain the user's interactions with the feedback form itself, which provides minimal debugging value.
- Early sampling ensures the complete user context is preserved regardless of whether the user ultimately submits the feedback.

</SpecSection>

<SpecSection id="replay-manual-api-sampling-timing" status="stable" since="1.2.0">

### Replay Sampling Timing for Manual API

When feedback is captured using the manual API (`capture_feedback`), SDKs **MUST** sample and flush the replay during the execution of the feedback capture API to ensure session context is preserved.

</SpecSection>

<SpecSection id="user-facing-platforms" status="stable" since="1.0.0">

### User-Facing Platforms

On user-facing platforms such as mobile, desktop, or browser, SDKs **SHOULD** provide first-class support for requesting user feedback when an error or crash occurs. See the user-facing docs for [Apple](https://docs.sentry.io/platforms/apple/enriching-events/user-feedback/) and [Java](https://docs.sentry.io/platforms/java/enriching-events/user-feedback/) for API examples.

On mobile and desktop, it is common to prompt the user for feedback after a crash happened on the previous run of the application. SDKs **SHOULD** implement the `onCrashedLastRun` callback on the options. This callback gets called shortly after the initialization of the SDK when the last program execution terminated with a crash. The SDK **SHOULD** execute the callback only once during the entire run of the program to avoid multiple callbacks if there are multiple crash events to send.

</SpecSection>

<SpecSection id="backend-platforms" status="stable" since="1.0.0">

### Backend Platforms

On backend platforms, SDKs **SHOULD** document how to use the [last event ID](/sdk/foundations/state-management/scopes/#event-id-retrieval) to prompt the user for feedback themselves.

</SpecSection>

---

## Wire Format

<SpecSection id="feedback-payload" status="stable" since="1.0.0">

### Feedback Payload

A feedback is sent as a `feedback` envelope item containing a JSON object. The object is an [event payload](/sdk/foundations/envelopes/event-payloads/) with an additional `feedback` context object.

#### Envelope Item Constraints

- This Item **MAY** occur at most once per Envelope.
- This Item is mutually exclusive with `"transaction"` items.

#### Envelope Headers

`event_id`

: **UUID String, required.** Corresponds to the `event_id` field of the event payload. **It is not equal to the `associated_event_id`** field in the feedback context. Clients are required to generate an event identifier ahead of time and set it at least in the Envelope headers. If the identifier mismatches between the Envelope and payload, the Envelope header takes precedence.

#### Additional Item Headers

_None_

#### Event Attributes

Below is a recap of the [required attributes](/sdk/foundations/envelopes/event-payloads/#required-attributes) for the event payload. For the full list of attributes, see [Event Payloads](/sdk/foundations/envelopes/event-payloads/).

| Field       | Type   | Required     | Since | Description                                      |
| ----------- | ------ | ------------ | ----- | ------------------------------------------------ |
| `event_id`  | String | **REQUIRED** | 1.0.0 | Hexadecimal string representing a UUID v4 value. |
| `timestamp` | Number | **REQUIRED** | 1.0.0 | UNIX timestamp of the current time (in seconds). |
| `platform`  | String | **REQUIRED** | 1.0.0 | Platform of the SDK.                             |

#### Feedback Context Attributes

The `contexts.feedback` object carries user-provided feedback data:

| Field                 | Type   | Required     | Since | Description                                                                                                                                                                           |
| --------------------- | ------ | ------------ | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `message`             | String | **REQUIRED** | 1.0.0 | Comments of the user, describing what happened and/or sharing feedback. Max length: **4,096 Unicode code points**. See [Client-Side Input Validation](#client-side-input-validation). |
| `contact_email`       | String | **OPTIONAL** | 1.0.0 | Email of the user who submitted the feedback. If excluded, Sentry (not the SDK) attempts to fill this in from user context. Anonymous feedbacks (no name or email) are accepted.      |
| `name`                | String | **OPTIONAL** | 1.0.0 | Name of the user who submitted the feedback. If excluded, Sentry (not the SDK) attempts to fill this in from user context.                                                            |
| `url`                 | String | **OPTIONAL** | 1.0.0 | URL of the webpage the user was on when submitting feedback. May be used for search or alerts.                                                                                        |
| `associated_event_id` | String | **OPTIONAL** | 1.0.0 | Identifier of an error event in the same project. Links a related error in the feedback UI.                                                                                           |
| `replay_id`           | String | **OPTIONAL** | 1.0.0 | Identifier of a related Session Replay in the same project. Sentry uses this to render a Replay clip in the feedback UI.                                                              |

</SpecSection>

<SpecSection id="file-attachments" status="stable" since="1.0.0">

### File Attachments

SDKs **MAY** attach files of any type to a feedback (screenshots, logs, documents, etc.) by sending them as [attachment items](/sdk/telemetry/attachments/), with `event_id` corresponding to the feedback item. SDKs **MUST** send attachment items in the same envelope as the feedback item. (since 1.3.0) Attachments are not limited to screenshots — any file type is supported.

</SpecSection>

---

## Public API

<SpecSection id="capture-feedback" status="stable" since="1.0.0">

### Capture Feedback

SDKs **MUST** expose a top-level function to send a feedback:

```
captureFeedback(feedback) -> eventId
```

**Parameters:**

- `feedback` — Object containing at minimum `message`. Optionally includes `contact_email`, `name`, `url`, `associated_event_id`, and `replay_id`.

**Returns:** The `eventId` (string).

Naming **SHOULD** follow the SDK's language conventions:

- `captureFeedback` (JavaScript, Java)
- `capture_feedback` (Python, Ruby)
- `CaptureFeedback` (Go, .NET)

</SpecSection>

---

## Examples

### Feedback Payload

```json
{
  "event_id": "9ec79c33ec9942ab8353589fcb2e04dc",
  "timestamp": "2011-05-02T17:41:36Z",
  "platform": "javascript",
  "contexts": {
    "feedback": {
      "contact_email": "john@example.com",
      "name": "John Smith",
      "message": "I love session replay!",
      "url": "https://sentry.io/replays/",
      "associated_event_id": "32fd1995636d446385016e2747623e11",
      "replay_id": "82840977e85b4ed3bc27f7b5b25cec15"
    }
  }
}
```

### Full Envelope with Attachments

```json
{"event_id":"9ec79c33ec9942ab8353589fcb2e04dc","sent_at":"2024-03-19T15:18:27.581Z","sdk":{"name":"sentry.javascript.react","version":"7.105.0"}}
{"type":"feedback"}
{
  "event_id": "9ec79c33ec9942ab8353589fcb2e04dc",
  "timestamp": "2011-05-02T17:41:36Z",
  "platform": "javascript",
  "contexts": {
    "organization": { "id": "0", "slug": "sentry" },
    "feedback": {
      "contact_email": "john@example.com",
      "name": "John Smith",
      "message": "I love session replay!",
      "url": "https://sentry.io/replays/",
      "associated_event_id": "32fd1995636d446385016e2747623e11",
      "replay_id": "82840977e85b4ed3bc27f7b5b25cec15"
    }
  },
  "level": "error",
  "environment": "prod",
  "release": "frontend@f00",
  "sdk": {
    "integrations": [
      "BrowserTracing",
      "BrowserProfiling",
      "Replay",
      "ReplayCanvas"
    ],
    "name": "sentry.javascript.react",
    "version": "7.105.0"
  },
  "tags": {
    "sentry_version": "24.4.0.dev0"
  },
  "user": {
    "ip_address": "127.0.0.1",
    "email": "john@example.com",
    "id": 1,
    "name": "John Smith"
  }
}
{"type":"attachment","length":1234,"filename":"screenshot.png"}
<binary screenshot data>
{"type":"attachment","length":567,"filename":"debug-logs.txt"}
<text file content>
```

### Example Implementations

User Feedback class:

- [Objective-C](https://github.com/getsentry/sentry-cocoa/blob/9eedc425727f0daccca1bd2be6021f6c3d9c654c/Sources/Sentry/SentryUserFeedback.m)
- [Java](https://github.com/getsentry/sentry-java/blob/671f9e0b8b709ef18e8b0788c31df44f99d09d35/sentry/src/main/java/io/sentry/UserFeedback.java#L7)
- [C#](https://github.com/getsentry/sentry-dotnet/blob/6ddf8363170bffee74a8c3cbf44335797d39ca9c/src/Sentry/Protocol/UserFeedback.cs#L10)

Envelope item:

- [Objective-C](https://github.com/getsentry/sentry-cocoa/blob/fd3e46efe59324e894d5601a7b3028824a588dda/Sources/Sentry/SentryEnvelope.m#L145-L162)
- [Java](https://github.com/getsentry/sentry-java/blob/b2e8dd43ea2ef37201b215ec409655e49e3231c8/sentry/src/main/java/io/sentry/SentryEnvelopeItem.java#L131-L155)
- [C#](https://github.com/getsentry/sentry-dotnet/blob/3bc6c66a14e653982a6a25df033d68d8ad12c4af/src/Sentry/Protocol/Envelopes/EnvelopeItem.cs#L170-L178)

---

## Changelog

<SpecChangelog />
