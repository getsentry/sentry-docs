...
Files of any type (screenshots, logs, documents) can accompany a feedback event by sending them as [attachment items](/sdk/foundations/transport/envelope-items/#attachment) in the same envelope. Sentry uses the shared `event_id` to associate attachments with the feedback.
...
SDKs **MAY** attach files of any type to a feedback (screenshots, logs, documents, etc.) by sending them as [attachment items](/sdk/foundations/transport/envelope-items/#attachment), with `event_id` corresponding to the feedback item. SDKs **SHOULD** send attachment items in the same envelope as the feedback item. (since 1.3.0) Attachments are not limited to screenshots — any file type is supported.
