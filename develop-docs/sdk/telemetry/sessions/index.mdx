...
count along with session updates. Events that stem from `captureException` or
`captureError` must increment the errors counter and affect the session's error
status. Additionally, `captureEvent` can also increment the errors counter when
used to manually create exception events. Events stemming from `captureMessage`
must not affect the session's error count, regardless of their log level. A
session that is `ok` and has an error count of greater than zero is considered
an errored session. All crashed and abnormal sessions are also at all times
considered errored but subtracted from the final errored session count.
