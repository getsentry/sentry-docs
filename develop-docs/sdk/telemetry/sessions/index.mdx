...
count along with session updates. Only events that stem from `captureException` or
`captureError` must increment the errors counter and affect the session's error
status. Events stemming from `captureEvent` or `captureMessage` must not affect
the session's error count, regardless of their log level. A session that is `ok`
and has an error count of greater than zero is considered an errored session.
All crashed and abnormal sessions are also at all times considered errored but
subtracted from the final errored session count.
