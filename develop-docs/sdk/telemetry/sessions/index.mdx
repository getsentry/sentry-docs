...
count along with session updates. Only events with a level of `error` or higher
(such as `fatal`) must increment the errors counter. Events with a level of
`warning` or lower (such as `info` or `debug`) must not affect the session's
error count. A session that is `ok` and has an error count of greater than zero
is considered an errored session. All crashed and abnormal sessions are also at
all times considered errored but subtracted from the final errored session count.
