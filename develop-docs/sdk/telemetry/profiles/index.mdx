...
A V1 profile **MUST** be serialized as JSON and sent in the same [envelope](/sdk/foundations/transport/envelopes/) as the associated transaction with item type `profile`. An envelope **MUST** contain at most one `profile` item.
...
A V2 profile chunk **MUST** be serialized as JSON and sent in an [envelope](/sdk/foundations/transport/envelopes/) with item type `profile_chunk`.
