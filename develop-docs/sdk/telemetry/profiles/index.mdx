...
spec_changelog:
  - version: 2.5.1
    date: 2026-07-22
    summary: Clarified that an envelope must contain at most one profile_chunk item
  - version: 2.5.0
    date: 2026-02-20
...
- [Envelope Items](/sdk/foundations/envelopes/envelope-items/) — `profile` and `profile_chunk` item type constraints
- [Frame Attributes](/sdk/foundations/transport/event-payloads/stacktrace/#frame-attributes) — frame object format
...
| `debug_meta` | Object | **REQUIRED** (native) | 1.0.0 | Debug metadata for symbolication. Required on native platforms (`cocoa`, `rust`). Same payload as the [`debug_meta` interface](/sdk/foundations/transport/event-payloads/sdk/). |
...
| `debug_meta` | Object | **REQUIRED** (native) | 2.0.0 | Debug metadata for symbolication. Required on native platforms. Same payload as the [`debug_meta` interface](/sdk/foundations/transport/event-payloads/sdk/). |
...
| `frames` | List | **REQUIRED** | 1.0.0 | List of [frame objects](/sdk/foundations/transport/event-payloads/stacktrace/#frame-attributes). (since 1.1.0) Each frame **MUST** contain at least one of `filename`, `function`, or `instruction_addr`. For native platforms, `instruction_addr` is **REQUIRED** for symbolication. `module` or `package` is used for frame grouping. |
...
A V1 profile **MUST** be serialized as JSON and sent in the same [envelope](/sdk/foundations/envelopes/) as the associated transaction with item type `profile`. An envelope **MUST** contain at most one `profile` item.
...
A V2 profile chunk **MUST** be serialized as JSON and sent in an [envelope](/sdk/foundations/envelopes/) with item type `profile_chunk`. An envelope **MUST** contain at most one `profile_chunk` item.
