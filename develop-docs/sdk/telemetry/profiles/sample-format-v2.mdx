...
There are some additional contexts we need to attach to the *transactions/spans* payloads for continuous profiling to be connected to tracing.
...
The span data for a span needs to be updated to contain the active *thread id* so we know which thread is associated with the transaction.
...
```json
{
  "thread.id": "thread id", // this should be a string that matches the thread id in the thread_metadata of the profile chunk
	"thread.name": "thread name", // optional
	"profiler_id": "42928c7ee9174231956f077581145489" // optional to override the one from the trace context but should not be necessary
}
```
