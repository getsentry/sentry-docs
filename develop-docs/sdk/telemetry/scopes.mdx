...
Attributes are key-value pairs where each value is either a primitive value or an object containing:
...
- Values are attribute objects with `value`, and optionally `unit` properties. 
- For convenience, the SDK MAY also accept primitive values directly (see exampe), i.e. create a unit-less attribute.
