...
spec_changelog:
  - version: 2.3.0
    date: 2026-09-22
    summary: Added sentry.is_localhost default attribute for localhost inbound filtering
  - version: 2.2.0
    date: 2026-06-22
...
| `sentry.is_localhost` | 2.3.0 | Whether the SDK determined the application is running on localhost. Used to enable the localhost inbound filter. The definition of "localhost" is platform-specific. Follow [Relay's detection heuristics](https://github.com/getsentry/relay/blob/master/relay-filter/src/localhost.rs) if applicable |
...
| Attribute | Description |
|-----------|-------------|
| `server.address` | The server address. Equivalent to [`server_name`](/sdk/foundations/envelopes/event-payloads/#optional-attributes) on errors and transactions. |
...
The `log` envelope item **SHOULD** include `version` and `ingest_settings` properties to instruct Relay to infer IP address and user agent from the incoming request. See the [Ingest Settings](/sdk/foundations/envelopes/envelope-items/#ingest-settings) spec for the full definition, field reference, and behavior.
