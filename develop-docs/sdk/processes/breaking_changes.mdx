...
- **Issue detection:** Changing how or if certain spans are emitted can influence server-side issues (for example Performance issues like N+1 queries).

Changes to the above need closer inspection. In order to make a good decision, you can try to estimate how many users are affected using our data analytics tools. In the end, it always needs a human/team to weigh in.
