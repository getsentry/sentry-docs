...
An <Link to="/sdk/event-payloads/overview">event</Link> may contain one or more breadcrumbs in an attribute named `breadcrumbs`.
...
The following example illustrates the breadcrumbs part of the <Link to="/sdk/event-payloads/overview">event
payload</Link> and omits other attributes for simplicity.
