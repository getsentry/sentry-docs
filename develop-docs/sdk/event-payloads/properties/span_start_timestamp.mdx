...
: _Required_. A timestamp representing when the measuring started. The
  ...
  epoch](https://en.wikipedia.org/wiki/Unix_time). The `start_timestamp` value must be greater or
  equal the `timestamp` value, otherwise the Span is discarded as invalid.
