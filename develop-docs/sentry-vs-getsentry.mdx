...
Getsentry's settings are configured in `getsentry/conf/settings/defaults.py`.
Additional environment specific overrides are found in files matching the
environment name. For example, local development is found in `getsentry/conf/settings/dev.py`.
