...
- _Don't_ use an `index.tsx?$` file if the folder contains components used in other parts of the app regardless of the entrypoint file. (ie, actionCreators, panels)
- _Don't_ use an index file just to re-export. Prefer importing individual components instead.
...
<Alert level="info">
  For React Hooks tips check out{" "}
  <Link to="/frontend/using-hooks/">this page</Link>.
</Alert>
...
<Alert level="info">
  For React Testing Library (RTL) tips check out{" "}
  <Link to="/frontend/using-rtl/">this page</Link>.
</Alert>
