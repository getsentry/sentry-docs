...
```bash
pnpm update --latest $(pnpm outdated | cut -d' ' -f1 | grep [group-name])
```
...
6. **Read the `pnpm-lock.yaml` diff**
   ...
   Generally you will want to make sure nothing has duplicated versions, in that case you may need to use `pnpm dedupe` or in the worst case you may need to upgrade the package which is pulling in the offending older versions.
