...
function ProjectList({organization}) {
...
URL patterns use `$param` placeholders (e.g. `$organizationIdOrSlug`, `$projectIdOrSlug`) which TypeScript extracts to enforce a matching `path` object. If the URL has parameters, you must supply all of them - or pass `skipToken` to disable the query.
...
function useUpdateProjectNameOptimistic({orgSlug}) {
  ...
  return useMutation({
    ...
    onMutate: async variables => {
      ...
      queryClient.setQueryData(queryClient, projectQueryKey, oldData => {
