...
`NEW_FEEDBACK_ENVELOPE`: [The new format](/sdk/telemetry/feedbacks/) created by the Replay team when adding
...
`USER_REPORT_ENVELOPE`: [The older format](/sdk/telemetry/user-reports/) with name/email/comments, that requires
...
saved via the [issues platform](/backend/issue-platform/).
...
The preferred way of sending feedback from the SDK is in [feedback envelope](/sdk/telemetry/feedbacks/).
...
Attachments are another [item type](/sdk/foundations/envelopes/#attachment)
...
User reports are also sent to Relay in envelope format, item type [user_report](/sdk/telemetry/user-reports/).
