...
- **Spans v2**: The DSC is reconstructed, but minimally so; the reconstructed DSC only contains public key (of the spans' project) and trace id. Span attributes like `release`, `environment`, and `segment.name` are not used because there is no guarantee that an envelope's v2 spans all share the same value for these attributes.
