...
Each of these metrics can be filtered and grouped by tags [configured for metric extraction in Relay](https://github.com/getsentry/relay/blob/master/relay-dynamic-config/src/metrics.rs#L323-L367).
...
When you go into Trace Explorer or Errors, you might want to now split the data by a custom tag you’ve added to your events. This granularity is not offered by metrics, so **these queries need to use stored events**.

</Alert>

If you want to learn more about Dynamic Sampling, continue to the [next page](/application-architecture/dynamic-sampling/biases/).
