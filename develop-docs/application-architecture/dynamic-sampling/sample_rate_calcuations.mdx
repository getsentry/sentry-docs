...
```
boost_latest_releases_multiplier = boost_factor / (boost_factor ** base_sample_rate)
```

The dynamic factor function makes the multiplier smaller as the base sample rate gets larger. This prevents the boost from oversampling too aggressively when the project is already sampling a large share of its traffic.

Sentry only emits these rules for releases that are currently in the boosted release set for a project. The rule matches on `trace.release` and `trace.environment`, starts when the release was first boosted, and decays linearly back to `1.0` over the platform-specific time-to-adoption window.
