...
Because release adoption varies over time, a system of _decaying_ rules interpolates between two sample rates within a specified time window and interpolation function (e.g. `linear`). This approach gradually lowers the sample rate as user adoption increases and the volume of samples grows.
