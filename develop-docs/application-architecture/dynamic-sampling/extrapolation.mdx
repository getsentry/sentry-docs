...
1. When the sample rate and the sample size are low, the extrapolation will be inaccurate and should not be used to draw conclusions. In this case should educate users towards that they can increase the value they can get out of the product by increasing the sample rate. We should also inform users that if they want to investigate individual events, they can switch to sample mode.
