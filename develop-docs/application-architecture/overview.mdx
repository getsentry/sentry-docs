...
Each region silo can be scaled independently, and is isolated from other regions. Within each region exists separate, dedicated infrastructure and applications as outlined in the [application overview](/application-architecture/#high-level-overview).

### Silo Configuration

Silo modes are defined as environment variables (and django settings). The relevant settings are:

- `SILO_MODE` either `CONTROL`, `REGION` , `MONOLITH` or None. Use `SENTRY_SILO_MODE` environment variable to define this setting.
- `SENTRY_LOCAL_CELL` The name of the region the application is in (eg. `us`, `us2` or `de`) or `None`. Defined by `SENTRY_REGION` environment variable.
- `SENTRY_LOCALITIES` A list of localities/region that the application has. A locality is composed of multiple cells.
- `SENTRY_CELLS` A list of cells that the application has. This list defines names, URLs, for each cell/region.
- `SENTRY_FALLBACK_CELL` The region name of the former ‘monolith’ region. For [sentry.io](http://sentry.io) this is the `us` region. The fallback cell has special behavior that is required for backwards compatibility.
