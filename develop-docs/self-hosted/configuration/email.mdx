...
Sentry only supports SMTP for sending outbound email notifications. It does not support other protocols or APIs for sending email. However, Sentry's built-in SMTP server powered by [egos-tech/smtp](https://gitlab.com/egos-tech/smtp) is sufficient enough to handle multiple use-cases. For more details on other use-cases, please refer to their documentation.

Without a working email backend, Sentry can't send verification emails. This means:
- Users can't verify their primary email address
- Users can't add secondary or backup emails
- Password reset flows won't work

Email verification confirms that a user owns the address on their account, which protects against unauthorized access. Even if you don't need alert notifications, we recommend enabling email for account security.
