...
4. **kafka-ui showing "no end offset."** kafka-ui sometimes can't render an
   ...
   not a consumer problem. `docker compose exec kafka kafka-consumer-groups --bootstrap-server kafka:9092 --describe` will show
...
- `docker compose exec kafka kafka-consumer-groups --bootstrap-server kafka:9092 --describe --group snuba-events-subscription-executor`
