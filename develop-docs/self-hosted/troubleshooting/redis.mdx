...
If the results show that the majority of keys are from `e`, it means the event-processing backlog is causing the issue. In this case, you can try increasing the memory limit for Redis, or reducing the number of ingested events by applying server-side rate limiting or decreasing the client-side sample rate. These keys are supposedly transient and have a TTL, so they should be automatically removed after a while.
