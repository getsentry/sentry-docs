...
title: "Event Detail"
sidebar_order: 20
description: "Learn more about how to drill down into a span of a single transaction to debug performance issues, identify associated errors, and root out bottlenecks."
---

From [Performance](/product/performance/) and [Discover](/product/explore/discover-queries/), you can drill all the way down into a span for a single transaction and traverse multiple directions. This will accelerate your ability to debug slow HTTP requests, database queries, identify associated errors, and root out other bottlenecks.

![Event detail showing critical event information, breakdowns, minimaps and waterfall view](./img/transaction-detail.png)

You can get to an event detail from either a [Transaction Summary](/product/performance/transaction-summary/) or [Query Builder](/product/explore/discover-queries/query-builder/). In [Discover](/product/explore/discover-queries/), performing a search with the `event.type:transaction` or going into a pre-built query such as "Transactions by Volume" will generate a list of event IDs. Event IDs will be linked to open the corresponding **Event Detail** page.

<Alert>
  Currently, only root transactions are searchable. Any span data that inherits
  from its root are not.
</Alert>
...
![Trace navigator showing related events](./img/trace-navigator.png)
...
![Span detail view shows the span id, trace id, parent span id, and other data such as tags.](./img/span-detail.png)
...
![Operations breakdown in the event detail.](./img/operations-breakdown.png)
...
![Operations calculation method.](./img/operations-calc.png)
...
![Adding or removing child transactions.](./img/addremove-transaction.png)
