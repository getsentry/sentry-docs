...
If you're building with AI agents, LLMs, or MCP servers, Sentry's [Agent Tracing](/product/ai-observability/) extends your existing tracing setup to capture the full picture — model calls, tool executions, token usage, and agent handoffs — all connected to the same traces as your HTTP requests, database queries, and application errors. Learn more about [setting up Agent Tracing](/product/ai-observability/getting-started/).
