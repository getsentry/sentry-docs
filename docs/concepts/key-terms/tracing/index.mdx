...
sidebar_order: 70
description: >-
  Learn more about the tracing features Sentry offers to help you track your
  software performance across multiple systems.
...
Transactions are a legacy concept replaced by service spans, which are entry-point spans that mark a service boundary within a trace. New SDKs and features use spans exclusively. See [Trace Explorer](/product/trace-explorer/) for the recommended way to query performance data.
...
### What's a Service Span?

A service span is the entry point of a service within a trace — the span-based replacement for a transaction. Like a transaction, a service span has a name, a status, and child spans that capture the operation's full lifecycle from start to finish.

A trace can contain multiple service spans, typically one for each service involved. The first service span in a trace (the one with no parent) is also the trace's root span.
...
Depending on the size and volume of your app, you may have a lot of traces to sort through. The [Sentry Dashboards](https://sentry.io/orgredirect/organizations/:orgslug/dashboards/) page in Sentry is a good place to get a high-level, aggregated view of your application's performance and focus in on the traces you care about. You'll see a list of transactions for specific pages, endpoints and parts of your application. Clicking on the transaction ID will take you to the Trace View page for that transaction. Learn more about using [Sentry Dashboards](/product/dashboards/sentry-dashboards/).
