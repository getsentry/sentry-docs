...
sidebar_order: 20
description: >-
  With distributed tracing, you can track software performance and measure
  throughput & latency, while seeing the impact of errors across multiple
  systems.
...
Learn more in our [tracing documentation](/concepts/key-terms/tracing/).
...
Traces are defined as the collection of all transactions and spans that share a `trace_id` value.

#### Transactions

<Alert level="info">

Transactions are a legacy concept replaced by service spans, which are entry-point spans that mark a service boundary within a trace.
When working with Sentry's current tools, use [Trace Explorer](/product/trace-explorer/) and the [Spans](/product/dashboards/widget-builder/#spans) dataset.

</Alert>

Transactions and their parent spans share most properties. They also have a `transaction` property, which holds the transaction name used in the UI to identify the transaction. Common examples include endpoint paths for backend request transactions, task names for cron job transactions, and URLs for page-load transactions.
...
In the span-based model, scope-level tags and data are replaced by span attributes — refer to your SDK's migration guide for details.
...
- `parent_span`: ties the span to its parent span
...
- `status`: short code indicating the operation's status
- and many others, including custom set attributes

An example use of the `op` and `description` properties together is `op: db.query` and `description: SELECT * FROM users WHERE last_active < %s`. The `status` property indicates the success or failure of the span's operation, or a response code for HTTP requests.
...
- _Browser Page-load Transaction_: 7 spans
  - 1 span for the entire page load
...
Some spans in the browser transaction correspond to backend transactions. When a span in one service gives rise to a transaction in another, the original span is a parent span to both the transaction and its first span. In the diagram below, squiggly lines represent this parent-child relationship.

![Diagram illustrating the trace-transaction-span relationship applied to the example.](./img/diagram-transaction-example.png)

In our example, every transaction other than the initial browser page-load transaction is the child of a span in another service. In a fully-instrumented system, this pattern always holds true. The only parentless span is the root of the initial transaction; every other span has a parent. Parents and children live in the same service, except when the child span is the first span of a child transaction, in which case the parent span lives in the calling service and the child transaction/child span lives in the called service.
...
- _Backend HTML/CSS/JS Request Transactions_: 1 span each
  - 1 span for the entire request (child of a browser span)
...
- _Database Server Request Transaction_: 3 spans
  - 1 span for the entire request (child of the backend span above)
...
- Browser's full process (_T_ and span _S_)
  - XHR request to backend\* (_S_)
  - Rendering confirmation screen (_S_)
- Backend's processing of the request (_T_ and span _S_)
...
- Database's work updating the customer's order history (_T_ and span _S_)
  - Individual SQL queries (_S_)
- Queued task of sending the email (_T_ and span _S_)
  - Function call to populate email template (_S_)
  - API call to email-sending service (_S_)

\* Starred spans are parents of later transactions (and their first spans).
...
A trace begins when the earliest transaction starts, and ends when the last transaction ends. You can't "start" or "end" a trace, but you can initiate a transaction which will trigger a trace. Completing all transactions ends a trace.
...
In a fully instrumented system, each trace should have exactly one root span, which is the first span in the trace and has no parent. All other transactions and spans in the trace are connected to this root through distributed trace propagation.
However, you may not have tracing enabled in every service, or an instrumented service may fail to report a transaction. This can result in gaps in your trace hierarchy, with non-originating, parentless transactions called **orphan transactions**.
...
In Sentry's current span-based SDKs, individual spans are sent to Sentry in batches as they finish, without waiting for a parent transaction to close. The description below applies to the legacy transaction-based model.
