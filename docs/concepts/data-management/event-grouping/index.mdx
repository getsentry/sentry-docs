...
sidebar_order: 0
description: >-
  Learn about fingerprinting, Sentry's default error grouping algorithms, and
  other ways to customize how events are grouped into issues.
...
When Sentry's default fingerprinting algorithm generates a new hash, it performs the following steps:

1. It generates a vector representation of the error's message and in-app stack frames, or all frames when no in-app frames are present, using a transformer-based text embedding model
2. It compares this embedding against existing error embeddings for that project
