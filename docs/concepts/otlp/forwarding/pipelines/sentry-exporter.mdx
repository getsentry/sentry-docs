...
<Expandable title="Interactive Setup with AI Agent Skills" level="success">

If you use an AI agent (like Claude Code or Cursor), the `sentry-otel-exporter-setup` skill can guide you through configuring the Sentry Exporter interactively.

The skill walks you through:

- Choosing an installation method (binary or Docker)
- Configuring automatic project creation
- Generating a complete collector configuration
- Setting up environment variables for authentication

**Installation:**

```bash
npx skills add https://github.com/getsentry/agent-skills --skill sentry-otel-exporter-setup
```

See the [Sentry Agent Skills](https://github.com/getsentry/agent-skills) repo for other agents.
...
```
"Set up OTel Collector with Sentry"
```

Or:

```
"Configure OpenTelemetry for Sentry with multi-project routing"
```
