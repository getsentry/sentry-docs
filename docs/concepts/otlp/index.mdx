...
## Propagating traces from the Sentry SDK

If you use the Sentry SDK on your frontend or React Native project alongside a backend service instrumented with OpenTelemetry, you can connect your Sentry and OpenTelemetry traces using [the `propagateTraceparent` option](/platforms/javascript/configuration/options/#propagateTraceparent) when initializing the Sentry SDK. The trace ID started on your frontend or mobile app will be propagated to your backend app, associating all of your telemetry together into a single trace inside Sentry.
