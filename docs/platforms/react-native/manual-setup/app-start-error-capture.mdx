...
If you need different `environment` values per build (e.g., production vs staging), set the `SENTRY_ENVIRONMENT` environment variable at build time. The SDK build scripts will override the `environment` in your `sentry.options.json` without modifying the source file.
