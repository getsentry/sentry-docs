...
Recursively reads linked errors up to a specified limit using a lookup key on the captured Error object. This is the React Native-specific version that also processes linked errors from native layers (Java, Objective-C) and collects debug images.
