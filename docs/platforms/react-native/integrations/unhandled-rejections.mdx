...
**Important Note:** Enabling Hermes rejection tracking will overwrite existing hooks due to limitations of the engine. Only one promise rejection tracker can be active at a time.
...
Sentry.init({
  ...
  integrations: [
    Sentry.reactNativeErrorHandlersIntegration({
      patchGlobalPromise: false, // Disables Promise patching for JSC
    }),
  ],
});
```

**Note:** The Promise polyfill is used to ensure compatibility and provide rejection tracking functionality that may not be available natively in older JSC versions.
...
## Troubleshooting (Legacy behavior and JavaScriptCore)
