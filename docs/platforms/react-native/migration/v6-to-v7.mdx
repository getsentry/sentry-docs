...
The main goal of version 7 of the Sentry React Native SDK is to provide compatibility with Sentry JavaScript version 10. This update includes breaking changes due to the upgrade to JavaScript SDK v10, the removal of deprecated APIs, and a reorganization of the npm package structure.
...
Sentry JavaScript mainly focus on OpenTelemetry changes. If you don't have any integration with OpenTelemetry.
There will also be changes in regard of the use of `BaseClient`, `hasTracingEnabled`, `logger` from `@sentry/core`, .
Other general JavaScript SDK version 10 changes are described in the [JavaScript SDK 9.x to 10.x migration guide](/platforms/javascript/guides/react/migration/v9-to-v10/).
