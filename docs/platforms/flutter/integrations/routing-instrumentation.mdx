...
In order to report TTID, manually wrap the desired widget with `SentryDisplayWidget`, as shown below:
...
Since the approximation is enabled by default, you can combine both approaches by manually calculating TTID for certain Widgets that need more accurate measurements and leave the rest to be automatically approximated.
...
TTFD is disabled by default. In order to get TTFD measurements, follow these steps:
...
If the span finishes through the API, its status is set to `SpanStatus.OK`. 
