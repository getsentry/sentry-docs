...
- Auto-installation of the Sentry Cocoa SDK via CocoaPods (deprecated, only if the Kotlin CocoaPods plugin is applied).
...
sentryKmp {
  ...
  autoInstall {
    ...
    // Configure the Cocoapods plugin (deprecated).
    cocoapods {
    ...
    // The linker is skipped when the Kotlin CocoaPods plugin is applied.
    linker {
...
## How Auto-Install and Linking Interact

The plugin decides how to provide Sentry Cocoa based on whether the Kotlin CocoaPods plugin is applied to the module. The two paths don't combine: when the CocoaPods plugin is applied, linker configuration is skipped, even if you set `xcodeprojPath` or `frameworkPath`.

| Your setup | What the plugin does | Supported |
| --- | --- | --- |
| Sentry Cocoa added with Swift Package Manager, Kotlin CocoaPods plugin **not** applied | Links the Kotlin/Native framework against Sentry Cocoa, using `linker` settings to locate it | Yes (recommended) |
| Kotlin CocoaPods plugin applied, `autoInstall.cocoapods.enabled = true` | Adds the `Sentry` pod if you haven't declared one yourself. A pod you declared is left unchanged | Yes (deprecated) |
| Kotlin CocoaPods plugin applied, `autoInstall.cocoapods.enabled = false` | Does nothing for Apple targets. You must declare the `Sentry` pod yourself | Yes (deprecated) |
| Kotlin CocoaPods plugin applied **and** Sentry Cocoa added with Swift Package Manager | Skips linking, so the SPM-provided framework isn't linked | No |

A few additional rules apply:

- Sentry Cocoa installation and linking only run on a macOS host. On other hosts, only the `commonMain` dependency is installed.
- Linking is set up only for the Apple targets being compiled in the current build, so Android-only builds skip it.
- If you set both `frameworkPath` and `xcodeprojPath`, `frameworkPath` takes precedence. It must point to a `Sentry.xcframework` or `Sentry-Dynamic.xcframework` folder.
