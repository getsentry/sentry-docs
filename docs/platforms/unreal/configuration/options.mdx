...
The default engine-based detection is available on Windows and Linux only, and only works in packaged non-debug builds. See <PlatformLink to="/configuration/app-hangs/">App Hangs</PlatformLink> for setup details and prerequisites.
...
Detects hangs using the sentry-native SDK's built-in app-hang watchdog instead of Unreal Engine's `FThreadHeartBeat` watcher. It works with both the Crashpad and Native backends and doesn't depend on the engine's heartbeat configuration.

Only takes effect when <PlatformIdentifier name="EnableHangTracking" /> is enabled.

<Alert>

This feature is available on Windows, Linux, and macOS. On macOS it requires the <PlatformLink to="/configuration/native-backend/">native backend</PlatformLink>. See <PlatformLink to="/configuration/app-hangs/">App Hangs</PlatformLink> for details.
