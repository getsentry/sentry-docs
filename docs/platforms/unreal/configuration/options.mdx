...
This feature is available on Windows, Linux, macOS, and Android. On macOS it requires the <PlatformLink to="/configuration/native-backend/">native backend</PlatformLink>. On Android, it uses the NDK integration and monitors the game thread. See <PlatformLink to="/configuration/app-hangs/">App Hangs</PlatformLink> for details.
