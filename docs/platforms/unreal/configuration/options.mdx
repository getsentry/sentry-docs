...
The default engine-based detection is available on Windows and Linux only, and only works in packaged non-debug builds. See [App Hangs and ANRs](/platforms/unreal/app-hangs/) for setup details and prerequisites.
...
Enables mobile app-not-responding detection. On iOS and macOS, this configures Cocoa app hang detection. On Android, it configures JVM ANR detection.
...
Sets how long the main thread can remain unresponsive before the SDK captures an app-not-responding event. On Android 11 and later, Android reports ANRs through `ApplicationExitInfo`, so the operating system controls the threshold.
