...
The engine's heartbeat monitor thread must be enabled for hang tracking to work. Set `HangDuration` to a value greater than `0` in your project's `DefaultEngine.ini`:
...
`HangDuration` controls how long the engine waits (in seconds) before terminating a hung process. Setting it to a high value (e.g. `300`) ensures the engine doesn't kill the application before Sentry can capture the hang event. If `HangDuration` is `0` (the engine default), the heartbeat monitor thread won't run and hang tracking will be silently skipped.
