...
The SDK can detect hangs in one of two ways, selected with the <PlatformLink to="/configuration/app-hangs/#native-app-hang-detection">UseNativeHangTracking</PlatformLink> setting:
...
This mechanism requires additional engine configuration — see [Prerequisites](#prerequisites).
...
The hang event includes:
- Event level: `error`
- Exception type: `AppHang`
- Exception value: `App hung for at least <duration> ms.`
- Mechanism type: `AppHang`
- Stack trace of the hung thread

Unlike the engine watcher, this mechanism works with both the Crashpad and Native backends, doesn't depend on the engine's heartbeat configuration, and is also available on macOS (with the native backend).

## Prerequisites

<Alert>

This section applies to the default engine watcher only. The native watchdog (<PlatformIdentifier name="UseNativeHangTracking" />) doesn't require any engine configuration.

</Alert>

The engine's heartbeat monitor thread must be enabled for the engine watcher to work. By default, `HangDuration` in `[Core.System]` is set to `0`, which disables the heartbeat monitor. Set it to a value greater than `0` in your project's `DefaultEngine.ini`:
