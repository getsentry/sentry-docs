...
> **_NOTE:_** Currently on Windows we provide the option to automaticly compile Sentry Native from source when building the plugin. This can be useful if you require custom modifications such as specific dependency versions. To enable this feature you can either Run the [Unreal Automation Tool](https://dev.epicgames.com/documentation/en-us/unreal-engine/unreal-automation-tool-overview-for-unreal-engine) with `-buildNativeForWindows` in the arguments list, or alternativly set `bBuildNativeForWindows` to true in `<your_project_root>/Plugins/Sentry/Sentry.Build.cs`
