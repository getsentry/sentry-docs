...
On Windows and Linux, `EnableHangTracking` uses Unreal Engine's `FThreadHeartBeat` watcher. It requires packaged, non-debug builds and `HangDuration` greater than `0`:
...
The engine watcher waits until `HangTimeoutDuration` before capturing a stuck thread's stack trace. It uses Unreal Engine's existing heartbeat system plus a Sentry watchdog thread.
...
The native watchdog sends one game-thread heartbeat per frame and runs a background watchdog thread. It does not require Unreal Engine's heartbeat configuration. On macOS, it requires the [native backend](/platforms/unreal/configuration/native-backend/). On Android, it runs alongside JVM ANR detection, so a freeze that blocks both threads can create an ANR and an `AppHang` event.
...
| Option                           | Default   | Platforms                        |
| -------------------------------- | --------- | -------------------------------- |
| `EnableAppNotRespondingTracking` | Enabled   | Android, iOS, macOS              |
| `AppNotRespondingTimeout`        | 5 seconds | iOS, macOS, Android below API 30 |

On iOS and macOS, these options configure Cocoa app hang detection. On Android, they configure JVM ANR detection. Android 11 and later reports ANRs through `ApplicationExitInfo`, so the operating system controls the timeout.
