...
The easiest way to get started with Sentry is to use the Sentry [Lambda Layer](https://docs.aws.amazon.com/lambda/latest/dg/adding-layers.html) instead of adding `@sentry/serverless` with `npm` or `yarn` manually.
...
<LambdaLayerDetail canonical="aws-layer:node:v7" />
