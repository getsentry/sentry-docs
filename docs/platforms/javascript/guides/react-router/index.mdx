...
### Alternative: Top level import (Limited Instrumentation)

If you're unable to use the `NODE_OPTIONS` approach (for example, when deploying to **Vercel** or other hosting platforms that don't support Node.js CLI flags), you can import the instrumentation file directly at the top of your `entry.server.tsx`:
...
<Alert level='warning' title='Limitations of top level import'>

**This approach results in incomplete instrumentation.** When you import the instrumentation file directly in your entry server instead of using `NODE_OPTIONS`, Sentry is initialized after some modules have already been loaded by the ESM module system.

**What this means:**
- Only some native Node.js APIs (like `fetch` and the `http` module) will be instrumented
- You'll miss automatically captured spans and traces for some of your application's server-side operations

**Why this happens with ESM:**
In ECMAScript Modules (ESM), the module loading and initialization happen in a specific order. When Sentry is imported directly in your entry file, the ESM loader has already processed and cached many modules. Sentry's automatic instrumentation relies on intercepting module imports as they happen, but this can't work retroactively on already-loaded modules.

The `--import` flag solves this by loading Sentry's instrumentation hooks before **any** application modules are processed, ensuring comprehensive coverage.

**Recommendation:** Use this method only if the `--import` flag is not supported by your deployment platform.
