...
This installation method has the fundamental restriction that only native Node.js APIs can be instrumented (like for example `fetch` and the `http` module).
In other words, this means that the Sentry SDK will not capture data for database calls, queues, ORMs, libraries or other framework-specific data.
