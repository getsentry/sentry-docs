...
With this setup, Sentry captures errors thrown by your agents and AI spans for every run — agent runs, model generations, tool calls, workflows, token usage, and latency — along with outgoing HTTP and `fetch` requests. Prompts and model outputs are recorded on your AI spans by default, so the Agent Tracing transcript shows the full exchange without any extra configuration.
