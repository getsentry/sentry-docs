...
- **No AI spans at all.** Confirm `instrument(Sentry.createFlueInstrumentation())` actually runs. Importing `sentry-init.ts` at the top of `src/app.ts` runs it during app startup, before any agent takes a turn.
