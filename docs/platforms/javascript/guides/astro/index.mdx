...
Install the SDK using the `astro` CLI:
...
To keep your auth token secure, set the `SENTRY_AUTH_TOKEN` environment variable in your build environment or store it in an environment variable:
...
<script>
  ...
  async function throwApiError() {
    ...
    // throw an error if the API call's return is not `ok`
    if (!res.ok) {
      throw new Error("Sentry Example Frontend Error");
    }
  }
...
<button type="button" onclick="throwFrontendError()">
  Throw frontend error
</button>
<button type="button" onclick="throwApiError()">Throw API error</button>
```

Then also create the route we're calling in our test page, like `src/pages/api/test-error.(js|ts)`:

```javascript {filename: test-error.ts}
...
<button type="button" onclick="throwApiError()">Throw error with trace</button>
```
