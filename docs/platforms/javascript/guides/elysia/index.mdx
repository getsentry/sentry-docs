...
- One of these runtimes:
  - Bun
  - Node.js 18+ (with the [@elysiajs/node](https://elysiajs.com/integrations/node) adapter)
  - Deno (with `@sentry/deno` alongside `@sentry/elysia`)
  - Cloudflare Workers, see [Elysia on Cloudflare](/platforms/javascript/guides/cloudflare/frameworks/elysia/)
...
```bash {tabTitle:Deno}
npm install @sentry/elysia @sentry/deno elysia
```
...
```javascript {tabTitle:Deno} {filename: index.ts}
import * as Sentry from "@sentry/deno";
import { withElysia } from "@sentry/elysia";
import { Elysia } from "elysia";

// On Deno, initialize with `@sentry/deno` so that `Deno.serve` is
// instrumented. `@sentry/elysia`'s own `init()` targets Bun.
Sentry.init({
  dsn: "___PUBLIC_DSN___",
  // ___PRODUCT_OPTION_START___ performance
...
// withElysia returns the app instance, so you can chain routes directly
const app = withElysia(new Elysia()).get("/", () => "Hello World");
...
On Deno, set `"nodeModulesDir": "manual"` in your `deno.json` and install the packages with npm, so that Deno resolves them from a real `node_modules` directory.

If you prefer to keep `Sentry.init()` from `@sentry/elysia`, pass `integrations: [denoServeIntegration()]` from `@sentry/deno` instead. Without one of these two wirings there is no `http.server` span, every lifecycle phase becomes its own root transaction, and incoming trace headers are ignored.
