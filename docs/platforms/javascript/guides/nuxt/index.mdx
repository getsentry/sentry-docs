...
After building with `nuxi build`, run your project. The Sentry server config is bundled into the server build and initializes itself at startup, so no `--import` flag is needed.
