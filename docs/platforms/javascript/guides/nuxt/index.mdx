...
The SDK bundles `sentry.server.config.ts` into the Nitro server build and initializes Sentry when the server starts. To learn more, see <PlatformLink to="/install/">Server-Side Initialization</PlatformLink>.
