...
Server-side spans will display `0ms` for their durations. This is due to a security measure Cloudflare Workers implements to [mitigate against timing attacks](https://developers.cloudflare.com/workers/runtime-apis/performance/). In the Cloudflare Workers runtime, `performance.now()` and `Date.now()` only advance after I/O occurs, which means CPU-bound operations will show zero duration.
