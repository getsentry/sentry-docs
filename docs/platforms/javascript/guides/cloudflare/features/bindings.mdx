...
For RPC calls to Durable Objects and service bindings, add the binding to <PlatformLink to="/tracing/distributed-tracing/#rpc-trace-propagation">`rpcTracePropagationBindings`</PlatformLink>. The <PlatformLink to="/install/vite-plugin/#derived-rpc-trace-propagation">Vite plugin</PlatformLink> adds the bindings to classes in the same Worker for you.

## Which `env` Is Instrumented

The SDK instruments the `env` object that Cloudflare gives to the handlers and classes that Sentry wraps, such as your Worker, Durable Objects, and Workflows. The <PlatformLink to="/install/vite-plugin/">Vite plugin</PlatformLink> wraps these for you at build time.
