...
[Cloud Functions for Firebase](https://firebase.google.com/docs/functions) lets you automatically run backend code in response to events triggered by Firebase features and HTTPS requests. Sentry's Node SDK works out of the box with Cloud Functions for Firebase.
...
Create an initialization file (for example, `instrument.js`) that requires and initializes Sentry. This file must be required at the very top of your functions entry point, before any other requires.
...
Require the initialization file at the very top of your functions entry point (for example, `index.js`), before any other requires:
...
<Alert>
  The key is to require the initialization file **before** any other requires,
  including Firebase requires. This ensures Sentry can properly instrument all
  modules.
</Alert>
