...
If the defaults pick up too much or too little, narrow the upload with the `sourcemaps` options. These are top-level options and apply to both Webpack and Turbopack builds.

<Alert level="info">
  Earlier versions reached these settings through
  `unstable_sentryWebpackPluginOptions`. That escape hatch was removed — every
  option it exposed is now a first-class build option. See [Build
  Configuration](/platforms/javascript/guides/nextjs/configuration/build/) for
  the full list.
</Alert>
