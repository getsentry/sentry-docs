...
<Expandable title="Using Next.js middleware/proxy with tunneling">

If you're using Next.js middleware (`middleware.ts`) or proxy (`proxy.ts`) that intercepts requests, you may need to exclude the tunnel route. When using `tunnelRoute: true`, Sentry generates a random route on each build. For compatibility, use a fixed route instead:
...
Then exclude it in your middleware or proxy:

```typescript {filename:middleware.ts or proxy.ts}
export const config = {
