...
`global-error.tsx` catches errors in your root layout. It must include `<html>` and `<body>` tags since it replaces the entire document.
...
Pass a second argument to `captureException` to add debugging context:
