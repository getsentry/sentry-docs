...
First, add the `nodejs_compat` [compatibility flag](https://developers.cloudflare.com/workers/runtime-apis/nodejs/), which configures the Cloudflare Workers runtime to provide the necessary Node.js APIs required by the Sentry SDK.
...
Then, set the [compatibility date](https://developers.cloudflare.com/workers/configuration/compatibility-dates/) to `2025-08-16` or any later date. This date or later dates are required to introduce `https.request` to the Cloudflare Workers runtime which is needed by the Sentry SDK to send requests.
