...
`withSentryConfig` uses a [custom Webpack plugin](https://www.npmjs.com/package/@sentry/webpack-plugin) to manage your sourcemaps and releases under the hood. If you need to access the `sentryWebpackPluginOptions` via `withSentryConfig` you can do so with `unstable_sentryWebpackPluginOptions`.
