...
Update your `next.config.(js|mjs)` file with the auth token option and set the `SENTRY_AUTH_TOKEN` environment variable in your `.env` file.
...
Update `withSentryConfig` in your `next.config.(js|mjs)` file with the `automaticVercelMonitors` option.
...
### Configure Component Annotation

Update `withSentryConfig` in your `next.config.(js|mjs)` file with the `reactComponentAnnotation` option.
...
To test tracing, create a test API route like `/api/sentry-example-api` and update your test button to call this route and throw an error if the response isn't `ok`.
...
```javascript {filename:app/api/sentry-example-api/route.js}
