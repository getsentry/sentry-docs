...
We recommend you include your DSN directly in these three files. Alternatively you can pass the DSN it via a _public_ environment variable like `NEXT_PUBLIC_SENTRY_DSN`.
...
If you find that there are frames in your client-side stack traces that aren't getting source-mapped even when most others are, it's possible because they are from files in `static/chunks/` rather than `static/chunks/pages/`. By default, such files aren't uploaded because the majority of the files in `static/chunks/` only contain Next.js or third-party code.
...
Use the `tunnelRoute` option to provide a route the SDK will use to tunnel events to Sentry:
