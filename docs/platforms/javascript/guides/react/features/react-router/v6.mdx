...
These snippets render with `ReactDOM.createRoot` from `react-dom/client`, which needs React 18 or later. On React 17, render with `ReactDOM.render` from `react-dom` instead.
...
```javascript {2-12, 17-23, 28, 31-33}
import React from "react";
import ReactDOM from "react-dom/client";
import {
  BrowserRouter,
