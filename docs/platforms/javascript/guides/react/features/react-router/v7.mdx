...
<Alert level="warning" title="Looking for framework mode?">
  - React Router v7 (framework mode) is not yet supported, but we do provide a guide on how to set this up using existing SDKs [here](/platforms/javascript/guides/react-router/).
</Alert>
