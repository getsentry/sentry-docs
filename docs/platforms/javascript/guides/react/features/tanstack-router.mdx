...
_(Available in version TODO and above)_

TanStack Router support is included in the `@sentry/react` package since version `TODO` and is only compatible with version `1.34.5` of ` `@tanstack/router` and above.
