...
<Alert level="info" title="Using React 19+?">

For React 19 and above, we recommend using `Sentry.reactErrorHandler()` with React's built-in error hooks (`onUncaughtError`, `onCaughtError`, `onRecoverableError`) for more comprehensive error coverage. See the [getting started guide](../) for setup instructions.

The `ErrorBoundary` component described on this page is useful for:
- React 18 and below
- Granular error handling in specific component subtrees
- Custom fallback UIs for specific sections of your app
...
function App() {
  ...
  return (
    ...
    >
      ...
      <button onClick={() => setMessage({ text: "Hello World" })}>
...
Custom error boundaries in React **must be class components** — this is a React requirement, not a Sentry limitation. React does not provide a hook-based API for error boundaries.
