...
`solidRouterBrowserTracingIntegration` comes from `@sentry/solid/solidrouter`, which imports [`@solidjs/router`](https://docs.solidjs.com/solid-router). If your app doesn't use Solid Router, the build fails with `Could not resolve "@solidjs/router"`. Use `Sentry.browserTracingIntegration()` instead, which works without a router but doesn't add route names to your spans.
