...
Passing `dataCollection: {}` opts generative AI inputs and outputs **on**, along with other permissive defaults (bodies, cookies, user info, and more). Review the data your agent handles and tighten these before production. See [Privacy Controls](#privacy-controls) for how to turn inputs and outputs off.
...
With `dataCollection: {}` (or any `dataCollection` config), generative AI inputs and outputs default to **on**. To turn them off, set `genAI.inputs` and `genAI.outputs` to `false`:
...
You can also set `recordInputs` / `recordOutputs` on a specific AI integration to control input and output recording per integration.
