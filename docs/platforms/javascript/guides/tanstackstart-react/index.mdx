...
#### Add the Sentry Vite Plugin

Add `sentryTanstackStart` to your `vite.config.ts` to enable automatic source map uploads during production builds:
...
For all available options, see the [Vite Plugin documentation](/platforms/javascript/guides/tanstackstart-react/features/sentryTanstackStart/).
