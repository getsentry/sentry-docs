...
The build and start commands further down expect a `.output/server` directory, which Nitro produces. Add `nitro` to your dependencies with `npm install nitro`, and keep `nitro()` after `tanstackStart()`. Without it, `vite build` writes `dist/` instead, the copy step fails with `No such file or directory`, and the start command fails with `ERR_MODULE_NOT_FOUND`.
