...
Load this file with the `--import` flag when starting your app:
...
Add the `sentry()` middleware as early as possible in your main Hono app. For Cloudflare Workers and Bun, pass your Sentry options directly to the middleware. For Node.js, Sentry is already initialized in the instrument file, so no options are needed here.
...
### Migrating from @hono/sentry

If you're using the deprecated community middleware `@hono/sentry`, switch to `@sentry/hono`:

1. Uninstall `@hono/sentry` and `toucan-js`
2. Follow the install and configure steps above for your runtime
