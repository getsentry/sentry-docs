...
**Tracing is disabled by default.** You must explicitly enable it by setting either `tracesSampleRate` or `tracesSampler` in your Sentry configuration. Without setting one of these options, no transaction data will be sent to Sentry, even if you have tracing integrations configured.
