...
This example demonstrates proper queue instrumentation patterns. For more details on instrumenting queues, see the [Queues Module documentation](/platforms/javascript/guides/node/tracing/instrumentation/queues-module/).
...
## Manual LLM Instrumentation (Custom Agent + Tool Calls)

Example Repository: [Customer Service Bot](https://github.com/getsentry/llm-manual-agent-monitoring-example)

**The Challenge:** You're building a custom agent that uses a proprietary LLM API (not OpenAI/Anthropic), performs multi-step reasoning with tool calls, and needs comprehensive monitoring to track token usage, tool performance, and agent effectiveness across the entire conversation flow.

**The Solution:** Manually instrument each component of the AI pipeline using Sentry's agent span conventions. Create spans for agent invocation, LLM calls, tool executions, and handoffs between agents, with rich attributes for monitoring costs, performance, and business metrics.

<Alert>

This example follows Sentry's AI Agents Module conventions. For detailed specifications on `gen_ai.*` span attributes and requirements, see the [AI Agents Module documentation](https://develop.sentry.dev/sdk/telemetry/transactions/#ai-agents-module).
  ...
  await Sentry.startSpan(
    ...
    async (agentSpan) => {
      try {
        ...
        agentSpan.setAttribute('gen_ai.output.messages', JSON.stringify([
          { role: 'assistant', parts: [{ type: 'text', content: aiResponse.message }] }
        ]));
  ...
  await Sentry.startSpan(
    ...
    async (agentSpan) => {
      try {
        ...
        const llmResponse = await Sentry.startSpan(
          ...
          async (llmSpan) => {
            ...
            llmSpan.setAttribute(
              ...
              JSON.stringify([
                { role: "assistant", parts: [{ type: "text", content: llmData.choices[0].message.content || "" }] },
              ])
            );
            ...
            if (llmData.choices[0].message.tool_calls) {
              llmSpan.setAttribute(
                "gen_ai.output.messages",
                JSON.stringify([
                  { role: "assistant", parts: llmData.choices[0].message.tool_calls.map(tc => ({ type: "tool_call", name: tc.function.name, arguments: tc.function.arguments })) },
                ])
              );
            }
          ...
          for (const toolCall of llmResponse.choices[0].message.tool_calls) {
            await Sentry.startSpan(
              ...
              async (toolSpan) => {
                ...
                toolSpan.setAttribute("gen_ai.tool.call.result", toolOutput);
        ...
        agentSpan.setAttribute("gen_ai.output.messages", JSON.stringify([
          { role: "assistant", parts: [{ type: "text", content: finalResponse }] },
        ]));
