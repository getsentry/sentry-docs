...
Stream mode sends span records instead of assembling one transaction event with embedded spans. Use `beforeSendSpan` and `ignoreSpans` to filter streamed spans instead of `beforeSendTransaction` and `ignoreTransactions`. See <PlatformLink to="/tracing/streamed-spans/">Streamed Spans</PlatformLink> for details.
