...
| `forceTransaction` | `boolean`                   | If true, ensure this span shows up as transaction in the Sentry UI. Not available in <PlatformLink to="/tracing/streamed-spans/" fallbackPlatform="javascript">stream mode</PlatformLink>. Use `parentSpan: null` instead to ensure this span shows up as service span. |
...
Note that the property names differ between transaction mode (the default) and <PlatformLink to="/tracing/streamed-spans/" fallbackPlatform="javascript">stream mode</PlatformLink>:
...
Sentry maintains a [list of well-known span operations](https://develop.sentry.dev/sdk/telemetry/transactions/#list-of-operations) and it is recommended that you use one of those operations if it is applicable to your span.
