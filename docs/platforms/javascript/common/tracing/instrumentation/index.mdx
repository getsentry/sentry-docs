...
| `forceTransaction` | `boolean`                   | If true, ensure this span shows up as transaction in the Sentry UI. Not available in <PlatformLink to="/tracing/streamed-spans/">stream mode</PlatformLink>. Use `parentSpan: null` instead to ensure this span shows up as service span. |
...
Note that the property names differ between transaction mode (the default) and <PlatformLink to="/tracing/streamed-spans/">stream mode</PlatformLink>:
