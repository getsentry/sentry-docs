...
<Alert level="info" title="Default Tracing Behavior">

When neither `tracesSampleRate` nor `tracesSampler` is set in your Sentry configuration, **no transactions will be sent to Sentry** by default. This makes tracing opt-in - you must explicitly enable it by setting one of these options.

Setting `__SENTRY_TRACING__` to `false` is only safe when:
- You have not configured `tracesSampleRate` or `tracesSampler`
- You are not using any tracing APIs like `Sentry.startTransaction()`, `Sentry.startSpan()`, etc.
- You are not using integrations that automatically create transactions (like `browserTracingIntegration`)

If you're using tracing features but want to temporarily disable sending transactions, set `tracesSampleRate` to `0` instead of using this tree-shaking flag.
