...
<PlatformSection supported={["javascript.electron", "javascript.react-router"]}>

To get the full agent tree (`gen_ai.invoke_agent` → `gen_ai.chat` + `gen_ai.execute_tool`), use a framework integration that owns the tool loop (for example <PlatformLink to="/configuration/integrations/vercelai/">Vercel AI</PlatformLink>), or create `gen_ai.execute_tool` spans yourself around your local tool handlers.
