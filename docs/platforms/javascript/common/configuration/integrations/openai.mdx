...
<PlatformSection notSupported={["javascript.node", "javascript.connect", "javascript.hapi", "javascript.koa"]}>
  ...
  If you call OpenAI from a Durable Object, wrap the DO with <PlatformLink to="/features/durableobject/">`instrumentDurableObjectWithSentry`</PlatformLink> and set `enableRpcTracePropagation: true` so Worker → DO → LLM stays on one trace.
...
<PlatformSection notSupported={["javascript", "javascript.angular", "javascript.ember", "javascript.gatsby", "javascript.react", "javascript.solid", "javascript.svelte", "javascript.vue", "javascript.wasm"]}>

To get the full agent tree (`gen_ai.invoke_agent` → `gen_ai.chat` + `gen_ai.execute_tool`), wrap your tool loop with <PlatformLink to="/agent-tracing/manual-instrumentation/">manual instrumentation</PlatformLink>, or use a framework integration that owns the tool loop (for example <PlatformLink to="/configuration/integrations/vercelai/">Vercel AI</PlatformLink>).
