    ...
    }),
    {
      async fetch(request, env) {
        const openai = new OpenAI({
          ...
          // OpenAI-compatible providers (for example OpenRouter) work the same way:
...
Streaming and non-streaming requests are automatically detected and handled appropriately. Each call creates a `gen_ai.chat` span (including `responses.create()`).
...
OpenAI-compatible providers may still return usage without `stream_options`; prefer setting `include_usage: true` whenever you need reliable token attributes.
