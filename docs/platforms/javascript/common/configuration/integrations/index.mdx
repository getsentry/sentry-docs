...
You can also lazy load pluggable integrations via `Sentry.lazyLoadIntegration()`. This will try to load the integration from the CDN (not from npm).
Note that this function will reject if it fails to load the integration from the CDN, which can happen if a user has an ad-blocker or if there is a network problem. You should always make sure to handle rejections for this function in your application.
