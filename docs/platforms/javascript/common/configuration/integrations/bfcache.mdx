...
| `browser.bfcache.reload.duration` | distribution | Duration (in milliseconds) of the fallback reload when a back/forward navigation missed the bfcache. Only emitted when the browser reports a duration. |
...
| `browser.bfcache.not_restored_reason_count` | The number of reasons the browser reported for a miss. Omitted on hits and on misses where the browser reported no reasons.             |
| `sentry.segment.name`                       | The route the navigation landed on, taken from the scope's transaction name. Falls back to `window.location.pathname` when none is set. |
