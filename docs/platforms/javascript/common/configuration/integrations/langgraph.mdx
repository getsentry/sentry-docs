...
The `langGraphIntegration` adds instrumentation for LangGraph to capture spans by automatically wrapping LangGraph operations and recording AI agent interactions including agent invocations, graph executions, and node operations.

<Alert>

This integration is **enabled by default for Node.js-based platforms** and automatically captures spans for LangGraph operations. It requires SDK version `10.25.0` or higher.
...
For Cloudflare Workers edge runtime, manual instrumentation is required using `instrumentLangGraph`.
...
For Next.js applications using the Edge runtime, manual instrumentation is required using `instrumentLangGraph`. This integration is automatically instrumented in the Node.js runtime.

</Alert>

</PlatformSection>

The `instrumentLangGraph` helper adds instrumentation for LangGraph to capture spans by wrapping a compiled LangGraph graph and recording AI agent interactions with configurable input/output recording. You need to manually wrap your compiled graph with this helper. See example below:
...
const llm = new ChatOpenAI({
  modelName: "gpt-4o",
  apiKey: process.env.OPENAI_API_KEY, // Warning: API key will be exposed in browser!
});
