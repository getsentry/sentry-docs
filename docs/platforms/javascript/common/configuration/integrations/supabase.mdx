...
By default, unsampled requests only send a bare `traceparent` (no `tracestate`/`baggage`), so correlation still works but with less context. Set `respectSamplingDecision: false` on the Supabase client if you want full trace context regardless of sampling.
