...
The `vercelAIIntegration` is **not supported in React Native**. It relies on the Vercel AI SDK's OpenTelemetry-based telemetry and Node.js runtime APIs that aren't available in React Native. For React Native apps, use the manual client wrappers for [OpenAI](/platforms/react-native/configuration/integrations/openai/), [Anthropic](/platforms/react-native/configuration/integrations/anthropic/), or [Google Gen AI](/platforms/react-native/configuration/integrations/google-genai/) instead — see [AI Agent Monitoring for React Native](/platforms/react-native/ai-agent-monitoring/).
