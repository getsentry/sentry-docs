...
To enhance the spans collected by this integration, we recommend providing a `functionId` to identify the function that the telemetry data is for. In order to make sure that the inputs and outputs are correctly captured, we recommend adding `recordInputs` and `recordOutputs` to all functions. For more details, see the [AI SDK Telemetry Metadata docs](https://sdk.vercel.ai/docs/ai-sdk-core/telemetry#telemetry-metadata).
