...
To control input and output recording on the default entrypoint, set `experimental_telemetry.recordInputs` and `experimental_telemetry.recordOutputs` per call instead (see [Configuration](#configuration)).
