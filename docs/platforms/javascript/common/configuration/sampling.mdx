...
<Alert level="info" title="Tracing is Opt-in">

**Tracing is disabled by default.** When you initialize Sentry without setting `tracesSampleRate` or `tracesSampler`, the SDK will not send any transaction data to Sentry. This is true even if you have tracing integrations like `browserTracingIntegration` configured - without a sampling rate, no spans will be sent.

To enable tracing, you must explicitly set one of:
- `tracesSampleRate`: A number between 0 and 1 (e.g., `0.1` for 10% sampling)
- `tracesSampler`: A function that returns a sample rate for each transaction

Note that setting `tracesSampleRate` to `0` is different from not setting it at all. A rate of `0` means tracing is enabled but no transactions are sampled, while not setting it means tracing is completely disabled.
