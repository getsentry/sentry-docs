...
When using custom instrumentation to create a span, the `tracesSampler` function will automatically receive the span `name` and the initial span `attributes` to use in its decision.
