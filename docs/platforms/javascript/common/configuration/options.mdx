---
title: Options
description: "Learn more about how the Sentry JavaScript SDK can be configured via options. These are being passed to the init function and therefore set when the SDK is first initialized."
...
you can provide the ID with this option. The organization ID is used for trace propagation.

<PlatformCategorySection supported={["server", "serverless"]}>

It is also used for features like <PlatformLink to="/configuration/options#strictTraceContinuation">strict trace continuation</PlatformLink>.
...
In version 10 of the SDK, the defaults below apply only when you set the `dataCollection` option.
Without it, the deprecated `sendDefaultPii` option controls data collection.
Starting with version 11, the defaults always apply.
...
The SDK matches denylist terms against key names. The match is partial and case-insensitive. For example, the term `auth` matches `Authorization` and `X-Auth-Token`. The built-in sensitive denylist always applies, also to keys in an `allow` list:

`["auth", "token", "secret", "password", "passwd", "pwd", "key", "jwt", "bearer", "sso", "saml", "csrf", "xsrf", "credentials", "session", "sid", "identity"]`

Some headers, cookies, and query parameters identify a user without being credentials, such as `x-forwarded-for` or `x-user-id`. To filter these values too, extend the denylist with these terms: `["forwarded", "-ip", "remote-", "via", "-user"]`.

<PlatformCategorySection supported={["browser"]}>

`dataCollection` does not affect <PlatformLink to="/session-replay/privacy/">Session Replay</PlatformLink>. Use the Replay privacy options to control what Replay captures.
...
This callback has no effect in stream mode. Use [`beforeSendSpan`](#beforeSendSpan) to modify spans and check `span.is_segment` to target service spans. Use [`ignoreSpans`](#ignoreSpans) to drop spans.
...
<Expandable title="Example in transaction mode">
...
In transaction mode, `ignoreSpans` is evaluated when the transaction ends, using the span's final name and attributes.
...
This option has no effect in stream mode. Use [`ignoreSpans`](#ignoreSpans) to filter spans by name, operation, or attributes.
...
<SdkOption name="traceLifecycle" type="'static' | 'stream'" defaultValue="'stream'" defaultNote='since version 11.0.0' availableSince='10.53.1'>
...
Logs are captured when you call `Sentry.logger.*` or add a logging integration. Use [`beforeSendLog`](#beforeSendLog) to filter logs before sending them to Sentry.
...
To discard all logs, set `beforeSendLog: () => null`.
