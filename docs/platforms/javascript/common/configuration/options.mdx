...
<Alert>
  Passing `dataCollection` (even an empty `dataCollection: {}`) opts you into the more permissive `dataCollection` defaults. To preserve the old behavior while using `dataCollection`, opt out of each category explicitly:
