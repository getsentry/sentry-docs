...
```
  ...
  <Expandable title='What is the difference to `beforeSend`, `beforeSendTransaction`, and `beforeSendSpan`?'>
    ...
    If you're using <PlatformLink to="/tracing/new-spans/">stream mode</PlatformLink>, use `beforeSendSpan`, since event processors are not applied to spans.
...
<SdkApi name="spanToJSON" signature="function spanToJSON(span: Span): SpanJSON">
  ...
  <PlatformLink to="/tracing/new-spans/">stream mode</PlatformLink>, we
...
>
  ...
  <PlatformLink to="/tracing/new-spans/">stream mode</PlatformLink>.
</SdkApi>
...
See <PlatformLink to="/ai-agent-monitoring/">AI Agent Monitoring</PlatformLink> for more information on tracking AI conversations.
