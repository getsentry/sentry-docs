...
```
  ...
  <Expandable title='What is the difference to `beforeSend` and `beforeSendSpan`?'>
    ...
    Event processors do not run on streamed spans. Use `beforeSendSpan` to modify each finished span before it is sent. In transaction mode, event processors also run on transaction events, followed by `beforeSendTransaction`. You can configure one `beforeSendTransaction` callback.
  </Expandable>
</SdkApi>
...
<SdkApi name="lazyLoadIntegration" signature="function lazyLoadIntegration(name: string, scriptNonce?: string): Promise<IntegrationFn>" categorySupported={['browser']}>
...
>
Set a single attribute. Unlike tags, attributes are applied to your logs and metrics (rather than errors).
...
>
  ...
  <Expandable title="Capturing User IP-Addresses">
    ...
    This is automatically done if you have configured <PlatformLink to="/configuration/options/#dataCollection">`dataCollection: { userInfo: true }`</PlatformLink> in your SDK configuration.
    </PlatformCategorySection>
    ...
    server. `{{auto}}` is automatically set if you have configured <PlatformLink to="/configuration/options/#dataCollection">`dataCollection: { userInfo: true }`</PlatformLink> in your SDK configuration.
    </PlatformCategorySection>
...
<SdkApi
  name="withActiveSpan"
  signature="function withActiveSpan<T>(span: Span | null, callback: (scope: Scope) => T): T"
>
...
<SdkApi
  ...
  signature="function setConversationId(conversationId: string | null): void"
>
...
See <PlatformLink to="/agent-tracing/">Agent Tracing</PlatformLink> for more information on tracking AI conversations.
...
<SdkApi name="startSession" signature="function startSession(): Session">
...
<SdkApi
  name="handleRequest"
  signature={`function handleRequest(options?: {
  ...
  parameters={[
  {
    name: "trackClientIp",
    type: "boolean",
    description: "When enabled, Sentry attaches the client's IP address to the event. This requires your Astro app to be configured for SSR or Hybrid rendering. Defaults to `false`.",
    defaultValue: "false" },
  ]}
>
Returns an Astro middleware handler that instruments incoming server requests with Sentry error and performance monitoring. It lets you control which additional data should be added to recorded request spans.
