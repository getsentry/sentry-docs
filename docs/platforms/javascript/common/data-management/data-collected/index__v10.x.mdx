...
Headers attached as span attributes and headers captured by the `httpClient` integration are filtered: the header names are kept, and the values of sensitive headers (names that match terms such as `auth`, `token`, or `password`) are replaced with `[Filtered]`.
Request headers attached to error events on the server are sent with their original values in version 10 (version 11 filters them as well).
...
Cookie values attached as span attributes or captured by `httpClient` are filtered: the values of sensitive cookies (like `token` or `key`) are replaced with `[Filtered]`. Cookies attached to error events on the server are sent with their original values in version 10 (version 11 filters them as well).
...
The `dataCollection.httpBodies` option controls data taken from the body: tRPC input and Remix action form data (`"incomingRequest"`), and the results of Next.js server actions (`"outgoingResponse"`). All body types are included by default. Set `dataCollection: { httpBodies: [] }` to opt out:
...
On the server-side, the <PlatformLink to="/configuration/integrations/requestdata/">RequestData Integration</PlatformLink> captures incoming request data including cookies, headers, query strings, request body (`data`), URL, and user information. By default, all of these fields are captured. The IP address is included when `dataCollection.userInfo` is enabled.
...
Response body sizes, based on the `content-length` header, may still be sent.
