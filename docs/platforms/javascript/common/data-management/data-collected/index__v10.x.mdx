...
You can control many of the categories listed here with the <PlatformLink to="/configuration/options/#dataCollection">`dataCollection` option</PlatformLink> (available since version `10.57.0`), which lets you opt in or out of each data category individually.

When you set `dataCollection` (even to an empty object `{}`), the SDK collects all categories by default and scrubs values whose keys match the built-in sensitive denylist. To collect less, opt out of a category or restrict it.

When you don't set `dataCollection`, the deprecated `sendDefaultPii` option determines what the SDK collects. With `sendDefaultPii: true`, all categories are collected. Without it, the SDK does not collect user identity fields, IP addresses, request and response bodies, generative AI content, or database query data, and it applies stricter filtering to headers, cookies, and query parameters.
...
By default, the Sentry SDK sends HTTP request and response headers. The SDK always keeps the header names and replaces the values of sensitive headers (names that match terms such as `auth`, `token`, or `password`) with `[Filtered]`.

Use the `dataCollection.httpHeaders` option to control this. Set `dataCollection: { httpHeaders: { request: false, response: false } }` to disable header collection, or use `{ allow: [...] }` or `{ deny: [...] }` on `request` and `response` to restrict which header values are sent. Since version `10.75.0`, you can also set one value that applies to both directions, for example `dataCollection: { httpHeaders: false }`.

When you don't set `dataCollection`, header collection depends on the deprecated `sendDefaultPii` option:

- With `sendDefaultPii: true`, the SDK sends headers with the standard filtering described above.
- Without it, the SDK still sends headers, but it also replaces the values of IP-related headers (names that match `forwarded`, `-ip`, `remote-`, `via`, or `-user`) with `[Filtered]`. The browser `httpClient` integration is an exception: when `dataCollection` is not set, it captures headers and cookies of failed requests only if `sendDefaultPii` is `true`.
...
With `userInfo: false`, user identity fields are not sent automatically. Some integrations (e.g. <PlatformLink to="/user-feedback/">User Feedback</PlatformLink>) may still send data like the user ID, username, and email address when explicitly configured.
...
<PlatformSection notSupported={["javascript", "javascript.angular", "javascript.capacitor", "javascript.cordova", "javascript.effect", "javascript.elysia", "javascript.ember", "javascript.firebase", "javascript.gatsby", "javascript.nitro", "javascript.react", "javascript.solid", "javascript.svelte", "javascript.vue", "javascript.wasm", "javascript.cloudflare", "javascript.deno"]}>
  On the server-side, the incoming request body is captured by default. You can disable sending the incoming request body by configuring `ignoreRequestBody` in the <PlatformLink to="/configuration/integrations/http/">HTTP Integration</PlatformLink>.
</PlatformSection>
