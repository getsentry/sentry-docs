...
You can control many of the categories listed here with the <PlatformLink to="/configuration/options/#dataCollection">`dataCollection` option</PlatformLink>, which lets you opt in or out of each data category individually.
...
Use the `dataCollection.httpHeaders` option to control header collection. Set `dataCollection: { httpHeaders: false }` to disable it, or use `{ allow: [...] }` or `{ deny: [...] }` to restrict which header values are sent.
To control each direction separately, use the `{ request: ..., response: ... }` form, for example `{ request: true, response: false }`.

## Cookies

By default, cookie collection is enabled. On the server, the cookies of incoming requests are sent with error events and server spans. In the browser, the SDK doesn't send cookies unless you enable the `httpClient` integration, which captures the cookies of failed requests. The SDK replaces the values of sensitive cookies (like `token` or `key`) with `[Filtered]`.
...
In some integrations such as [`handleRequest`](/platforms/javascript/guides/astro/#customize-server-instrumentation) in Astro, the `trackClientIp` option also controls this. It defaults to the `dataCollection.userInfo` setting.
...
Use the `dataCollection.httpBodies` option to control this. It takes a list of the body types to collect: `"incomingRequest"`, `"outgoingRequest"`, `"incomingResponse"`, and `"outgoingResponse"` — all included by default. Set it to `[]` to disable body collection:
...
<PlatformSection supported={["javascript.remix"]}>
  ...
  Captured fields are sent as `remix.action_form_data.<field>` span attributes.
</PlatformSection>
