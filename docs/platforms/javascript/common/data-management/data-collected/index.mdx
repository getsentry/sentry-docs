...
Without `dataCollection` (and with `sendDefaultPii` unset or `false`), the user's IP address is not sent. You can enable it with the deprecated `sendDefaultPii: true`. In some integrations such as <PlatformLink to="/guides/astro/#customize-server-instrumentation">`handleRequest`</PlatformLink> in Astro, the user's IP address can also be sent by enabling `trackClientIp`.
