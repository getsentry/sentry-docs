...
Canvas is supported in SDK versions >= `7.98.0`. Please see the <PlatformLink to="/session-replay/#canvas-recording">canvas setup documentation</PlatformLink> to get started with canvas recordings.
...
<Expandable title="My same-origin (srcdoc) iframe is empty in replays" permalink>

Same-origin iframes that use the `srcdoc` attribute (rather than `src`) are blocked by default and render as an empty placeholder. This is because our masking logic doesn't run inside `srcdoc` iframe content, so blocking prevents that content from being recorded unmasked.

If you understand that the iframe's contents will be recorded **without masking**, you can re-enable recording with `unblock`:

```javascript
Sentry.replayIntegration({
  unblock: ["iframe[srcdoc]"],
});
```

Only do this for iframes whose contents you know are safe to record in full.
...
If you want to turn off [rage click issues](/product/issues/issue-details/replay-issues/) altogether, you can disable them by going to the **Settings** page in Sentry, clicking "Projects", and selecting the project for which you want to disable rage clicks. From here, select ["Replays"](https://sentry.io/orgredirect/organizations/:orgslug/settings/projects/:projectId/replays/) under the "PROCESSING" heading and toggle off "Create Rage Click Issues".
