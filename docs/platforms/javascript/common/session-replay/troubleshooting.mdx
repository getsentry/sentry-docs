...
<Expandable title="Clicks are incorrectly resulting in a dead click" permalink>

There are certain cases (for example, clicking a button to play an audio clip) where Sentry incorrectly records a click as a dead click. As a work-around you can use the [`slowClickIgnoreSelector`](https://docs.sentry.io/platforms/javascript/session-replay/configuration/#general-integration-configuration) configuration option to ignore dead clicks for specific selectors.
</Expandable>
