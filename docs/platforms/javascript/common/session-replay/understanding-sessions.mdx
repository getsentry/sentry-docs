...
When you add the `replayIntegration()` to your SDK, it automatically starts based on your sample rate settings:

| Configuration | What Happens |
|--------------|-------------|
| `replaysSessionSampleRate > 0` | Starts in <PlatformLink to="/session-replay/understanding-sessions/#session-mode">**session mode**</PlatformLink> |
| `replaysOnErrorSampleRate > 0` only | Starts in <PlatformLink to="/session-replay/understanding-sessions/#buffer-mode">**buffer mode**</PlatformLink> |
| Both rates are `0` | Replay is installed but inactive until you <PlatformLink to="/session-replay/understanding-sessions/#manually-starting-replay">**manually start it**</PlatformLink> |
