...
You can configure which issue types are created by visiting Project Settings > [Replay](https://sentry.io/orgredirect/organizations/:orgslug/settings/projects/:projectId/replays/) and toggling detection on or off for each issue type. By default all issue types are enabled.
