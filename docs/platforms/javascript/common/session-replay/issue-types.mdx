...
The error event, before considering available quota, is subject to [Inbound Filters](/concepts/data-management/filtering/) and may be dropped. If the inbound filters are disabled, the error is accepted (consuming error quota) and an issue is created. Error events may or may not have an attached replay to help debug the problem.
