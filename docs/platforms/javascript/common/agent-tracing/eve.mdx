...
The generated instrumentation needs two values from your Sentry project. Both are read from environment variables.
...
Each turn produces a trace shaped like this:
...
- **Token and cost values are doubled.** Don't add the Sentry SDK with the <PlatformLink to="/configuration/integrations/vercelai/">Vercel AI integration</PlatformLink> to the same agent. Eve already produces the AI spans, so a second producer double-counts them.
