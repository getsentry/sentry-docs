...
[Agents](/product/ai-observability/agents/) groups multi-turn AI activity into a single replay of messages and tool calls. Use `setConversationId()` so every AI span in a chat session shares the same `gen_ai.conversation.id`.
...
The [Agents](/product/ai-observability/agents/) view includes a **User** column. To populate it, call `setUser` once per request or session, before any AI calls:

```javascript
Sentry.setUser({ id: "user_123", email: "jane@example.com", username: "jane" });
```

Any of `id`, `email`, or `username` is sufficient — the Agents view displays whichever fields are present. See the <PlatformLink to="/configuration/apis/#setUser">setUser API reference</PlatformLink> for the full list of supported fields.
...
From SDK `10.61.0`, `gen_ai` spans are sent as standalone envelope items (avoids large-payload drops; required for <Link to="/product/ai-observability/agents/">Agents</Link>).
