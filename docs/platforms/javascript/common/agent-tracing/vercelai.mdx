...
Two steps are required. `force: true` on its own is not enough — without the second step you get no AI spans at all.

Add the integration with `force: true`:
...
And pass `experimental_telemetry` on every AI SDK call:
