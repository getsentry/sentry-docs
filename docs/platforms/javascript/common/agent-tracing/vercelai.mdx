...
Next.js runs your server code in two runtimes, but Sentry only instruments the AI SDK in the Node runtime. The instrumentation relies on diagnostics channels, which the Edge runtime doesn't support.

In the Edge runtime, `vercelAIIntegration` does nothing. You can still import it from `@sentry/nextjs` without errors, but it captures no spans. To monitor AI calls, run them in the Node runtime. See the <PlatformLink to="/migration/v10-to-v11/#vercel-ai-is-not-instrumented-on-the-edge-runtime">migration guide</PlatformLink>.
...
The integration is enabled by default in the Node runtime:
...
You must also pass `experimental_telemetry` on every call. See [Turn on telemetry](#turn-on-telemetry).
...
`force` only registers the span processors. You still need `experimental_telemetry` on every call.
...
- Sentry SDK: `10.6.0`+
- Edge runtime: Not supported in Sentry SDK `11.0.0`+. Run AI calls in the Node runtime instead.
