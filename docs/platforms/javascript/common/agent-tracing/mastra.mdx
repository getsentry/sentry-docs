...
<PlatformSection supported={["javascript.nextjs"]}>
  <Alert level="warning">
    ...
    `condition` field, which Next.js 15 doesn't support, so on Next.js 15 the
    SDK skips it and Mastra produces no spans under `--turbopack` (in `dev` or
    `build`). Use webpack instead, or [**Manual
