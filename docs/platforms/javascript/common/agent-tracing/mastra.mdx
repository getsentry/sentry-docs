...
>
  <Alert level="warning">
    In meta-frameworks, Mastra is usually bundled into the server build, which
    the SDK can't hook at runtime. Register the exporter yourself using
...
Register `SentryMastraExporter` yourself when the SDK can't hook the `Mastra` constructor — most often because Mastra is bundled into your server build:
