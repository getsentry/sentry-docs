...
The wrapper covers the agent Durable Objects only. It does not instrument the outer Worker or an authored Hono application. If you also need request instrumentation, add the <PlatformLink to="/install/vite-plugin/">Vite plugin</PlatformLink>, which wraps the Worker at build time and leaves the Durable Objects that the blueprint already wraps unchanged. Don't use `@sentry/node` on Cloudflare.
