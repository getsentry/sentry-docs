...
By default, Sentry derives [model cost](/product/agents/costs/) from the model name and token counts. Pass the raw provider model string unchanged so pricing can resolve. If your provider reports costs or you use custom pricing, see [Send Custom Costs](/product/agents/costs/#send-custom-costs).
