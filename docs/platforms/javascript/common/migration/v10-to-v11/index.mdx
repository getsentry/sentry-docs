...
Streamed spans only carry attributes, so scope `tags` and `extra` no longer reach any span, including the segment span that replaced the transaction. They still apply to errors, so you don't have to remove them. Set attributes for everything that should be searchable on spans, which also applies them to logs and metrics:
...
- Database spans are named after a summary of the query (`SELECT "User"`), not the statement. The sanitized statement stays on `db.query.text`.
...
| `cache.get`, `cache.put`, `cache.remove`                                 | The cache keys (`user:123`), or for dataloader the operation and loader name        | The cache operation (`cache.get`, `cache.put`, `cache.remove`)                                                                            |
...
If you only use the Sentry SDK, tracing works as before and there's nothing to do. In that setup, spans created through `@opentelemetry/api` are ignored.
