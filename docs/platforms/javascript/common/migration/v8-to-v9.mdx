...
- The `fetchProxyScriptNonce` option in `sentryHandle()` was removed due to security concerns.
  If you previously specified this option for your CSP policy, specify a <PlatformLink to="/manual-setup/#configure-csp-for-client-side-fetch-instrumentation">script hash</PlatformLink> in your CSP config or <PlatformLink to="/manual-setup/#disable-client-side-fetch-proxy-script">disable</PlatformLink> the injection of the script entirely.
