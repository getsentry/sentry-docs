...
Tags are **not** applied to logs, metrics, or spans when <PlatformLink to="/tracing/streamed-spans/" fallbackPlatform="javascript">span streaming</PlatformLink> is enabled. Tags still apply to errors, and in transaction mode (`traceLifecycle: "static"`) they're still added to the transaction.

If you're using SDK version `10.61.0` or above and want data to be searchable on spans, logs, or metrics, use <PlatformLink to="/enriching-events/attributes/">Attributes</PlatformLink> instead.
