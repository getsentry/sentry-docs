...
the scope. When a scope is forked, it inherits all data from the parent scope.
...
Scopes are basically a stack of data that is attached to events. When an event is captured, the SDK will merge the data from the current scope into the event. This allows you to attach data to events that is relevant to the context in which the event was captured.
...
- The [isolation scope](#isolation-scope)
- The [current scope](#current-scope)

### Global Scope

The global scope is applied to _all_ events, no matter where they originate. You can use it to store e.g. environmental information, or other things that should apply to all events.
...
The current scope is the local scope that is currently active. In contrast to the isolation scope, which is usually only forked rarely, e.g. per request, the current scope may be forked more frequently and under the hood. It can be used to store data that should only be applied to specific events. In most cases, you should not access this scope directly, but use `Sentry.withScope` to create a new scope that is only active for a specific part of your code:
...
You can access the curent scope via `Sentry.getCurrentScope()`, but usually you should use `Sentry.withScope()` to interact with local scopes instead.

## How Scope Data is Applied to Events

Before an event (e.g. an error or transaction) is sent to Sentry, the currently active scopes are applied to it.

The global scope is applied first, followed by the isolation scope, and finally the current scope. This means that any data set on the current scope will take precedence over data set on the isolation or global scope:
