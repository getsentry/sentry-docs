...
Use `beforeSendSpan` to change the name of a finished service span before it is sent. Check `is_segment` so that child spans keep their names. In transaction mode, use `beforeSendTransaction` to set the transaction name:

```javascript {tabTitle:Stream Mode (Default)} {mdExpandTabs}
Sentry.init({
  beforeSendSpan(span) {
    if (span.is_segment) {
      span.name = "UserListView";
    }
    return span;
  },
});
