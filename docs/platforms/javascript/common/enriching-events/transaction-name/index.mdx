...
Use `beforeSendSpan` to change the name of a finished service span before it is sent. Check `is_segment` so that child spans keep their names. In static mode, use `beforeSendTransaction` to set the transaction name:
...
```javascript {tabTitle:Static Mode}
Sentry.init({
