...
Users will be able to send screenshots with their feedback by default. You can configure this using the `showScreenshot` option. Screenshots aren't supported on mobile devices, so the screenshot button will be hidden in this case. This feature is only available on SDK v8.
