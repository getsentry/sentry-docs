...
Sentry can pair feedback with an associated event, giving you additional insight into issues. To do this, Sentry needs an `eventId`. For example, to get the `eventId`, you can:

1. Use the <PlatformLink to="/configuration/options/#before-send"><PlatformIdentifier name="before-send" /></PlatformLink> configuration, or
2. Use the return value of a method capturing an event, such as `captureMessage`.
