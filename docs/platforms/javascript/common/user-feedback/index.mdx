...
On SDK version 8.0.0 and above, users can send screenshots with their feedback. If you're using on-prem, you also need release 24.4.2 and above. You can configure this using the `showScreenshot` option, by default it is set to `true`. Screenshots aren't supported on mobile devices, so the screenshot button will be hidden automatically in this case.
