...
![An example feedback button image of the User Feedback Widget on a demo site](./img/user-feedback-widget-button.png)
![An example feedback form image of the User Feedback Widget on a demo site](./img/user-feedback-widget-form.png)
...
On SDK version 8.0.0 and above, users can send screenshots with their feedback. If you're self-hosting, you also need release 24.4.2 and above. You can configure this using the `showScreenshot` option, by default it is set to `true`. Screenshots aren't supported on mobile devices, so the screenshot button will be hidden automatically in this case.
