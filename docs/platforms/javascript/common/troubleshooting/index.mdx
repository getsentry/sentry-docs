...
<PlatformSection supported={['javascript.nuxt']}>
  ...
  <Expandable permalink title="Nuxt: Server-side Nitro is not sending events">
    ...
    - With a Cloudflare Nitro preset, the SDK doesn't use `sentry.server.config.ts`. Use the [Cloudflare Nitro plugin](/platforms/javascript/guides/cloudflare/frameworks/nuxt/) instead.
