...
<PlatformSection supported={['javascript.nuxt']}>
  <Expandable permalink title="Failed to register 'import-in-the-middle' during build time">
    ...
    for `@vercel/nft` to fix this. This will add the `hook.mjs` file to your build output. See the [underlying issue in the UnJS Nitro project](https://github.com/nitrojs/nitro/issues/2703).
