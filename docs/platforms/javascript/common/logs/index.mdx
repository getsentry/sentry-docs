...
- <PlatformCategorySection notSupported={["server-only", "mobile"]}>**[Session Replay](/product/session-replay/)** — Logs include `sentry.replay_id` when a replay is active. Jump from a log entry directly to the replay to see what the user was doing when the log was emitted.</PlatformCategorySection>
...
- <PlatformCategorySection notSupported={["server-only", "mobile"]}><PlatformLink to="/session-replay/">Session Replay</PlatformLink> — Logs are automatically linked to replays, letting you jump from a log entry to see what the user was doing.</PlatformCategorySection>
