...
title: Set Up Logs
sidebar_title: Logs
description: "Structured logs allow you to send, view and query logs sent from your applications within Sentry using the JavaScript SDK."
sidebar_order: 4
...
Use `Sentry.setAttribute` and `Sentry.setAttributes` to attach attributes that are automatically included in all logs (as well as your metrics). These work just like <PlatformLink to="/apis/#setTag">`Sentry.setTag` and `Sentry.setTags`</PlatformLink>, but they accept `string`, `number`, and `boolean` values or arrays of these.
...
## Electron Automatic Logs

Capture Electron lifecycle events, network requests, and process exits as logs without adding `Sentry.logger` calls:

- <PlatformLink to="/configuration/integrations/net/">ElectronNet</PlatformLink> logs responses and errors from Electron's `net.request` API. These logs have `sentry.origin: auto.electron.net`.
- <PlatformLink to="/configuration/integrations/electronbreadcrumbs/">ElectronBreadcrumbs</PlatformLink> logs selected events from `app`, `BrowserWindow`, `webContents`, `autoUpdater`, `screen`, and `powerMonitor`. These logs have `sentry.origin: auto.electron.events`.
- <PlatformLink to="/configuration/integrations/childprocess/">ChildProcess</PlatformLink> logs Electron child process and renderer exits and crashes. These logs have `sentry.origin: auto.electron.child-process`.

These integrations are included by default in the main process SDK. Starting with Electron SDK `7.18.0`, their log capture is opt-in: set `logs: true` on each integration you want to capture logs from. Earlier versions emitted these logs when SDK logging was enabled.

Log capture follows each integration's breadcrumb settings. Keep breadcrumbs enabled for ElectronNet, select the events to capture for ElectronBreadcrumbs, and include the exit reasons to capture in ChildProcess's `breadcrumbs` option.
...
- <PlatformSection notSupported={["javascript.node", "javascript.aws-lambda", "javascript.azure-functions", "javascript.elysia", "javascript.express", "javascript.fastify", "javascript.gcp-functions", "javascript.hapi", "javascript.hono", "javascript.koa", "javascript.nestjs", "javascript.nitro", "javascript.bun", "javascript.deno", "javascript.cloudflare", "javascript.wasm","javascript.eve", "javascript.mastra"]}><PlatformLink to="/session-replay/">Session Replay</PlatformLink> — Logs are automatically linked to replays, letting you jump from a log entry to see what the user was doing.</PlatformSection>
