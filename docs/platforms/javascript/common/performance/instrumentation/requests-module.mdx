...
As a prerequisite to setting up [Requests](/product/performance/requests/) you’ll need to first <PlatformLink to="/performance/">set up performance monitoring</PlatformLink>. Once this is done, the JavaScript SDK will automatically instrument outgoing HTTP requests. If that doesn't fit your use-case, you can also set up [custom instrumentation](#custom-instrumentation).

## Custom Instrumentation

For detailed information about which data can be set, see the [Requests Module Developer Specification](https://develop.sentry.dev/sdk/performance/modules/requests/).
