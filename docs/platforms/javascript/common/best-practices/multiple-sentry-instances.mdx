...
Integrations are setup on the `Client`, if you need to deal with multiple clients you have to make sure to also do the integration handling correctly.
  ...
  return {
    ...
    setupOnce() {
      Sentry.addEventProcessor((event) => {
        ...
        // Run the integration ONLY when it was installed on the current client
        if (self) {
