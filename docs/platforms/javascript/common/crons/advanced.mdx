...
- Just getting started with cron monitoring → Try [UI Setup](ui-setup) instead
- Using standard cron libraries → Try [Automatic](automatic) instead
- Need simple manual integration → Try [Manual Integration](manual) instead
...
async function runMonitoredJob() {
  // 🟡 Start check-in with monitor creation/update
