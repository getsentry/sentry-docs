...
<PlatformSection notSupported={['aws-lambda', 'gcp-functions']}>
  <Note>
    Note that even though all performance instrumentations will not be registered
    if tracing is not enabled, it will still be included in the bundle. If you
