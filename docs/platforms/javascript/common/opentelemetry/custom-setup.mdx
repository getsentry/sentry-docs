...
- **SentrySampler**: Ensures that the Sentry `tracesSampleRate` is respected. Even if you do not use Sentry for tracing, you still need this in order for trace propagation to work as expected. See [Using a Custom Sampler](./#using-a-custom-sampler) if you want to use a custom sampler.
