...
If OpenTelemetry owns tracing in your application and you only want Sentry's errors, logs and metrics on the same traces, use <PlatformLink to="/configuration/integrations/opentelemetry/">`openTelemetryIntegration`</PlatformLink> instead. It requires significantly less manual wiring.
