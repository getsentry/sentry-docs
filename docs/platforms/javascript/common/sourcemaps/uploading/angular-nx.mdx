...
<Alert>
  ...
  source code to be leaked. You can prevent your source maps from being publicly
  accessible by configuring your server to deny access to `.js.map` files, or by
  using the [Sentry Webpack Plugin's
  `sourcemaps.filesToDeleteAfterUpload`](https://www.npmjs.com/package/@sentry/webpack-plugin#sourcemapsfilestodeleteafterupload)
  option to delete source maps after they have been uploaded to Sentry.
</Alert>
...
<Alert>
  ...
  source code to be leaked. You can prevent your source maps from being publicly
  accessible by configuring your server to deny access to `.js.map` files, or by
  using the [Sentry Esbuild Plugin's
