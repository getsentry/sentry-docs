...
To group AI spans in [Conversations](/product/agents/conversations/), add
`conversationIdIntegration` and call `setConversationId`. Neither is included in
lightweight mode by default:

```javascript
import * as Sentry from "@sentry/node-core/light";
import { conversationIdIntegration, setConversationId } from "@sentry/core";
