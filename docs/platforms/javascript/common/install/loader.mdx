...
## Performance & Feedback Bundle

To use Sentry for error monitoring, tracing, and [User Feedback](../../user-feedback), you can use the following bundle:

```html {tabTitle: CDN}
<script
  src="https://browser.sentry-cdn.com/{{@inject packages.version('sentry.javascript.browser') }}/bundle.tracing.feedback.min.js"
  integrity="sha384-{{@inject packages.checksum('sentry.javascript.browser', 'bundle.tracing.feedback.min.js', 'sha384-base64') }}"
  crossorigin="anonymous"
></script>
```
...
- `bundle.tracing.feedback.<modifiers>.js` - Error monitoring, tracing, and user feedback
