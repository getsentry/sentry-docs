...
This approach involves initializing the SDK with the platform's specific options directly and is ideal for projects that require configuring options that are not yet supported by the KMP SDK such as experimental options.
