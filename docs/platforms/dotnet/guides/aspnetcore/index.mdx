...
When set to `true`, the SDK will detect blocking calls in async methods (e.g. the use of `Task.Wait()` or `Task.Result`) and report these to Sentry as errors. Blocking calls can lead to [Threadpool starvation](https://learn.microsoft.com/en-nz/archive/blogs/vancem/diagnosing-net-core-threadpool-starvation-with-perfview-why-my-service-is-not-saturating-all-cores-or-seems-to-stall).
...
This feature makes use of the `Ben.BlockingDetectore` library and so has [the same caveats](https://github.com/benaadams/Ben.BlockingDetector?tab=readme-ov-file#caveats):
...
> 2. Won't alert for blocking calls that don't block, like on precompleted Tasks (e.g. a single small Body.Write)
