...
Sentry provides an integration with ASP.NET Core through the [Sentry.AspNetCore NuGet package](https://www.nuget.org/packages/Sentry.AspNetCore).
...
When configuring the SDK via the frameworks configuration system, it's possible to add the SDK by simply calling `UseSentry` without providing any further information. The SDK provides extension methods to support ASP.NET Core `2.X`, `3.X`, and `5.X` and later:
