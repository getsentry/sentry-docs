...
When enabled, the SDK reports all historical tombstones from Android's exit reasons list, not just the latest one. Historical tombstones receive only minimal enrichment, as the available data is likely out of sync.
