...
```shell {tabTitle:.NET Core CLI} {"onboardingOptions": {"profiling": "2-3"}}
dotnet add package Sentry -v {{@inject packages.version('sentry.dotnet') }}
// Sentry profiling for .NET is available in Alpha on .NET 6.0+
dotnet add package Sentry.Profiling -v {{@inject packages.version('sentry.dotnet.profiling') }}
```

```powershell {tabTitle:Package Manager} {"onboardingOptions": {"profiling": "2-3"}}
...
```shell {tabTitle:Paket CLI} {"onboardingOptions": {"profiling": "2-3"}}
