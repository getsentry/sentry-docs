...
This layout is designed for PlayStation 5 (Prospero) symbol servers. It uses a simple
path structure based on the code file name and the ELF build ID.

Sony's `prospero-symupload` tool stores symbols using uppercase build IDs in the
directory path. Sentry will look for symbols using both lowercase and uppercase
build IDs, so symbols uploaded by Sony's tooling will be found automatically.

Only ELF files are supported with this layout.
