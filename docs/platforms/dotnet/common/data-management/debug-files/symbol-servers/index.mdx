...
When Sentry cannot find the symbols on your symbol server it will provide the details of what it attempted
in the "Images Loaded" section on an error event.  When you click on "details" of an image you can see
where it attempted to locate the debug file, under which file path and what happened.  You can see if the
information in a file is incomplete (for instance unwind information is missing) or if a 404 ocurred
when attempting to load the file.  You will need to turn on "Not Found" errors in the selector to see
all attempts of locating files.
