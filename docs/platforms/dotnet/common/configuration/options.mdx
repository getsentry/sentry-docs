...
Disables the `SentryHttpMessageHandler`, which normally instruments outgoing HTTP requests to create spans and propagate trace headers. Set this to `true` when using `OpenTelemetry.Instrumentation.Http` with `AddHttpClientInstrumentation()` to avoid duplicate spans — in that scenario, OpenTelemetry handles HTTP instrumentation and Sentry's handler is not needed.
