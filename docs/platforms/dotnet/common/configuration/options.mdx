...
A callback used to name transactions for requests that the routing system doesn't resolve to a route template (for example, requests that don't match any endpoint). It receives the current `HttpContext` and returns the transaction name to use, or `null`/`string.Empty` to fall back to the URL path.
...
<SdkOption name="DisableSentryHttpMessageHandler" type="bool" defaultValue="false">

Disables the `SentryHttpMessageHandler`, which normally instruments outgoing HTTP requests to create spans and propagate trace headers. Set this to `true` when using `OpenTelemetry.Instrumentation.Http` with `AddHttpClientInstrumentation()` to avoid duplicate spans — in that scenario, OpenTelemetry handles HTTP instrumentation and Sentry's handler is not needed. See <PlatformLink to="/tracing/instrumentation/opentelemetry/#usage">OpenTelemetry Support</PlatformLink> for more details.
