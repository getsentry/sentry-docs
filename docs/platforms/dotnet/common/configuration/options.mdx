...
If filtering based on exception types, the original exception (including any `AggregateException`) will be stored in `SentryEvent.Exception`. However Sentry's `MainEventProcessor` unpacks AggregateExceptions automatically and so what you will see in the Sentry UI will be the various inner exceptions. These are also available in the SDK by inspecting the `SentryEvent.SentryExceptions` collection.
