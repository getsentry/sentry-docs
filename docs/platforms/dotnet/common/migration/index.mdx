...
- The `User.IpAddress` is now set to `{{auto}}` by default, even when `sendDefaultPII` is disabled.
  - To prevent sending IP addresses, use the "Prevent Storing of IP Addresses" option in the ["Security & Privacy"](https://sentry.io/orgredirect/organizations/:orgslug/settings/projects/:projectId/security-and-privacy/) project settings on sentry.io.
