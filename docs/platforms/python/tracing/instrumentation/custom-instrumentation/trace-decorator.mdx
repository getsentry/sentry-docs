...
Adding the `@sentry_sdk.trace()` decorator to a function will create a [span](/concepts/key-terms/tracing/distributed-tracing/#traces-transactions-and-spans) for each execution of this function. The `name` of the span will be the function's fully qualified name and `op` will be set to `"function"`.
