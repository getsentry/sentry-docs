...
To change data in the current span, use ` sentry_sdk.get_current_scope().span`. This property will return a span if there's one running, otherwise it will return `None`.
