...
with sentry_sdk.start_span(
    ...
    span.set_data("mcp.session.id", "session_xyz789")
    span.set_data("mcp.transport", "pipe")
...
with sentry_sdk.start_span(
    op="mcp.resource",
    name=f"mcp.resource {resource_uri}",
) as span:
    # Set MCP-specific attributes
