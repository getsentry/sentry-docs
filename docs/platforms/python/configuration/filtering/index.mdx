...
Since the SDK sends to Sentry the exact payload that the callback returns, the callback can perform arbitrary modifications to the event payload, including adding data to the event and modifying or deleting existing event fields. It is even possible to use `before_send` to construct a completely different event payload from scratch.
