...
Since the SDK sends the same payload to Sentry that the callback returns, the callback can perform arbitrary modifications to the event payload, including adding data to the event and modifying or deleting existing event fields.
