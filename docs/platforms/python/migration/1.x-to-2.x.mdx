...
There is no stable 2.0 release of the Sentry Python SDK yet. A release candidate (`2.0.0rc3`) is available for testing. If you decide to give it a try, please [let us know](https://github.com/getsentry/sentry-python/issues) if you run into any issues!
...
<Alert level="warning" title="New scope-based APIs">

The SDK will take care of deciding what data belongs on which scope as long as you use the top-level API, so for instance `sentry_sdk.set_tag()` as opposed to `scope.set_tag()`. We recommend you use the top-level API even if you're creating your own scopes since setting data on the wrong scope might lead to unexpected results.
...
If you want your scope change to affect the whole transaction, use the `isolation_scope`.
...
If you want your scope change to affect a smaller unit of code such as a span, use the `current_scope`.
...
If you're wrapping a whole request-response cycle or a whole execution of a task, fork the `isolation_scope`:
...
If you only want to apply something to a smaller, localized part of the code, fork the `current_scope` with `new_scope()`:
