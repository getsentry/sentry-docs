...
- `name`: Required.
- `attributes`: See [Span Attributes](#span-attributes).
...
- `active`: Defaults to `True`. If set to `False`, the span will not be set as
  active, meaning that other spans won't consider it their parent, unless
  explicitly set.
...
name), `attributes`, and `active` flag.
...
types. Notably, `None` attribute values are not supported.

<Alert level="warning" title="Unsupported values">

If you set an attribute of an unsupported type (for example, an object), it will
be cast to string before it's set on the span. If you need access to specific
properties on an object or to the individual elements of a list, we recommend
picking the object apart into separate attributes.
...
Since spans no longer support tags, tags set on the scope with the
global `sentry_sdk.set_tag()` API will not be applied to spans. You can use the
the `sentry_sdk.set_attribute()` API to set attributes.
...
`sentry_sdk.traces.continue_trace()` works slightly differently than the legacy
...
Additionally, span streaming mode introduces a new
`sentry_sdk.traces.new_trace()` function that resets the trace.
...
`sampling_context["span_context"]["attributes"]`. You can provide attributes via
the `attributes` argument to `sentry_sdk.traces.start_span()`.

The sampling decision is made on `start_span(...)`.

As before, sampling is only applied to top-level spans. Children spans
...
Each span is matched against your `ignore_spans` rules when it's created. This
...
In practice, ignoring a span means it will be unsampled. This might lead to 
other spans in the same tree also being unsampled:
- If a top-level span is ignored, all of its children (its whole span
  tree) will be ignored as well.
- If a non-top-level span is ignored, its children will NOT be ignored by default,
  unless they themselves match an ignore rule.
