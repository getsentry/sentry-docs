...
Add `AsyncioIntegration()` to your list of `integrations`, enable tracing and be sure to call `sentry_sdk.init()` **at the beginning of the first `async` function you call**, as shown in the example below. `AsyncioIntegration` works by patching the current event loop, which is why it needs to be enabled when there's an event loop running.
