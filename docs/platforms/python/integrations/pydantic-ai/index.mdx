...
- `handled_tool_call_exceptions`:
  ...
  Option to capture tool call exceptions that Pydantic AI prevents from bubbling up. These include validation errors when the agent is configured to
  retry tool calls. All tool call exceptions reported when this option is `True` are handled errors in Sentry. This option has no effect on exceptions
  that are not handled by Pydantic AI.
