...
- If beat is running in your worker process (i.e., you're running your worker with the `-B`/`--beat` option), initialize Sentry in either he `celeryd_init` or the `beat_init` signal.
- If beat is running in a separate process, you need to initialize Sentry in *both* the `celeryd_init` and `beat_init` signals.
