...
- Sentry is configured not to send LLM and tokenizer inputs/outputs by default. If you want to include them, add the `include_prompts=True` flag to `OpenAIIntegration`, or set `send_default_pii=True` in the `sentry_sdk.init()` call.
...
sentry_sdk.init(
    ...
    integrations = [
        OpenAIIntegration(
            include_prompts=True, # now LLM/tokenizer inputs/outputs will be sent to Sentry
        ),
    ],
)
```
