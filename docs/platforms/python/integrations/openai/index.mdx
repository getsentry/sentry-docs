...
- Sentry is configured to send LLM and tokenizer inputs/outputs by default. If you want to exclude them, set the `exclude_prompts=True` flag.
...
sentry_sdk.init(
    ...
    integrations = [
        OpenAIIntegration(
            exclude_prompts=True, # now LLM/tokenizer inputs/outputs will not be sent to Sentry
        ),
    ],
)
```
