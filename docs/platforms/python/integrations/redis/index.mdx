...
If you, for example, set `cache_prefixes` to `["template.cache", "middleware.cache"]` access to all Redis keys starting with `template.cache` or `middleware.cache` will show up in the [cache-monitoring dashboard](https://sentry.io/orgredirect/organizations/:orgslug/performance/cache/).
