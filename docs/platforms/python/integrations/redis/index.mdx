...
You can specify a list of prefixes to Redis keys to define a key space that should be considered as cache. Cache keys will show up in the [cache-monitoring dashboard](https://sentry.io/orgredirect/organizations/:orgslug/performance/cache/). Giving you more insight into your caching strategy.

If you, for example, set `cache_prefixes` to `["template.cache", "middleware.cache"]` access to all Redis keys starting with `template.cache` or `middleware.cache` will show up in your [cache-monitoring dashboard](https://sentry.io/orgredirect/organizations/:orgslug/performance/caches/).
