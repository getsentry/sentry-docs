...
Regardless of how `failed_request_status_codes` is set, any exceptions raised by the handler, which are not of type `HTTPResponse` (or a subclass) are reported to Sentry. For example, if your request handler raises an unhandled `AttributeError`, the `AttributeError` gets reported to Sentry, even if you have set `failed_request_status_codes=set()`.
