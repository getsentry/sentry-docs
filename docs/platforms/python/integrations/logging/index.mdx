...
Some frameworks, such as `uvicorn`, may reset or override your `logging` configuration. To ensure your log level is active and logs are sent to Sentry, you can apply the `force=True` flag in `logger.setLevel(level, force=True)` or `logging.basicConfig(level=level, force=True)`.
