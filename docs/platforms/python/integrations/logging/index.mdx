...
Some frameworks, such as `uvicorn`, may reset or override your `logging` configuration. To ensure your log level is active and logs at or above the configured level are picked up by the SDK, you can use the `force=True` flag in `logger.setLevel(level, force=True)` or `logging.basicConfig(level=level, force=True)`.
