...
The short answer to the first two questions: check if your `contextvars` work and
you clone the hub where needed.
...
It might happen that your SDK might have issues sending events to Sentry.
In case of errors and tracing data, this manifests as errors and transactions
missing in Sentry. In case of cron monitors, you might be seeing crons being
marked as timed out in Sentry when you know they've run successfully. You might
also be seeing errors from the SDK about the connection getting reset or the
