...
To show inline source context in Sentry, `sentry-cli` can scan debug files for source references, resolves them locally, and creates a source bundle—an archive of all referenced source files. Run these commands to create the bundle(s):

```PowerShell {tabTitle:Windows}
sentry-cli debug-files bundle-sources .\bin\godot.windows.template_release.x86_64.pdb
Rename-Item .\bin\godot.windows.template_release.src.zip .\bin\godot.windows.template_release.x86_64.src.zip
```
...
```bash {tabTitle:Linux}
sentry-cli debug-files bundle-sources ./bin/godot.linux.template_release.x86_64.debugsymbols
mv bin/godot.linux.template_release.src.zip bin/godot.linux.template_release.x86_64.src.zip
```

To upload debug symbols and source bundles to Sentry, run:

```bash {tabTitle:Bash/PowerShell}
sentry-cli debug-files upload --org ___ORG_SLUG___ --project ___PROJECT_SLUG___ bin
```

This uploads debug information files from the `./bin` directory to Sentry.
