...
To show inline source context in Sentry, `sentry-cli` can scan debug files for source references, resolves them locally, and creates a source bundle—an archive of all referenced source files. `sentry-cli` can create such bundles on the fly during the upload of debug information files.

To upload the debug information files to Sentry with sources included on the fly, run:
...
<Expandable title="Generating source bundle archives">

You can also create the bundles separately:
