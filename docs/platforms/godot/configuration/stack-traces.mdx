...
# For Android development, you'll need both debug and release templates.
# The debug template is required for running the project from the editor in debug mode.
...
- `generate_android_binaries`: generate Android templates and Gradle template project after build is complete
...
**Android:** The build produces `android_source.zip`, which contains the Gradle build template with the compiled export template libraries, and `android-template-release-native-symbols.zip` & `android-template-debug-native-symbols.zip` archives containing the debug symbols.
