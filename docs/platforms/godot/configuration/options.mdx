...
If `true`, the SDK will capture logged errors as events, logs and/or breadcrumbs, as defined by [`godot_logger.event_mask`](#godot_logger.event_mask) and [`godot_logger.breadcrumb_mask`](#godot_logger.breadcrumb_mask) options. Crashes are always captured. See also the [`enable_logs`](#enable_logs) option.
