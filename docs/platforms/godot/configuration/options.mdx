...
<SdkOption name="enable_app_hang_tracking" type="bool" defaultValue="false">
...
`SentrySDK.logger`. Godot logger events listed in [`godot_logger.log_mask`](#godot_logger.log_mask) are also automatically captured as [Sentry Logs](/product/explore/logs/);
`godot_logger.log_mask` is empty by default, so no events are auto-captured. Use [`godot_logger.enabled`](#godot_logger.enabled) and the other
[`godot_logger` options](#godot-logger-options) to control how Godot's error messages and log output (including `print()` statements) are captured.
