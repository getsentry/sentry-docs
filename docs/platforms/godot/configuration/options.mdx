...
If `true`, enables [Sentry's structured logs](/product/explore/logs/) feature. When enabled, logs you record with the dedicated logging APIs through `SentrySDK.logger` are sent to Sentry. When disabled, those logging APIs remain safe to call, but they won't send logs.
...
Specifies the Godot logger events captured as [Sentry Logs](/product/explore/logs/). Accepts a single value or a bitwise combination of `GodotLoggerEventMask` masks. Empty by default, so no Godot logger events are captured as logs. Requires [`enable_logs`](#enable_logs) to be turned on.
