...
If assigned, this callback runs before an event is sent to Sentry. It takes `SentryEvent` as a parameter and returns either the same event object, with or without modifications, or `null` to skip reporting the event. You can assign it [programmatically](#programmatic-configuration). This can be used, for instance, for stripping PII before sending.
