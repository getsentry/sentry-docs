...
Disable **Auto Init** and register the callback during <PlatformLink to="/configuration/options/#programmatic-configuration">programmatic initialization</PlatformLink>. The following example drops transactions from editor runs and adds a tag to the remaining transactions. In C#, use `options.Native.SetBeforeSendTransaction` for transactions created by the native layer, including GDScript and GDExtension code:
