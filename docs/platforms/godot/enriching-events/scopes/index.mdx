...
A scope holds the data that rides along with your telemetry: [tags](../tags/), [contexts](../context/), [breadcrumbs](../breadcrumbs/), the [user](../identify-user/), the severity level, the fingerprint, and the attributes attached to <PlatformLink to="/logs/">logs</PlatformLink> and <PlatformLink to="/metrics/">metrics</PlatformLink>. `SentryScope` has a setter for each, plus `clear()` to wipe everything written to that scope.
...
The two layers don't share local scope data: what you write to a GDScript scope doesn't reach C# events, and what you write to a C# scope doesn't reach GDScript and engine events. Tags, breadcrumbs, and the user set globally still sync both ways. See <PlatformLink to="/dotnet/#how-the-two-layers-work-together">C#/.NET Support</PlatformLink> for what else crosses the boundary.
