...
`SentrySDK.with_span()` always forks the current scope and restores the previous scope when its callable returns. Starting an inactive span with `active` set to `false` doesn't fork or change the current scope.
