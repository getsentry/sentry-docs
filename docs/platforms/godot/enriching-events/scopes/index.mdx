...
A scope holds the data that rides along with your telemetry: [tags](../tags/), [contexts](../context/), [breadcrumbs](../breadcrumbs/), the [user](../identify-user/), the severity level, the fingerprint, and the attributes attached to [logs](/platforms/godot/logs/) and [metrics](/platforms/godot/metrics/). The `SentryScope` class exposes one method per kind of data — `set_tag()`, `set_context()`, `add_breadcrumb()`, `set_user()`, `set_level()`, `set_fingerprint()`, and `set_attribute()` — plus `clear()`, which wipes everything written to that scope.
