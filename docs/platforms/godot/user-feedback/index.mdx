...
For a quick implementation, add `user_feedback_gui.tscn` to your UI scene's `CanvasLayer` node and call `show()` to display the feedback form. The form automatically scales to different viewport resolutions and hides itself when the user clicks either "Submit" (which sends the feedback to Sentry) or "Cancel".
...
To customize the form's appearance, use the provided reference theme file (`sentry_theme.tres`). You can duplicate this file to create custom styling, then assign your theme to the `theme` property in the Inspector for the feedback UI control. It can be used to customize fonts, colors, separation, etc. For more advanced customizations involving layout changes, copy the entire `user_feedback/` folder into your project and modify the scenes and scripts as needed.
