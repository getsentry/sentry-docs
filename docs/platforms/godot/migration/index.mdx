...
By default, the Godot logger integration no longer sends Godot log output to [Sentry Logs](/product/explore/logs/). If you want to capture those logs, you must opt in by setting `godot_logger.log_mask` to the event types you want to send:
...
SentrySdk.Init(options =>
{
	options.GodotLogger.LogMask =
		GodotLoggerEventMask.Error | GodotLoggerEventMask.Script | GodotLoggerEventMask.Shader;
});
```
