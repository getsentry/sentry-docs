...
- **The global scope syncs both ways.** Tags, breadcrumbs, and the user you set in GDScript appear on C# events, and the ones you set in C# appear on GDScript and engine events. There's no need to set them twice.
...
- Data written to a local scope doesn't cross layers either. What you set inside GDScript's [`with_scope()`](/platforms/godot/enriching-events/scopes/) reaches only GDScript and engine events, and what you set inside a C# local scope reaches only C# events. Only the globally set tags, breadcrumbs, and user sync.
