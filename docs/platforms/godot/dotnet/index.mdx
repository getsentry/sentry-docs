...
- Local scope data doesn't cross layers: GDScript's [`with_scope()`](/platforms/godot/enriching-events/scopes/) reaches only GDScript and engine events, and a C# capture-method scope callback reaches only C# events. `ConfigureScope()` isn't local, so those writes sync like the global ones.
