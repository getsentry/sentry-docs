...
| Name | Type | Value |
| ---- | ---- | ----- |
| `DumpFolder` | `REG_EXPAND_SZ` | Path to a local directory (e.g., `<install-dir>\.sentry-native\wer`) |
...
```shell
reg add "HKLM\SOFTWARE\Microsoft\Windows\Windows Error Reporting\LocalDumps\MyApp.exe" /v DumpFolder /t REG_EXPAND_SZ /d "<install-dir>\.sentry-native\wer" /f
...
/* for each minidump_path in L".sentry-native/wer" */ {
