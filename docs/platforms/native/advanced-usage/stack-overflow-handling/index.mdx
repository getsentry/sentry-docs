...
it can no longer use stack variables or call functions. This results in a crashed handler and application without sending
...
the heap, where a handler overflow caused by a stack hungry `on_crash` implementation could lead to arbitrary heap 
corruption).
...
<Alert>
This means that on macOS, when using `breakpad` or `crashpad, ' a stack overflow does not require any different setup 
from other crashes.
</Alert>
