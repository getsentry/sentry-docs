...
`before_send_feedback` is called with the feedback object and a hint. Return the feedback to send it, or call `sentry_value_decref(feedback)` and return `sentry_value_new_null()` to drop it. The hint is always provided, so you can also use this hook to attach files to every feedback.
