...
This function is called with a feedback event and a hint, and can return a modified event, or `sentry_value_new_null()` to drop the feedback. The feedback itself is nested under the event's `contexts.feedback`. To drop it, call `sentry_value_decref(event)` first — the SDK doesn't release the event you were handed, so skipping this leaks it. Use the hint to add attachments.
