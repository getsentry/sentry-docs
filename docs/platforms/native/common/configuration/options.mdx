...
This option enables the [metrics integration](/platforms/native/metrics), which allows the SDK to capture metrics and send them to Sentry. When disabled, all calls to `sentry_metrics_count()`, `sentry_metrics_gauge()`, `sentry_metrics_distribution()`, and `sentry_scope_capture_metric()` are no-ops.

</SdkOption>

<SdkOption name="before_send_metric" type="function">

This function is called with an SDK-specific metric object, and can return a modified metric object, or `sentry_value_new_null()` to drop the metric. In that case, you **must** call `sentry_value_decref(metric)` before returning. An example can be found on the [Metrics page](/platforms/native/metrics/#options).
