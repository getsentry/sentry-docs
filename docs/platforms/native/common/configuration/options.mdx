...
This function is called with a user feedback object and a hint, and can return a modified feedback object, or `sentry_value_new_null()` to drop the feedback. To drop it, call `sentry_value_decref(feedback)` first — the SDK doesn't release the feedback you were handed, so skipping this leaks it. Use the hint to add attachments.

User feedback does not go through <PlatformIdentifier name="before_send" />, so this is the only hook that sees it. See [Filtering](/platforms/native/configuration/filtering/#using-before_send_feedback).
