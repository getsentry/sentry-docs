...
Two options control app-hang detection. See the [configuration reference](/platforms/native/configuration/options/) for full details.

| Option                                                                                          | Description                                                                                                                  |
| ----------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| [`enable_app_hang_tracking`](/platforms/native/configuration/options/#enable_app_hang_tracking) | Turns the watchdog on or off. Disabled by default.                                                                           |
| [`app_hang_timeout`](/platforms/native/configuration/options/#app_hang_timeout)                 | How long (in milliseconds) the watched thread can go without a heartbeat before an event is captured. Defaults to `5000` ms. |
