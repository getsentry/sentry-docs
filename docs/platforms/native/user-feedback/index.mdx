...
Context that describes a single piece of feedback belongs on a scope rather than the global scope, where it would apply to every subsequent event. `sentry_scope_capture_feedback()` layers that scope on top of the global scope and returns the feedback event ID.
...
The scope constructor determines who frees the scope; see <PlatformLink to="/enriching-events/scopes/#creating-a-scope">Scopes</PlatformLink>.
