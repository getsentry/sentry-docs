...
This applies to the default transaction mode. In stream mode, there's no separate transaction name — set the span name when you create it with `Sentry.startSpan('name', ...)`. See <PlatformLink to="/tracing/new-spans/">New Spans</PlatformLink>.
