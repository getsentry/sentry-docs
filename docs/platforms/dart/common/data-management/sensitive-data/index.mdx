...
description: "Learn about filtering or scrubbing sensitive data within the Sentry Dart SDK, so that data is not sent with the event. You can also configure server-side scrubbing to ensure the data is not stored."
sidebar_order: 100
keywords:
  - pii
  - gdpr
