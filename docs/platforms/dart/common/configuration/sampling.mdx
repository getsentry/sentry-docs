---
title: Sampling
description: "Learn how to configure the volume of error and transaction events sent to Sentry using the Dart SDK."
sidebar_order: 50
---
...
If `tracesSampler` throws an exception, the SDK falls back to the inherited sampling decision in transaction mode when one is available, then to `tracesSampleRate`. In stream mode, it falls back to `tracesSampleRate`. Without an applicable inherited decision or configured sample rate, the trace is not sampled. Return `0.0` from the sampler to reject a trace explicitly.
