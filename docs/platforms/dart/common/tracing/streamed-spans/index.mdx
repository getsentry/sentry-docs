...
Only **root spans** are sampled and child spans inherit the root span's decision. When a root span isn't sampled, its callback still executes with a no-op span, so all span operations remain safe to call.
...
- **Check the envelopes**: With `options.debug = true` or network inspection, check if the span envelopes are sent with the content type `application/vnd.sentry.items.span.v2+json` instead of transaction envelopes.
