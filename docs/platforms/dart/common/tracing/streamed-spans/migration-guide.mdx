...
+   span.setAttribute('sentry.op', 'task');
+   await runCheckout();
+ },
+ parentSpan: null,
+);
...
+   checkoutSpan.setAttribute('sentry.op', 'task');
...
Tags set on the scope with (`Sentry.configureScope((scope) => scope.setTag(...))`) aren't applied to spans in stream mode. Use `Sentry.setAttributes(...)` to set attributes that apply to your spans instead.
