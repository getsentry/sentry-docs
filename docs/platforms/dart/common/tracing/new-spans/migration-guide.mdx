...
Tags set on the scope (`Sentry.configureScope((scope) => scope.setTag(...))`) aren't applied to spans in stream mode. Use `Sentry.setAttributes(...)` to set attributes that apply to your spans instead.
