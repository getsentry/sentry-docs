...
You can set any `SentrySpanStatusV2` value: `ok`, `error`, `cancelled`, or `deadlineExceeded`. When you don't set it, the status defaults to `ok`, or `error` if the callback throws.
