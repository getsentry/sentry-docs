...
## Include a Session Replay

To include the activity leading up to user feedback, enable buffered replays in Sentry Flutter SDK `9.0.0` or later:
...
The SDK keeps up to 30 seconds of replay activity on the device for every session not selected by `sessionSampleRate`. It sends the buffer when your app calls `Sentry.captureFeedback()` or opens the built-in `SentryFeedbackForm`. Capturing at form open preserves the activity that led the user to report the problem instead of recording only their interactions with the form.

Buffered replays are available on iOS and Android. To adjust the percentage of sessions buffered, set `onErrorSampleRate` between `0.0` and `1.0`. See [Session Replay sampling](/platforms/dart/guides/flutter/session-replay/#sampling) for details.
