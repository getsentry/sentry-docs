...
### `SentryFlutter.init` throws an error `sentry_init` failed

On Linux, `SentryFlutter.init` automatically tries to use the crashpad backend.
There are cases where the Flutter tooling overrides the permission of the crashpad binary therefore failing the initialization.
