...
- If you enable the `split-debug-info` and `obfuscate` features, you must upload [debug symbols](/platforms/dart/guides/flutter/debug-symbols/).
- Issue titles might be obfuscated (or minified on web) as we rely on the `runtimeType`, but they may not be human-readable. For iOS and Android, follow the [Sentry Dart Plugin guide](/platforms/dart/guides/flutter/debug-symbols/) to set up the obfuscation map which allows us to deobfuscate the issue tile. We’re currently exploring a solution for Web.
...
Source Context support requires compiling your app using the `split-debug-info` build parameter on Flutter `3.10.0` and above. You must also upload [debug symbols](/platforms/dart/guides/flutter/debug-symbols/) with the `upload_sources` option enabled.

## Sentry Dart Plugin

If you are using the Sentry Dart Plugin to upload [Debug Symbols](/platforms/dart/guides/flutter/debug-symbols/#automatically-upload-debug-symbols), refer to the points below to resolve potential issues.
...
In v10, the Sentry Flutter SDK uses Breakpad for native crash reporting on Linux and Windows. Crash reports are uploaded when the application starts again. Restart the application after a test crash to verify delivery.

Breakpad handles crashes in the application process, without Crashpad's out-of-process isolation. It also lacks Crashpad's Windows Error Reporting (WER) and fast-fail coverage.

To use Crashpad, set `SENTRY_NATIVE_BACKEND=crashpad` in your local build environment and CI before building the application:
...
This is a build-time setting. An unset or empty `SENTRY_NATIVE_BACKEND` keeps the default backend; it does not disable native crash reporting.
...
Sentry Flutter SDK v10 requires Swift Package Manager on iOS and macOS. CocoaPods cannot install the v10 Sentry Flutter plugin. Enable Swift Package Manager and set the deployment targets to iOS 15.0 or macOS 12.0 or later, as described in <PlatformLink to="/manual-setup/#apple-platform-setup">Apple Platform Setup</PlatformLink>.

### `SentryFlutter.init` Throws a `sentry_init failed` Error

If you build your Linux app with `SENTRY_NATIVE_BACKEND=crashpad`, Flutter tooling can override the permissions of the Crashpad handler binary, causing initialization to fail. This workaround does not apply to the default Breakpad backend.
...
## Which Native SDK Versions Are Bundled With Each Flutter SDK Release?
