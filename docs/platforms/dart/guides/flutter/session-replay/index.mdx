...
If you prefer not to record an entire session, you can elect to capture a replay only when an error occurs or when a user submits feedback. In this case, the integration will buffer up to 30 seconds worth of events prior to the trigger. It will continue to record the session, following the rules above regarding session life and activity. For the built-in <PlatformLink to="/user-feedback/#sentryfeedbackform">`SentryFeedbackForm`</PlatformLink>, the buffer is sent when the form opens, so the replay shows what happened before the user started entering feedback. Read the [sampling](#sampling) section for configuration options.
