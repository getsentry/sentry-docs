...
| ----------------- | ------------------------------------------------------------------------------------------------------------------ | ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| sessionSampleRate | `double`                                                                                                           | `0`                          | The sample rate for replays that begin recording immediately and last the entirety of the user's session. `1.0` automatically records every session, and `0` disables automatic session recording.                                                                                                             |
...
In v10, set both `sessionSampleRate` and `onErrorSampleRate` to `0.0` to prevent automatic recording and error-triggered uploads. Explicit `SentryFlutter.replay.start()` and `startBuffering()` calls bypass session sampling. To send a manually started buffer when error sampling is zero, call `SentryFlutter.replay.flush()`. See <PlatformLink to="/session-replay/#manual-control">Manual Control</PlatformLink> for setup and lifecycle examples.
