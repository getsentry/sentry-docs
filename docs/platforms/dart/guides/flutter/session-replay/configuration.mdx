...
| onErrorSampleRate | `double` | `0`     | The sample rate for replays that are recorded when an error happens or when a user submits feedback. This type of replay will record up to 30 seconds of events prior to the trigger and continue recording until the session ends. `1.0` captures all sessions with an error or feedback, and `0` captures none. |
