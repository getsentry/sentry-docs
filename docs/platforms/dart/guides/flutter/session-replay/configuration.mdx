...
| onErrorSampleRate | `double` | `0`     | The sample rate for buffered replays sent when the SDK captures an error or user feedback. The replay includes up to 30 seconds of activity before the trigger and continues recording until the session ends. `1.0` buffers every session not selected by `sessionSampleRate`, and `0` disables buffering. |
