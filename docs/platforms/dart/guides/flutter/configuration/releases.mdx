...
On web, v10 uses Sentry JavaScript SDK 11. Unhandled errors captured from Dart or JavaScript set the browser session status to `unhandled` instead of `crashed`. Unlike the native non-terminating behavior above, `unhandled` is a terminal status for browser sessions.

Account for these changes when comparing crash-free metrics and alerts before and after upgrading.
