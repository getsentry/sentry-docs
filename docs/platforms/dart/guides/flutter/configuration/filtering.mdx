...
In v10, an exception in an event processor or a filtering callback drops the affected item instead of sending it unchanged. This applies to event processors, `beforeSend`, `beforeSendTransaction`, `beforeSendFeedback`, `beforeBreadcrumb`, `beforeSendLog`, and `beforeSendMetric`.

`beforeSendSpan` behaves differently: the SDK logs the exception and does not drop the span.

Handle expected failures inside these callbacks and return the intended item or `null` explicitly. A throwing `tracesSampler` uses the <PlatformLink to="/configuration/sampling/#precedence">sampling fallback</PlatformLink>; return `0.0` to reject a trace explicitly.
