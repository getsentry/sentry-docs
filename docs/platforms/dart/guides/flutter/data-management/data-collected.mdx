...
- **The size of the request body:** There's a <PlatformLink to="/configuration/options/#maxRequestBodySize">maxRequestBodySize</PlatformLink> option that's set to `MaxRequestBodySize.never` by default. This means by default no request body is sent to Sentry.
...
In v10, widgets marked as sensitive with Flutter's `SensitiveContent` widget are also masked by default. Review your app's replay output and configure <PlatformLink to="/session-replay/privacy/">Replay privacy settings</PlatformLink> for your content.
