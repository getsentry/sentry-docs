...
Max response body APIs have been removed. Due to PII concerns, response bodies will no longer be added to Sentry events by the SDK. 
...
In version `9.0.0`, only the `dio` integration is supported.
