...
- Usage of `dart:html` has been removed in favor of `package:web`. The SDK is now packaged with the `package:web` dependency for better interopability with web APIs.

#### User Feedback

The old user feedback API has been removed. The new API is `Sentry.captureFeedback`.

#### Response Body Handling

Max response body APIs have been removed. Due to PII concerns, response bodies will no longer be added to Sentry events by the SDK. Responses are now attached to the `Hint` object, which can be read in `beforeSend`/`beforeSendTransaction` callbacks via `hint.response` so you can manually attach the response to your event. In version `9.0.0`, only the `dio` integration is supported.

#### Screenshots

Screenshots are now masked by default. For example, text fields are automatically masked to prevent sensitive information from being leaked.

#### Logging

The default log level is now `warning` when `debug = true`. This can be adjusted by setting `options.diagnosticLevel = SentryLevel.info`.

#### SDK Data Classes

SDK data classes are now mutable. For backwards-compatibility, `copyWith` and `clone` can still be used but are officially deprecated.
