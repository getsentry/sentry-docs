...
In older versions, the SDK failed to provide line numbers and source context in combination with Cysharp. The SDK relies on Unity's IL2CPP backend to provide instruction addresses for Sentry to generate line numbers in events. When using Cysharp, those instruction addresses would default back to `0x0` once the stacktrace goes into the library.
