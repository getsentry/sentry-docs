...
When targeting WebGL, the SDK has specific limitations due to the platform's unique characteristics. For complete details about WebGL support and configuration requirements, see the [WebGL section in Native Support](/platforms/unity/native-support/#webgl).

The most significant limitation is that line numbers are not available because the IL2CPP backend required for symbolication is not available on WebGL.
