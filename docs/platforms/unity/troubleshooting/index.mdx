...
|---------|---------------|---------|
| `YourClass+<>c` | Cached delegate class for simple lambdas | `CompilerGeneratedDelegateSamples+<>c in ThrowExceptionInLinq { <lambda> }` |
| `YourClass+<>c__DisplayClass#_#` | Display class for closures that capture local variables | `CompilerGeneratedDelegateSamples+<>c__DisplayClass12_0 in ThrowExceptionInClosure { <lambda> }` |
...
```
CompilerGeneratedDelegateSamples+<>c in ThrowExceptionInLinq { <lambda> }
```
