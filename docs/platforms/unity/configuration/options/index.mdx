...
Enables Cocoa app hang detection on iOS. When the main thread is unresponsive for longer than <PlatformIdentifier name="AppHangTimeout" />, the SDK captures an `AppHang` event with a native stack trace.
...
<SdkOption name="Experimental.EnableNativeAppHangTracking" type="bool" defaultValue="false">

Enables experimental sentry-native app hang tracking on macOS, Windows, and Linux. The native watchdog captures a stack trace from the stalled Unity main thread and disables the managed Unity watchdog to avoid duplicate reports.

On macOS, set `Experimental.MacosBackend` to the native backend.

</SdkOption>

<SdkOption name="AnrDetectionEnabled" type="bool" defaultValue="true">
