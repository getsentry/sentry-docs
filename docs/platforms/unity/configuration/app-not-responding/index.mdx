...
On WebGL, ANR detection uses a single coroutine with a `Stopwatch` to measure elapsed time. When the timeout is exceeded, it reports the ANR event.

**Limitation**: ANR detection requires the game to recover from the not-responding state to report the event.
