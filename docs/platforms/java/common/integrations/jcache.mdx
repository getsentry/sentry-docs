...
sentryCache.clear();                   // cache.flush span
```

## Traced Operations

| JCache Method(s) | Span Op |
|---|---|
| `get`, `getAll`, `invoke`, `invokeAll` | `cache.get` |
| `put`, `getAndPut`, `putAll` | `cache.put` |
| `remove`, `getAndRemove`, `removeAll(keys)` | `cache.remove` |
| `clear`, `removeAll()` | `cache.flush` |

Conditional writes — `putIfAbsent`, `replace`, `getAndReplace` — are **not** traced because the outcome (write vs no-op) is not known ahead of time.

## Span Data

| Key | Type | Description |
|---|---|---|
| `cache.hit` | boolean | Whether the cache lookup returned a value (`cache.get` spans only) |
...
ITransactionContext txCtx = new TransactionContext("test-cache", "task");
ITransaction tx = Sentry.startTransaction(txCtx);
Sentry.getCurrentScopes().getScope().setTransaction(tx);
