...
Sentry's Kafka integration lets you trace Kafka producer sends and consumer record processing. In Spring Boot, this works automatically. With raw `kafka-clients`, you instrument producers and consumers with `sentry-kafka`. Queue spans appear in Sentry's [Queues dashboard](https://sentry.io/orgredirect/organizations/:orgslug/insights/backend/queues/).
...
implementation 'io.sentry:sentry-kafka:{{@inject packages.version('sentry.java.kafka', '8.37.0') }}'
...
<dependency>
    ...
    <version>{{@inject packages.version('sentry.java.kafka', '8.37.0') }}</version>
</dependency>
<dependency>
...
That's it. Every `KafkaTemplate.send(...)` call produces a `queue.publish` span, and every `@KafkaListener` method produces a `queue.process` transaction. Sentry injects propagation headers (`sentry-trace`, `baggage`) into outgoing records and continues the trace on the consumer side.
...
- **Not active with OpenTelemetry.** When using the Sentry OpenTelemetry agent or `opentelemetry-spring-boot-starter` alongside Sentry, OpenTelemetry's messaging instrumentation provides the Kafka spans instead.
