...
Now every `KafkaTemplate.send(...)` call produces a `queue.publish` span if there's a transaction running, and every `@KafkaListener` method produces a `queue.process` transaction. Sentry injects propagation headers (`sentry-trace`, `baggage`) into outgoing records and continues the trace on the consumer side.
