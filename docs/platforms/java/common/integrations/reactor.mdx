...
Due to how Reactor works under the hood, if you don't use this integration, you might run into issues such as unrelated breadcrumbs appearing the events that are sent to Sentry within the context of Reactive Streams.
...
Then, enable the `SentryReactorThreadLocalAccessor`:
```java
