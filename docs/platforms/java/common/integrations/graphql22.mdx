...
If you're using version 21 or older of `graphql-java`, please refer to <PlatformLink to="/integrations/graphql/">GraphQL docs</PlatformLink>.
...
## Control GraphQL Content

GraphQL documents, variables, and supported request and response bodies can contain sensitive application data. Use <PlatformLink to="/configuration/options/#dataCollection">Data Collection</PlatformLink> to control them independently:
...
Sentry.init(
    options -> {
      options.getDataCollection().getGraphql().setDocument(false);
      options.getDataCollection().getGraphql().setVariables(false);
      options.getDataCollection().setHttpBodies(Collections.emptySet());
    });
```

Configuring any field activates the Data Collection defaults for omitted fields. Operation metadata used for tracing and grouping can still be collected when document or variable content is disabled.
