...
`FunctionTimer` doesn't expose individual durations, so the integration can't produce a distribution or percentiles for it. To calculate a weighted mean, divide the sum of `${exportedName}.total_time` by the sum of `${exportedName}.count`.
