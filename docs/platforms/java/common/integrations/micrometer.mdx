...
Call `recordCompletedOrder`, then open [Metrics in Sentry](/product/metrics/) and query for `checkout_completed`. The default naming convention converts dots in metric names to underscores. The `payment_method` tag is available as a metric attribute.

## Metric Mappings

The registry applies its configured Micrometer `NamingConvention` to metric names, tag keys, and tag values. The default convention converts dots to underscores.
