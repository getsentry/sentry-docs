...
When you call a global function such as `Sentry.captureException`, internally Sentry discovers the active scopes and uses them when capturing the event.
...
Global scope, isolation scope and current scope are combined before an event (like an error or transaction) is sent to Sentry.
