...
## Control URL Query Parameters

OpenFeign adds URLs to HTTP spans and breadcrumbs. Use <PlatformLink to="/configuration/options/#dataCollection">`dataCollection.urlQueryParams`</PlatformLink> to filter or omit their query parameters:
...
Configuring this field activates the Data Collection defaults for omitted fields. The base URL is still sent when query parameter collection is disabled.
