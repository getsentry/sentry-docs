...
When using `org.springframework.boot:spring-boot-docker-compose` mind that this extension overwrites the `spring.datasource.url` property and therefore removes the `p6spy` prefix. Therefore if you want to test this locally you need to disable/remove spring-boot-docker-compose
