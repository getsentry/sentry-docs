...
Add `Interceptors` to `ApolloClient.Builder` using `SentryApollo4BuilderExtensions`:

```java
import com.apollographql.ApolloClient;
import io.sentry.apollo4.SentryApollo4BuilderExtensionsKt;

ApolloClient apollo = SentryApollo4BuilderExtensionsKt
...
ApolloClient apollo = SentryApollo4BuilderExtensionsKt.sentryTracing(
