...
---
Sentry currently does not provide any automatic instrumentation for messaging queue systems in Java at this moment. In the meantime, you can still instrument custom spans and transactions around your queue producers and consumers to ensure that you have performance data about your messaging queues.
...
Your `queue.process` span must exist inside a transaction in order to be recognized as a consumer span. You can start a new transaction in Java using `Sentry.startTransaction()`.
...
<Note>
Depending on the messaging system you are instrumenting, you may need to adjust your `sentry-trace` header key to a valid format. If you are using JMS, the `sentry-trace` header should be transported with a  different name (e.g. `sentry_trace`) since `-` characters are not permitted in JMS property names.
</Note>
