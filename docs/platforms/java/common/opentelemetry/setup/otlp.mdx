...
dependencies {
    implementation platform('io.sentry:sentry-opentelemetry-bom:{{@inject packages.version('sentry.java', '8.49.0') }}')
}
```
...
<dependencyManagement>
    <dependencies>
        ...
        <dependency>
            ...
            <version>{{@inject packages.version('sentry.java', '8.49.0') }}</version>
            <type>pom</type>
            <scope>import</scope>
        </dependency>
    </dependencies>
...
dependencyManagement {
    imports {
        mavenBom 'io.sentry:sentry-opentelemetry-bom:{{@inject packages.version('sentry.java', '8.49.0') }}'
    }
}
```
...
The OTLP endpoint and authentication details are shown in the code examples below. You can also find the **OTLP Traces Endpoint** and **OTLP Traces Endpoint Headers** in your Sentry project under **Settings > Projects > [Project] > [Client Keys (DSN)](https://sentry.io/orgredirect/organizations/:orgslug/settings/projects/:projectId/keys/)**.
