...
For Spring and Spring Boot, incoming request body content is limited to JSON and form bodies that meet the configured <PlatformLink to="/configuration/options/#maxRequestBodySize">`maxRequestBodySize`</PlatformLink>. The default is `NONE`, so incoming request body content isn't collected until this option is configured.
