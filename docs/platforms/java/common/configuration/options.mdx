...
File paths and unencoded Logback messages aren't Data Collection categories. OpenTelemetry-derived HTTP header attributes also aren't controlled by Data Collection yet. These paths continue to use `sendDefaultPii`.
...
Environment variables use the same names in uppercase with the `SENTRY_` prefix, for example, `SENTRY_DATA_COLLECTION_USER_INFO` and `SENTRY_DATA_COLLECTION_HTTP_BODIES`.
