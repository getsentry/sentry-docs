...
For request bodies to be captured, the <PlatformIdentifier name="send-default-pii" /> option must also be set to `true`.
Please consult the documentation for <PlatformIdentifier name="send-default-pii" /> to evaluate whether you would want to enable it, making sure to manually remove the data you don't want to send using our features for managing [_Sensitive Data_](../../data-management/sensitive-data/).
