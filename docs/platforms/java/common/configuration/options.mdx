...
Whether cache operations (`get`, `put`, `remove`, `flush`) should be traced. When enabled, the SDK creates spans for cache operations performed through the <PlatformLink to="/integrations/jcache/">JCache integration</PlatformLink> or Spring Cache (`@Cacheable`, `@CachePut`, `@CacheEvict`). This is opt-in because cache operations can be high-volume, especially with in-memory caches.

In Spring Boot, set `sentry.enable-cache-tracing=true` in `application.properties`.
