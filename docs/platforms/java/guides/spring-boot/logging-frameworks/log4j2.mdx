...
Auto-configuration only runs when Log4j 2 Core is the active logging backend. If Log4j 2 API calls are routed through SLF4J to another backend, such as Logback, the appender isn't added.
...
Spring Boot 2 doesn't support `SentryAppender` auto-configuration, and errors logged before the Spring context refreshes aren't captured by it. In both cases, configure the appender yourself and leave `sentry.logging.enabled` unset.
