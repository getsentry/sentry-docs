...
When a custom `SentryAppender` uses an encoder, Sentry sends the encoded message and omits the original message template and parameters by default. Set `<includeUnencodedMessage>true</includeUnencodedMessage>` directly on the appender to include them without enabling the broader `sendDefaultPii` option. See the [Logback configuration example](/platforms/java/guides/logback/#encoded-messages).
