...
To record the user's IP address and `Principal#name` as the username, enable automatic user information through Data Collection:

```properties {tabTitle:application.properties}
sentry.data-collection.user-info=true
```
...
Configuring this field activates the <PlatformLink to="/configuration/options/#dataCollection">Data Collection defaults</PlatformLink> for omitted fields.
