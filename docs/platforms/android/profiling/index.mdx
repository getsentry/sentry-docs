...
<Alert level="info" title="Looking for the old profiler?">
  On devices running below Android 15, the SDK automatically falls back to the
  legacy profiler, which samples threads using the Android runtime's `tracer`.
  That implementation, along with the transaction-based profiler, is now
  considered legacy. See{" "}
  <PlatformLink to="/profiling/legacy">Legacy Profiling</PlatformLink> for
  details, including how to disable it.
</Alert>
