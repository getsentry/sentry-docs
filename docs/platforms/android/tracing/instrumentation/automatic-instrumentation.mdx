...
For the first Activity in a cold start, the measurement includes work performed before the Activity is created. This means code in `Application.attachBaseContext()` and `Application.onCreate()` contributes to TTID.
