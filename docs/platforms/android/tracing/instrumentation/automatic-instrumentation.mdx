...
sidebar_order: 10
description: "Learn what transactions are captured after tracing is enabled in the Sentry Android SDK."
og_image: >-
...
The transaction starts before each Activity's `onCreate` method is called. For the first Activity in an app launch, the SDK may use an earlier app-start timestamp.
...
For example, when the SDK uses process-start timing, the duration includes time spent initializing Content Providers and ends with the first rendered frame.

</Alert>

The SDK sets the Span operation to `app.start.cold` for _Cold start_ and `app.start.warm` for _Warm start_.

On Android 7.0 (API 24) and later, the SDK can use process start as the beginning of an app start when Performance V2 is enabled. If that timestamp is unavailable or Performance V2 is disabled, it falls back to the SDK initialization time, usually recorded by `SentryPerformanceProvider`. The measurement ends when the first frame is drawn.
...
Unlike the `app.start.cold` and `app.start.warm` spans described above, standalone app start tracing uses the `app.start` operation for its transaction. You can use a custom `tracesSampler` to set a dedicated sample rate for app starts without increasing your overall sample rate:
...
Call `Sentry.extendAppStart()` in `Application.onCreate`, after the SDK is initialized, so the SDK doesn't automatically finish the app start transaction when the first frame is drawn. With automatic initialization, the SDK is ready before `Application.onCreate` runs. If you initialize the SDK manually, call `SentryAndroid.init()` before `Sentry.extendAppStart()`.

Optionally, retrieve the extended app start span with `Sentry.getExtendedAppStartSpan()` to add child spans that break down the extended launch period. Perform your launch-time work, finish any child spans, and call `Sentry.finishExtendedAppStart()` when your app is fully ready.
...
public final class MyApplication extends Application {
  ...
  public void onCreate() {
    ...
    ISpan child =
        extendedSpan != null
            ? extendedSpan.startChild("preload", "Preload resources")
            : null;
...
class MyApplication : Application() {
  override fun onCreate() {
    ...
    // Optionally, retrieve the extended app start span to attach your own child spans
    val extendedSpan = Sentry.getExtendedAppStartSpan()
    val child = extendedSpan?.startChild("preload", "Preload resources")
...
These examples use `Sentry.finishExtendedAppStart()` to finish the extended app start. Alternatively, if you retrieved the extended span, you can call `finish()` on it directly. Both methods finish the same extension, so use only one.

Finish any child spans first, then finish the extended app start once your launch-time work is done. By default, if the extended app start isn't finished within the 30-second [deadline timeout](/platforms/android/configuration/options/#deadlineTimeout), the SDK finishes the extended span with `DEADLINE_EXCEEDED` and suppresses the cold or warm app start measurement. The standalone `app.start` transaction and its spans are still sent.
...
Extending the app start requires standalone app start tracing to be enabled. If `enableStandaloneAppStartTracing` is `false`, or the app start transaction was already created, `Sentry.getExtendedAppStartSpan()` returns `null` and `Sentry.finishExtendedAppStart()` is a no-op.
...
For more information see our [Apollo integration](/platforms/android/integrations/apollo4/).
...
The Plugin injects a code snippet that starts a span out of the active span bound to the scope for each `CRUD` operation. The SDK sets the span `operation` to `db.sql.query` for SQLite calls (or `db.sql.room` for Room DAO methods), and the span `description` to the SQL query when available. (For Room DAOs, the description is the DAO class name.)
...
When the operation throws an `Exception`, Sentry's SDK attaches the exception to the running span and rethrows it. If the exception isn't caught by your code and is captured by Sentry, the span and SentryEvent will be linked when viewing it on the **Issue Details** page in sentry.io.

For more information see our [SQLite instrumentation](/platforms/android/integrations/room-and-sqlite/).
...
For the first Activity in a cold start, the TTID span starts at the app-start timestamp rather than just before `Activity.onCreate()`. On Android 7.0 and later, with Performance V2 enabled, the SDK can use process start for this timestamp.

When it does, TTID includes `Application.attachBaseContext()` and `Application.onCreate()`. If the SDK falls back to its initialization timestamp, work before SDK initialization may not be included.

On Android 12 and later, Sentry does not separately instrument the system splash screen. TTID still finishes after the Activity draws its first frame, so any delay that postpones that frame contributes to TTID.
