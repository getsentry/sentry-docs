...
For the first Activity in a cold start, the TTID span starts at the app-start timestamp rather than just before `Activity.onCreate()`. On Android 7.0 and later, with Performance V2 enabled, this timestamp comes from process start and includes `Application.attachBaseContext()` and `Application.onCreate()`. If the SDK falls back to its initialization timestamp, work before SDK initialization may not be included.
