...
Retrieve the extended app start span with `Sentry.getExtendedAppStartSpan()` to optionally add child spans that break down the extended launch period. Perform your launch-time work, finish any child spans, and call `Sentry.finishExtendedAppStart()` when your app is fully ready.
...
These examples finish the extended app start by calling `finish()` on the span returned by `Sentry.getExtendedAppStartSpan()`. Alternatively, call `Sentry.finishExtendedAppStart()` if you don't need the span reference. Both methods finish the same extension, so use only one.
