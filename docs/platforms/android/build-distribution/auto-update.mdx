...
| `installGroups` | Set of group names used to tag the build at upload **and** filter available updates at runtime | `[]` (empty set) |
...
Install groups let you control which builds the Auto-Update SDK considers as available updates. When configured, the SDK only sees builds that share at least one install group with the current app. See the [Install Groups guide](/platforms/android/build-distribution/install-groups/) for full upload instructions.
...
The Gradle plugin handles both sides automatically — it tags the uploaded build with these groups **and** embeds them in the app so the SDK sends them when checking for updates.

### Programmatic Override

You can override install groups at runtime if you need dynamic control (for example, based on a remote config or user setting):
...
This only affects which updates the SDK requests — it does not change the groups the build was uploaded with.
