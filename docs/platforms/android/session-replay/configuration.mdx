...
The SDK exposes the following main options for Session Replay:
...
By default, Replay will capture basic information about all outgoing http requests in your application. This includes the URL, request and response body size, method, and status code. The intention is to limit the chance of collecting private data.
...
### Requirements
- SDK version >= [8.27.x](https://github.com/getsentry/sentry-java/releases/tag/8.27.1)
- OkHttp

**Important:** Network details uses `SentryOkHttpInterceptor` under the hood to instrument http requests. The [Sentry gradle plugin](/platforms/android/configuration/gradle/) handles adding necessary dependencies and so works out of the box. Projects that don’t use the plugin will include the interceptor themselves
`val client = OkHttpClient.Builder().addInterceptor(SentryOkHttpInterceptor()).build()`:
