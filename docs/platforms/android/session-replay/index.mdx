...
Q: **How can I debug which UI elements are being masked?**

A: You can enable a debug overlay that highlights which UI widgets are masked. Call `Sentry.replay().enableDebugMaskingOverlay()` to turn the overlay on and `Sentry.replay().disableDebugMaskingOverlay()` to turn it off. These methods must be called after the SDK is initialized. The masks will be invalidated at most once per frameRate (default 1 fps).
