...
When a variant is excluded via `ignoredVariants`, `ignoredBuildTypes`, or `ignoredFlavors`, most Sentry Gradle plugin tasks are disabled for that variant — including [Size Analysis](/platforms/android/size-analysis/) and [Build Distribution](/platforms/android/build-distribution/) uploads. Snapshot uploads are the exception and will still run for excluded variants. To upload builds for a variant, it must not be excluded by these filters. To turn off only bytecode features (tracing instrumentation or runtime optimizations) for some variants while keeping mapping uploads and other plugin tasks, use [Per-variant Feature Overrides](#per-variant-feature-overrides) instead.

</Alert>

### Per-variant Feature Overrides

Use the `variants` block when you want to change Sentry behavior for one Android variant without disabling the whole plugin for that variant.

Matching uses the **full AGP variant name** (the same suffix as `assemble{Variant}`). Examples:

- No product flavors: `debug`, `release`
- Flavor `staging` + build type `debug`: `stagingDebug`
- Multiple flavor dimensions: camelCase join in dimension order, such as `stagingFreeDebug`

Today you can override:

- `tracingInstrumentation.enabled`
- `runtimeOptimizations.enabled`

Unset override properties inherit the global `sentry { ... }` defaults. Top-level `ignoredVariants` / `ignoredBuildTypes` / `ignoredFlavors` still mean “Sentry off for this variant” and take precedence as a hard disable.
...
sentry {
    ...
    // Disable bytecode features on debug only; mapping upload and other tasks stay enabled.
    variants {
        debug {
            tracingInstrumentation {
                enabled = false
...
sentry {
    ...
    // NamedDomainObjectContainer: use create("...") or maybeCreate("...") in Kotlin DSL.
    variants {
        create("debug") {
            tracingInstrumentation {
                enabled.set(false)
...
With product flavors, list each full variant you care about:
...
There is no build-type-wide shortcut in this API yet (for example “all `debug` variants”). Name each full variant under `variants { }`, or use top-level `ignoredBuildTypes` only if you intend to disable nearly all Sentry Gradle plugin behavior for that build type.
