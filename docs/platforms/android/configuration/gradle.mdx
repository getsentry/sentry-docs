...
<Alert level="warning" title="Minimum Gradle version for plugin 6.x">

`io.sentry.android.gradle` >= `6.0.0` is built against Gradle `8.14.2` and requires Gradle `8.14.2` or later at runtime. Older Gradle versions (including `8.10.x`) will fail at task registration with a `java.lang.NoSuchMethodError` such as `'org.gradle.api.tasks.Exec org.gradle.api.tasks.Exec.setIgnoreExitValue(boolean)'`. This is a Gradle ABI incompatibility, not a feature flag, so you can't work around it by disabling individual upload tasks. If you can't update Gradle yet, stay on `io.sentry.android.gradle:5.x` until you can. The plugin's minimum supported [Android Gradle Plugin](https://developer.android.com/studio/releases/gradle-plugin) version is `7.4.0`, and the minimum [Kotlin language version](https://kotlinlang.org/docs/compatibility-modes.html) is `1.8`.
