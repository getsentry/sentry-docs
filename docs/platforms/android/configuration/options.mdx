...
## Memory Limiter Options

<SdkOption name="memoryLimiterEnabled" type="bool" defaultValue="false" availableSince="8.57.0">

Enables experimental detection of Android Memory Limiter process deaths through retained [ApplicationExitInfo](https://developer.android.com/reference/android/app/ApplicationExitInfo) records.

When enabled, the SDK checks for Memory Limiter terminations on the next app start and reports the latest match as a fatal event. If release health is enabled, the previous session is also marked as abnormal.

This feature is available only on supported devices running on Android 17 (API level 37) and higher. See <PlatformLink to="/configuration/memory-limiter/">Memory Limiter</PlatformLink> for setup details and limitations.

AndroidManifest.xml key: `io.sentry.memory-limiter.enable`.

</SdkOption>

<SdkOption name="reportHistoricalMemoryLimiterExits" type="bool" defaultValue="false" availableSince="8.57.0">

Reports older retained Memory Limiter process deaths in addition to the latest one.

Historical Memory Limiter terminations are not enriched with persisted context from the previous process. This option only takes effect when <PlatformIdentifier name="memoryLimiterEnabled" /> is enabled.

AndroidManifest.xml key: `io.sentry.memory-limiter.report-historical`.
