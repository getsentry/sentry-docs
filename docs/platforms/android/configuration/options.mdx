---
title: Options
description: "Learn more about how the Sentry Android SDK can be configured via options. These are being passed to the init function and therefore set when the SDK is first initialized."
sidebar_order: 1
---
...
File paths aren't a Data Collection category. They continue to use `sendDefaultPii`.

<Alert>

Read our [frequently asked questions about mobile data privacy](/security-legal-pii/security/mobile-privacy/) when completing Apple App Store or Google Play privacy details.
...
| `databaseQueryData`    | `boolean`                    | `true`         | Allows supported integrations to collect associated query data. Sanitized statements and structural metadata aren't affected. |
...
The `cookies`, HTTP header, and URL query parameter fields support `off`, `deny_list`, and `allow_list`. Matching terms are comma-separated. Matching is partial and case-insensitive, and the built-in sensitive list always applies. Filtered values are replaced with `"[Filtered]"`.
...
If you previously used `sendDefaultPii=false`, either leave Data Collection unset to preserve that behavior or explicitly disable every category you don't want collected. If you used `sendDefaultPii=true`, an empty `DataCollection` opts into the new filtered defaults. Keep `sendDefaultPii=true` only if you also need file paths.
...
<SdkOption name="enableLegacyProfiling" type="bool" defaultValue="true" availableSince="8.51.0">
