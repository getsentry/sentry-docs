...
The crash context data in this case is collected by the operating system as part of the shutdown of your crashed application, and partially runs out-of-process, which allows for a richer collection of crash context data vs. using the [NDK integration](/platforms/android/configuration/using-ndk/).
...
It is a sensible practice to run the [NDK integration](/platforms/android/configuration/using-ndk/) (`io.sentry.ndk.enable`) and the Tombstone integration at the same time:
...
This option is useful after updating the SDK to the version that introduced the Tombstone integration, to report all Tombstones that occurred before the SDK update.
