...
### Finding ANRs with Profiles

To find ANR issues that have a profile attached, search your issue stream for:

```
mechanism:ANR has:profiler.id
```

This filters for ANR events that include a `profiler.id`, meaning a stack profile and flamegraph were successfully captured and attached to the event.

![ANR Profile Search](./img/anr-profile-search.png)
