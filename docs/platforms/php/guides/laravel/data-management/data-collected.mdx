...
When <PlatformLink to="/agent-tracing/">Agent Tracing</PlatformLink> is active, Sentry collects metadata about AI agent activity. This can include agent names, provider names, model names, token usage, streaming state, finish reasons, conversation IDs, tool names, tool descriptions, and tool schemas.
