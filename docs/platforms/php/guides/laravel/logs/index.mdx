...
Ensure you are not testing in Tinker (`php artisan tinker`) as it does not trigger the logs to be flushed, you can run `\Sentry\logger()->flush();` manually to flush the logs and send them to Sentry.
