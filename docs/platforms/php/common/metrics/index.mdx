...
In Sentry, metrics are extracted from your span data, to ensure they are contextually linked to the rest of your data through Tracing, so you can do more than just observe. If you notice an unexpected pattern, you can zoom in to see a snapshot through your entire system with a trace and get rich additional debugging information through issues, replays or profiles to help you understand the root cause and take action.

## How to attach data to a span

Sentry's tracing product has to be enabled in order for Metrics to work. To begin capturing metrics data, you first need to start a transaction. Check out the <PlatformLink to="/tracing/">performance setup</PlatformLink> and <PlatformLink to="/tracing/instrumentation/custom-instrumentation/">custom instrumentation</PlatformLink> documentation for more detailed information.
...
After you emit your span data, you need to configure the metric you want to track in the Sentry UI from this data field. Learn about this [here](https://docs.sentry.io/product/explore/metrics/metrics-set-up/).
