...
title: Options
sidebar_order: 0
description: "Learn more about how the Sentry Go SDK can be configured via options. These are being passed to the init function and therefore set when the SDK is first initialized."
---
...
<Alert>
  Passing [`DataCollection`](#DataCollection) opts you into the more permissive `DataCollection` defaults. To preserve the old `SendDefaultPII: false` behavior while using `DataCollection`, explicitly disable or filter each category. See the [`DataCollection`](#DataCollection) example below.
