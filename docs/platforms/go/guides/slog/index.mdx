...
func main() {
	...
	handler := sentryslog.Option{
		LogLevel:   slog.LevelInfo, // LogLevel is the minimum level for sending logs to Sentry.
		EventLevel: slog.LevelError, // EventLevel is the minimum level for sending events to Sentry.
...
Sentry allows you to send logs either as log entries or as events. The minimum log level defaults to `slog.LevelDebug` and logs will be send if the `EnableLogs` option is set. The minimum event level defaults to `slog.LevelError`.
...
```go
logger := slog.New(sentryslog.Options{
  EventLevel: slog.LevelError + 1 // manually set level to disable event logs
