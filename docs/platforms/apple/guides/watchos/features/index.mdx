...
For a complete list of all available features, see the <PlatformLink to="/common/features">main Features page</PlatformLink>. This page highlights watchOS-specific features and limitations.
...
- Manual error reporting using `SentrySDK.capture(error:)` or `SentrySDK.capture(message:)`
...
1. **Use Manual Error Reporting** - Since crash detection is not available, use `SentrySDK.capture(error:)` to manually report important errors
2. **Add Context** - Use scopes to add context and tags to help debug issues
3. **Test Thoroughly** - Since symbolication is limited, ensure your error messages and context are descriptive
4. **Consider Companion App** - For critical error tracking, consider implementing error reporting in your companion iOS app
