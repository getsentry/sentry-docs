...
To visualize which elements are being masked, you can enable a masking overlay, which will be displayed in a semi-transparent red overlay on top of the masked elements.
This works for the simulator, device and Xcode Preview.

Call the following method from anywhere in your app to display the red masking overlay on all masked views:

```swift
  SentrySDK.replay.showMaskPreview()
```

By default the overlay will be opaque. To configure the opacity of the overlay to be semi-transparent, you can pass a custom value as a parameter:

```swift
  SentrySDK.replay.showMaskPreview(0.5)
```

To preview masking during the design phase of your SwiftUI views, you can use the `sentryReplayPreviewMask` modifier.
This helps ensure proper masking without affecting the final release build.

This view modifier works on the simulator, device and Xcode Preview, therefore we recommend to apply the modifier only in your preview code:
