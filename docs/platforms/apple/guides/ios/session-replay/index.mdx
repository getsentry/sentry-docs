...
A: We don`t actively prevent you from using Session Replay on these platforms, but currently we only officially support it on iOS and cannot make guarantees about the functionality and the performance on other platforms.
