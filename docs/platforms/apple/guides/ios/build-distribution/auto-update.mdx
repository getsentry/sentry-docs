...
Unlike Android, iOS builds are identified by their `mainBinaryIdentifier` (Mach-O UUID), which is unique per binary. This means the server can always reliably look up the correct build and its install groups, so no additional SDK-side configuration is needed.
