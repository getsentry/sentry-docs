...
- Memory introspection → The Apple SDK includes memory introspection data in crash reports by default, which captures stack contents near the crash site. This data can help diagnose issues but may contain sensitive information. You can disable this feature by setting <PlatformIdentifier name="enableMemoryIntrospection" /> to `false`. Learn more in the <PlatformLink to="/configuration/options/#enableMemoryIntrospection">configuration options</PlatformLink>.
