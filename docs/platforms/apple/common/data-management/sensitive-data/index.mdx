...
description: "Learn about filtering or scrubbing sensitive data within the Sentry Apple SDK, so that data is not sent with the event. You can also configure server-side scrubbing to ensure the data is not stored."
sidebar_order: 100
keywords:
  - pii
  - gdpr
...
- Memory introspection → The Apple SDK does not include memory introspection data in crash reports by default. When enabled by setting <PlatformIdentifier name="enableMemoryIntrospection" /> to `true`, it captures stack contents near the crash site, which can help diagnose issues but may contain sensitive information. Learn more in the <PlatformLink to="/configuration/options/#enableMemoryIntrospection">configuration options</PlatformLink>.
