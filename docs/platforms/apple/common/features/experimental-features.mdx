...
Enable <PlatformLink to="/tracing/instrumentation/automatic-instrumentation/#standalone-app-start-tracing">`enableStandaloneAppStartTracing`</PlatformLink> to send app start data as a dedicated transaction instead of attaching it to the first UIViewController transaction. You can also set a dedicated <PlatformLink to="/configuration/options/#experimental.appStartSampleRate">sample rate</PlatformLink> for app start transactions.
