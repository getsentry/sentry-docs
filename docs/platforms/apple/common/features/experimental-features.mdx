...
### Watchdog Terminations V2

Enable the `enableWatchdogTerminationsV2` option to use a more efficient mechanism for detecting watchdog terminations.
...
```objc {tabTitle:Objective-C}
options.experimental.enableWatchdogTerminationsV2 = YES;
```
...
You can also use the `beforeSendMetric` callback to filter or modify metrics before they're sent:

```swift
options.experimental.beforeSendMetric = { metric in
    var metric = metric
    metric.attributes["processed"] = true
    return metric // return nil to drop the metric
}
```
