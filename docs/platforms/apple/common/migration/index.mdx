...
#### Swift Type Name Changes

Several SDK classes were rewritten in Swift in v9. While Objective-C names are preserved via `@objc(...)` annotations, the Swift type names have changed. If your project defines types with the same name as SDK types (e.g. a local `Breadcrumb` type), you may see ambiguity errors like `Cannot convert value of type 'Sentry.SentryLevel' to expected argument type 'Sentry.SentryLevel'`. Resolve these by fully qualifying the SDK type, e.g. `Sentry.SentryLevel`.
...
- We renamed `SentryOptions` to `Options` in Swift. The class is still available as `SentryOptions` in Objective-C. If you reference the type explicitly in Swift, update to `Options` (or `Sentry.Options` to avoid ambiguity).
- We renamed `SentryStructuredLogLevel` to `SentryLogLevel`
- We renamed `SentryStructuredLogAttribute` to `SentryLogAttribute`
- We changed the `SentryFeedback` initializer: the `screenshot` parameter has been replaced with `attachments: [Attachment]?` to support multiple attachments.
