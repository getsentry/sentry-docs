...
In 6.x, we introduce a new type [SentryMessage](https://develop.sentry.dev/sdk/foundations/data-model/events/message/) for `event.message`. SentryMessage provides you the ability to pass a format string with parameters to Sentry, which can help group similar messages into the same issue.
