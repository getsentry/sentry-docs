...
In that case, this can be because of a [potential bug in Swift](https://github.com/apple/swift/issues/72657)
when trying to get the class reference of a Swift class that references a class in RoomPlan or
ActivityKit on an iOS version lower than the minimum required version of RoomPlan or ActivityKit.
For example, running the code below on a Simulator on iOS 15 with the Sentry Cocoa SDK enabled
...
If you can't upgrade to at least Sentry Cocoa SDK version `8.23.0`, you can also disable swizzling,
but this disables a few features, such as automatic performance monitoring and network breadcrumbs:
