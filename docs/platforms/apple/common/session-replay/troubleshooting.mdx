...
Due to potential masking issues introduced by Apple's Liquid Glass rendering changes in iOS 26.0, Session Replay is <strong>automatically disabled</strong> by default on apps running iOS 26.0+ when built with Xcode 26.0 or later. This defensive measure protects user privacy and prevents potential PII leaks until masking is reliably supported.
