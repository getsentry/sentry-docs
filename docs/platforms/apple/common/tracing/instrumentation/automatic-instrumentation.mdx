...
Since Cocoa SDK version 8.33.0, the SDK doesn't create time to initial display (TTID) and [time to full display (TTFD)](#time-to-full-display) for UIViewControllers presented in the background because the logic requires UI frames to be drawn.

The SDK doesn't create time to initial display (TTID) and [time to full display (TTFD)](#time-to-full-display) spans for UIViewControllers presented after receiving the [UIApplicationDidEnterBackgroundNotification](https://developer.apple.com/documentation/uikit/uiapplicationdidenterbackgroundnotification), because the logic requires UI frames to be drawn.
...
The span starts when the view of a view controller is loaded, and there is no other view controller transaction happening at the moment. Since Cocoa SDK version 8.33.0, the SDK doesn't create [time to initial display (TTID)](#time-to-initial-display) and time to full display (TTFD) spans for UIViewControllers presented in the background because the logic requires UI frames to be drawn.
