...
The method signatures of these extensions resemble those of the original types, but they add a span to the current transaction:
...
let data = Data(contentsOfUrlWithSentryTracing: url, options: [.atomic])
...
// Reading a file with tracing enabled:
NSURL *url = [NSURL URLWithString:@"path/to/file"];
NSData *data = [NSData dataWithContentsOfUrlWithSentryTracing:url options:NSDataReadingAtomic error:nil];

// Writing to a file with tracing enabled:
[data writeWithSentryTracingToURL:url options:NSDataWritingAtomic error:nil];
```
