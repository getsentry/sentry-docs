...
When enabled, you can also set a dedicated sample rate for app start transactions using `experimental.appStartSampleRate`. This overrides `tracesSampleRate` for app start transactions only, so you can collect all app starts without increasing your overall sample rate. A custom `tracesSampler` callback still takes precedence.
...
SentrySDK.start { options in
    ...
    options.experimental.enableStandaloneAppStartTracing = true
    options.experimental.appStartSampleRate = 1.0
}
```
