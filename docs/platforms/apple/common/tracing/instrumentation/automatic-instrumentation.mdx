...
Starting with iOS 18, macOS 15 and tvOS 18, the `NSFileManager` is not backed by `NSData` anymore and needs to be swizzled directly.
This feature is currently experimental and disabled by default, as it might cause issues with the internal behaviour of these classes.
...
Starting with iOS 18, tvOS 18 and macOS 15 the Swift classes `Data` and `FileManager` are not backed by their Objective-C counterparts `NSData` and `NSFileManager` anymore and can therefore not be instrumented automatically anymore. Instead, please use [Manual File I/O Tracing](#manual-fileio-tracing).
...
The Sentry SDK offers extensions for the types `Data` and `FileManager` to track reading and writing data, as well as creating, moving, copying and deleting files.
...
We recommend to disable swizzling of `NSData` and `NSFileManager` when using manual file I/O tracing, as the internal behaviour of these classes differs between platform versions and can cause duplicate spans in your traces.
