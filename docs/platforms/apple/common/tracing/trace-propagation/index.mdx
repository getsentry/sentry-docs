...
description: "Learn how to connect events across applications/services using the Sentry Apple SDK."
sidebar_order: 3000
---
...
If you also need the W3C `traceparent` header for OpenTelemetry-compatible backends, enable it with:

```swift
options.enablePropagateTraceparent = true
```

When enabled, the SDK sends the `traceparent` header alongside `sentry-trace` and `baggage` on outgoing requests that match `tracePropagationTargets`.

If you run any JavaScript applications in your distributed system, make sure that these headers are added to your CORS allowlist and won't be blocked or stripped by your proxy servers, gateways, or firewalls.
