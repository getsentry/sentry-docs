...
| `customButton`\* | `UIButton?`               | `nil`                       | Install a hook for the specified button to show the form when it is pressed. If this is set, `configureWidget` is ignored. \*This property is on `SentryUserFeedbackConfiguration`, not `SentryUserFeedbackWidgetConfiguration`. |
