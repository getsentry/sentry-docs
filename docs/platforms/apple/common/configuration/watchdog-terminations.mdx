...
A common reason the watchdog can terminate your app is an Out Of Memory problem. Other less likely reasons are crashes that lead to immediate termination of your app due to security reasons, such as a stack buffer overflow or exceptions with a termination reason of codesigning.
No third-party crash library can report those because the OS immediately terminates the app.
