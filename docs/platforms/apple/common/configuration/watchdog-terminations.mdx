...
Note that there are some security-related types of crashes which third-party libraries like Sentry can not report, where the OS immediately terminates the app. These are usually infrequent, and include stack buffer overflows, or exceptions with a termination reason related to codesigning.
