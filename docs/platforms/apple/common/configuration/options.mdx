...
<SdkOption name="experimental.appStartSampleRate" type="float" defaultValue="nil">

The sample rate for standalone app start transactions, overriding <PlatformIdentifier name="tracesSampleRate" />. When set, standalone app start transactions use this rate instead of <PlatformIdentifier name="tracesSampleRate" />. A custom <PlatformIdentifier name="tracesSampler" /> callback still takes precedence.

Requires <PlatformIdentifier name="experimental.enableStandaloneAppStartTracing" /> to be enabled.

The value must be between `0.0` and `1.0`. Setting `nil` (the default) falls back to <PlatformIdentifier name="tracesSampleRate" />. Setting `0` discards all app start transactions, and `1.0` collects all of them. Values outside the valid range are clamped to `0`.
