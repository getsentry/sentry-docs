...
When enabled, the SDK includes memory introspection data in crash reports. Memory introspection captures stack contents near the crash site, which can help diagnose issues but may contain sensitive information.

When disabled, the SDK excludes this data from crash reports, reducing the potential for sensitive data disclosure. This can be useful if your application handles particularly sensitive information in memory.
