...
The DSN tells the SDK where to send the events. If this value is not provided, the SDK will try to read it from the `SENTRY_DSN` environment variable on macOS. On iOS, tvOS, watchOS, and visionOS, the environment variable fallback does not apply. If neither is set, the SDK will not send any events.
...
Sets the environment. This string is freeform and defaults to `production`. A release can be associated with more than one environment to separate them in the UI (think `staging` vs `prod` or similar).
...
<SdkOption name="enableSigtermReporting" type="bool" defaultValue="false" availableSince="8.27.0">
...
<SdkOption name="enableUncaughtNSExceptionReporting" type="bool" defaultValue="false" availableSince="8.40.0">
...
Learn more in the <PlatformLink to="/usage/#capturing-uncaught-exceptions">capturing uncaught exceptions</PlatformLink> documentation.
...
A list of string prefixes of framework names that belong to the app. By default, this contains `CFBundleExecutable` to mark the app's main executable as "in-app".

Sentry differentiates stack frames that are directly related to your application ("in application") from stack frames that come from other packages such as the standard library, frameworks, or other dependencies. The difference is visible in [sentry.io](https://sentry.io), where only the "in application" frames are displayed by default.

This property is read-only. Use the `add(inAppInclude:)` method to append additional framework name prefixes:
...
[SentrySDK startWithConfigureOptions:^(SentryOptions *options) {
    options.dsn = @"___PUBLIC_DSN___";
    [options addInAppInclude:@"MyFramework"];
}];
```
...
<SdkOption name="idleTimeout" type="double" defaultValue="3.0">

The idle time, measured in seconds, to wait until a <PlatformLink to="/tracing/instrumentation/automatic-instrumentation/#user-interaction-tracing">user interaction transaction</PlatformLink> will be automatically finished. The transaction will use the end timestamp of the last finished span as the end time for the transaction.
...
<SdkOption name="configureUserFeedback" type="function">

A closure that configures the User Feedback widget. The closure receives a `SentryUserFeedbackConfiguration` object that you can use to customize the feedback form's appearance and behavior.
...
Learn more in the <PlatformLink to="/user-feedback/">User Feedback documentation</PlatformLink>.
...
**Deprecated.** Use <PlatformIdentifier name="onLastRunStatusDetermined" /> instead, which is called regardless of whether the app crashed.

A block called after SDK initialization when the last execution terminated with a crash. This callback is invoked synchronously during SDK initialization, so keep any work here minimal. It is only executed once during the entire run of the program. Crash reporting is automatically disabled if a debugger is attached, and there is no guarantee this callback runs on the main thread.
...
[SentrySDK startWithConfigureOptions:^(SentryOptions *options) {
    options.dsn = @"___PUBLIC_DSN___";
    options.onCrashedLastRun = ^(SentryEvent * _Nonnull event) {
        // Perform actions when a crash was detected on last run
...
<SdkOption name="onLastRunStatusDetermined" type="function">

A block called shortly after SDK initialization when the crash status of the last program execution has been determined. Unlike <PlatformIdentifier name="onCrashedLastRun" />, this callback is invoked regardless of whether the app crashed:
...
This callback is only executed once per `SentrySDK.start(configureOptions:)` lifecycle, and it is not guaranteed to be called on the main thread. Crash reporting is automatically disabled while a debugger is attached, in which case the callback reports `didNotCrash` even if the app crashed.
...
SentrySDK.start { options in
    ...
    options.onLastRunStatusDetermined = { status, crashEvent in
        switch status {
        case .didCrash:
            print("App crashed on last run: \(String(describing: crashEvent))")
        case .didNotCrash:
            print("App did not crash on last run")
        @unknown default:
            break
        }
    }
...
[SentrySDK startWithConfigureOptions:^(SentryOptions *options) {
    options.dsn = @"___PUBLIC_DSN___";
    options.onLastRunStatusDetermined = ^(SentryLastRunStatus status, SentryEvent * _Nullable crashEvent) {
        if (status == SentryLastRunStatusDidCrash) {
            NSLog(@"App crashed on last run: %@", crashEvent);
        } else {
            NSLog(@"App did not crash on last run");
        }
    };
}];
```
...
<SdkOption name="sessionReplay.sessionSampleRate" type="float" defaultValue="0">

The sample rate for replays that begin recording immediately and last the entirety of the user's session. Specifying `0` means no sessions are recorded, `1.0` means all sessions are recorded.

Learn more in the <PlatformLink to="/session-replay/">Session Replay documentation</PlatformLink>.

</SdkOption>

<SdkOption name="sessionReplay.onErrorSampleRate" type="float" defaultValue="0">

The sample rate for replays that are recorded when an error occurs. A 30-second replay buffer prior to the error is included. Specifying `0` means no error replays are recorded, `1.0` means all error replays are recorded.

Learn more in the <PlatformLink to="/session-replay/">Session Replay documentation</PlatformLink>.

</SdkOption>

<SdkOption name="sessionReplay.maskAllText" type="bool" defaultValue="true">
...
<SdkOption name="sessionReplay.quality" type="enum" defaultValue="medium">

The quality of the session replay video. Higher quality increases CPU and bandwidth usage. Available values: `low` (80% scale, 20k bitrate), `medium` (100% scale, 40k bitrate), `high` (100% scale, 60k bitrate).

</SdkOption>

<SdkOption name="sessionReplay.maskedViewClasses" type="array" defaultValue="[]">
...
<SdkOption name="sessionReplay.excludeViewTypeFromSubtreeTraversal" type="method">

Adds a view type pattern to the set of excluded view types, preventing the SDK from traversing their subtrees during redaction. Matching uses partial string containment against `type(of: view).description()`. Use this to prevent crashes caused by traversing problematic view hierarchies.
...
<SdkOption name="sessionReplay.includeViewTypeInSubtreeTraversal" type="method">

Removes a view type from the excluded set, allowing its subtree to be traversed again. Matching uses exact string matching. Use this to re-enable traversal for a view type that was excluded by default.
...
<SdkOption name="sessionReplay.enableViewRendererV2" type="bool" defaultValue="true">

When enabled, uses a faster view renderer for session replays that can be up to 4-5x faster than the default renderer. This reduces main thread interruptions and visual lag.

</SdkOption>

<SdkOption name="sessionReplay.enableFastViewRendering" type="bool" defaultValue="false">

When enabled, uses faster but potentially incomplete view rendering for session replays. This can improve performance but may result in some views not being fully rendered in the replay. Requires `enableViewRendererV2` to be enabled.
...
options.configureProfiling = ^(SentryProfileOptions *profiling) {
    ...
    profiling.sessionSampleRate = 1.0;
    profiling.profileAppStarts = YES;
};
```

<SdkOption name="profiling.lifecycle" type="enum" defaultValue="manual">

The mode for starting and stopping the profiler:

- `manual` — Profiling is controlled manually via `SentrySDK.startProfiler()` and `SentrySDK.stopProfiler()`. Profiling is independent of transactions and spans.
- `trace` — Profiling starts automatically when there is at least one active root span, and stops when there are no root spans. This mode requires tracing to be enabled.

</SdkOption>

<SdkOption name="profiling.sessionSampleRate" type="float" defaultValue="0">
...
When enabled, the profiler starts as early as possible during the app lifecycle, allowing you to profile app startup performance. The `sessionSampleRate` is evaluated on the previous launch and only takes effect when app start profiling activates on the next launch.
...
These options are under `options.experimental` and control experimental features that are subject to change or may be removed in future versions.

<SdkOption name="experimental.enableUnhandledCPPExceptionsV2" type="bool" defaultValue="false">

Enables a more reliable mechanism for reporting unhandled C++ exceptions. This approach hooks into all instances of `__cxa_throw`, which provides more comprehensive and consistent exception handling across an app's runtime, regardless of the number of C++ modules or how they're linked.

<Alert level="warning">

The mechanism of hooking into `__cxa_throw` could cause issues with symbolication on iOS due to caching of symbol references.
...
<SdkOption name="experimental.enableSessionReplayInUnreliableEnvironment" type="bool" defaultValue="false">

Forces enabling of session replay in environments where masking of text and images cannot be reliably guaranteed (such as iOS 26.0+ with Liquid Glass).

The SDK normally disables session replay in these environments unless it can detect indicators of a reliable environment (iOS 18 or earlier, `UIDesignRequiresCompatibility` set to `YES` in Info.plist, or built with Xcode before 26.0).

<Alert level="warning">

Enabling this flag on iOS 26.0+ may result in text and images not being masked as expected in session replays.
