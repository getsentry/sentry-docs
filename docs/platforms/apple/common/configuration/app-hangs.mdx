...
Starting with version 9.0.0, App Hangs V2 is the default on iOS and tvOS, and the `enableAppHangTrackingV2` option has been removed.

For version 8.50.0 to 8.x, you can enable it with:
...
SentrySDK.start { options in
    options.dsn = "___PUBLIC_DSN___"
    options.enableAppHangTrackingV2 = true // Only available in 8.50.0–8.x
}
```
