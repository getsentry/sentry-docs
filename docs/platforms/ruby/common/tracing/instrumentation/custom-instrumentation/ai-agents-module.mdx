...
── invoke_agent My Agent          (gen_ai.invoke_agent)
   ├── chat claude-sonnet-4-20250514            (gen_ai.chat)         ← 1st LLM call
   ├── execute_tool get_weather   (gen_ai.execute_tool)   ← tool run
   ├── chat claude-sonnet-4-20250514            (gen_ai.chat)         ← 2nd LLM call
   └── ...
```
...
Sentry.with_child_span(op: 'gen_ai.request', description: 'chat o3-mini') do |span|
  ...
  span.set_data('gen_ai.response.finish_reasons', [result.choices[0].finish_reason])
