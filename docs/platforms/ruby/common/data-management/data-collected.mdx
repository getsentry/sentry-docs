...
Please note that `send_default_pii` is not considered for a request query string in span attributes. We recommend utilizing your own redactions in `before_send` or similar hooks.
