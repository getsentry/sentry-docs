...
Rails structured logging is enabled by default once you include `sentry-rails` and call `Sentry.init`.

If you're also capturing logs from `Rails.logger`, make sure you also enable the `:logger` patch with `config.enabled_patches << :logger`.

```ruby
Sentry.init do |config|
  config.rails.structured_logging.enabled = true # enabled by default
