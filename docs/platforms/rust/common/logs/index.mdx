...
To customize this behavior, define a custom `event_filter`.
Multiple event filters can be combined using the bitwise or operator.
The following code snippet shows the default `event_filter` used when the `sentry` crate is used with the `logs` feature.
