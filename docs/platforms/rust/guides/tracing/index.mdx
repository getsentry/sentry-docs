...
By default, error level events from `tokio-rs/tracing` are captured as Sentry events, while anything at or above info level is added as a breadcrumb and captured as a structured log (if the `logs` feature flag is enabled).
