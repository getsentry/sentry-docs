...
Additionally, if the `logs` feature flag of the `sentry` crate is enabled, events from `tokio-rs/tracing` at info level or above are also captured as structured logs.
