---
title: Options
description: "Learn more about how the Sentry Elixir SDK can be configured via options. These are being passed to the init function and therefore set when the SDK is first initialized."
sidebar_order: 1
---
...
The function runs after the event has been sent and its return value is ignored, so a crash doesn't change the send result.
...
<SdkOption name="traces_ignore_http_status_codes" type='list' defaultValue='[404]'>
...
The SDK drops the transaction only once the response status is known, after the request has been handled. By then, the trace has already been propagated as sampled, so services called while handling the request still report their spans. In Sentry, those spans appear without their root transaction.

This option is available since SDK version 14.0.0.
