...
Oban wraps *unhandled* crashes, exits, and throws in jobs inside [`Oban.CrashError`](https://hexdocs.pm/oban/2.18.0/Oban.CrashError.html) exceptions. If jobs return errors manually however, such as by returning `{:error, reason}` tuples, then Oban wraps those inside [`Oban.PerformError`](https://hexdocs.pm/oban/2.18.0/Oban.PerformError.html) exceptions. If you want to only report crashes to Sentry, you can configure an event filter that doesn't report `Oban.PerformError` events or that inspects its `:reason` field to determine whether to report them. For example:
