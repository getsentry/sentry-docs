...
The request-scrubbing options below are passed to `Sentry.PlugContext` in your endpoint or router, not set with `config :sentry`. If a custom scrubber raises, the SDK logs the failure at the `:error` level and uses its default scrubber for that field instead.
...
- Request parameters, including body, query, and path parameters, are included
