...
It's important to note that even when Sentry begins to only store the most valuable data based on Dynamic Sampling Priorities, the [performance metrics](/product/performance/metrics/) you see are still based on **all** the events you send to Sentry. This ensures that you get a full and complete view of your application's health.
