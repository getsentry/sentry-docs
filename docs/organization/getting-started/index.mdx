...
[Single sign-on (SSO)](/organization/sso/) allows your team to log in quickly, streamlines the on/off-boarding process for member accounts, and strengthens your login with secure credentials. Sentry provides out-of-the-box configuration for integrating SSO providers like [Okta](/organization/sso/#okta) and [Azure Active Directory](/organization/sso/#azure-active-directory) (SAML) or [Google](/organization/sso/#google-business-app) and [GitHub](/organization/sso/#github-organizations) (Oauth). In addition, we provide a generic configuration option for any other [SAML2 Identity Provider](/organization/sso/saml2/).

Sentry also supports a subset of the specification for System for Cross-Domain Identity Management (SCIM) for [Okta](/organization/sso/okta-sso/okta-scim/) and [Azure AD](/organization/sso/azure-sso/#scim-integration).
...
- Which errors should be ingested into your Sentry account and which should be dropped, through [Inbound Filters](/organization/quotas/#inbound-data-filters)
