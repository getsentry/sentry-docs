...
[Single sign-on (SSO)](/account/sso/) allows your team to log in quickly, streamlines the on/off-boarding process for member accounts, and strengthens your login with secure credentials. Sentry provides out-of-the-box configuration for integrating SSO providers like [Okta](/account/sso/#okta) and [Azure Active Directory](/account/sso/#azure-active-directory) (SAML) or [Google](/account/sso/#google-business-app) and [GitHub](/account/sso/#github-organizations) (Oauth). In addition, we provide a generic configuration option for any other [SAML2 Identity Provider](/account/sso/saml2/).

Sentry also supports a subset of the specification for System for Cross-Domain Identity Management (SCIM) for [Okta](/account/sso/okta-sso/okta-scim/) and [Azure AD](/account/sso/azure-sso/#scim-integration).
...
Learn more about different user roles in [Organization Management](/account/membership/).
...
- Which errors should be ingested into your Sentry account and which should be dropped, through [Inbound Filters](/account/quotas/#inbound-data-filters)
