...
Because Sentry uses Just-In-Time (JIT) provisioning, new members are registered with Sentry automatically during their first login attempt with SAML SSO. These accounts will have their membership types delegated based on the default membership role in [**Settings > Auth**](https://sentry.io/orgredirect/organizations/:orgslug/settings/auth/).
