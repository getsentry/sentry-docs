...
If you change your organization slug, you'll also need to update it in the ["Auth" section of your "Organization Settings"](https://sentry.io/orgredirect/organizations/:orgslug/settings/auth/) in Sentry.
...
1. Sign in to [sentry.io](https://sentry.io). Select [**Settings > Auth**](https://sentry.io/orgredirect/organizations/:orgslug/settings/auth/)
...
3. Select "Enable API Integration", then enter the SCIM URL from the [auth settings page](https://sentry.io/orgredirect/organizations/:orgslug/settings/auth/) as the Base URL field.

4. For the API Token, copy the Auth Token value from the [auth settings page](https://sentry.io/orgredirect/organizations/:orgslug/settings/auth/).
...
Once these changes have been made, newly assigned users will be provisioned in Sentry. They can then access Sentry through their SSO provider (for example, the Sentry app tile in Okta). If needed, you can also send invitation emails manually from [**Settings > Members**](https://sentry.io/orgredirect/organizations/:orgslug/settings/members/) in Sentry. If a user gets un-assigned, they'll be removed from their organization in Sentry.
...
- If the `sentryOrgRole` field is left blank, group members will be provisioned with the default organization-level role. This default role can be configured in Sentry under [**Settings > Auth**](https://sentry.io/orgredirect/organizations/:orgslug/settings/auth/). Otherwise, the role must be one of the following:
  - admin
  - manager
  - billing
  - member
