...
1. Navigate to your [**Org Settings > Auth**](https://sentry.io/orgredirect/organizations/:orgslug/settings/auth/) and click on "Configure" for Active Directory.
...
As a result of these changes, users who are assigned will be provisioned in Sentry. They can then access Sentry through their SSO provider (for example, the Sentry tile in Azure). If needed, you can also send invitation emails manually from [**Settings > Members**](https://sentry.io/orgredirect/organizations/:orgslug/settings/members/) in Sentry. When a user is un-assigned, their membership in Sentry will be deleted.
