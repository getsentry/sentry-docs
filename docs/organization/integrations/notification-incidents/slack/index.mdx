...
When you install the Sentry Slack app, it requests the following OAuth scopes. These are the minimum permissions required for the integration to function.

### Bot Token Scopes

| Scope | Reason |
|---|---|
| `channels:read` | Verify that a public channel exists when you set it as an alert destination. |
| `groups:read` | Verify that a private channel exists when you set it as an alert destination. |
| `users:read` | Find Slack users so Sentry can connect them to their Sentry accounts and deliver personal notifications. |
| `chat:write` | Send alert and notification messages to channels and direct messages. |
| `chat:write.public` | Send alerts to public channels the Sentry bot hasn't been invited to yet. |
| `chat:write.customize` | Display the Sentry name and icon on messages, show temporary prompts to individual users, and update existing notifications. |
| `links:read` | Detect when someone shares a Sentry link in Slack. |
| `links:write` | Show a rich preview of Sentry issues, Discover queries, and metric alerts when their links are shared in Slack. |
| `team:read` | Show your workspace name and icon in the Sentry integration settings page. |
| `im:read` | Send direct messages to individual users for personal notifications. |
| `im:history` | Read and respond to direct messages sent to the Sentry bot. |
| `commands` | Power the `/sentry` slash commands (`link`, `unlink`, `link team`, etc.). |

### User Token Scopes

These scopes are requested from the individual user who installs the integration:
