...
Sentry will validate access it when saving the alert, but you can also send a test alert to double check.
...
// Image of it being initially mentionned
...
// Image of it working from thread context

If you're debugging solo, you can also open it as an Agent in Slack, using the dropdown in the top-right of your Slack window.

// Image of the dropdown, with Sentry as an option

This will open a new persistent panel in your Slack window for personal conversations with Seer Agent. In these conversations, you don't have to mention it every time and new chats will open with a few example prompts to get you started.

// image of the suggested promptså

If you happen to have a single Slack Workspace connected to multiple Sentry organizations, Seer Agent will automatically try to infer the Sentry organization to search based on thread context (e.g. Sentry links). If this behaviour doesn't fit your needs, you can use `/sentry set-preferred-organization [organization_slug]` command to manually set your preference when interacting with Seer Agent. Your chats and mentions will fallback to this set organization if no organization could be inferred.

// Image of command example
