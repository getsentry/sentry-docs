...
While you can install the GitHub integration from [GitHub](https://github.com/apps/sentry-io), we recommend installing it from Sentry.

### GitHub Permissions

The following permissions are required for the GitHub integration to work correctly. They are full opt in to be able to use the app.
...
| Members            | Read-only     | To map organization users to GitHub identities for improved issue attribution and collaboration.                                   |
...
Occassionally, Sentry will request additional permissions to your GitHub account, as new features are introduced. Denying or ignoring the request to update will not affect your current Sentry usage, but may prevent access to future features. 
