...
sidebar_order: 1
description: >-
  Learn more about Sentry’s GitLab integration and how it helps you track and
  resolve bugs faster by using data from your GitLab commits.
...
To minimize duplication in issue tracking, you can sync comments, assignees, and status updates for Sentry issues to GitLab issues and vice versa. When you delegate an issue to an assignee or update a status on GitLab, the updates will also populate in Sentry. When you resolve an issue in Sentry, the issue status will automatically update in GitLab. When GitLab issues are marked as closed, you can configure how the corresponding issue in Sentry should be resolved.
...
To configure Issue sync, navigate to **Organization Settings** > **Integrations**, and click "Configure" next to your GitLab instance. On the following page, you can configure how you want information to sync between Sentry and GitLab.

![Sync GitLab statuses to Sentry](./img/gitlab-sync.png)

#### User Mappings

For assignee syncing to work properly between Sentry and GitLab, you'll need to set up user mappings. This ensures that when an issue is assigned to someone in GitLab, you can correctly assign it to the corresponding user in Sentry, and vice versa. **NOTE:** Without proper user mappings, assignee information will not sync between Sentry and GitLab.

To set up user mappings:

1. Navigate to **Organization Settings** > **Integrations** > **GitLab**.
2. Click "Configure" next to your GitLab instance.
3. Select the **User Mappings** tab.
4. For each team member, map their GitLab username to their Sentry account.
5. Click **Save Changes** when you're done.

![Configure GitLab user mappings](./img/gitlab-user-mappings.png)

<Alert title="Important">
  Make sure there is a 1:1 mapping between a GitLab user and a Sentry user. If there is not, assignee syncing will not work.
</Alert>
