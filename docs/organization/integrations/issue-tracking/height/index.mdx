...
To configure issue management automatically, create an [issue alert](/product/alerts/alert-types/#issue-alerts) or a [performance metric alert](/product/alerts/alert-types/#performance-metric-alerts). Performance metric alerts create issues in Sentry when triggered, so you can send those issues to Height the same way as issue alerts. When selecting the [action](/product/alerts/create-alerts/issue-alert-config/#then-conditions-actions), choose "Notify Integration" and then "Height" for issue alerts, or select "Height" from the action dropdown for performance metric alerts:
