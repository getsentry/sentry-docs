...
Notifications for Spike Protection are turned off by default to avoid excessive noise for users. Read the [notification](/account/quotas/spike-protection/#notifications) section below to learn how to turn them on for key projects.
...
- Set [rate limits](/account/quotas/manage-event-stream-guide/#6-rate-limiting) on the DSN keys for the projects related to the spike.
- Set up [metric alerts](/product/alerts/alert-types/#metric-alerts) for the number of events in a project.
- If a specific release version has caused the spike, add the version identifier to the project's [inbound filters](/account/quotas/manage-event-stream-guide/#3-inbound-data-filters) to avoid accepting events from that release.
- Set up an [on-demand budget](/account/quotas/manage-event-stream-guide/#on-demand-budget) to make sure you have time to adjust your volume in the event of a future spike.
- Set up [spend allocations](/account/quotas/#spend-allocation) to ensure your critical projects are guaranteed a portion of your reserved volume, even if there are spikes in other projects.
