...
In your project's [Client Keys (DSN)](https://sentry.io/settings/projects/:projectId/keys/) settings, click "Configure", and you can create multiple DSN keys per project and assign different (or no) rate limits to each key. This will allow you to dynamically allocate keys (with varying thresholds) depending on release, environment, and so on.
...
1. Go to your project's [Client Keys (DSN)](https://sentry.io/settings/projects/:projectId/keys/) settings and open the project DSN key configuration by clicking "Configure".
...
You can find a list of deleted and discarded issues in the "Discarded Issues" tab in your project's [Inbound Filters](https://sentry.io/settings/projects/:projectId/filters/) settings. From here, you can un-discarded any of these issues to receive future events.
...
While SDK configuration requires changes to your source code and depends on your next deployment, server-side filters can be easily configured per project in the "Data Filters" section of your project's [Inbound Filters](https://sentry.io/settings/projects/:projectId/filters/) settings.
...
If you have a rogue client, Sentry supports blocking an IP from sending data. Go to your project's [Inbound Filters](https://sentry.io/settings/projects/:projectId/filters/) settings to add the IP addresses (or subnets) in the "IP Addresses" field.

### Filter by Release

<Include name="feature-available-for-plan-business.mdx" />

If you discover a problematic release causing excessive noise, Sentry supports ignoring all events and attachments from that release. Go to your project's [Inbound Filters](https://sentry.io/settings/projects/:projectId/filters/) settings, then add the release to the "Releases" field.

### Filter by Error Message

<Include name="feature-available-for-plan-business.mdx" />

Sentry supports filtering out a specific or certain kind of error as well. Go to your project's [Inbound Filters](https://sentry.io/settings/projects/:projectId/filters/) settings, then add the error message to the "Error Message" field.
