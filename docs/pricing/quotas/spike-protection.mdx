...
Spike Protection and its notifications are configured separately. Enabling Spike Protection for a project protects its quota by dropping events during a spike, even if the project has no notification actions. Notification actions are not added by default.
...
2. Expand the project, click **Add Action**, and select a notification action:
   - **Sentry Notification** sends a personal notification to eligible Owner, Manager, and Billing members. Each recipient's personal settings determine whether and where they receive it.
...
![Spike limit threshold =800x](./img/spike-limit-threshold.png)
...
![Spike zoomed out =800x](./img/spike-protection-zoomed-out.png)
...
![Spike zoomed in plotted with spike limits =800x](./img/spike-protection-zoomed-in.png)
...
![Spike limit projections with seasonality =800x](./img/spike-protection-steady-state.png)
...
![A "bursty" project with intentional spikes. =800x](./img/bursty_projects.png)
