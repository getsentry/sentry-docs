...
Seer is Sentry's AI debugging agent that catches breaking changes before deploys, and debugs and fixes issues in production.

Access Seer by adding it to your existing subscription for $40 per active contributor per month, where an active contributor is defined as any user who opens 2 or more PRs to a Seer-connected repository. At the end of the month, counts reset.
