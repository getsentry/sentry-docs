...
<Alert>
  All profiles are billed under Profile Hours regardless of whether they are [Transaction-Based, Continuous or UI&nbsp;Profiling](/product/explore/profiling/transaction-vs-continuous-profiling/).
  Transaction-Based profiling duration is extracted from the samples that make up the profile.
</Alert>
