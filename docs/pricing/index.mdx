...
Seer is Sentry's AI debugging agent. It catches breaking changes before deploy and debugs and fixes issues in production. Access Seer by enabling it on your existing subscription in [settings](https://sentry.io/orgredirect/organizations/:orgslug/settings/). Seer usage is billed separately as its own monthly charge and does not count against your PAYG budget.

Access Seer by adding it to your existing subscription for $40 per active contributor per month, where an active contributor is defined as any user who makes 2 or more PRs to a [Seer-connected repository](/organization/integrations/source-code-mgmt/github/#seer). At the end of the month, contributor counts reset.
...
You can see how you're using Seer and manage your usage by going to the [stats page](https://sentry.io/orgredirect/organizations/:orgslug/stats/) and selecting the **Issue Fixes** or **Issue Scans** categories, and updating the automation settings for when to run Seer in your project settings.
...
Go to [**Organization Settings**](https://sentry.io/orgredirect/organizations/:orgslug/settings/) and click "Subscription" (under "Usage & Billing"), then click "Start Trial" for Seer. Alternatively, you can find a similar flow on the **Issue** details page from the Seer panel or in the **Seer settings** page.
![Seer trial in subscription settings](./img/seer_trial.jpg)
