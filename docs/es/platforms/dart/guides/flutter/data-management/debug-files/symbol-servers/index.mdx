...
description: "Conoce los servidores de símbolos y cómo configurarlos con tus proyectos de Sentry."
sidebar_order: 4
---

Sentry puede descargar archivos de información de depuración desde repositorios externos. Esto te permite dejar de subir archivos de depuración y, en su lugar, configurar un servidor de símbolos público o gestionar uno propio. También es posible configurar repositorios externos y subir archivos de depuración al mismo tiempo.
...
Sentry consulta los repositorios externos en el orden en que se configuran para obtener los archivos de información de depuración. Si se configuran repositorios personalizados, estos se consultan primero. Solo los archivos de información de depuración que no se encuentran en alguno de los repositorios personalizados se buscan en los integrados.
...
Los cambios al agregar o eliminar repositorios externos se aplican de inmediato. Como resultado,
los eventos pueden agruparse de forma diferente con la nueva información y generar nuevos issues.
...
- **Bucket de Amazon S3**: Un bucket de S3 completo o un subdirectorio. Esto
  ...
  recomendamos almacenar todos los archivos en minúsculas y usar una configuración de capitalización de ruta en minúsculas.
...
2. **Path Casing**: Sobrescribe qué capitalización usa Sentry para consultar los archivos de
   información de depuración. El valor predeterminado es una capitalización mixta, que usará la capitalización
   descrita en la siguiente sección. Cuando se sobrescribe, todo el acceso es
...
3. **Directory Layout**: La estructura interna del bucket o el protocolo
   del servidor de símbolos. Hay tres diseños para elegir, que se
   describen en la siguiente sección. El valor predeterminado es _"Platform Specific"_.
...
La convención en el protocolo Symbol Server de Microsoft es almacenar estos archivos con el último carácter de la extensión del archivo reemplazado por `_`. Un ejemplo completo sería: `KERNEL32.dll/590285E9e0000/KERNEL32.dl_`. Esto no es necesario en tus propios repositorios, ya que Sentry detecta la compresión en todas las rutas.

<div id="note-about-portable-pdbs-and-nuget">
  ## Nota sobre los PDB portátiles y NuGet
</div>

Para descargar un PDB portátil desde el servidor de símbolos de NuGet, es necesario proporcionar la suma de verificación de depuración mediante el encabezado `SymbolChecksum`.
...
siguientes reglas de uso de mayúsculas/minúsculas:
...
la extensión del archivo (ya sea `.exe`, `.dll` o `.pdb`) se reemplaza por `.sym`. En las
...
LLDB suele esperar un enlace simbólico cuyo nombre son los últimos 12 dígitos hexadecimales, que
...
Los dígitos hexadecimales van en **mayúsculas**, el sufijo de la app va en **minúsculas**.
...
GDB admite varios métodos de búsqueda, según cómo se especifique el archivo de información de depuración. Sentry usa el [Build ID Method]: si se ha escrito una nota o sección GNU build ID en el archivo ELF, esta proporciona un identificador único para el ejecutable que también se conserva en el archivo de depuración.
...
- PE: `<Signature><Age>` (edad en hexadecimal, sin relleno)
- PDB: `<Signature><Age>` (edad en hexadecimal, sin relleno)
- Portable PDB: `<Signature><Age>` (edad en hexadecimal, sin relleno)
- ELF: la secuencia de bytes de la nota gnu-build-id.
- Mach-O: bytes de `LC_UUID`
- WASM: la sección `build_id` como bytes
...
La capitalización de todas las rutas de búsqueda es generalmente en **minúsculas**, excepto el
...
para más información. Esto da lugar a las siguientes rutas para todos los posibles
tipos de archivo:
...
Path: `<FileName>/<SIGNATURE><AGE>/<FileName>` (`<FileName>/<SIGNATURE>FFFFFFFF/<FileName>` para PDB portables)
...
nombre de archivo para localizar símbolos. Las rutas de archivos son idénticas a SSQP, salvo por
...
no recomendamos usar esta convención para estos tipos. No obstante, Sentry
admitirá las convenciones de SSQP con reglas de mayúsculas/minúsculas adaptadas cuando se seleccione este diseño.
...
Nota: Los archivos deben residir en `buildid/` en el servidor.
