...
| networkDetailAllowUrls   | `(string|RegExp)[]`     | `[]`        | Captura detalles de solicitud y respuesta para XHR y fetch que coincidan con las URL indicadas.                                                                              |
| networkDetailDenyUrls    | `(string|RegExp)[]`     | `[]`        | No captura detalles de solicitud y respuesta para estas URL. Tiene prioridad sobre `networkDetailAllowUrls`.                                                                 |
...
| beforeAddRecordingEvent  | `(event) => event | null` | `i => i`    | Filtra eventos de grabación adicionales que incluyen logs de consola y solicitudes/respuestas de red.                                                                         |
