...
description: "Obtenga información sobre los requisitos de las herramientas de compilación que permiten a Sentry identificar de forma única los archivos de información de depuración entre compilaciones."
sidebar_order: 2
supported:
...
Cada archivo de información de depuración especifica un identificador único. Los informes de fallas declaran
estos identificadores para que los depuradores y los sistemas de informes de fallas resuelvan los
...
* **Identificador de código**: El identificador único del ejecutable o la biblioteca
  dinámica —el archivo de código—. El contenido de este identificador depende de la plataforma:
  los archivos Mach-O usan un UUID, los archivos ELF un hash SHA y los archivos PE
...
* **Identificador de depuración**: El identificador único del archivo auxiliar de depuración. A
  diferencia del identificador de código, Sentry aplica la misma estructura en todas
  las plataformas. En Windows, es el identificador único real del archivo PDB; en todas
...
que se puedan ubicar de forma única mediante un mecanismo común.

<Alert>
  La información de depuración no tiene que estar asociada a versiones (releases). El identificador
  de depuración único garantiza que Sentry pueda elegir los archivos correctos para cada informe
  de falla. Sin embargo, se recomienda configurar versiones en el cliente para
  aprovechar otras funciones.
</Alert>

Para eventos nativos, la página de detalles del problema muestra una lista de *Imágenes cargadas*.
...
identificador de depuración. Todos los compiladores y enlazadores recientes admiten la generación de
...
*El identificador debe estar presente e idéntico tanto en el binario como en los archivos de información de depuración con símbolos eliminados.*
Si por alguna razón falta el ID, sube los archivos antes de eliminar los símbolos para que `sentry-cli` pueda calcular un
identificador estable a partir del archivo sin despojar.

<div id="pdb-age-mismatches">
  ## Desajustes de edad en PDB
</div>

Los PDB de Microsoft componen sus identificadores a partir de dos partes: una firma única y
...
del PDB correspondiente en su encabezado. Esto incluye la edad. Si el PDB se
...
asignar el mismo identificador a ambos archivos. Sin embargo, esto requiere que ambos
...
A diferencia de otros archivos de información de depuración, los archivos de ProGuard no tienen un identificador único intrínseco. Sentry CLI les asigna un UUID SHA1 según la suma de verificación del archivo. Puedes usar `sentry-cli debug-files check` sobre un archivo de ProGuard para ver el UUID generado.

Si necesitas generar el UUID por tu cuenta, puedes hacerlo con el siguiente algoritmo (código en Python como referencia):
