...
title: ESM (MJS)
sidebar_order: 10
description: "Conoce cómo ejecutar Sentry en una aplicación ESM."
supported:
  - javascript.nestjs
---
...
Cuando ejecutes tu aplicación en modo [ECMAScript Modules](https://nodejs.org/api/esm.html#introduction) (ESM), no puedes usar `require()` para cargar módulos. En su lugar, debes usar la opción de línea de comandos `--import` para cargar un módulo antes de que la aplicación se inicie.
...
<Alert>No ofrecemos compatibilidad con ESM en versiones de Node anteriores a la 18.19.0.</Alert>


<div id="troubleshooting-esm-instrumentation">
  ## Solución de problemas de la instrumentación ESM
</div>
...
desactíalo configurando `registerEsmLoaderHooks` en false. Ten en cuenta que esto también
