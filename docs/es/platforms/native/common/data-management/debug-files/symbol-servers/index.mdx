...
2. **Built-In Repositories:** Permite seleccionar de una lista de servidores de símbolos preconfigurados. De forma predeterminada, _iOS_ y _Microsoft_ están habilitados en SaaS. El repositorio de _iOS_ no está disponible en instalaciones autogestionadas (self-hosted), porque Apple no proporciona un servidor de símbolos público ni nos permite distribuir los Debug Information Files (DIFs) por nuestra cuenta.

Sentry consulta los repositorios externos en el orden en que se configuran. Si se configuran repositorios personalizados, estos se consultan primero. Solo los archivos de información de depuración que no se encuentren en uno de los repositorios personalizados se solicitarán a los integrados.
...
simbolicar nuevos informes de fallos. Del mismo modo, cualquier repositorio integrado se puede deshabilitar
...
los eventos pueden agruparse de manera diferente con la nueva información y generar nuevos problemas.
...
- **Bucket de Amazon S3**: Puede ser un bucket S3 completo o un subdirectorio. Esto
  ...
  recomendamos almacenar todos los archivos en minúsculas y usar una configuración de mayúsculas/minúsculas en la ruta también en minúsculas.

- **Bucket de Google Cloud Storage**: Puede ser un bucket GCS completo o un
...
2. **Formato de mayúsculas/minúsculas en la ruta**: Define qué capitalización usa Sentry para consultar los
   archivos de información de depuración. El valor predeterminado es mixto, que usa el formato
   descrito en la siguiente sección. Si se anula, todo el acceso será
...
3. **Distribución del directorio**: La estructura interna del bucket o el protocolo
   del servidor de símbolos. Hay tres distribuciones entre las que elegir, que se
   explican en la siguiente sección. El valor predeterminado es _"Específico de la plataforma"_.
...
La convención en el protocolo Symbol Server de Microsoft es almacenar estos archivos con
...
Para descargar un PDB portátil desde el servidor de símbolos de NuGet, es necesario proporcionar la suma de comprobación de depuración mediante el encabezado `SymbolChecksum`.
...
La siguiente tabla contiene la correspondencia entre las estructuras compatibles y los esquemas de ruta
...
El esquema de ruta unificado es específico de Sentry y permite una estructura coherente
para todos los archivos de depuración, independientemente de la plataforma. Puede almacenar archivos Breakpad, PDB,
PE y todo lo demás. 

La [herramienta symsorter] puede ordenar automáticamente los
símbolos de depuración en este formato y también crear automáticamente paquetes de código fuente.

La ruta se basa en un `build_id` formateado en hexadecimal con
letras minúsculas. Los dos primeros bytes de ese ID se usan como carpeta raíz.
...
- ELF: la secuencia de bytes de la nota de código gnu-build-id.
...
siguientes reglas de mayúsculas/minúsculas:
...
El nombre del archivo de símbolos depende de la plataforma. En Windows, la extensión
del archivo (ya sea `.exe`, `.dll` o `.pdb`) se reemplaza por `.sym`. En todas las
demás plataformas, la extensión `.sym` se **añade** al nombre completo del archivo,
...
El UUID se divide tomando los primeros 20 dígitos hexadecimales en bloques de 4 caracteres,
y se crea un directorio para cada bloque. En el directorio final,
LLDB suele esperar un enlace simbólico con el nombre de los últimos 12 dígitos hexadecimales, que
apunta al archivo dSYM real.

Esto no es realmente una característica de LLVM. De hecho, es una característica de
`CoreFoundation` y está implementada exclusivamente en macOS sobre Spotlight.
Spotlight indexa estas rutas y la API privada `DBGCopyFullDSYMURLForUUID` es
utilizada por lldb para localizar los símbolos. macOS usa los enlaces simbólicos de esas
ubicaciones.

Dado que el ejecutable o la biblioteca comparten el mismo UUID que el archivo dSYM, los
primeros se distinguen con el sufijo `.app`.
...
GDB admite varios métodos de búsqueda, según cómo se especifique el archivo de información de depuración. Sentry utiliza el [Build ID Method]: suponiendo que se haya escrito una nota o sección de GNU Build ID en el archivo ELF, esto define un identificador único para el ejecutable que también se conserva en el archivo de depuración.

El GNU Build ID es una cadena binaria de longitud variable, que generalmente consiste en un hash SHA1 de 20 bytes de la sección de código (`.text`). La ruta de búsqueda es
_pp/nnnnnnnn.debug_, donde _pp_ son los 2 primeros caracteres hexadecimales de la cadena de bits del Build ID, y _nnnnnnnn_ son el resto de la cadena hexadecimal. Para buscar ejecutables, se omite el sufijo `.debug`.
...
[Convenciones clave de SSQP] son una extensión del protocolo original Microsoft Symbol
Server para .NET. Especifica rutas de búsqueda para archivos PE, PDB, Mach-O y ELF.
La capitalización de todas las rutas de búsqueda es generalmente en **minúsculas**, excepto para el
campo age de los identificadores de PDB, que debe ir en **MAYÚSCULAS**.

Para archivos Mach-O y ELF, SSQP indica usar los mismos identificadores que
se usan en LLDB y en el método GNU build-id, respectivamente. Consulta las secciones anteriores
...
SSQP especifica un método de búsqueda adicional mediante suma de comprobación SHA1 del
...
plataforma Windows. Estos usan un identificador de depuración de _firma-edad_ además del
nombre de archivo para localizar los símbolos. Las rutas de archivo son idénticas a SSQP, salvo por
las reglas de uso de mayúsculas y minúsculas predeterminadas:

- Los nombres de archivo se mantienen **tal como están**.
- La firma de un identificador de PDB está en **mayúsculas**, pero la edad en
  **minúsculas**.
- La marca de tiempo de un identificador de PE está en **mayúsculas**, pero el tamaño en
  **minúsculas**.
...
Este diseño es idéntico al de _SymStore_, salvo que los dos primeros caracteres
...
`<build id>` es la secuencia de bytes de la nota de código gnu-build-id, igual que en el caso de ELF del esquema de rutas unificado.
