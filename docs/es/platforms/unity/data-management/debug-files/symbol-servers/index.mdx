...
Sentry consulta los repositorios externos en el orden en que se configuran. Si hay repositorios personalizados, se consultan primero. Solo los archivos de información de depuración que no se encuentren en uno de los repositorios personalizados se consultan en los integrados.
...
los eventos pueden agruparse de manera diferente con la nueva información y generar nuevas incidencias.
...
- **Servidor de símbolos HTTP**: Un servidor HTTP que entrega archivos de depuración en una
  ruta configurable. Las búsquedas en el servidor generalmente deben ser no sensibles a mayúsculas y minúsculas,
  aunque puede configurarse una escritura con mayúsculas específica en los ajustes. Ten en cuenta
...
- **Bucket de Amazon S3**: Ya sea un bucket completo de S3 o un subdirectorio. Esto
...
- **Bucket de Google Cloud Storage**: Ya sea un bucket completo de GCS o un
  ...
  entre mayúsculas y minúsculas, por lo que recomendamos almacenar todos los archivos en minúsculas.
...
2. **Casing de ruta**: Define qué uso de mayúsculas utiliza Sentry para consultar los archivos de
   información de depuración. El valor predeterminado es mixto y utilizará el uso
...
3. **Estructura del directorio**: La estructura interna del bucket o el protocolo
   del servidor de símbolos. Hay tres estructuras a elegir, que se
   describen en la siguiente sección. El valor predeterminado es _"Específica de la plataforma"_.
...
La descarga de un Portable PDB desde el servidor de símbolos de NuGet requiere proporcionar la suma de comprobación de depuración mediante el encabezado `SymbolChecksum`.
...
La siguiente tabla muestra el mapeo de las estructuras admitidas a los esquemas de rutas
...
calculase a partir de _Debug Identifiers_ eliminando los guiones y aplicando las
...
El UUID se divide separando los primeros 20 dígitos hexadecimales en bloques de 4 caracteres, y se crea un directorio para cada bloque. En el directorio final, LLDB suele esperar un enlace simbólico con el nombre de los últimos 12 dígitos hexadecimales, que apunta al archivo dSYM real.

Esto no es en realidad una función de LLVM. De hecho, es una función de `CoreFoundation` y está implementada exclusivamente en macOS sobre Spotlight. Spotlight indexa estas rutas y la API privada `DBGCopyFullDSYMURLForUUID` es empleada por lldb para localizar los símbolos. macOS utiliza los enlaces simbólicos de esas ubicaciones.
...
La ruta se basa en un `build_id` formateado en hexadecimal con letras minúsculas. Los dos primeros bytes de ese ID se usan como carpeta raíz.

La construcción de `build_id` difiere entre formatos de archivo:
...
- WASM: sección `build_id` en bytes
...
La grafía de todas las rutas de búsqueda es generalmente en **minúsculas**, excepto el
campo age de los identificadores PDB, que debe ir en **MAYÚSCULAS**.

Para los archivos Mach-O y ELF, SSQP especifica usar los mismos identificadores que
se usan en los métodos de LLDB y GNU build id, respectivamente. Consulta las secciones anteriores
...
SSQP especifica un método de búsqueda adicional mediante checksum SHA1 del
...
El servidor público de símbolos proporcionado por Microsoft solía alojar solo PDB para la
...
Como el Microsoft Symbol Server original no servía archivos ELF ni Mach-O,
...
Este diseño es idéntico a _SymStore_, excepto que los dos primeros caracteres
