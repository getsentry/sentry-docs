...
Los ejecutables, las bibliotecas dinámicas y los acompañantes de depuración en todas las plataformas de Apple usan
el formato contenedor *Mach Object*, o abreviado *Mach-O*. Esto aplica a iOS,
iPadOS, tvOS, watchOS, macOS y visionOS.

* **Ejecutables** no llevan extensión de archivo. En aplicaciones de escritorio, a menudo se
  ubican dentro de estructuras de paquetes de app con el sufijo `.app`. A menos que se
  eliminen manualmente, los ejecutables contienen información de unwinding y una tabla de símbolos.
...
* **Acompañantes de depuración** se colocan en una estructura de carpetas con la extensión `.dSYM`,
  y se encuentran en `<name>.dSYM/Contents/Resources/DWARF/<name>`.
  Por lo general contienen una tabla de símbolos e información de depuración, pero rara vez información de unwinding.
...
el archivo dSYM debe crearse con el siguiente comando:
...
En las distribuciones de Linux, los ejecutables y la información de depuración se almacenan en contenedores *ELF*. A diferencia de otras plataformas, no existe un contenedor ni un especificador específico para archivos de depuración asociados.

La información de depuración forma parte del binario (ejecutable o biblioteca) y se elimina al generar compilaciones de lanzamiento debido a su tamaño. Sin embargo, hay una manera de conservarla en un archivo separado (ya sea en una ubicación diferente o con la extensión `.debug`):
...
Las bibliotecas compartidas instaladas a través de gestores de paquetes suelen proporcionar su información de
...
el tiempo requerido para subir archivos de información de depuración a Sentry y así mejorar
...
* **Bibliotecas dinámicas** usan la extensión de archivo `.dll` y, por lo demás, se comportan
  exactamente igual que los ejecutables.

* **Archivos de depuración** se almacenan en archivos `.pdb`. Por lo general, contienen información
...
convertir archivos de depuración nativos en símbolos de Breakpad. Estos conversores reúnen toda
la información disponible en un solo archivo, de modo que solo sea necesario subir
un único archivo.

A diferencia de los archivos de depuración nativos, los símbolos de Breakpad descartan mucha información
que no se requiere para procesar minidumps. En particular, las funciones inline no están
declaradas, por lo que Sentry no puede mostrar marcos inline en las trazas de pila.
...
Ten en cuenta que no admitimos los source maps, que también son un formato usado para la depuración de WASM, pero que tienen limitaciones que los hacen poco prácticos para una herramienta de notificación de fallos como Sentry.
...
está disponible en `LLVM` [desde la versión 17](https://github.com/llvm/llvm-project/commit/c7af9ae577bb04c5fe120fc07844a500818c8f47) (y, con ella, en la cadena de herramientas correspondiente de `emscripten`).
...
a la vez que se elimina toda la información de depuración del binario de la versión.

<div id="proguard-mappings">
  ## Asignaciones de ProGuard
</div>

Los archivos de asignación de ProGuard permiten que Sentry resuelva rutas de clases y
nombres de métodos de Java ofuscados a su forma original. En ese sentido, actúan como archivos de
información de depuración para aplicaciones Java y Android.