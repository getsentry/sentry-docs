...
description: "Obtén información sobre los requisitos de las herramientas de compilación que permiten a Sentry identificar de forma unívoca los archivos de información de depuración entre compilaciones."
sidebar_order: 2
---
...
* **Code Identifier**: El identificador único del ejecutable o de la biblioteca
  dinámica (el archivo de código). El contenido de este identificador depende de la
  plataforma: los archivos Mach-O usan un UUID, los archivos ELF un hash SHA y los archivos PE usan
...
* **Debug Identifier**: El identificador único del archivo de depuración asociado. En
  contraste con el identificador de código, Sentry aplica la misma estructura en todas
  las plataformas. En Windows, este es el id único real del archivo PDB; en todas
...
calcularán un *Debug Identifier* para cada archivo cargado. Este identificador se
asocia con ejecutables y bibliotecas, así como con sus archivos de depuración asociados, para garantizar
que puedan localizarse de forma unívoca mediante un mecanismo común.

<Alert>
  La información de depuración no tiene que estar asociada con versiones. El identificador
  único de depuración garantiza que Sentry pueda elegir los archivos correctos para cada informe
  de fallos. Sin embargo, se recomienda configurar versiones en el cliente para
  aprovechar otras funciones.
</Alert>

Para eventos nativos, la página de detalles de la incidencia muestra una lista de *Loaded Images*.
...
que coincidan con él en la pantalla de configuración de *Debug Files*.

`sentry-cli` puede ayudar a imprimir propiedades de los archivos de información de depuración, como su
identificador de depuración. Consulta [*Checking Debug Information Files*](/es/cli/dif/#checking-files) para obtener más información.
...
ID de compilación, pero a veces pueden requerir configuración adicional. `gcc` lo hace
...
*El identificador debe estar presente e idéntico tanto en el binario como
en los archivos de información de depuración separados.* Si falta el ID por algún motivo,
carga los archivos antes de separar la depuración para que `sentry-cli` pueda calcular un
identificador estable a partir del archivo sin depurar.

<div id="pdb-age-mismatches">
  ## Desajustes de edad de los PDB
</div>
