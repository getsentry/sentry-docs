...
Agregar Sentry a tu aplicación te proporciona una gran cantidad de información muy valiosa sobre errores y rendimiento que de otro modo no obtendrías. Y mucha información es buena, siempre que sea la correcta y en un volumen razonable.

<div id="sampling-error-events">
  ## Muestreo de eventos de error
</div>

Para enviar una muestra representativa de tus errores a Sentry, configura la opción <PlatformIdentifier name="sample-rate" /> en tu SDK con un número entre `0` (se envía el 0% de los errores) y `1` (se envía el 100% de los errores). Es una tasa estática que se aplica por igual a todos los errores. Por ejemplo, para muestrear el 25% de tus errores:
...
Cambiar la tasa de muestreo de errores requiere volver a desplegar. Además, establecer una tasa de muestreo en el SDK reduce la visibilidad del origen de los eventos. Definir un límite de tasa para tu proyecto (que solo descarta eventos cuando el volumen es alto) puede adaptarse mejor a tus necesidades.
...
1. Capturar un único rastro implica una sobrecarga mínima, pero capturar rastros de _cada_ carga de página o _cada_ solicitud a la API puede añadir una carga indeseada a tu sistema.
2. Habilitar el muestreo te permite gestionar mejor la cantidad de eventos enviados a Sentry y adaptar el volumen a las necesidades de tu organización.

Elige una tasa de muestreo que busque equilibrar el rendimiento y el volumen con la precisión de los datos. No quieres recopilar _demasiados_ datos, pero sí los suficientes para extraer conclusiones significativas. Si no sabes qué tasa elegir, empieza con un valor bajo y auméntalo gradualmente a medida que conozcas mejor tus patrones de tráfico y volumen.
...
1. Tasa de muestreo uniforme (<PlatformIdentifier name="traces-sample-rate" />):
   ...
   - Ofrece una sección transversal uniforme de transacciones, sin importar dónde ocurran en tu aplicación ni bajo qué circunstancias.
...
De forma predeterminada, ninguna de estas opciones está configurada, lo que significa que no se enviarán transacciones a Sentry. Debes configurar una de las opciones para comenzar a enviar transacciones.
...
Sea cual sea la decisión de muestreo de una transacción, esa decisión se pasará a sus spans secundarios y, a partir de ahí, a cualquier transacción que posteriormente se inicie en otros servicios.
