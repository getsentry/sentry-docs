...
description: "Conoce los requisitos de las herramientas de compilación que permiten a Sentry identificar de forma unívoca los archivos de información de depuración entre compilaciones."
sidebar_order: 2
---
...
* **Identificador de depuración**: El identificador único del archivo de depuración asociado. A
  diferencia del identificador de código, Sentry aplica la misma estructura en todas las
...
que puedan ubicarse de forma unívoca mediante un mecanismo común.
...
<Alert>
  ...
  de fallo. Sin embargo, se recomienda configurar versiones en el cliente para
...
en los archivos de información de depuración “stripped” (sin símbolos).* Si falta el ID por alguna razón,
sube los archivos antes de quitar símbolos para que `sentry-cli` pueda calcular un
identificador estable a partir del archivo sin “strip”.
...
un campo de edad. La firma se genera cuando se crea inicialmente el PDB y
...
modifica después de generarse el PE, su edad podría divergir. Esto
...
también puede usarse para añadir un ID de compilación si tu toolchain aún no lo admite. Además, te permite
