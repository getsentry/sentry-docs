...
description: "Obtén información sobre los requisitos de las herramientas de compilación que permiten a Sentry identificar de manera única los archivos de información de depuración entre compilaciones."
sidebar_order: 2
---
...
* **Identificador de código**: El identificador único del ejecutable o la biblioteca
  ...
  los archivos Mach-O usan un UUID, los archivos ELF un hash SHA, los archivos PE usan
  una concatenación de ciertos atributos de cabecera. En WebAssembly usamos un
  UUID incrustado en la sección `build_id` del archivo.

* **Identificador de depuración**: El identificador único del archivo de acompañamiento de depuración. A
  diferencia del identificador de código, Sentry impone la misma estructura en todas
...
calcularán un *Identificador de depuración* para cada archivo subido. Este identificador está
asociado con ejecutables y bibliotecas, así como con archivos de acompañamiento de depuración, para garantizar
que se puedan localizar de forma única mediante un mecanismo común.

<Alert>
  La información de depuración no tiene que estar asociada a versiones. El identificador
...
`sentry-cli` puede ayudar a mostrar propiedades de archivos de información de depuración como su
identificador de depuración. Consulta [*Comprobar archivos de información de depuración*](/es/cli/dif/#checking-files) para obtener más información.

<div id="gnu-build-identifiers">
  ## Identificadores de compilación de GNU
</div>

Para archivos ELF en Linux, Sentry utiliza el identificador de compilación de GNU para calcular el
...
archivos de información de depuración con símbolos eliminados.* Si el ID falta por alguna razón,
sube los archivos antes de eliminar los símbolos para que `sentry-cli` pueda calcular un
identificador estable a partir del archivo sin modificar.
...
un campo de edad. La firma se genera cuando se crea el PDB por primera vez y
...
asigna el mismo identificador a ambos archivos. Sin embargo, esto requiere que ambos
archivos se carguen en la misma ejecución del comando de carga. De lo contrario, los
identificadores divergirán y Sentry podría no ser capaz de localizar el archivo correcto
...
`LLVM` (y, con él, la correspondiente toolchain `emscripten`) es compatible con esto [desde la versión 17](https://github.com/llvm/llvm-project/commit/c7af9ae577bb04c5fe120fc07844a500818c8f47).
