---
title: Muestreo
description: "Aprende a configurar el volumen de eventos de error y transacciones que se envían a Sentry."
sidebar_order: 50
---

Incorporar Sentry en tu aplicación te ofrece mucha información valiosa sobre errores y rendimiento que de otro modo no tendrías. Y mucha información es buena, siempre que sea la adecuada y en un volumen razonable.

<div id="sampling-error-events">
  ## Muestreo de eventos de error
</div>

Para enviar una muestra representativa de tus errores a Sentry, establece la opción <PlatformIdentifier name="sample-rate" /> en la configuración de tu SDK a un número entre `0` (0% de errores enviados) y `1` (100% de errores enviados). Esta es una tasa fija que se aplicará por igual a todos los errores. Por ejemplo, para muestrear el 25% de tus errores:
...
1. Capturar un solo rastro implica una sobrecarga mínima, pero capturar rastros para _cada_ carga de página o _cada_ solicitud de API puede añadir una carga indeseada a tu sistema.
...
Elige una tasa de muestreo que busque equilibrar el rendimiento y el volumen con la precisión de los datos. No quieres recopilar _demasiados_ datos, pero sí los suficientes para extraer conclusiones significativas. Si no estás seguro de qué tasa elegir, empieza con un valor bajo y ve aumentándola gradualmente a medida que conozcas mejor tus patrones de tráfico y volumen.
...
1. Tasa de muestreo uniforme (<PlatformIdentifier name="traces-sample-rate" />):
   ...
   - Ofrece un conjunto uniforme de transacciones, sin importar dónde ocurran en tu app ni bajo qué circunstancias.
...
Cuando uses instrumentación personalizada para crear una transacción, puedes agregar datos al <PlatformIdentifier name="sampling-context" /> pasándolos como segundo argumento opcional a <PlatformIdentifier name="start-transaction" />. Esto resulta útil si hay datos a los que quieres que el muestreador tenga acceso pero que no quieres adjuntar a la transacción como `tags` o `data`, por ejemplo porque son sensibles o demasiado grandes para enviarlos junto con la transacción. Por ejemplo:
...
Sea cual sea la decisión de muestreo de una transacción, esa decisión se propagará a sus spans secundarios y, desde ahí, a cualquier transacción que posteriormente inicien en otros servicios.
...
Si la transacción que se está creando actualmente es una de esas transacciones posteriores (es decir, si tiene una transacción principal), la decisión de muestreo ascendente (principal) se incluirá en los datos del contexto de muestreo. Tu <PlatformIdentifier name="traces-sampler" /> puede usar esta información para decidir si heredar esa decisión. En la mayoría de los casos, heredar es lo adecuado para no romper trazas distribuidas. Una traza rota no incluirá todos tus servicios.

<PlatformContent includePath="performance/always-inherit-sampling-decision">

En algunos SDK, por conveniencia, la función <PlatformIdentifier name="traces-sampler" /> puede devolver un valor booleano, de modo que la decisión del principal pueda devolverse directamente si ese es el comportamiento deseado.
...
Si al crear la transacción ya sabes si quieres que se envíe a Sentry o no, puedes pasar una decisión de muestreo directamente al constructor de la transacción (ojo: no en el objeto <PlatformIdentifier name="custom-sampling-context" />). Si lo haces, la transacción no quedará sujeta a <PlatformIdentifier name="traces-sample-rate" /> ni se ejecutará <PlatformIdentifier name="traces-sampler" />, por lo que puedes tener la certeza de que la decisión que pases no se sobrescribirá.
...
1. Si <PlatformIdentifier name="traces-sampler" /> no está definido, pero existe una decisión de muestreo del padre, se usará la decisión del padre.
