---
title: Sentry for AI
sidebar_order: 1
description: "Connect Sentry to your AI coding assistants and workflows. Install agent skills, use the MCP server, or add monitoring to your AI applications."
new: true
keywords:
  - AI
  - artificial intelligence
  - LLM
  - machine learning
  - MCP
  - Model Context Protocol
  - agent skills
  - AI monitoring
  - vibe coding
  - Claude
  - Cursor
  - Copilot
---

<LinkCardGrid>
  <LinkCard
    href="/ai/agent-skills/"
    iconSrc="/ai/img/IconBot.svg"
    title="Agent Skills"
    description="Teach your AI coding assistant how to set up and configure Sentry. Works with Claude Code, Cursor, GitHub Copilot, and more."
    className="w-full md:w-[calc(50%-12px)]"
  />
  <LinkCard
    href="/ai/mcp/"
    iconSrc="/ai/img/IconCode.svg"
    title="Sentry MCP Server"
    description="Connect your AI assistant directly to Sentry using the Model Context Protocol for real-time error analysis and debugging."
    className="w-full md:w-[calc(50%-12px)]"
  />
  <LinkCard
    href="/ai/monitoring/"
    iconSrc="/ai/img/IconStats.svg"
    title="AI Monitoring"
    description="Add Sentry monitoring and debugging to your AI agents and MCP servers. Track LLM calls, token usage, and tool executions."
    className="w-full md:w-[calc(50%-12px)]"
  />
  <LinkCard
    href="/ai/dotagents/"
    iconSrc="/ai/img/IconPackage.svg"
    title="dotagents"
    description="A package manager for .agents directories. Declare agent skill dependencies, lock versions for reproducibility, and share skills across your team."
    className="w-full md:w-[calc(50%-12px)]"
  />
</LinkCardGrid>

## AI In Sentry

Looking for Sentry's AI-powered debugging features? Sentry uses AI to help you debug faster with automated root cause analysis, fix suggestions, and AI-powered PR reviews.

**Key features:**

- **[Seer](/product/ai-in-sentry/seer/)** - AI debugging agent
  - [Root Cause Analysis & Issue Fix](/product/ai-in-sentry/seer/root-cause-analysis/) - Automated root cause analysis and code fixes for your issues
  - [Bug Prediction / AI Code Review](/product/ai-in-sentry/seer/ai-code-review/) - Get error predictions and code reviews on your pull requests
- **[AI-Powered Query Builder](/product/explore/trace-explorer/)** - Use natural language queries to search your traces and spans data in Trace Explorer
- **[AI Replay Summaries](/product/explore/session-replay/replay-details/#ai-powered-replay-summary-web-only)** - Get AI-generated summaries of session replays to debug issues faster

[Learn more about AI In Sentry](/product/ai-in-sentry/)
