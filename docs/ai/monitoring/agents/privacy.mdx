...
- `gen_ai.input.messages` (previously `gen_ai.prompt` or `gen_ai.request.messages`)
- `gen_ai.tool.call.arguments` (previously `gen_ai.tool.input`)
- `gen_ai.tool.call.result` (previously `gen_ai.tool.output`)
- `gen_ai.output.messages` (previously `gen_ai.response.tool_calls` or `gen_ai.response.text`)
