...
If you already manage agent tooling with [dotagents](/ai/dotagents/), install the same plugin from there:

```bash
npx @sentry/dotagents add getsentry/agent-plugin
```

This installs into your global dotagents scope by default. Pass `--project` to install it for the current repository instead.
