...
1.  In sentry.io, navigate to [**Settings > Integrations**](https://sentry.io/orgredirect/organizations/:orgslug/settings/integrations/) > **Slack**.
...
You can receive personal workflow, deploy, and alert notifications from our Slack integration. Manage your [personal notification settings](/product/notifications/notification-settings/) by navigating to **User Settings > Notifications**.
...
You can receive [team alert notifications](/product/monitors-and-alerts/alerts/) from our Slack integration. To enable this feature, type `/sentry link team [organization_slug]` in the desired Slack channel. To view a team's associated Slack channel in sentry.io, navigate to [**Settings > Teams**](https://sentry.io/orgredirect/organizations/:orgslug/settings/teams/) > **[Team] > Notifications**.
...
For example, Error alerts will have tools to "Resolve", "Archive" and "Assignee" right from Slack. If you've enabled [Seer](/product/ai-in-sentry/seer/), you will also the option to fix the issue right from Slack.
...
![Slack notification of an alert =500x](./img/slack-alert-metric.png)
...
| `links:write`          | Allows Sentry to [unfurl](https://docs.slack.dev/messaging/unfurling-links-in-messages/) `sentry.io` links shared in your workspace into rich previews.                                                           |
...
Lastly, we also require the following scopes on each Slack user's behalf. These are limited to read-only access, meant for [unfurls](https://docs.slack.dev/messaging/unfurling-links-in-messages/) (rich previews) and account linking.
...
### Disabling the AI Assistant Experience
...
![Example of Seer chats now in Slack App DMs =600x](./img/slack-agent-dms.png)
