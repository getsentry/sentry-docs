...
The [issue linking](/integrations/integration-platform/ui-components/issue-link) and [alert action](/integrations/integration-platform/ui-components/alert-action) UI components can use `FormField` components to display attributes in their respective modals. These components can render custom forms with inputs specified according to the schemes below. They behave similarly to their HTML equivalents.
