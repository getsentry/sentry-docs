...
def setup():
    ...
    return redirect('https://sentry.io/orgredirect/organizations/:orgslug/settings/integrations/')
```
...
Public integrations generate [authentication tokens](/integrations/integration-platform/#auth-tokens) after a successful [OAuth installation](#oauth-process).
...
When a user uninstalls your integration, you'll receive an `installation.deleted` request to your webhook URL. For more information, check out the [full Webhooks documentation](/integrations/integration-platform/webhooks/).
