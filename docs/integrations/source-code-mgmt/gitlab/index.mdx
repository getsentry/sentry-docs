...
The GitLab integration connects your repositories to Sentry to bring [commit tracking](#commit-tracking), [suspect commits](#suspect-commits), [stack trace linking](#stack-trace-linking), and [issue management](#issue-management) into the contexts that help Sentry debug issues and power [Seer AI features](#seer) like Autofix and Code Review. It also supports [resolving issues via commit or MR](#resolve-via-commit-or-pull-request) and [Code Owners](#code-owners) for automatic issue routing.
