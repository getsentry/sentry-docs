...
This is optional but can help you identify the correct file when multiple mapping files exist for the same app.

```bash
# Creates an association to the release my.app.id@1.0.0+1
sentry-cli upload-proguard \
    --uuid A_VALID_UUID \
    app/build/outputs/mapping/{BuildVariant}/mapping.txt \
    --app-id my.app.id \
