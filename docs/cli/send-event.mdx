...
As of version `1.71`, the `send-event` command can accept an optional argument that specifies a path to the stored JSON representation of an [event payload](https://develop.sentry.dev/sdk/foundations/data-model/events/). When used, it will load the file, validate the event and send it to Sentry.
