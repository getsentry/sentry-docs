...
- **Frontmatter descriptions are injected into MD exports** as italic text after the H1 heading. Pages without a `description` in their frontmatter lose this LLM relevance signal — LLM agents read the first ~500 lines to decide if a page is useful, so a good description helps them prioritize correctly.
