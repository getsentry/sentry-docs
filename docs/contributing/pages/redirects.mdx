...
For simple one-to-one redirects, set them up in `src/middleware.ts`.

To add a new redirect in [`src/middleware.ts`](https://github.com/getsentry/sentry-docs/blob/master/src/middleware.ts#L60), add a new object to `REDIRECTS` with the following properties:

- `from`: The incoming request path.
- `to`: The new destination path you want to route to instead.
