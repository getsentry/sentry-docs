...
When a variable call is invalid (or errors), or doesn't match something in the known scope, it will produce an error message in the output in development mode,
and force the build to fail in production.

If you need to escape an expression you can add a leading `\` to the `@inject`.
