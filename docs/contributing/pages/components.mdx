...
Something that applies to guides and other platforms, but not the base JavaScript platform.
...
`PlatformSection` follows explicit `fallbackGuide` and `fallbackPlatform` rules,
but a guide does not automatically inherit its parent platform's key. For example,
`javascript.express` can inherit `javascript.node` through `fallbackGuide`, while
`supported={["javascript"]}` matches only the base JavaScript platform, not its guides.
Page-frontmatter support rules differ: they also check the guide's parent platform.
...
Use the `defaults` prop to select one or more options when the page loads:
...
Each option in `defaults` must also be included in `options`.
