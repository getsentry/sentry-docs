...
You can call out specific lines in a code block using the `{fromLineA-toLineB}` for ranges
or `{lineA,lineB}` for specific lines (or a combination of both):
...
Highlighting diffs is also supported using the `diff` language:
...
```
````
Or if you want to preserve syntax highlighting, you can add a `diff` metadata to any code block,
