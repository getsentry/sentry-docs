...
You can use the `|inline` modifier within an image alt text to render it inline with the text like in the example below:
...
Note, that the `|inline` modifier is a hack and gets stripped out by the markdown parser.
