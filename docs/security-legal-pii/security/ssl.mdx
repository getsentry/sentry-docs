...
Starting on 2025-06-20, the following domains with certificates previously
issued by DigiCert will be transitioned to being issued by Let's Encrypt:
...
At the moment, all Sentry TLS certificates use either Digicert and Let's Encrypt as certificate authorities. If this list of roots needs to be changed, we will publish an announcement on [status.sentry.io](https://status.sentry.io/).
