---
title: "What Errors to Prioritize"
sidebar_order: 5
description: "Practical guidance on what errors to catch, how to search issues in Sentry, and when to set alerts."
---
...
When that alert fires, [triage and fix from Slack](https://sentry.io/cookbook/fix-sentry-issues-from-slack/) (~10 min recipe) so the team can ask questions and kick off Seer without leaving the channel.
...
When a resolved issue comes back, [auto-open a PR on the regression](https://sentry.io/cookbook/regressed-issue-to-pr-cursor/) (~20 min recipe).
...
Once feedback is assigned, [auto-triage it with a Cursor agent](https://sentry.io/cookbook/fix-user-feedback-with-cursor-agents/) (~20 min recipe).

## Error Handling Strategy

### Capture Context, Not Just Exceptions

When you catch an error, add tags and context to make debugging instant. Set user info globally so it applies to all events. Enable [Session Replays](/product/session-replay/) and [Logs](/product/logs/) for even more visibility.
