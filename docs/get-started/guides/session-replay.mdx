...
You've set up [Sentry Session Replay](/product/session-replay/). Now what? 
...
Explore the [Session Replay product walkthrough guides](/product/session-replay/) to learn more about the Sentry interface and discover additional tips.

## Go one step further

Ready for a full workflow? Try a Cookbook recipe:

- [Fix user feedback with Cursor agents](https://sentry.io/cookbook/fix-user-feedback-with-cursor-agents/) (~15 min) — turn feedback + session context into fixes
- [Fix Sentry issues from Slack](https://sentry.io/cookbook/fix-sentry-issues-from-slack/) (~15 min) — triage with replay-linked issues in Slack
- [Self-healing workflow with Seer](https://sentry.io/cookbook/self-healing-workflow-seer/) — close the loop from error to PR
