...
Still on `console.log`? [Replace it with LogTape](https://sentry.io/cookbook/structured-logging-logtape/) (~20 min recipe) so the same structured logs go to the console locally and to Sentry in production.
...
Make your logs queryable, and far less noisy, by [Structuring your logs](https://sentry.io/cookbook/structure-your-logs/) (~30 min recipe).
...
To pipe Supabase infrastructure logs into Sentry and alert on them, see [Set up a monitor from a Supabase log drain](https://sentry.io/cookbook/setup-supabase-log-drain-monitoring/) (~15 min recipe).
...
Explore the [Logs product walkthrough guides](/product/logs/) to learn more about the Sentry interface and discover additional tips.
