...
All of the configurations, team and project structure, and most metadata will be relocated. Specifically, for every organization you [choose to relocate](#choose-which-organizations-to-relocate) this includes:
...
One thing to note is that while we won't be able to transfer over historical event data, new incoming events will populate the projects in your relocated organization as soon as you [update your DSNs](#update-your-dsns). All relocated organization members will be automatically notified and re-invited via email once the relocation completes.

### Version support window

Self-serve relocation is only possible from any of the last two minor self-hosted releases. That is, if the latest self-hosted release is 24.1.2, your self-hosted install must be on at least 24.1.0 to be successfully relocate.
