...
Every release of Relay also provides a set of binaries that come with a minimum required OS version.
...
We support LTS versions of the listed Operating Systems as long as they receive updates from their vendors.
The only exception is Ubuntu x86, which will be supported 2 years after LTS support has officially ended to allow
customers with extended support to continue using Relay.
