...
Add another filter by clicking the "Add Query" button. In this example, we're using the [Apdex](/product/insights/overview/metrics/#apdex) definition where tolerable response times are between the satisfactory threshold and four times the satisfactory threshold:
