...
Once you've [configured the Sentry SDK](/ai/monitoring/mcp/getting-started/) for your MCP project, you'll start receiving data in MCP Dashboards, found in [Sentry Dashboards](https://sentry.io/orgredirect/organizations/:orgslug/dashboards/).
...
Underneath these widgets are tables that allow you click on a row to view data in more detail on their own dashboards:

- **MCP Tools**: Performance metrics for each MCP tool including request count, error rate, average duration, and P95 latency
- **MCP Resources**: Access patterns and performance for MCP resources by URI, showing requests, error rate, average duration, and P95 latency
- **MCP Prompts**: Usage statistics for MCP prompt templates by name, including requests, error rate, average duration, and P95 latency

You can also search for these dashboards on the Dashboards home page.
