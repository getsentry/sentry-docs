...
Since the Dev Toolbar will be visible to users within your app, it's important to consider which environments the Toolbar should apply to.

In dev & staging environments, include the Toolbar so that all developers and testers can use it and quickly go from the page they're looking at back to Sentry for further debugging.
...
You will need to edit the [Project Settings](https://sentry.io/orgredirect/organizations/:orgslug/settings/projects/) page to allow the Toolbar to connect to Sentry by configuring your dev, staging, and production domains. Only add domains to this list that you trust and control.
...
If the toolbar `<script>` ([step #3 Install](3-install)) is included on your site, and `SentryToolbar.init()` is called, then a "Login to Sentry" button will be visible to the public. This is not ideal, but your data in Sentry will still be safe as users outside of your Sentry organization will not be able to login.
...
<Alert>
  If you're using feature flags inside your product then also setup [Feature Flag Evaluation and Change Tracking](https://docs.sentry.io/platforms/javascript/feature-flags/) within your SDK.
</Alert>
