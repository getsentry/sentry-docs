...
If the SDK you are using doesn't automatically instrument HTTP requests, you can use <PlatformLink to="/performance/instrumentation/requests-module/">custom instrumentation</PlatformLink> instructions to instrument your application. If your platform doesn't have dedicated custom instrumentation instructions, try adapting the code from another platform, since all the concepts are the same.
