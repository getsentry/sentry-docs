...
- the `description` field contains the full URL of the request (e.g., `"GET http://my.service.io/some-data"`)
- if the request is to a relative URL (e.g., `"GET /data.json"`) the `server.address` span data value is set to the server address (e.g., `"my.service.io"`)
