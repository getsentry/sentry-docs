...
At the top of the page, there are three graphs that summarize the overall behavior of outgoing HTTP requests. The first is a requests per minute (throughput) chart. The second is an average duration chart. The third is a chart that shows the percentage of 3xx, 4xx, and 5xx HTTP response status codes. You can click and drag to select a specific time range directly in a graph.

TODO: Do we need to describe Time Spent in detail? We did on the Queries page. Should we do it again here?
...
TODO: Search is not implemented, but will be for GA

By default, the table is sorted by descending time spent. You can also sort by throughput, duration, and HTTP response status code rate. The domain search input lets you search for specific domains.
