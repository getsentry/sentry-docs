...
Sentry expects that you have an orchestrator (like langchain) creating pipelines of one or more AI models (such as gpt-4)

In the AI Monitoring dashboard, we show you a table of the AI pipelines, and pull the token usage from your AI models.

If you are using OpenAI without langchain, you'll need to manually create "pipelines" with the `@ai_track` annotation.

If you are using langchain without OpenAI, you might have to manually record token usage with `record_token_usage()`.

Both of these manual helpers are documented below.
