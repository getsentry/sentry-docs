...
Install groups let you tag builds with one or more group names to control update visibility. When the Auto-Update SDK checks for updates, the server finds the single latest build (by semver version, then build number) that shares at least one install group with the request.
...
When the SDK checks for updates, the server returns the single latest build (highest semver version, with build number as tiebreaker) whose install groups overlap with the filter. Install groups for the filter are determined as follows:

- If the SDK **sends groups explicitly**, the server uses those.
- If the SDK **doesn't send groups**, the server looks up the uploaded build matching the device's current version and uses that build's upload groups. On iOS, the lookup uses the `mainBinaryIdentifier` (Mach-O UUID), which is unique per binary. On Android, it uses `buildVersion` and `buildNumber` — if multiple builds share the same values, the server picks the most recently uploaded one, which may lead to unexpected results.

We recommend configuring the SDK to send install groups explicitly rather than relying on server-side inheritance. On Android, the [Gradle plugin](/platforms/android/build-distribution/install-groups/#gradle-plugin) `installGroups` config handles this automatically — it tags the upload **and** embeds the groups in the app so the SDK sends them with every update check.
