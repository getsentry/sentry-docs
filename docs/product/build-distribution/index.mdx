...
1. If install groups are provided when checking for updates, only builds with at least one matching group are considered.
2. If no install groups are provided, all builds for that app and build configuration are eligible.
