...
After Seer opens a PR, it can update that same PR to address failing CI checks and your feedback. PR iteration is supported on GitHub.com only.
...
- **Feedback on GitHub:** Add a PR comment starting with `@sentry`, such as `@sentry Add a regression test for this fix`. Seer also acts on submitted PR reviews, including their summary and inline comments, without requiring an `@sentry` mention. You need write access to the repository to trigger iteration through GitHub comments or reviews.
- **Stop iteration:** Comment `@sentry stop iterating` on the PR to stop further automatic and manual iteration for that Autofix run. This also requires write access to the repository.
