...
How far Autofix proceeds automatically depends on two factors:
...
2. **Your automation preferences** — In your project's Seer settings, you can cap how far automation goes, regardless of the fixability confidence.

The automation preference acts as a maximum stopping point. Even if an issue has high fixability confidence, Autofix will respect your configured limit. The available stopping points are:
...
For example, if you've set **Stop after Plan**, Autofix will perform root cause analysis and solution planning, but won't create a PR—even for issues with very high fixability confidence. Conversely, if fixability confidence is low, Autofix may stop at root cause analysis even if your settings allow it to proceed further. 
