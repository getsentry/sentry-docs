...
Instead of having Seer generate the code fix directly, you can hand off the root cause analysis and solution plan to an external coding agent for implementation. This lets you leverage the strengths of specialized code generation tools while still benefiting from Seer's deep Sentry-context-aware debugging.

Supported coding agents for handoff:

- **Claude Code** — Anthropic's agentic coding tool. Seer passes the root cause and solution as a structured prompt that Claude Code can act on directly in your terminal or CI environment.
- **Cursor Cloud Agents** — Cursor's cloud-hosted background agents. Seer can trigger a Cursor Cloud Agent to implement the fix asynchronously, without requiring your local IDE to be open.
- **GitHub Copilot** — GitHub Copilot coding agent. Seer opens a GitHub issue or PR with the full context, which Copilot can then pick up and implement.
...
<Alert>
Coding agent integrations handle their own authentication and repository access. Make sure the agent has access to the same repositories connected to your Sentry project.
</Alert>
