...
With automation, you can combine these tools into powerful workflow - automatically identifying actionable issues in real time with scanner, and then using Seer to generate high quality fixes.
...
3. Generate a code patch that and open a PR

For a full product walk-through, please visit [issue fix](./issue-fix/).

<Arcade src="https://demo.arcade.software/ck6xlQQnspwe5RfFnjfy?embed&embed_mobile=tab&embed_desktop=inline&show_copy_link=true" />

### Issue Scan

Seer is able to automatically analyze issues as they are ingested by Sentry. This initial review is used to:

- Generate an "actionability" score - which indicates how likely it is that a particular issue is straightforward to fix
...
You can use automation to automatically trigger issue scans and fixes. There are three automation settings you can configure:
