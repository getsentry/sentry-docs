...
<Alert>
  When creating new metric alerts, the `is:unresolved` filter is added by
  default. This filter excludes events from archived and resolved issues. If you
  want your metric alerts to include archived issues, you can manually remove
  `is:unresolved` from your [filters](#filters).
</Alert>
...
### Anomaly Alerts Algorithm

Anomaly Alerts are powered by a blend of two core algorithms: Matrix Profiling and Prophet Forecasting. We leverage [TD Ameritrade's Matrix Profile](https://stumpy.readthedocs.io/en/latest/index.html) implementation and [Meta’s Prophet Library](https://facebook.github.io/prophet/), which are both Open Source.

Our Matrix Profile algorithm checks how “surprising” the most recent time interval is, compared with what we have seen in the past. More specifically, Matrix Profiling is a pattern matching and comparison technique which calculates a score based off of the minimum euclidean distance of the current subsequence to all previous subsequences. If the introduction of a new point (or set of points) causes this score to jump significantly, it can indicate that there is an anomaly.

![Matrix Profile visualization showing pattern matching and anomaly detection](./img/matrix_profile.jpeg)

We use Prophet to build a forecast based representation of what is “normal” for a particular time series - significant deviations from these forecasts can indicate an anomaly. Our forecasts use an additive approach where the time series is broken down into trend and seasonalities. We build a point estimate along with prediction intervals (PIs) indicating the range of values that we are expecting to see.

![Prophet forecast visualization showing trend, seasonality, and prediction intervals](./img/prophet_meta.png)

In our system, these approaches complement each other. Specifically, we flag an anomaly any time both algorithms agree _or_ one emits a very high confidence anomaly. By combining the results in this way, we are able to effectively reduce false positives and false negatives.
