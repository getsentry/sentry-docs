...
sidebar_order: 20
description: Learn more about the options for configuring a performance metric alert.
og_image: /og-images/product-alerts-create-alerts-metric-alert-config.png
---

Sentry provides several configuration options to create a performance metric alert based on your organization's needs. When a performance metric alert is triggered, Sentry automatically creates an [issue](/product/issues/) that you can view, triage, and assign on the [Alert Details](/product/alerts/alert-types/#performance-metric-alerts) page.
