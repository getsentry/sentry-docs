...
- **Prioritize using tags**: Filter issue alerts based on important tags, such as `customer_type=enterprise` or `url=/very/important/page`. You can find the list of tags available in your project in your project's [Tags](https://sentry.io/settings/projects/:projectId/tags/) settings. The list is an aggregation of all tag keys (default and custom) encountered in events for that project.
...
- **Ownership Rules**: Use [ownership rules and code owners](/product/issues/ownership-rules/) to let Sentry automatically send alerts to the right people, as well as to ease configuration burden. You can configure ownership in your project's [Ownership Rules](https://sentry.io/settings/projects/:projectId/ownership/) settings. In the case of ownership rules, when there are no matching owners, the alert goes to all project members by default. If this is too broad, and you'd like a specific owner to be the fallback, end your ownership rules with a rule like `*:<owner>`.
