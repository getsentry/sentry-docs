...
Sentry supports the following characters for the components of a metric:
- Metric name -> all word characters including `_`, `-`, `.`. Any character which is not supported, will be replaced by `_`.
- Metric tag keys -> all word characters including `_`, `-`, `.`, `/`. Any characters that is not supported, will be removed from the tag key.
