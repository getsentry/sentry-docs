...
- **Timers:** Tracks the execution time for a specific block of code (for example, the time to load a user profile).
...
By default, Sentry adds some common tags, such as `transaction`, `environment`, and `release`. You can also create your own custom tags to track attributes that help organize your data for your specific use case. For example, some useful custom tags are browser name, region, language, and customer.
