...
sidebar_order: 120
description: Tool calls available for use by the Sentry MCP Server
---

## Overview

MCP tools are the available commands that can be used by the MCP server. These tools are often called by eachother for additional information as needed. The tool calls descriptions within the MCP server provide the necessary details for clients to detect an available tool and call it. 
