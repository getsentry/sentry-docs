...
- **Android** and **React Native** — These SDKs support a [`beforeErrorSampling`](/platforms/android/session-replay/) callback that lets you filter which errors trigger a replay capture (for example, only capturing replays for unhandled crashes).
- **Flutter** — Replay recording is currently configuration-driven only. There are no manual start/stop APIs.

For full programmatic control over replay sessions, including starting, stopping, and flushing replays based on arbitrary conditions, see the [Web SDK's manual session control](/platforms/javascript/session-replay/understanding-sessions/#manually-starting-replay).
