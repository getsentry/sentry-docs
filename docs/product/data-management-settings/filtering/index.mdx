...
The web crawler filter allows you to filter out errors from web crawlers, determined by the event's user-agent, from the following sites: Baidu, Yahoo, Sogou, Facebook, Alexa, generic bots and spiders, Slack, Google Indexing, Pingdom, Lytics, AWS Security Scanner, Hubspot, and Bytedance. Errors from Slack's Slackbot web crawler will not be filtered.
