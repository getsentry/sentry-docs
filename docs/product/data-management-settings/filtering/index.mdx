...
- Transactions from legacy browsers
- Known web crawlers
...
### Transactions from Legacy Browser 

The legacy browser filters allow you to filter out certain versions of legacy browsers that are known to cause problems. 

As of February 2024, we have new, updated legacy browser filters, that are more up to date than the previous ones. These new filters will be periodically evaluated and updated to filter out versions of the browsers that we believe should be considered legacy. 

Any old legacy browser filters will still show up in Settings, but will be labeled with "Deprecated". They will continue to work, but you will not be able to turn them on if you don't already have them on. Once you turn an old filter off, you will be unable to turn it on again. 
...
![](/product/data-management-settings/filtering/builtin-inbound-filters.png)
