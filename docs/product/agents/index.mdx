---
title: Agents
sidebar_order: 215
sidebar_section: debug
description: >-
  Use AI observability in Sentry to trace and debug agent workflows and
  conversations. Monitor agent runs, tool calls, model interactions, token
  usage, and errors.
keywords:
  - agents
  - AI observability
  - LLM monitoring
  - agent tracing
  - MCP monitoring
  - tool calls
  - OpenAI
  - Anthropic
  - LangChain
  - token usage
  - agent tracing
  - AI tracing
og_image: /og-images/product-agents.png
---

Sentry helps you understand what's going on with your agent workflows. Using [agent tracing](/concepts/key-terms/agent-tracing/) automatically collects information about agent runs, tool calls, model interactions, and errors across your entire AI pipeline—from user interaction to final response. You can also replay past conversations. If you are looking for MCP monitoring, see [MCP Servers](/product/mcp-servers/).

![Agents Overview](./img/agent-dashboard.png)
![Agents Conversations](./img/transcript-span.png)

## Get Started

To start debugging agents, you must have an existing Sentry account and project set up. If you don't have one, [create an account here](https://sentry.io/signup/).

Use agent tracing by [setting up Sentry for Agents](/product/agents/getting-started/) and [name your agents](/product/agents/naming/) so they're identifiable in the dashboard. Then choose a [sampling strategy](/product/agents/sampling/) that keeps complete agent runs without sampling all application traffic.

## Example Use Cases

- Your agent is failing silently during tool execution, and you want to trace the complete agent flow to identify where it's breaking.
- Users report that your agent is returning unexpected or malformed responses, and you need to debug the full context of prompts, model calls, and outputs.
- Your agent workflows are experiencing performance issues, and you want to identify which steps (model calls, tool usage, or custom logic) are causing bottlenecks.

## Related Features

<LinkCardGrid>
  <LinkCard
    href="/product/agents/conversations/"
    iconSrc="/ai/img/IconCompass.svg"
    title="Conversations"
    description="Replay past conversations with your AI assistants. See every message and tool call in a chat-like view."
    className="w-full md:w-[calc(50%-12px)]"
  />
  <LinkCard
    href="/product/agents/dashboards/"
    iconSrc="/ai/img/IconStats.svg"
    title="Agents Dashboards"
    description="View agent executions, model costs, token usage, tool calls, and recent errors in the Agents Dashboards."
    className="w-full md:w-[calc(50%-12px)]"
  />
</LinkCardGrid>
