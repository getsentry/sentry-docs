...
You can also assign a team member to a specific user feedback submission on the **User Feedback Details** view. For user feedback submissions you consider spam, you can mark them as such selecting the feedback(s) and clicking "Mark as Spam". These user feedback submissions will then appear under the Spam heading. 
...
In order to do auto-spam detection, we use a Large Language Model to classify messages. We are using a language model internal to Google Cloud Platform where no data is stored or persisted. This is part of our existing [[sub-processor agreement](https://sentry.io/legal/dpa/#subprocessors)](https://sentry.io/legal/dpa/#subprocessors).
