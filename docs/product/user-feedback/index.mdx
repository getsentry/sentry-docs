...
You can also assign a team member to a specific user feedback submission on the **User Feedback Details** view. If you come across user feedback that you consider spam, select the feedback(s) and click "Mark as Spam". This feedback will then be filtered out and appear under the Spam heading. 
...
By default, Sentry auto-detects user feedback submissions that are likely spam and places them inside the “Spam” folder on the **User Feedback** page to improve the quality and actionability of feedbacks in the main list. Feedbacks detected as spam will also bypass any Alert Rules you have configured for User Feedback so you only get alerted when it matters.

In order to do auto-spam detection, we use a Large Language Model to classify messages. We are using a language model internal to Google Cloud Platform where no data is stored or persisted. This is part of our existing [sub-processor agreement](https://sentry.io/legal/dpa/#subprocessors).

If you’d like to opt out of spam detection, turn off "Enable Spam Detection" in Settings > Projects > [Project Name] > User Feedback (under “Processing”).
