...
The detector for this performance issue runs on a schedule and checks the 7 day p75 (75th percentile) Web Vitals scores for the top 5 highest opportunity pages on your website (_the pages that are both high traffic, and have issues worth fixing_).
...
* A _loading / render performance issue_ will be opened, if `TTFB`, `LCP`, or `FCP` falls below the `Meh` threshold.
* A _page stability issue_ will be opened, if `CLS` falls below the `Meh` threshold.
* An _interaction performance issue_ will be opened, if `INP` falls below the `Meh` threshold.
