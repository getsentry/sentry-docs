...
This view is useful for diagnosing the impacted audience of feature flag change. For example, if a new issue occurs and the flag distribution view shows that 100% of impacted users are receiving a recently changed feature flag value, this could support the hypothesis that the feature flag change is responsible for the new error.
