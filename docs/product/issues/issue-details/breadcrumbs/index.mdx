...
- **Type**: `type` is a semi-internal attribute that controls how breadcrumbs are categorized. If left unchanged, all breadcrumbs are recorded as `default`, but Sentry provides other [types](https://develop.sentry.dev/sdk/foundations/data-model/events/breadcrumbs/#breadcrumb-types) that each have their own color/icon in the UI.
...
Learn more detailed information about breadcrumb data in the [Breadcrumbs Interface developer documentation](https://develop.sentry.dev/sdk/foundations/data-model/events/breadcrumbs/).
