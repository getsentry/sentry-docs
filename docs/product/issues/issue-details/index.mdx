...
Sometimes the Trace Navigator will be replaced by a Related Issue. A Related Issue is a different issue which occurred during the same trace associated to the event in view. This is useful for understanding the context of the current issue and may help you identify the root cause of the current issue. A Related Issue allows the developer to quickly navigate between the two related events.
