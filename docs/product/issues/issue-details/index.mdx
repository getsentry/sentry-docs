...
While the <SandboxLink scenario="oneIssue" projectSlug="react">Issue Details</SandboxLink> page displays information about a specific event that's part of an issue. You can navigate between the events of an issue using the event navigator.
...
- **Event Navigator** - Allows you to select the latest, oldest, and recommended event from an issue and navigate between events in sequence.
