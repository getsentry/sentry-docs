...
You can manually mark an issue as `Resolved` when it’s been fixed. A plain **Resolve** treats any later event as a regression. With [releases](/product/releases/), you can instead resolve in a specific version so older traffic doesn't reopen the issue. See [3 Different Ways to Resolve Issues](/product/releases/#3-different-ways-to-resolve-issues).
