...
The general format of a rule is: `type:pattern owners`
 ...
 - **`pattern`** - The pattern you're matching on, which supports unix-style [glob syntax](<https://en.wikipedia.org/wiki/Glob_(programming)>). For example, add `*` to match anything and `?` to match a single character. _This is not a regex._ For example:
   - `src/javascript/*` for a `path` type
   - `https://www.example.io/checkout` for a `url` type
   - `Chrome 81.0.*` for a `tags.browser` type
 - **`owners`** - The owner, or list of owners, identified by either the email of the Sentry user or the name of the team, prefixed with `#`. Thus, the owner may be `example@company.com` or `example1@company.com example2@company.com` or `#backend-team`. A space separates each owner in the list of owners.
...
![Configuration options for defining ownership rules in project settings.](/product/issues/ownership-rules/project_settings_edit_ownership_rules.png)
...
![Assigned to with cog](/product/issues/ownership-rules/assigned_to_cog.png)

![Configuration options for defining ownership rules from an issue page.](/product/issues/ownership-rules/issue_details_edit_ownership_rules.png)
...
![Code Owners in UI](/product/issues/ownership-rules/codeowners.png)
...
1.  Go to an issue in your project. You should see the following option to link to your source code in the stack trace:
    ![Link Stack Trace](/product/issues/ownership-rules/link_stack_trace.png)
...
![External team mappings for GitHub/Gitlab](/product/issues/ownership-rules/external_team_mappings.png)

![External user mappings for GitHub/Gitlab](/product/issues/ownership-rules/external_user_mappings.png)
...
![Select from dropdown to automatically assign issues to their owners.](/product/issues/ownership-rules/auto-assign-issue-owner.png)
