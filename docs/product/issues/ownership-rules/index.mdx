...
Ownership rules and code owners are matched against individual events in an issue. Sentry evaluates an event by checking if _any_ of its in-app frames match a rule from:

1. Code owners, top-to-bottom
2. Ownership rules, top-to-bottom

Sentry takes the _last_ rule that matches for an event (if there are multiple owners in a rule, we take the leftmost owner). The order of in-app frames in an event is not used in determining the rule assignment.
...
Sentry will return the last matching rule, which in this case is `path:backend/endpoints/auth/* #auth-team #enterprise-team`. Since there are two owners in the last matching rule, Sentry will assign to the first owner, `#auth-team`. 
