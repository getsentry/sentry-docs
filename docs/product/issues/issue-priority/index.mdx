...
The default view on the **Issues** page is the "Prioritized" view, which shows high and medium priority issues. But all views can be filtered to include, for example, only high priority issues. You can do this by adding the query: `issue.priority:high` to the search bar and saving the view.
