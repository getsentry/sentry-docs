...
The enhanced priority calculations will consider additional factors when determining the priority of an issue for Python or JavaScript projects. In addition to the log level or actionability, Sentry will also consider the error message, whether or not the error is handled, and historical actions taken on similar issues.
