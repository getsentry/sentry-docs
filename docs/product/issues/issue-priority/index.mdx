...
Have feedback? Please let us know in the [GitHub discussion](...).
...
When Sentry receives an event, it assigns a priority to the issue based on the event's **log level** (for error issues) or **severity** (for non-error issues like performance). For Python and Javascript projects, Sentry will also consider the error message and whether or not the error is handled.
