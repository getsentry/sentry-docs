...
By default, all Sentry accounts have 100 builds of Size Analysis / month. Below our options for controlling which builds will process for Size Analysis.
...
See [Integrating Into CI](/product/size-analysis/integrating-into-ci/) for complete workflow examples, and the [Upload Guides](#upload-guides) for platform-specific upload instructions.

### Configuring Processing Filters in Sentry Settings

You can further tune your builds for Size Analysis in Sentry Settings. Under a (project's settings)[https://sentry.io/settings/projects/],
there is a settings page for "Mobile Builds".

[!Mobile Builds Settings Page](./images/mobile-builds-settings.png)

By default, Size Analysis processes for all uploaded builds. You can configure filters to only process builds for specific criteria, i.e. `git_head_ref: main`, `build_configuration_name: Release`, etc. Below is an example filter for a specific bundle (`com.emergetools.hackernews`) and branch (`main`).

[!Example Filter for specific bundle + branch](./images/example-filter-specific-bundle-branch-main.png)
