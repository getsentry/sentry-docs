...
Time Spent is a performance metric that measures the total cumulative time your application spends executing a specific operation (like database queries, API calls, or other transactions) over a given time period. Time Spent serves as a way to measure the relative performance impact of your transactions towards your application.
