...
Not all browsers fully support all web vitals. Sentry also considers certain web vitals necessary to calculate [Performance Score](/product/insights/web-vitals/#performance-score). 
...
<Note>
\* These Web Vitals are required to calculate [Performance Scores](/product/insights/web-vitals#performance-score), for the respective browsers. Some browsers may lack support for certain Web Vitals, depending on the web page and/or browser version. Page loads that are missing required Web Vitals are not scored due to incomplete data.
</Note>
