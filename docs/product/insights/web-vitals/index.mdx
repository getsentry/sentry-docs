...
**Page Loads** contribute up to 4 individual **Web Vital** components found on each event. Interactions contribute to **INP** (Interaction to Next Paint). The default weights and score rating thresholds for each **Web Vital** can be found in the tables below.
...
Some browsers may not support all **Web Vitals** used in Sentry's **Performance Score** calculation. Find out which Web Vitals are supported by your browser in the [Browser Support](/product/insights/web-vitals/web-vitals-concepts/#browser-support) table.
</Note>
