...
If the SDK you're using doesn't automatically instrument HTTP requests, you can instrument your application by following platform-specific custom instrumentation instructions. Dedicated platform documentation is available for these platforms:
...
If your platform doesn't have dedicated custom instrumentation instructions, you can try adapting the code from one of the supported platforms. In addition, the [Requests Modules SDK development guide](https://develop.sentry.dev/sdk/performance/modules/requests/) lists which span properties are required, and their expected format.
