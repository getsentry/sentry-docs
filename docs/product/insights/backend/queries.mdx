...
You can see the query's source underneath the query description on the **Query Summary** page, as shown in the image below. If you have a [source code management integration](/integrations/source-code-mgmt/) enabled, Sentry will also show a link to open the query's source in the relevant repository.
