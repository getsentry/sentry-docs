...
1. Click "Create Project" and configure it as appropriate for your application:
   ...
   - You'll notice that the Sentry React SDK setup code comes pre-populated with the project's unique DSN (Data Source Name). The DSN tells the SDK where to send events, so events are associated with the right project. You'll need to paste the DSN key into the SDK configuration code later in the tutorial. Jot it down now and keep it handy, you can also find a project's DSN anytime in **[Project] > Settings > SDK Setup > Client Keys (DSN)**
