...
- Go directly to [the next step](/product/sentry-basics/integrate-backend/configuration-options/).
...
1. Click "Create Project" and configure it as appropriate for your application:
    ...
    - **Set your alert frequency**: For the purpose of this tutorial, select **I'll create my own alerts later**.
      ...
      > If you're wary of alert fatigue, learn more about how to set up your alerts in [Alerts](/product/monitors-and-alerts/alerts/).
...
1. Open your Sentry account and navigate to **Settings > Integrations** to enable the GitHub integration and add your `backend-monitoring` repository. For more detailed information, follow the steps described in [our GitHub documentation](/integrations/source-code-mgmt/github/).
...
2. Open the `settings.py` file (located under `./backend-monitoring/myproject/settings.py`). This is where we import, initialize, and configure the Sentry SDK in our application.
   ...
   ```
   > In `settings.py` replace `YOUR_DSN` with the `DSN` key value you copied from the project created in the [project creation tutorial](/product/sentry-basics/integrate-frontend/create-new-project/).
...
[Configuration Options](/product/sentry-basics/integrate-backend/configuration-options/)
