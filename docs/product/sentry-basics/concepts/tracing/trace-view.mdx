...
Sentry provides a waterfall trace view similar to what you see in other APM solutions. The goal of this view is to show you everything that may have occurred during that trace, be it any errors, performance issues or profiles that may have been collected. The trace view is designed to be your one stop shop when debugging performance or errors, and it is the one place where you can gather full context on what was happening during an error or a performance issue. This experience is critical to us, so feedback (especially candid) is always appreciated.
