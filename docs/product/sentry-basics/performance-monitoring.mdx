---
title: "Performance Monitoring V2"
sidebar_order: 1
...
In many tools, Performance Monitoring is just about tracking a few key metrics on your web pages; Sentry takes a more granular approach. By setting up [Distributed Tracing](/product/sentry-basics/tracing/distributed-tracing/), Sentry captures detailed performance data for [every transaction](/concepts/key-terms/tracing/) in your entire application stack and automatically presents it in a variety of easy-to-use but powerful features so you can rapidly identify and resolve performance issues as they happen - all in one place.
...
After turning on Distributed Tracing, you'll automatically see a set of [Insights](/product/insights/) pages that give you a high-level view of performance data for each layer of your stack. This is useful for quickly seeing any major performance outliers that need optimization, or tracking potential regressions across time, environments, releases, etc.

- [**Frontend Insights**](/product/insights/frontend/) give you a view of performance data for your frontend (browser) transactions. These are broken down into pageview performance with things like Core Web Vitals, as well as transaction performance with things like duration, and error rate.
...
Just like typical [error issues](/product/issues/issue-details/error-issues/), Sentry will automatically detect common performance issues and group them into [performance issues](/product/issues/issue-details/performance-issues/). For instance: 
...
While Performance Issues and Insights give you a high-level view of performance data, the [Trace Explorer](/product/issues/performance-issues/trace-explorer/) lets you slice and dice all your performance data by any attribute. This is a powerful tool for finding root causes and common patterns for performance regressions or potential optimizations. For instance, you can find the p95 duration of all spans on the `GET /users/:id` endpoint, or find all pages slowing down because of a particular middleware routing. 

The newest version of the Trace Explorer also lets you quickly calculate metrics based on your span data, like p50, p95, and p99 durations, and then group them by any attribute. This lets you quickly see if there are specific endpoints, versions, user groups, etc. that are performing poorly. For a more thorough dive into how you can use the new Trace Explorer and Span Metrics, take a look at our most recent [blog post](https://blog.sentry.io/find-and-fix-performance-bottlenecks-with-sentrys-trace-explorer/). 
...
Instead of instrumenting pageloads with specific metrics at set points, like `onLoad`, `onUnload`, `onError`, etc., Sentry automatically traces every network request, file I/O, and other operation in your application. This lets you see the performance impact of every request, and any errors that occured during that request. 
