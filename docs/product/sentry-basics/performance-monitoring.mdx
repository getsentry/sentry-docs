...
### 1: pre-built Sentry Dashboards: Opinionated views of performance data for each layer of your stack

After turning on Distributed Tracing, you'll automatically see a set of [Sentry Dashboards](/product/dashboards/sentry-dashboards/) pages that give you a high-level view of performance data for each layer of your stack. This is extremely useful for quickly seeing any major performance outliers that need optimization, or tracking potential regressions across time, environments, releases, etc.

- [**Frontend Dashboards**](/product/dashboards/sentry-dashboards/frontend/) gives you an easy but powerful way to drill into common frontend performance issues with things like Core Web Vitals, as well as transaction performance with things like duration, and error rate.
- [**Backend Dashboards**](/product/dashboards/sentry-dashboards/backend/) show your backend transaction performance, across things like database queries, external API calls, caches, queues, crons, and more.
- [**Mobile Dashboards**](/product/dashboards/sentry-dashboards/mobile/) track vital performance metrics for your mobile apps, including your cold & warm app start times, TTID & TTFD, and time to load or render key screens throughout your app.
- [**AI Dashboards**](/ai/monitoring/agents/) give you a view of your LLM performance. This is currently in beta and only available for certain integrations.
...
By default, Sentry automatically tracks a set of [performance metrics](/platforms/javascript/tracing/span-metrics/) for every trace. These are used to populate the Sentry Dashboards, but you can also use them to build custom [dashboards](/product/dashboards/) and [alerts](/product/monitors-and-alerts/alerts/).
...
For basic frontend or backend performance monitoring, just turn on Tracing in your relevant SDK. This will automatically trace every network request and give you a basic set of performance metrics. However, to make sure Sentry is capturing accurate and complete performance data, you need to set up [Distributed Tracing](/product/sentry-basics/getting-started-tutorial/). NextJS, Laravel, Ruby on Rails and a few others have strong out-of-the-box support for distributed tracing, but for more complex or multi-language applications, you'll need to configure this manually. Learn more about trace propagation [here](/platforms/javascript/tracing/distributed-tracing/).
