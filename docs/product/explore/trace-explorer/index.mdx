...
## Cross-Event Querying (Beta)

<Alert level="info">

Cross-Event Querying is currently in beta. To access this feature, enable the Early Adopter flag in your [organization settings](https://sentry.io/orgredirect/settings/:orgslug/).
...
Currently, span and log events are supported as join targets.
...
2. Choose the event type you want to join on, for example, **Logs** or another **Span**.
3. Add filters for that event type, such as `severity:error` or `db.query:"SELECT *"`.
...
[Application Metrics](/product/explore/metrics/) are typically sampled at 100%, while traces are sampled at a much lower rate, so a specific metric sample often won't have a trace attached to it. Cross-Event Querying helps you find a comparable trace for the same pattern:

1. In Trace Explorer, filter spans by attributes your metric shares (for example, `release:myapp@1.2.3` or `http.route:/checkout`).
2. Click the **Plus (+)** next to the search bar and join on **Logs**, adding filters that describe the behavior the metric is tracking (for example, `severity:error` for a failure metric).
3. Pick a matching trace from the results. It gives you the same debugging context (spans, logs, related errors) as a comparable trace would.
