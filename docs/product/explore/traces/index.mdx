...
**Example:** Single condition: Find traces where a user with email `foo@example.com` loaded a specific frontend route.
...
**Example:** Multiple conditions: Find traces where a user with the email `foo@example.com` loaded a page on your frontend on a specific route AND a slow database query was present.
...
- **Matching Spans**: Number of spans matching your query versus the total spans in the trace.
...
Expand a trace to see the matching spans in the below table format:

- **Span ID**: Links to the span in the Trace View.
- **Description**: Includes project, `span.op`, and `span.description`.
- **Timeline**: Visual representation of the span's position within the trace.
- **Duration**: Time taken by the individual span.
