...
In addition to counting the number of [slow and frozen UI frame renders](/product/dashboards/sentry-dashboards/mobile/mobile-vitals/#slow-and-frozen-frames) for Mobile Vitals Insights, Sentry now records the timestamp for every frame and overlays it on top of profiling flame charts:
