...
In addition to counting the number of [slow and frozen UI frame renders](/product/insights/mobile/mobile-screens/#slow-and-frozen-frames) for Mobile Screens Insights, Sentry now records the timestamp for every frame and overlays it on top of profiling flame charts:
