...
If your [**Profiling**](https://sentry.io/orgredirect/organizations/:orgslug/profiling/) page displays no profiling data, and you're seeing a "look at the aggregate flamegraph" banner, it means that Sentry has received profiling data from your project, but no [spans or transactions](/product/tracing/). While this can be a valid use case when profiling is set up, but tracing is not [WOULD BE GOOD TO ADD A LINE ABOUT WHAT HAVING PROFILES, BUT NO TRACING WOULD BE GOOD FOR]...., we recommend setting up tracing, which can complement profiling in valuable ways [WOULD BE GOOD TO ADD A QUICK EXAMPLE OF VALUABLE WAYS TRACING + PROFILING CAN WORK TOGETHER].

If you've configured tracing and are sending us tracing data, but still see this banner, please reach out to our [customer support](https://sentry.zendesk.com/hc/en-us/).
