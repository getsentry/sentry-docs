...
The **Profiling** page contains two main tabs which serve as the entrypoint to profiling - transactions and profiles. Depending on how you setup profiling, you might be interested in one or the other. The transactions tab lists transactions in descending order of execution time for your selected project(s) while the flamegraph tab shows an aggregate flamegraph that spans across transactions.
...
The "Profiles by Percentiles" timeline chart can be used to filter the transaction data by highlighting the desired time period.

The "Slowest Functions" widget shows you a list of the functions that took the most time in your application. It lists the transactions the function was called from. Clicking on a transaction will take you to an example profile.
...
The flamegraph tab shows an aggregate flamegraph based off the query you selected. This tab is a great place to visualize a flamegraph across transaction boundaries and better understand how the system as a whole might be bottlenecked. The aggregate flamegraph is a more advanced workflow that serves to users which have setup continuous profiling and are interested in how their entire system performs.

@TODO add flamegraph tab photo

The aggregate flamegraph shows a flamegraph where the width of functions are represented by the amount of times a particular function had been sampled. By using this view, we can understand where our system is spending time. It is generally recommended to look for functions with large sample count and optimize them, as they are the ones most frequently occupying the program's stack.

By right clicking on function, a dropdown will open, giving you the ability to open some example profiles and see an instance of that function executing - this is useful to gain context into how and why that function might be running.

@TODO show right click dropdown option
