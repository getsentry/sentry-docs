...
The flamegraph tab shows an aggregate flamegraph based off the query you selected. This tab is a great place to visualize a flamegraph across transaction boundaries and better understand how the system as a whole might be bottlenecked. The aggregate flamegraph serves users who have already set up continuous profiling and are interested in how their entire system performs.

@TODO add flamegraph tab photo

The aggregate flamegraph shows a flamegraph where the width of functions are represented by the amount of times a particular function had been sampled. By using this view, you can understand where your system is spending time. It is generally recommended to look for functions with large sample count and optimize them, as they are the ones most frequently occupying the program's stack.

By right clicking on a function, a dropdown will open, giving you the ability to open some example profiles and see an instance of that function executing - this is useful to gain context into how and why that function might be running.
