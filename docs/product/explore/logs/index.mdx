...
- **Debugging information**: `navigation.action`, `page.interaction`, `query.result`, `cache.hit`
- **User behavior**: `user.login`, `checkout.started`, `feature.used`, `preference.changed`
- **Application state**: `config.loaded`, `service.initialized`, `connection.established`
- **Business events**: `order.placed`, `payment.processed`, `notification.sent`
