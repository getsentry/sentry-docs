...
There are five main building blocks that impact the results of your saved query. You can use a combination of these to narrow down your search.
...
The dataset selector allows you to choose between querying for errors or transactions. This will determine the type of data you are querying for. You can only query one dataset at a time. The errors dataset will return events related to errors, while the transactions dataset will return transactions events for viewing performance data. 
