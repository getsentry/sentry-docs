...
- `params` - Filtered request parameters (collected when `data_collection.url_query_params` is enabled)
...
- `arguments` - Job arguments (collected when `data_collection.queues` is enabled, filtered for sensitive data)
...
- `message_id` - Email message ID (collected when `data_collection.user_info` is enabled)
...
- `params` - Mailer parameters (filtered using `data_collection.url_query_params`)
