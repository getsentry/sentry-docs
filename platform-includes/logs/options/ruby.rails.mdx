...
Structured logging is enabled by default. You can customize which Rails events are captured.
...
<Alert level="warning">
  This captures ALL Rails framework logs as well as ANY OTHER logs produced by
  instances of the `Logger` class and this can be very noisy. We recommend using
  `Sentry.logger` for better control over what gets logged.
</Alert>
