...
Use `sentry_sdk.set_attribute` and `sentry_sdk.set_attributes` to attach attributes that are automatically included on all logs, metrics, and spans (only in <PlatformLink to="/tracing/streamed-spans">stream mode</PlatformLink>). These write to the isolation scope, so they apply for the current request or session:
