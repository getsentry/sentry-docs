...
Godot logger events can also be captured automatically as [Sentry Logs](/product/explore/logs/). The `godot_logger.log_mask` option controls which events are captured — engine errors, script errors, shader errors, warnings, and `print()` output. This mask is empty by default, so no Godot logger events are captured as logs unless you configure it:
...
events and breadcrumbs — see `godot_logger.event_mask` and `godot_logger.breadcrumb_mask`.
