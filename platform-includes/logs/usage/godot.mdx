...
By default, Godot's own log output isn't captured as [Sentry Logs](/product/explore/logs/). To capture Godot logger events as logs — engine errors, GDScript errors, shader errors, warnings, and log messages such as `print()` output — set the `godot_logger.log_mask` option to the events you want:
