...
SentrySDK.set_attribute("map_name", "dust_arena")
SentrySDK.set_attribute("crossplay", true)

# All subsequent logs will include these attributes
SentrySDK.logger.info("Match started")

# Clear when returning to the main menu
SentrySDK.remove_attribute("map_name")
```

Global attributes support `bool`, `int`, `float`, and `String` data types. Other types will be converted to strings. Attributes passed directly to a log call take precedence over global attributes with the same key.
