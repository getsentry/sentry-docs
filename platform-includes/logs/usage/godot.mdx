...
By default, Godot's own log output isn't captured as [Sentry Logs](/product/explore/logs/). To capture Godot logger events as logs, set the `godot_logger.log_mask` option to the events you want to include, such as engine errors, script errors, shader errors, warnings, or `print()` output:
