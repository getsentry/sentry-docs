...
When the parts are built at runtime, `sentry_scope_set_fingerprints` takes a list created with `sentry_value_new_list` instead of a variadic argument list. `sentry_scope_remove_fingerprint`, available since SDK version [0.16.2](https://github.com/getsentry/sentry-native/releases/tag/0.16.2), clears a fingerprint set on a scope.
