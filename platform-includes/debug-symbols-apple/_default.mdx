...
3.  Set `ENABLE_USER_SCRIPT_SANDBOXING` in your Xcode project settings to `NO`. <img src="enable_user_script_sandboxing.png"/>
...
ERROR=$(sentry-cli debug-files upload --force-foreground --include-sources "$DWARF_DSYM_FOLDER_PATH" 2>&1 >/dev/null)
if [ ! $? -eq 0 ]; then
...
export SENTRY_PROJECT=___PROJECT_SLUG___
export SENTRY_AUTH_TOKEN=___ORG_AUTH_TOKEN___
ERROR=$(sentry-cli debug-files upload --force-foreground --include-sources "$DWARF_DSYM_FOLDER_PATH" --force-foreground 2>&1 >/dev/null)
if [ ! $? -eq 0 ]; then
