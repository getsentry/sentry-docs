...
You can also define the configuration via command-line arguments or a separate properties file.
...
**Command-line arguments**: configure value using `--sentry-define`:
...
The final configuration is the result of merging all the configuration sources together and the value with the highest priority wins.
...
| Readable Stacktrace  | ✓     | ✓     | ✓           | ✓           | ✓           |
| Source Context       | ✓\*\*   | ✓\*\*   | ✓\*\*         | ✓\*\*         | ✓\*\*         |
| Readable Issue Title | ✓       | ✓       | Not Supported | Not Supported | Not Supported |
...
| Readable Stacktrace  | ✓             |
| Source Context       | ✓             |
| Readable Issue Title | Not Supported |

## Configuration Reference

The following table lists all available configuration options for the Sentry Dart Plugin.

The **Key** column applies to the following configuration sources:

- **pubspec.yaml** 
- **sentry.properties**
- **--sentry-define**

Environment variables (where available) have the highest priority and override all other methods.
...
### Missing Release in Git History

**Problem:** The plugin fails because a previous release cannot be found in the git history.

**Solution:** Set `ignore_missing: true` in your configuration to bypass this validation:
...
### Debug IDs Not Working For Older SDK Versions (Flutter Web)

**Problem:** Symbolication isn't working for Flutter Web with plugin version 3.0.0+.

**Solution:** Debug IDs require Sentry Flutter SDK version 9.1.0 or higher. Either:

1. Upgrade your Sentry Flutter SDK to 9.1.0+, or
2. Set `legacy_web_symbolication: true` to use the legacy symbolication method:
...
### URL Prefix (Flutter Web)

**Problem:** Source maps aren't matching for a Flutter Web app that's not deployed at the root URL (e.g., `https://example.com/my-app/` instead of `https://example.com/`).

**Solution:** You need to configure both the `url_prefix` in the plugin and update your stack frame paths in the SDK to match.
