...
```JavaScript diff {tabTitle:CommonJS} {filename: instrument.js}
 const Sentry = require('@sentry/node');
...
If you cannot run node with [--require](https://nodejs.org/api/cli.html#-r---require-module) or [--import](https://nodejs.org/api/cli.html#--importmodule), [add a top-level import of `instrument.js` in your application](/platforms/javascript/guides/express/initialize-sentry/add-as-toplevel-import).
