...
The `before_send_metric` argument is a callback function that is called before a metric is emitted. If the callback returns `None`, the metric is not emitted. Attributes can also be updated in the callback function.
...
def before_metric(metric: Metric, _hint: Hint) -> Optional[Metric]:t
    if metric["name"] == "removed-metric":
        return None

    metric["attributes"]["extra"] = "foo"
    del metric["attributes"]["release"]
