...
Metrics are disabled by default. To enable metrics, set `enableMetrics` to `true` in the experimental options when initializing the SDK:
...
- `value`: The metric value (counter, distribution, or gauge)
- `unit`: The unit of measurement (`SentryMetricsUnit?`)
