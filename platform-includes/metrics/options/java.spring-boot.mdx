...
public class SentryMetricsConfiguration {
  @Bean
  public SentryOptions.Metrics.BeforeSendMetricCallback beforeSendMetric() {
    return (metric, hint) -> {
...
class SentryMetricsConfiguration {
  @Bean
  fun beforeSendMetric(): SentryOptions.Metrics.BeforeSendMetricCallback =
    SentryOptions.Metrics.BeforeSendMetricCallback { metric, hint ->
      // Drop metrics with specific attributes
