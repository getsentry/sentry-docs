...
| **Collect GC pause metrics** | Emit a metric for each garbage collection pause duration. Requires metrics to be enabled and Unreal Engine 5.5+. | `false` |
| **Collect game stats metrics** | Periodically collect game stats such as process memory usage and active UObject count. Requires metrics to be enabled. | `false` |
| **Sample interval (seconds)** | How often to sample game stats metrics. Higher values reduce overhead. | `60` |
