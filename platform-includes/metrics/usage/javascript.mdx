...
Use `Sentry.setAttribute` and `Sentry.setAttributes` to attach attributes that apply to all metrics (as well as your logs). These work just like <PlatformLink to="/apis/#setTag">`Sentry.setTag` and `Sentry.setTags`</PlatformLink>, but accept `string`, `number`, and `boolean` values.

<Alert level="note">
  The top-level `Sentry.setAttribute` and `Sentry.setAttributes` APIs require SDK version `10.61.0`+. If these functions aren't available, upgrade your Sentry SDK package to at least version `10.61.0`.
</Alert>
