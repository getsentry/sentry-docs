...
Each metric has a **1 MiB size limit**. If you exceed this, the metric will be dropped.
...
Use `Sentry.setAttribute` and `Sentry.setAttributes` to attach attributes that apply to all metrics (as well as your logs). These work just like [`Sentry.setTag` and `Sentry.setTags`](/platforms/javascript/configuration/apis/#setTag), but accept `string`, `number`, and `boolean` values.
