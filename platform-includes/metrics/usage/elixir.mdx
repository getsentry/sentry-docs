Metrics are always on. Once you initialize the SDK, you can send metrics using the `Sentry.Metrics` module. To have the SDK also report BEAM runtime metrics on its own, see [Automatic Metrics](#automatic-metrics).
