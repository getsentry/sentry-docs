...
The scrubber will check parts of the event that are most likely to contain data from the denylist, like user and request data. It will, however, not work recursively by default for performance reasons. If you want the scrubber to be recursive, initialize it with `recursive=True`.
