...
However, if you need finer control — for example, to define callbacks like [<PlatformIdentifier name="before-send" />](/configuration/options/#before-send) —
...
Prefer automatic initialization when possible — it runs before your code and helps ensure no startup events are missed. Use manual initialization only for cases that require runtime customization (custom [<PlatformIdentifier name="before-send" />](/configuration/options/#before-send) handler, dynamic release values, programmatic sampling, etc.).
