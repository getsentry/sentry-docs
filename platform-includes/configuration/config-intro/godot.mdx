...
The earliest point to configure and initialize the SDK is the main loop's `_initialize()` method (`_Initialize()` in C#).

To do this:

1. Disable **Auto Init** in **Project Settings -> Sentry -> Options**.
2. Create a new script that extends `SceneTree`. In GDScript, give it a `class_name`; in C#, mark the class `[GlobalClass]` and `partial`.
3. Assign that script as your main loop type in **Project Settings → Application → Run → Main Loop Type**.
4. Define the `_initialize()` method (`_Initialize()` in C#) and call `SentrySDK.init()` (`Sentry.Godot.SentrySdk.Init()` in C#) there.
