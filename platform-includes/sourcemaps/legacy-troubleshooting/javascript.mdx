...
```shell
sentry-cli sourcemaps upload --release VERSION --url-prefix '~/scripts' .
```
...
```
sentry-cli sourcemaps upload --release VERSION --url-prefix '~/scripts' script.min.js script.min.js.map
```

You can also upload it with the fully qualified URL. For example:

```
sentry-cli sourcemaps upload --release VERSION --url-prefix 'http://localhost:8000/scripts' .
```
