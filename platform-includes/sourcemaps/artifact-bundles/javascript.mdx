...
Sentry uses **Artifact Bundles** for managing un-minification of your stack traces. A bundle, at a higher level, contains all the minified sources and source maps (known as _artifacts_) required for Sentry to unminify the frames of a stack trace.
...
When using sentry-cli or the webpack, vite, or rollup bundler plugins Debug IDs are generated deterministicly based on the source file contents. Deterministic Debug IDs for esbuild are currently not supported.
