...
Vercel will create cron jobs depending on the `crons` field in your `vercel.json`. Set the `automaticVercelMonitors` option to `true` in your Sentry settings in `next.config.js` to automatically create cron job monitoring for them.
...
See <PlatformLink to="/manual-setup/#opt-out-of-auto-instrumentation-of-vercel-cron-jobs">Manual Setup</PlatformLink> for more details.
