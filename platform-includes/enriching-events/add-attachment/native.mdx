...
Adding attachments to the global scope at run-time after SDK initialization will monitor the file and upload it along with any event or crash that is sent to Sentry. Attaching to a [scope you created](/platforms/native/enriching-events/scopes/#creating-a-scope) instead uploads the file only with the captures that scope is passed to.

Only global scope attachments are included in crash reports. A file attached to a scope you pass is uploaded only when you explicitly capture an event with that scope. If you need the file included with a crash, attach it to the global scope or add it to the options instead.
