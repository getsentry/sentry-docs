...
Adding attachments to the global scope at run-time after SDK initialization will monitor the file and upload it along with any event or crash that is sent to Sentry, whereas adding attachments to a local scope will only upload the file with the specific event captured in that local scope. On a [scope you own](/platforms/native/enriching-events/scopes/#passing-a-scope-to-a-capture), the attachment is uploaded with every capture that scope is passed to, until you free the scope.

Only global scope attachments reach crash reports. A file attached to a scope you pass is uploaded when you capture against that scope, so if you need it on a crash, attach it to the global scope or to the options instead.
