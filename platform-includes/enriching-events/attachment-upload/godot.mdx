...
// Global scope: attached to every C# event.
SentrySdk.ConfigureScope(scope => scope.AddAttachment(attachment));

// Local scope: attached only to this C# event.
SentrySdk.CaptureMessage("Save failed", scope =>
{
    scope.AddAttachment(ProjectSettings.GlobalizePath("user://savegame.dat"));
});
```
