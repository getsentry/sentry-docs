...
In this example, we create a new transaction (or service span if you're using <PlatformLink to="/tracing/streamed-spans">stream mode</PlatformLink>) that is attached to the trace specified in the `sentry-trace` and `baggage` headers.
