...
{/* The first section should only apply for browser-only platforms, not for Meta-Frameworks - so we include browser but then exclude server/serverless platforms */}
