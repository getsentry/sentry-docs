...
The SDK bundles `sentry.server.config.ts` into the Nitro server build and initializes Sentry when the server starts. This works in production and in development mode, and you don't need the Node `--import` flag.

To learn more, see [Server-Side Initialization](/platforms/javascript/guides/nuxt/install/).
...
Once you have your test code in place, build and run your project.

</PlatformSection>

<PlatformSection notSupported={["javascript.cloudflare"]}>

Once you have your test code in place, run your project in production mode (`nuxi build`, then `node .output/server/index.mjs`) or in development mode (`nuxt dev`).
...
- Learn how the SDK <PlatformLink to="/install">initializes on the server side</PlatformLink>
