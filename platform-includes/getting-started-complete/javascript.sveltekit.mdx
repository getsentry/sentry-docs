...
Configure server-side instrumentation and tracing. Where this config lives depends on your SvelteKit version — SvelteKit 2 uses `svelte.config.js`, while SvelteKit 3 moved it into the `sveltekit()` Vite plugin instead (see [Configure Vite](#configure-vite) below).

</SplitSectionText>

<SplitSectionCode>

```js {tabTitle:SvelteKit 2} {mdExpandTabs} {filename:svelte.config.js} {3-12}
const config = {
  kit: {
    experimental: {
      instrumentation: {
...
```js {tabTitle:SvelteKit 3} {filename:vite.config.(js|ts)}
sveltekit({
  // ___PRODUCT_OPTION_START___ performance
  tracing: { server: true },
  // ___PRODUCT_OPTION_END___ performance
...
- Learn more about [deploying SvelteKit apps to Cloudflare Workers and Pages](/platforms/javascript/guides/cloudflare/frameworks/sveltekit/)
...
- Find various support topics in <PlatformLink to="/troubleshooting">Cloudflare troubleshooting</PlatformLink> and [SvelteKit troubleshooting](/platforms/javascript/guides/sveltekit/troubleshooting/)
- [Get support](https://www.sentry.help/en/)
