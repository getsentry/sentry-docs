...
SentryObjCTracesSamplerCallback callback = ^(SentryObjCSamplingContext *context) {
  if (context.transactionContext.parentSampled != SentryObjCSampleDecisionUndecided) {
    return context.transactionContext.parentSampled == SentryObjCSampleDecisionYes ? @1.0 : @0.0;
  }
  // the rest of sampling logic
};
```
