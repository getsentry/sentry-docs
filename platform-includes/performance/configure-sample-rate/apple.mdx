...
```objc {diff, tabTitle:Objective-C}
@import Sentry;
...
+       // Don't miss any transactions for VIP users
+       if ([samplingContext.customSamplingContext[@"vip"] boolValue] == YES) {
+           return @1.0;
+       } else {
+           return @0.25; // 25% for everything else
+       }
...
```objc {diff, tabTitle:Objective-C (SentryObjC)}
