...
SqlClient instrumentation is only available on .NET Core 3.0 or higher. SqlClient's `DiagnosticSource` events aren't emitted on .NET Framework, so this instrumentation has no effect there — even if you add the `Sentry.DiagnosticSource` package manually. On .NET Framework, only Entity Framework Core instrumentation is available via `Sentry.DiagnosticSource`.
