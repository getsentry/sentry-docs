...
SentryObjCSpan *transaction = [SentryObjCSDK startTransactionWithName:@"Transaction Name" operation:@"operation" bindToScope:YES];
NSError * error;
...
if (processItem(error: &error)) {
  ...
  [transaction finishWithStatus:SentryObjCSpanStatusInternalError];
}
```
