...
SentryObjCSpan *transaction =
    [SentryObjCSDK startTransactionWithName:@"processOrderBatch"
                              operation:@"task"
                            bindToScope:YES];
...
- (void)processOrderBatch {
    ...
    if (span == nil) {
        span = [SentryObjCSDK startTransactionWithName:@"processOrderBatch"
                                         operation:@"task"];
    }

    SentryObjCSpan *innerSpan = [span startChildWithOperation:@"subtask"];
    // omitted code
    [innerSpan finish];
}
```
