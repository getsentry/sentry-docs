...
[SentryObjCSDK startWithConfigureOptions:^(SentryObjCOptions * _Nonnull options) {
    options.dsn = @"___PUBLIC_DSN___";

    // To set a uniform sample rate
    options.tracesSampleRate = @1.0;

    options.tracesSampler = ^NSNumber * _Nullable(SentryObjCSamplingContext * _Nonnull samplingContext) {
