...
**Note:** Source maps, while also being debug information files, are handled differently
...
**Note:** At the moment, Sentry supports Native PDBs only. PDBs for the .NET platform are not supported.
...
**Note:** Debug information does not have to be associated with releases. The unique debug
...
<Alert title="Note" title="info"><markdown>
