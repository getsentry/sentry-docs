...
<Alert level="warning" level="Use of defer"><markdown>
...
<Alert level="info" level="Upgrading the SDK and want to understand what's new?"><markdown>

Have a look at the [Changelog](https://github.com/getsentry/sentry-javascript/releases).
...
[![Error in Unresolved Issues with title This is my fake error message](js-index/error-message.png)](js-index/error-message.png)

<Alert level="warning" level="Dealing with Ad-Blockers"><markdown>
...
[![Stack trace of a captured error](js-index/automatically-capture-errors.png)](js-index/automatically-capture-errors.png)
...
<Alert level="warning" level="Note"><markdown>

We **highly recommend** you incorporate source maps with the Sentry JavaScript SDK in order to receive the full benefit of error tracking and monitoring. Source maps will provide stack traces, which will provide more information regarding errors.
...
Make sure the `SentryWebpackPlugin` is set up as the last running plugin, otherwise, it could happen that the resulting source maps that the plugin receives are not the final one.
...
<Alert level="" level="Assets Accessible at Multiple Origins"><markdown>
...
<Alert level="" level="Validating source maps with Sentry CLI"><markdown>
...
<Alert level="" level="Working Behind a Firewall"><markdown>
...
<Alert level="" level="Secure Access to Source Maps"><markdown>
...
<Alert level="warning" level="Context Size Limits"><markdown>
...
![Sauce Test Status](browser-support.svg)



<Alert level="warning" level="Support for <= IE 11"><markdown>
...
[![](js-loader-settings.png)](js-loader-settings.png)
...
[![Modal popup asking user for more context on what occured before the break.](js-index/sentry-dialog.png)](js-index/sentry-dialog.png)
