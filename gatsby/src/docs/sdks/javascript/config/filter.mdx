...
![Built-in Inbound Filters](builtin-inbound-filters.png)
...
For example, with JavaScript errors, a minified source code can result in a nondeterministic stack trace that could interfere with event grouping, which can be avoided by ensuring Sentry has access to your [Source Maps](/platforms/javascript/sourcemaps) and minified artifacts.
 ...
 ![JavaScript stack trace without source maps](stacktrace-without-sourcemaps.png)
 ![JavaScript stack trace with source maps](stacktrace-with-sourcemaps.png)
...
Note also that breadcrumbs can be filtered, as discussed in [our Breadcrumbs documentation](sdks/javascript/errors/breadcrumbs/).
