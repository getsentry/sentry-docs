**Accepted Formats**: XCArchive

**Upload Mechanisms**: [Fastlane Plugin](#uploading-ios-builds-with-fastlane) (version 1.35.0 or higher) _or_ [Sentry CLI](#uploading-ios-builds-with-the-sentry-cli)

### Uploading iOS Builds With Fastlane

<Include name="size-analysis/upload-fastlane" />

### Uploading iOS Builds With the Sentry CLI

<Include name="size-analysis/upload-cli-ios" />
