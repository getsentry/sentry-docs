...
When manually setting token attributes, be aware of how Sentry uses them to [calculate model costs](/product/agents/costs/).
...
Because `gen_ai.usage.cache_read.input_tokens` (90) is larger than `gen_ai.usage.input_tokens` (10), the subtraction goes negative, resulting in a negative total cost.
