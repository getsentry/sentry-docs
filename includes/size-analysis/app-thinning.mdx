...
If you want results targeting a specific device type (recommended), apply app thinning **before** uploading to Sentry. Xcode only applies the `thinning` key in `ExportOptions.plist` to non-App Store exports, so create a separate `release-testing` export for Size Analysis. Continue using an `app-store-connect` export for TestFlight or App Store submission.

First create an `export_options.plist` file with the desired device type:
...
To retain the original XCArchive's dSYM information, upload a modified copy of the archive that uses the app from the thinned IPA. See the [Hacker News sample app's Fastfile](https://github.com/EmergeTools/hackernews/blob/main/ios/fastlane/Fastfile) for a complete example.
