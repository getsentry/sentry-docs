...
<Alert>
  Uploading IPA files requires Sentry Fastlane plugin version `2.0.0-rc.3` or
  later. Including dSYMs in IPA build analysis requires version `2.7.0` or later
  and an Apple silicon Mac. XCArchive uploads are supported in earlier versions.
</Alert>
...
3. In `FastFile`, add a call to `sentry_upload_build` after your build step.
   ...
   **Uploading an IPA:**
   ...
   IPA files typically don't contain dSYMs. Pass `dsym_path` to include them in build analysis.
