...
<PlatformSection notSupported={["react-native", "react-native.expo"]}>
...
<PlatformSection supported={["react-native", "react-native.expo"]}>

<IntegrationGrid
  integrations={[
    {to: "/integrations/openai/", title: "OpenAI", icon: "openai"},
    {to: "/integrations/anthropic/", title: "Anthropic", icon: "anthropic"},
    {to: "/integrations/google-genai/", title: "Google Gen AI SDK", icon: "google-genai"},
    {to: "/integrations/langchain/", title: "LangChain", icon: "langchain"},
    {to: "/integrations/langgraph/", title: "LangGraph", icon: "langgraph"},
  ]}
/>
...
If you're using a library that Sentry doesn't provide helpers for, create spans manually with well-defined names and data attributes. This gives Sentry a complete trace of how long the agent and each model or tool call took, where failures happened, and how the agent produced its final response.

The following sections describe the minimum span structure and attributes Sentry needs. You can add richer data, such as messages and streaming metrics, when it is available.
