...
```ts {filename:sentry.client.config.ts}
...
On `sentryAstro()`, `release` configures source map uploads and `debug` controls build-time logging. If you have no config files, the generated default snippets still pick up those values. The generated client snippet includes Replay with default sample rates.
