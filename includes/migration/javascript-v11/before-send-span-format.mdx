...
The `status` field is always set to `'ok'` or `'error'`. The finer-grained v10 statuses map to `'error'`, with the original value kept in `attributes['sentry.status.message']`.

Use `ignoreSpans` to drop spans. `beforeSendSpan` can only change spans, not drop them. A callback that does not match your `traceLifecycle` is never invoked: wrap it in `Sentry.withStaticSpan()` for `'static'`, leave it unwrapped for `'stream'`. Turn on debug logging to see a warning about a mismatch.
