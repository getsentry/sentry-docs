...
The app-start breakdown also changes in alpha.7. Update queries, dashboards, and `beforeSendSpan` callbacks that match `app.start.plugin_registration`, `app.start.sentry_setup`, or `app.start.first_frame_render`. These operations are no longer emitted. The SDK records measured startup work using:
...
**Unreleased after alpha.7:** The broad `app.start.pre_init` and `app.start.sentry_init` spans introduced in alpha.7 are no longer emitted. Startup frame builds that begin at or after the first frame's rasterization starts are also excluded. Review comparisons across prereleases because the breakdown has changed even when the standalone `app.start` root remains the same.
