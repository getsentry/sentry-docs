...
```dart {filename:http_client.dart}{tabTitle: default} {mdExpandTabs}
...
The marked elements (\*) are affected by the default enabled [server-side data scrubbing](/security-legal-pii/scrubbing/server-side-scrubbing/).
To implement client side data scrubbing, go to [client-side data scrubbing in Flutter](/platforms/dart/guides/flutter/data-management/sensitive-data/).
...
// or, in stream mode: await Sentry.startSpan('webrequest', (span) async { ... }, parentSpan: null)
