...
In v10, captured Dio errors use the stable type `DioException`, including in obfuscated builds. Default values are `HTTP Client Error with status code: <code>` for responses or `HTTP Client Error: <failure>` for connection failures. Custom Dio string builders are preserved.
