...
ElevatedButton(
  key: const Key('my_button_widget'),
  onPressed: () async {
    // The tap starts a root span automatically; nested spans attach to it
    await Sentry.startSpan('some operation', (span) async {
      // your operation
    });
  }, child: const Text('User Interaction Example'),
),
...
In stream mode, there's no public accessor for the active span, so manual finishing isn't available — keep the idle timeout enabled so the span finishes automatically.

To finish the transaction manually in transaction mode, use `Sentry.getSpan()` to retrieve the active transaction on the scope:
