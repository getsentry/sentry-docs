...
The decoder itself creates the span. In transaction mode, it starts a child off the active transaction; in stream mode, `Sentry.startSpanSync()` attaches the span to the active span automatically and sets its status based on whether the callback throws:
