...
Because the transaction uses the `app.start` operation, you can use `tracesSampler` to give app starts a dedicated sample rate without raising your overall sample rate:
...
Call `extendAppStart()` before the first frame renders. The `appRunner` callback of `SentryFlutter.init`, before `runApp`, is the safest place. Pair the two calls in a `try`/`finally` so an early return or a thrown exception can't leave the app start open.
