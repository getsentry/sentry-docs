...
<PlatformSection notSupported={["react-native"]}>
When you upload source maps, Sentry's bundler plugins or Sentry CLI prepare the artifacts before sending them to Sentry. During this upload workflow:

1. **A globally unique Debug ID is generated**: The upload tooling generates a Debug ID that is ideally deterministic.

2. **The Debug ID is embedded in the minified file**: The tooling adds a comment such as `//# debugId=DEBUG_ID` to each minified JavaScript file. This happens when the artifacts are prepared for upload, not when your application source is originally compiled. If you upload with Sentry CLI, run `sentry-cli sourcemaps inject` before `sentry-cli sourcemaps upload`.
</PlatformSection>
<PlatformSection supported={["react-native"]}>
...
3. **The same Debug ID is added to source maps**: The same workflow adds the Debug ID to each source map as a new attribute, linking the minified JavaScript file to its corresponding source map.
