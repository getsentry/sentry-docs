...
if (!issueBodyPath)
  fail('Usage: node scripts/generate-integration-doc.mjs <issue-body-file>');

const issueBody = fs.readFileSync(issueBodyPath, 'utf8');
const fields = parseIssueBody(issueBody);

// REQUIRED fields (per your requirement)
const title = pick(fields, [
  'title',
  'integration_name',
  'integration_name_title',
  'integration',
]);
const learn_about_sentence = pick(fields, [
  'learn_about_sentence',
  'lead_sentence_learn_about',
  'lead_sentence_learn_about_sentence',
  'lead_sentence',
]);
...
const installsteps = pick(fields, [
  'installsteps',
  'install_and_configure_steps',
  'install_and_configure',
]);
const configurationdetails = pick(fields, [
  'configurationdetails',
...
const indexPath = fs.existsSync(indexMdx)
  ? indexMdx
...
if (!indexPath) {
  console.warn(
    `No category index found at ${indexMdx} or ${indexMd}; skipping index update.`
  );
} else {
