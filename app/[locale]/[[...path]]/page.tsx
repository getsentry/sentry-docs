    ...
    return (
      <T>
        ...
        >
          ...
          <Alert level="info">
            The changelog updates automatically throughout the day. Summaries are
            generated by AI to provide quick, user-friendly insights into each update from
            the getsentry/sentry-docs repository.
          </Alert>
        </DocPage>
      </T>
    );
...
export async function generateMetadata(props: MetadataProps): Promise<Metadata> {
  ...
  // set locale early so doc tree lookups use localized titles/descriptions
  setServerContext({
    path: params.path ?? [],
    locale: params.locale,
  });
  ...
  if (params.path) {
    ...
    if (pageNode) {
      ...
      const docTitle = pageNode.frontmatter.title;
      title = guideOrPlatform
        ? t('{docTitle} | Sentry for {guideTitle}', {
            docTitle,
            guideTitle: guideOrPlatform.title,
          })
        : docTitle;
